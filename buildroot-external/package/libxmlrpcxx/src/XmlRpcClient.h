...
namespace XmlRpc {
  ...
  class XMLRPC_DLLEXPORT XmlRpcClient : public XmlRpcSource {
  public:
	  std::string getURL();
    ...
    //! Construct a client to connect to the server listening on the specified unix domain socket
    //!  @param path The path that is associated with the unix domain socket
    //!  @param uri  An optional string to be sent as the URI in the HTTP GET header
    XmlRpcClient(const char* path, const char* uri=0);

    //! Construct a client from an URL
    //!  @param url URL to connect to in the form <protocol>://<server>:<port>/<uri>
    XmlRpcClient(const std::string& url);
    ...
    //! Set the connection timeout. Use -1 for no timeout.
    void setTimeout(int timeout);

	//! Return a pointer to the dispatcher
  protected:
    // Execution processing helpers
    bool doExecute(const char* method, XmlRpcValue const& params, XmlRpcValue& result);
    ...
    // If true we use binary mode communication
    bool _isBinary;

    // Connection timeout in ms. Use -1 (default) for no timeout
    int _timeout;

    // Number of bytes expected in the response body (parsed from response header)
    int _contentLength;

	// Maximum number of connection attemts if windows reports WSAEADDRINUSE errors
	enum{
		ADDRINUSE_ERROR_MAX_RETRIES = 10
	};

	// Set to true if a WSAEADDRINUSE error is reported on a connection attempt
	bool _addrinuseErrorReported;
