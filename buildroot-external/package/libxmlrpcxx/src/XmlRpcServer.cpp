...
{
	return bindAndListen(NULL, port, backlog);
}

// Create a socket, bind to the specified port, and
// set it in listen mode to make it available for clients.
bool 
XmlRpcServer::bindAndListen(const char* ip, int port, int backlog /*= 5*/)
{
  ...
  if ( ! XmlRpcSocket::bind(fd, ip, port))
  {
    this->close();
  ...
  XmlRpcUtil::log(2, "XmlRpcServer::bindAndListen: server listening on port %d", port);
...
{
  XmlRpcUtil::log(5,"XmlRpcServer: handleEvent.");
  acceptConnection();
...
XmlRpcDispatch*
XmlRpcServer::getDispatcher()
{
	return &_disp;
}

int XmlRpcServer::getListenPort()
{
    return XmlRpcSocket::getSocketPort(getfd());
}
