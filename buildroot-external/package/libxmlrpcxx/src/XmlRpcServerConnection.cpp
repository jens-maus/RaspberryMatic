...
const char XmlRpcServerConnection::BINARY_SIGNATURE[] = "Bin";
const int XmlRpcServerConnection::BINARY_SIGNATURE_SIZE = sizeof(XmlRpcServerConnection::BINARY_SIGNATURE)-1;
...
{
  XmlRpcUtil::log(5,"XmlRpcServerConnection: handleEvent.");
...
XmlRpcServerConnection::writeResponse()
{
  ...
  if (_bytesWritten == int(_response.length())) {
	...
	return _keepAlive;    // Continue monitoring this source if true
	  ...
	  return true; //call us again to write the rest of the data
  }
...
XmlRpcServerConnection::parseRequest(XmlRpcValue& params)
{
  ...
  if(_isBinary){
	  XmlRpcValue methodName;
	  if(methodName.stringFromStream(_request, &offset)){
		  if(params.arrayFromStream(_request, &offset)){
...
void
XmlRpcServerConnection::generateResponse(XmlRpcValue& result)
{
	if(_isBinary){
