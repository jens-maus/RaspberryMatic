...
// Version id
const char XmlRpc::XMLRPC_VERSION[] = "XMLRPC++ 0.9";
...
XmlRpcUtil::findTag(const char* tag, std::string const& xml, int* offset)
{
  ...
  //strip the trailing ">" character from tag
  std::string tagstart( tag, strlen(tag) - 1 );
  size_t istart = xml.find(tagstart, *offset);
  if (istart == std::string::npos)
    return false;
  size_t iend = xml.find( '>', istart );
...
{
	return nextTagIs(tag, xml, offset, NULL);
}
...
bool 
XmlRpcUtil::nextTagIs(const char* tag, std::string const& xml, int* offset, bool* isEmpty)
{
  if (*offset >= int(xml.length())) return false;
  ...
  if  (*cp && (strncmp(cp, tag, len-1) == 0)) {
	  const char* tagEnd = strchr( cp + len-1, '>' );
	  if( !tagEnd )return false;
	  *offset += (tagEnd - cp) + nc +1;
	  tagEnd--;
	  if( *tagEnd == '/' )
	  {
	  	  if( isEmpty != NULL )
