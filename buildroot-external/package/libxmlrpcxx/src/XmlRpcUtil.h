...
namespace XmlRpc {
  ...
  class XMLRPC_DLLEXPORT XmlRpcUtil {
  public:
    // hokey xml parsing
    ...
    //! Returns true if the tag is found at the specified offset (modulo any whitespace)
    //! and updates offset to the char after the tag
	//! Sets *isEmpty to indicate if an empty tag was found. If isEmpty is NULL, an empty tag is not allowed.
    static bool nextTagIs(const char* tag, std::string const& xml, int* offset, bool* isEmpty);
