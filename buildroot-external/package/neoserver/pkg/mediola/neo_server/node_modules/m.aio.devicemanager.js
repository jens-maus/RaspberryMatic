...
m.aio.devicemanager._DeviceHandler={_findGateway:function(a,c){if(!a||!a.data)return null;a=a.data.gateways;if(!a)return null;for(var b=0;b<a.length;b++){var d=a[b];if(d&&d.index==c)return d}return null},_create:function(a,c,b){var d=m.aio.devicemanager.DMError,e=m.aio.devicemanager.DMError.ERROR_CODE,f=c.data.devices,g=m.aio.devicemanager._configmanager.LicenseManager,h=null;a.info&&a.info.gateway&&(h=this._findGateway(c,a.info.gateway));if(g.permitFeature("create",a.info,c.tenant.license,a.index,
f,h?h.info:null))if(a.name){if(a.index&&"string"===typeof a.index)if(a.index.match(/^[-]?\d+$/))a.index=parseInt(a.index);else{b(new d(e.INVALID,"index "+a.index+" invalid"));return}if(a.room){g=c.data.rooms;g||(c.data.rooms=[],g=c.data.rooms);h=!1;for(var k=0;k<g.length;k++)if(g[k].index==a.room){h=!0;break}if(h)if(a.info)if(a.info.sys)if(f||(c.data.devices=[],f=c.data.devices),g=require("m.aio.infomanager").getDeviceCount(a.info,c.tenant.license),0==g)b(Error("allow device count is 0"));else{if(-1!=
g){for(k=h=0;k<f.length;k++){var l=f[k];l.info&&l.info.sys==a.info.sys&&h++}if(h>=g){b(Error("reach maximum device count "+g));return}}g=1;for(h=0;h<f.length;h++){k=f[h];k.index===g&&g++;if(0<a.index&&k.index===a.index){b(new d(e.INDEX_EXISTS,"index "+a.index+" exists"));return}if(k.name===a.name&&k.room===a.room){b(new d(e.NAME_EXISTS,"name "+a.name+" exists"));return}}if(!a.index||0>=a.index)a.index=g;(f=c.dm.getModule(a.info.sys))?f.devicemanager?f.devicemanager.createDevice(a.info,c,function(d,
e){d?b(d):(a.info=e,c._insertIntoArray(a,c.data.devices),c._save(function(c){b(c,a)}))}):b(new d(e.INVALID,"sys "+a.info.sys+" do not have device manager implementation")):b(new d(e.UNKNOW,"sys "+a.info.sys+" unknow"))}else b(new d(e.INVALID,"sys "+a.info.sys+" invalid"));else b(new d(e.INVALID,"info "+a.info+" invalid"));else b(new d(e.INVALID,"room "+a.room+" do not exist"))}else b(new d(e.INVALID,"room "+a.room+" invalid"))}else b(new d(e.INVALID,"name "+a.name+" invalid"));else b(new d(e.NOT_PERMITTED,
...
c&&(e=c.findGatewayWithIndexIngnoreLicense(d.info.gateway));var f=c.dm.getModule(d.info.sys);f&&f.devicemanager&&f.devicemanager.applyIPEventFilter&&(e=f.devicemanager.applyIPEventFilter(d.info,a,c.deviceinfo,c,e?e.info:null))&&(d.info=e,b.push(d))}}):b=d;if(a&&a.ignoreLicense)return b;var e=[],f=m.aio.devicemanager._configmanager.LicenseManager,g=this;b.forEach(function(a){var b=null;a.info&&a.info.gateway&&(b=g._findGateway(c,a.info.gateway));var h=JSON.parse(JSON.stringify(a));f.permitFeature("read",
a.info,c.tenant.license,a.index,d,b?b.info:null)||(h._no_permission=!0);e.push(h)});return e},_update:function(a,c,b){var d=m.aio.devicemanager.DMError,e=m.aio.devicemanager.DMError.ERROR_CODE,f=c.data.devices;if(a.index){if("string"===typeof a.index)if(a.index.match(/^[-]?\d+$/))a.index=parseInt(a.index);else{b(new d(e.INVALID,"index "+a.index+" invalid"));return}if(a.name)if(a.room)if(a.info)if(a.info.sys){var g=null;a.info&&a.info.gateway&&(g=this._findGateway(c,a.info.gateway));var h=m.aio.devicemanager._configmanager.LicenseManager;
if(h.permitFeature("update",a.info,c.tenant.license,a.index,f,g?g.info:null)){var k,l=c.data.rooms,n;for(k=0;k<l.length;k++)a.room===l[k].index&&(n=l[k]);if(n){var q=-1;for(k=0;k<f.length;k++)if(a.index===f[k].index){var p=f[k];q=k}else if(f[k].name===a.name&&f[k].room===a.room){b(new d(e.NAME_EXISTS,"name "+a.name+" exists"));return}p?(p.info&&p.info.gateway&&(g=this._findGateway(c,p.info.gateway)),h.permitFeature("update",p.info,c.tenant.license,p.index,f,g?g.info:null)?(g=c.dm.getModule(a.info.sys))?
g.devicemanager?g.devicemanager.updateDevice(a.info,c,function(d,e){d?b(d):(a.info=e,f[q]=a,c._save(function(c){b(c,a)}))}):b(new d(e.INVALID,"sys "+a.info.sys+" do not have device manager implementation")):b(new d(e.UNKNOW,"sys "+a.info.sys+" unknow")):b(new d(e.NOT_PERMITTED,"sys "+p.info.sys+" not permitted"))):b(new d(e.NOT_FOUND,"no device has index "+a.index))}else b(new d(e.NOT_FOUND,"no room has index "+a.index))}else b(new d(e.NOT_PERMITTED,"sys "+a.info.sys+" not permitted"))}else b(new d(e.INVALID,
"sys "+a.info.sys+" invalid"));else b(new d(e.INVALID,"info "+a.info+" invalid"));else b(new d(e.INVALID,"room "+a.room+" invalid"));else b(new d(e.INVALID,"name "+a.name+" invalid"))}else b(new d(e.INVALID,"index "+a.index+" invalid"))},_delete:function(a,c,b){var d=m.aio.devicemanager.DMError,e=m.aio.devicemanager.DMError.ERROR_CODE;if(a){if(a&&"string"===typeof a)if(a.match(/^[-]?\d+$/))a=parseInt(a);else{b(new d(e.INVALID,"index "+a+" invalid"));return}for(var f=c.data.devices,g,h,k=0;k<f.length;k++)a===
f[k].index&&(g=f[k],h=k);g?(a=null,g.info&&g.info.gateway&&(a=this._findGateway(c,g.info.gateway)),m.aio.devicemanager._configmanager.LicenseManager.permitFeature("delete",g.info,c.tenant.license,g.index,f,a?a.info:null)?(f=c.dm.getModule(g.info.sys))?f.devicemanager?f.devicemanager.deleteDevice(g.info,c,function(a){a?b(a):(c.data.devices.splice(h,1),c._save(function(a){b(a,g)}))}):b(new d(e.INVALID,"sys "+g.info.sys+" do not have device manager implementation")):b(new d(e.UNKNOW,"sys "+g.info.sys+
" unknow")):b(new d(e.NOT_PERMITTED,"sys "+g.info.sys+" not permitted"))):b(new d(e.NOT_FOUND,"no device has index "+a))}else b(new d(e.INVALID,"index "+a+" invalid"))},_deleteUnlicensed:function(a,c,b){var d=[],e=c.devices;if(e){for(var f=m.aio.devicemanager._configmanager.LicenseManager,g=0;g<e.length;g++){var h=e[g],k=null;h.info&&h.info.gateway&&(k=this._findGateway(a,h.info.gateway));f.permitFeature("add",h.info,a.tenant.license,h.index,e,k?k.info:null)&&d.push(h)}c.devices=d}b&&b()}};
...
a.properties?(c.forEach(function(c){a.match(c)&&b.push(c)}),b):c},_update:function(a,c,b){var d=m.aio.devicemanager.DMError,e=m.aio.devicemanager.DMError.ERROR_CODE;if(a.index){if("string"===typeof a.index)if(a.index.match(/^[-]?\d+$/))a.index=parseInt(a.index);else{b(new d(e.INVALID,"index "+a.index+" invalid"));return}if(a.name){var f,g=c.data.rooms;for(f=0;f<g.length;f++)if(a.index===g[f].index)var h=g[f];else if(a.name===g[f].name){b(new d(e.NAME_EXISTS,"name "+a.name+" exists"));return}h?(d=
h.name!==a.name,a.info&&"object"===typeof a.info?(d=!0,h.info=a.info):h.info&&(d=!0,delete h.info),d?(h.name=a.name,c._save(function(a){b(a,h)})):b(null,h)):b(new d(e.NOT_FOUND,"no room has index "+a.index))}else b(new d(e.INVALID,"name "+a.name+" invalid"))}else b(new d(e.INVALID,"index "+a.index+" invalid"))},_delete:function(a,c,b){var d=m.aio.devicemanager.DMError,e=m.aio.devicemanager.DMError.ERROR_CODE;if(a){if(a&&"string"===typeof a)if(a.match(/^[-]?\d+$/))a=parseInt(a);else{b(new d(e.INVALID,
"index "+a+" invalid"));return}var f=c.data.rooms,g=-1,h;for(h=0;h<f.length;h++)if(a===f[h].index){var k=f[h];g=h}if(k){d=c.data.devices;for(h=0;h<d.length;h++)a===d[h].room&&(d[h].room=0);0<=g&&(c.data.rooms.splice(g,1),c._save(function(a){b(a,k)}))}else b(new d(e.NOT_FOUND,"no room has index "+a))}else b(new d(e.INVALID,"index "+a+" invalid"))}};
...
new m.aio.devicemanager.IntHandler(e,a,{devices:[],rooms:[],gateways:[]});this.handlers.push(h);a=g+f.sep+m.aio.devicemanager.DeviceManager.DB;var k=g+f.sep+m.aio.devicemanager.DeviceManager.DEVICE_INFO,l=require("fs-extra"),n=!1;l.existsSync(a+".enc")&&(a+=".enc",n=!0);var q=function(){d(h,k,function(){c&&c()})};(function(a,b,c){var d=require("fs");d.exists(b,function(f){f?d.readFile(b,{encoding:"utf8"},function(d,f){if(!d&&0<f.length)try{String(b).endsWith(".enc")&&(f=require("x.crypt").AES.decodeString(f,
e.ml_v1_info()));var g=JSON.parse(f);g&&(a.data=g,a.data.devices||(a.data.devices=[]),a._sort(a.data.devices),a.data.gateways||(a.data.gateways=[]),a._sort(a.data.gateways),a.data.rooms||(a.data.rooms=[]),a._sort(a.data.rooms))}catch(t){console.error(t)}c()}):d.writeFile(b,"",function(){c()})})})(h,a,function(){n?q():h._save(function(){var a=f.join(g,"cloud"),b=f.join(g,"automation"),c=f.join(g,"remotes");if(l.existsSync(a))try{l.remove(a)}catch(r){console.error(r)}if(l.existsSync(b))try{var d=l.readdirSync(b);
for(a=0;a<d.length;a++){var e=d[a];if(l.statSync(f.join(b,e)).isDirectory()){var h=f.join(b,e,"db");l.existsSync(h)&&l.remove(h)}}}catch(r){console.error(r)}if(l.existsSync(c))try{var k=l.readdirSync(c);for(b=0;b<k.length;b++){var n=k[b];if(l.statSync(f.join(c,n)).isDirectory()){var t=f.join(c,n,"device_db");l.existsSync(t)&&l.remove(t)}}}catch(r){console.error(r)}q()})})}else c()};
...
m.aio.devicemanager.DeviceManager.prototype.applySmartWidgetFilter=function(a,c,b,d){var e=this.getIntHandler(a);if(e)if(a=e.findDeviceWithIndex(c))if(a.info){var f=e.findRoomWithIndex(a.room),g=null;f&&(g=f.name);f=null;a.info.gateway&&e&&(f=e.findGatewayWithIndex(a.info.gateway));var h=require("m.aio.smartwidget.js");e=JSON.parse(JSON.stringify(a));if(c=h.applySmartWidgetFilter(a.info,f?f.info:null,b,{index:c,name:a.name,room:g}))e.smartwidget=c;d&&d(null,e)}else d&&d(null,[]);else c=Error("can not read device with id:"+
