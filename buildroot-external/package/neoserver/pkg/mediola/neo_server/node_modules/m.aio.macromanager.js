...
m.aio.macromanager.MacroCommand.parse=function(a){if(!a||!a.classid)return null;var b=null;switch(a.classid){case m.aio.macromanager.Command.CLASS_ID:b=m.aio.macromanager.Command.parse(a);break;case m.aio.macromanager.Navigate.CLASS_ID:b=m.aio.macromanager.Navigate.parse(a);break;case m.aio.macromanager.Sleep.CLASS_ID:b=m.aio.macromanager.Sleep.parse(a);break;default:return null}b&&(b.index=a.index);return b};
...
m.aio.macromanager.Navigate.parse=function(a){if(!a||!a.classid||a.classid!=m.aio.macromanager.Navigate.CLASS_ID)return null;var b=new m.aio.macromanager.Navigate;b.page=a.page;"number"===typeof a.history&&(b.history=a.history);return b};m.aio.macromanager.Navigate.prototype.toJSON=function(){var a=m.aio.macromanager.MacroCommand.prototype.toJSON.call(this);a.page=this.page;"number"===typeof this.history&&(a.history=this.history);return a};
...
m.aio.macromanager.Handler.prototype={execute:function(a,b,d,c,e){if(b=m.aio.macromanager.Path.build(b))switch(a){case "create":this._create(b,d,c,function(a,b){var c=this._toJSON(b);e&&e(a,c)}.bind(this));break;case "read":this._read(b,d,c,function(a,b){var c=this._toJSON(b);e&&e(a,c)}.bind(this));break;case "update":this._update(b,d,c,function(a,b){var c=this._toJSON(b);e&&e(a,c)}.bind(this));break;case "delete":this._delete(b,d,c,function(a,b){var c=this._toJSON(b);e&&e(a,c)}.bind(this));break;
default:e&&e(Error("invalid operation"))}else e&&e(Error("invalid path"))},findMacro:function(a,b,d){this.macros.findMacro(a,b,d)},findMacroIdx:function(a,b,d){this.macros.findMacroIdx(a,b,d)},_create:function(a,b,d,c){this.macros._create(a,b,d,function(a,b){a?c&&c(a):this._saveMacros(function(a){c&&c(a,a?null:b)})}.bind(this))},_read:function(a,b,d,c){this.macros._read(a,b,d,c)},_update:function(a,b,d,c){this.macros._update(a,b,d,function(a,b){a?c&&c(a):this._saveMacros(function(a){c&&c(a,a?null:
b)})}.bind(this))},_delete:function(a,b,d,c){this.macros._delete(a,b,d,function(a,b){a?c&&c(a):this._saveMacros(function(a){c&&c(a,a?null:b)})}.bind(this))},_saveMacros:function(a){this.runningTask?this.waitingTask?this.waitingTask.addCallback(a):this.waitingTask=new m.aio.macromanager.Handler.SaveMacrosTask(this,a):(this.runningTask=new m.aio.macromanager.Handler.SaveMacrosTask(this,a),this.runningTask.save())},_loadMacros:function(a){if(this.tenant){var b=this.tenant.getFolderPath();if(b){var d=
require("fs"),c=require("path"),b=b+c.sep+m.aio.macromanager.MacroManager.DB,e=this;this._fileExistsSync(b)?d.readFile(b,{encoding:"utf8"},function(b,c){if(!b&&c){var d;try{d=JSON.parse(c)}catch(e){}if(d=m.aio.macromanager.Macros.parse(d))this.macros=d}a&&a(b)}.bind(this)):d.writeFile(b,"",function(){e._loadMacros(a)})}else a&&a(Error("tenant folder invalid"))}else a&&a(Error("tenant is null"))},_saveMacrosFinish:function(){this.runningTask=null;this.waitingTask&&(this.runningTask=this.waitingTask,
this.waitingTask=null,this.runningTask.save())},_fileExistsSync:function(a){var b=require("fs");try{b.statSync(a)}catch(d){if("ENOENT"==d.code)return!1}return!0},_toJSON:function(a){if("undefined"==typeof a||null==a)return null;if(a.toJSON&&"function"==typeof a.toJSON)return a.toJSON();if(Array.isArray(a)){var b=[];a.forEach(function(a){b.push(this._toJSON(a))}.bind(this));return b}return a}};m.aio.macromanager.Handler.SaveMacrosTask=function(a,b){this._handler=a;this._callbacks=[];b&&this._callbacks.push(b)};
...
m.aio.macromanager.MacroManager.prototype={init:function(a,b,d){a&&a.children?(this._configmanager=b,require("async").each(a.children,function(a,b){var d=new m.aio.macromanager.Handler(a);this.handlers.push(d);d._loadMacros(function(){b()})}.bind(this),function(a){d&&d(a)})):d&&d()},addTenant:function(a,b){var d=new m.aio.macromanager.Handler(a);this.handlers.push(d);d._loadMacros(function(a){b&&b()})},removeTenant:function(a,b){this._removeHandler(a.index);b&&b()},execute:function(a,b,d,c,e,f){this._configmanager.trace("config/tenants/"+
a,function(k,g){if(k)g||this._removeHandler(a),f&&f(Error("no tenant with index "+a));else if(g.isAccessible()){var h=this._getHandler(a);h?h.execute(b,d,c,e,f):this.addTenant(g,function(a){a?f&&f(a):h.execute(b,d,c,e,f)})}else f&&f(Error("not permitted"))}.bind(this))},saveRemote:function(a,b){if(a.parent){var d=a.parent.parent;if(d)if(d=d.getFolderPath()){var c=a.getFolderPath(),e=require("fs"),f=require("fs-extra"),k=require("path"),g=d+k.sep+m.aio.macromanager.MacroManager.DB,h=c+k.sep+m.aio.macromanager.MacroManager.DB;
e.exists(g,function(a){a?f.copy(g,h,function(a){b&&b(a)}):b&&b()})}else b&&b(Error("invalid tenant folder"));else b&&b(Error("remote do not belong to any tenant"))}else b&&b(Error("remote is isolated"))},reloadTenant:function(a,b){for(var d=-1,c=0;c<this.handlers.length;c++){var e=this.handlers[c];if(e&&e.tenant&&e.tenant.index==a.index){d=c;break}}-1!=d&&this.handlers.splice(d,1);d=new m.aio.macromanager.Handler(a);this.handlers.push(d);d._loadMacros(function(){b()})},_getHandler:function(a){for(var b=
