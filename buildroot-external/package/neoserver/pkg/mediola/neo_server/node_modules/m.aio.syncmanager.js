...
m.aio.syncmanager.RemotesDownloader.prototype._doDownloadRemotes=function(d,b,c,a){var e=new m.aio.syncmanager.RemoteDownloader({host:this._host,port:this._port,username:this._username,password:this._password,credential:b}),f=new function(){this.idx=0;this.cb=a;this._onFinish=function(a){a?this.cb&&this.cb(a):(this.idx+=1,this.idx==c.length?this.cb&&this.cb():e.download(d,c[idx],function(a){f._onFinish.call(f,a)}))}};e.download(d,c[0],function(a){f._onFinish.call(f,a)})};
...
m.aio.syncmanager.RemoteDownloader.prototype.download=function(d,b,c){this._logger.log("trace","downlaod remote:"+b.name);var a=this,e=require("path");if(b.name){if(this._listener&&this._listener.onProgress)this._listener.onDownloadProgress(0,"start download");if(b.resources&&0!=b.resources.length){if(b.encrypt){this._encrypted=!0;if(!this._remotePassword){if(c){var f=Error("no remote password");f.code=1;c(f)}return}if(this._decrypt(b.encrypt,this._remotePassword)!=b.name){c&&(f=Error("remote password error"),
...
m.aio.syncmanager.RemoteDownloader.prototype._downloadFile=function(d,b,c){var a=this;this._logger.log("trace","downlaod file:"+b.file);b.file?b.file.match(/^resources.*/)&&this._s3cred?this._downloadFileFromS3(d,b,function(e){a._logger.log("trace","downlaod file finish:"+b.file);e&&a._logger.log("trace","downlaod file "+b.file+" from s3 error:"+e.message);c&&c(e)}):this._downloadFileFromServer(d,b,function(e){a._logger.log("trace","downlaod file finish:"+b.file);if(e)a._logger.log("trace","downlaod file "+
b.file+" from server error:"+e.message),c&&c(e);else if("device_db"==b.file&&a._encrypted){e=require("path");var f=b.file.replace(/\//g,e.sep);a._decryptFile(d+e.sep+f,function(a){c&&c(a)})}else c&&c()}):c&&c(Error("file info has no file path"))};
m.aio.syncmanager.RemoteDownloader.prototype._downloadFileFromServer=function(d,b,c){if(b.ref){var a=require("path"),e=require("fs"),f=b.file.replace(/\//g,a.sep),g=d+a.sep+f;this._logger.log("trace","downlaod file to:"+g);d=g.lastIndexOf(a.sep);d=g.substr(0,d);try{e.mkdirRecursiveSync(d)}catch(p){c&&c(p);return}d=require(m.aio.syncmanager.RemoteDownloader.PROTOCOL);a=this._host?this._host:m.aio.syncmanager.RemoteDownloader.OPTIONS.host;f=this._port?this._port:m.aio.syncmanager.RemoteDownloader.OPTIONS.port;
var h=m.aio.syncmanager.RemoteDownloader.OPTIONS.path+"/creatorneo/remote/downloadFile";b=b.ref.split("/");for(var k=0;k<b.length;k++)b[k]=encodeURI(b[k]);b=m.aio.syncmanager.RemoteDownloader.PROTOCOL+"://"+a+":"+f+h+"/"+b.join("/");a=this._username?encodeURIComponent(this._username):"";f=this._password?encodeURIComponent(this._password):"";b+="?username="+a+"&password="+f;this._logger.log("trace","downlaod file url:"+b);var l=e.createWriteStream(g);d.get(b,function(a){a.pipe(l);l.on("finish",function(){l.close(c)})}).on("error",
function(a){e.unlink(g);c&&c(a)})}else c&&c(Error("file info has no ref"))};
m.aio.syncmanager.RemoteDownloader.prototype._downloadFileFromS3=function(d,b,c){if(this._s3Client)if(b.ref){var a=require("path"),e=require("fs"),f=b.file.replace(/\//g,a.sep),g=d+a.sep+f;this._logger.log("trace","downlaod file to:"+g);d=g.lastIndexOf(a.sep);d=g.substr(0,d);try{e.mkdirRecursiveSync(d)}catch(h){c&&c(h);return}this._s3Client.getObject({Bucket:"mediola-creator",Key:"resources/"+b.ref},function(a,b){a?c&&c(a):e.writeFile(g,b.Body,function(a){c&&c(a)})})}else c&&c(Error("file info has no ref"));
...
m.aio.syncmanager.RemoteUploader.prototype._doRemoteUploadReq=function(d,b,c,a){var e=this,f=new m.aio.syncmanager.Assembler;if(this._listener&&this._listener.onProgress)this._listener.onProgress(1,"assemble files");f.generateRemoteUploadReq(b,c,function(c,b){if(e._listener&&e._listener.onProgress)e._listener.onProgress(2,"inform server for upcoming upload");e._doPostRequest("/creatorneo/remote/requestUpload?license="+d,null,JSON.stringify(b),function(c,b){if(c)a&&a(c);else try{var e=JSON.parse(b);
a&&a(null,e)}catch(n){a&&a(n)}})})};
...
m.aio.syncmanager.Assembler.prototype={generateRemoteDownloadRsp:function(d,b,c){var a=this;this._getRemoteDatabaseInfo(b,function(e,f){e?c&&c(e):(f&&f.forEach(function(a){a.file&&(a.ref="tenant/"+d.index+"/remote/"+b.index+"/"+a.file)}),a._getRemoteInfo(b,function(a,e){a?c&&c(a):(e.resources&&e.resources.forEach(function(a){a.file&&"device_db"!=a.file&&"macros_db"!=a.file&&"deviceinfo.xml"!=a.file&&(a.ref="tenant/"+d.index+"/remote/"+b.index+"/"+a.file,f.push(a))}),e.resources=f,c&&c(null,e))}))})},
generateRemoteUploadReq:function(d,b,c){this.generateRemoteDownloadRsp(d,b,c)},_getRemoteInfo:function(d,b){var c=require("unorm"),a=d.getFolderPath(),e={name:c.nfc(d.id),resources:[]};this._getFilesInfos(a,function(a,c){a?b&&b(a):(c&&(e.resources=c),b&&b(null,e))})},_getRemoteDatabaseInfo:function(d,b){var c=this,a=d.getFolderPath(),e=require("async"),f=require("fs");e.parallel([function(b){var e=d.getDeviceFile();e?f.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()},function(b){var e=
d.getMacrosFile();e?f.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()},function(b){var e=d.getDeviceinfoFile();e?f.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()}],function(a,c){var e=[];c&&c.forEach(function(a){a&&e.push(a)});b&&b(a,e)})},_getTenantDatabaseInfo:function(d,b){var c=this,a=d.getFolderPath(),e=require("async"),f=require("fs");e.parallel([function(b){var e=d.getDeviceFile();e?f.exists(e,function(d){d?c._getFileInfos(e,a,function(a,
c){b(a,c)}):b()}):b()},function(b){var e=d.getMacrosFile();e?f.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()},function(b){var e=d.getDeviceinfoFile();e?f.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()}],function(a,c){var e=[];c&&c.forEach(function(a){a&&e.push(a)});b&&b(a,e)})},_getFilesInfos:function(d,b){var c=[],a=require("node-dir"),e=require("async"),f=require("path"),g=this;a.files(d,function(a,k){a?b&&b(a):e.eachLimit(k,50,function(a,
b){if(0==f.basename(a).indexOf("."))b();else{var e=a.replace(d,"");"controls"==e.substr(1,8)&&".p"!=e.substr(e.length-2)?b():"resources_"==e.substr(1,10)?b():g._getFileInfos(a,d,function(a,e){a?b(a):(e&&c.push(e),b())})}},function(a){b&&b(a,c)})})},_getFileInfos:function(d,b,c){var a=require("fs"),e=require("path"),f=require("unorm");b=e.relative(b,d);b=f.nfc(b);d=f.nfc(d);"\\"==e.sep&&(b=b.replace(/\\/g,"/"));e=e.basename(d);var g={file:b,name:e},h=this;a.stat(d,function(a,b){a?c(a):b.isFile()?(g.size=
b.size,h._md5(d,function(a,b){a?c(a):(g.md5=b,h._sha1(d,function(a,b){a?c(a):(g.sha1=b,c(null,g))}))})):c(Error(d+" is not a file"))})},_md5:function(d,b){var c=require("fs"),a=require("crypto").createHash("md5"),e=c.createReadStream(d);e.on("readable",function(){for(var c;null!==(c=e.read());)a.update(c)}).on("end",function(){b(null,a.digest("base64"))})},_sha1:function(d,b){var c=require("fs"),a=require("crypto").createHash("sha1"),e=c.createReadStream(d);e.on("readable",function(){for(var c;null!==
...
"zip the user database.");var e=require("path"),d=b.getDeviceFile(),g=b.getMacrosFile(),h=b.getDeviceinfoFile(),k=b.getFolderPath()+e.sep+"automation",l=require("m.aio.filemanager.js"),p=require("fs"),n=Math.round((new Date).getTime()/1E3),r=0;l=l.getTmpDir();for(var q=e.join(l,n+".zip");a._fileExistsSync(q);){r++;n++;if(10<r){c&&c(Error("can not create temp file"));return}q=e.join(l,n+".zip")}a._logger.log("trace","zip file:"+q);n=require("archiver");e=p.createWriteStream(q);n=n("zip");e.on("error",
function(b){a._logger.log("warn","create user database error:"+b.message);c&&(c(b),c=null)});e.on("close",function(){c&&(c(null,q),c=null)});n.pipe(e);a._fileExistsSync(d)&&n.append(p.createReadStream(d),{name:"device_db"});a._fileExistsSync(g)&&n.append(p.createReadStream(g),{name:"macros_db"});a._fileExistsSync(h)&&n.append(p.createReadStream(h),{name:"deviceinfo.xml"});a._fileExistsSync(h)&&n.append(p.createReadStream(h),{name:"deviceinfo.xml"});if(a._fileExistsSync(k))if(p.statSync(k).isDirectory())n.directory(k,
"automation");else{d=Error("Automation folder is not a directory: "+k);a._logger.log("error",d.message);c&&c(d);c=null;return}n.finalize()})};d.prototype._requestUpload=function(b){var c=this;this._reportProgress(1,"send upload request",function(){c._logger.log("trace","send upload user database request.");c._doRequest("POST","/creatorneo/userdb/requestUpload",null,null,function(a,c){if(c)try{c=JSON.parse(c)}catch(f){a=f,a.code=103}a||(c&&"success"==c.status?c.result&&c.result.ref&&c.result.credential||
...
d);try{require("fs-extra").ensureDirSync(d)}catch(l){l.code=201;c&&c(l);return}f=new (require("adm-zip"))(b);try{f.extractAllTo(d,!0),c&&c(null,d)}catch(l){l.code=202,c&&c(l)}})};d.prototype._reloadUserDB=function(b,c,a){var d=this;this._reportProgress(3,"load user database",function(){var e=require("path"),g=c.getDeviceFile(),h=c.getMacrosFile(),k=c.getDeviceinfoFile(),l=e.join(c.getFolderPath(),"automation"),p=e.join(b,"device_db"),n=e.join(b,"macros_db"),r=e.join(b,"deviceinfo.xml");e=e.join(b,
"automation");d._copyFolder(e,l,function(b){b?(b.code=303,a&&a(b)):d._copyFile(r,k,function(b){b?(b.code=301,a&&a(b)):d._copyFile(n,h,function(b){b?(b.code=302,a&&a(b)):d._copyFile(p,g,function(b){if(b)b.code=302,a&&a(b);else{var d=require("m.aio.devicemanager.js");require("m.aio.macromanager.js").reloadTenant(c,function(){d.reloadTenant(c,function(){a&&a()})})}})})})})})};d.prototype._reportProgress=function(b,c,a){if(this._listener&&this._listener.onProgress)this._listener.onProgress(b,c,null,a);
else a&&a()};d.prototype._fileExistsSync=function(b){var c=require("fs");try{c.statSync(b)}catch(a){if("ENOENT"==a.code)return!1}return!0};d.prototype._copyFile=function(b,c,a){var d=require("fs");this._fileExistsSync(b)?d.readFile(b,function(b,e){b?a&&a(b):d.writeFile(c,e,function(b){a&&a(b)})}):d.writeFile(c,"",function(b){a&&a(b)})};d.prototype._copyFolder=function(b,c,a){var d=require("fs-extra"),f=this;d.remove(c,function(e){e?a&&a(e):f._fileExistsSync(b)?d.move(b,c,{overwrite:!0},function(b){a&&
a(b)}):a&&a()})};d.prototype._doRequest=function(b,c,a,d,f){var e=f;f=require(m.aio.syncmanager.PROTOCOL);var h={host:m.aio.syncmanager.HOST,port:m.aio.syncmanager.PORT,path:m.aio.syncmanager.PATH,headers:m.aio.syncmanager.HEADERS};h.path+=c;h.method=b;this._username&&this._password&&(h.auth=this._username+":"+this._password);a&&Object.keys(a).forEach(function(b){h.headers[b]=a[b]});var k="";b=f.request(h,function(a){a.setEncoding("utf8");a.on("data",function(a){k+=a});a.on("end",function(){200==
a.statusCode?e&&e(null,k):e&&e(Error("http error with status code:"+a.statusCode),k);e=null})});b.on("error",function(a){e&&(e(a),e=null)});d&&b.write(d);b.end()};return d}();m.aio.syncmanager.SyncManager=function(){this._configmanager=null};
...
"http");a&&a()},uploadRemote:function(d,b,c,a,e){if(c&&c.username&&c.password){var f=this;this._configmanager.trace("config/tenants/"+d,function(g,h){g?e&&e(g):h.license&&h.license.licenseTxt?f._configmanager.trace("config/tenants/"+d+"/remotes/"+b,function(b,d){b?e&&e(b):require("m.aio.devicemanager.js").saveRemote(d,function(b){b?e&&e(b):(new m.aio.syncmanager.RemoteUploader(c,a)).upload(h.license.licenseTxt,h,d,function(a){e&&e(a)})})}):e&&e(Error("no license"))})}else e&&e(Error("options format invalid"))},
downloadRemote:function(d,b,c,a,e,f){a&&a.username&&a.password?this._configmanager.trace("config/tenants/"+d,function(d,h){if(d)f&&f(d);else if(h.license&&h.license.licenseTxt)if(h.isAccessible())if(d=(new m.aio.syncmanager.RemotesDownloader)._findRemote(b,c)){a.credential=c.credential;var g=h.getRemotes().getFolderPath();(new m.aio.syncmanager.RemoteDownloader(a,e)).download(g,d,function(a){a?f&&f(a):setTimeout(function(){h.reloadRemotes(function(a){f&&f(a)})},100)})}else f&&f(Error("no such remote:"+
b));else f&&f(Error("not permitted"));else f&&f(Error("no license"))}):f&&f(Error("options format invalid"))},downloadRemoteList:function(d,b,c){(new m.aio.syncmanager.RemotesDownloader(null,null,d,b)).downloadRemoteList(c)},testRemotePassword:function(d,b,c,a){if(d)if(b)if(c){var e=new m.aio.syncmanager.RemotesDownloader;(c=e._findRemote(d,c))?c.encrypt?(b=e._decrypt(c.encrypt,b),a&&a(null,b==d)):a&&a(null,!0):a&&a(Error("no such remote:"+d))}else a&&a(Error("info is null"));else a&&a(Error("remote password is null"));
else a&&a(Error("remote name is null"))},requestDownload:function(d){var b=this;this._configmanager.trace("config/tenants",function(c,a){if(c)d&&d(c);else{var e=null;a.getChildren()?(a.getChildren().forEach(function(a){a.isDefault()&&(e=a)}),e&&e.license&&!e.license.isTestLicense()&&e.license.valid?b.requestDownloadTenant(e.index,d):e&&e.license&&e.license.isTestLicense()?(c=Error("can not download from test license"),c.code=400,d&&d(c,null)):d&&d(null,{infos:[]})):d&&d(null,{infos:[]})}})},requestDownloadTenant:function(d,
b){this._configmanager.trace("config/tenants/"+d,function(c,a){if(c)b&&b(c);else if(a.isAccessible()){var e={infos:[]},f=a.getRemotes();f&&0!=f.size()?require("m.aio.devicemanager.js").deleteUnlicensed(d,function(c){if(c)b&&b(c);else{var d=0,g=new m.aio.syncmanager.Assembler;f.getChildren().forEach(function(c){g.generateRemoteDownloadRsp(a,c,function(a,c){d+=1;a||e.infos.push(c);d==f.size()&&b&&b(null,e)})})}}):b&&b(null,e)}else b&&b(Error("not permitted"))})},getTenantFile:function(d,b,c){d&&b?this._configmanager.trace("config/tenants/"+
d,function(a,d){a?c&&c(a):(a=require("path"),b=b.replace(/\//g,a.sep),(d=d.getFolderPath())?c&&c(null,d+a.sep+b):c&&c(Error("tenant has no folder")))}):c&&c(Error("parameter invalid"))},getRemoteFile:function(d,b,c,a){d&&b&&c?this._configmanager.trace("config/tenants/"+d+"/remotes/"+b,function(b,d){b?a&&a(b):(b=require("path"),c=c.replace(/\//g,b.sep),(d=d.getFolderPath())?a&&a(null,d+b.sep+c):a&&a(Error("remote has no folder")))}):a&&a(Error("parameter invalid"))},uploadUserDB:function(d,b,c,a){if(b&&
b.username&&b.password)this._configmanager.trace("config/tenants/"+d,function(d,f){if(d){if(c&&c.onError)c.onError({error:d.message,error_code:100});a&&a(d)}else if(f.license&&f.license.licenseTxt)(new m.aio.syncmanager.UserDBUploader(b,c)).upload(f,function(b){a&&a(b)});else{if(c&&c.onError)c.onError({error:"no license",error_code:100});a&&a(Error("no license"))}});else{if(c&&c.onError)c.onError({error:"options format invalid",error_code:100});a&&a(Error("options format invalid"))}},downloadUserDB:function(d,
b,c,a){if(b&&b.username&&b.password)this._configmanager.trace("config/tenants/"+d,function(d,f){if(d){if(c&&c.onError)c.onError({error:d.message,error_code:100});a&&a(d)}else if(f.license&&f.license.licenseTxt)(new m.aio.syncmanager.UserDBDownloader(b,c)).download(f,function(b){a&&a(b)});else{if(c&&c.onError)c.onError({error:"no license",error_code:100});a&&a(Error("no license"))}});else{if(c&&c.onError)c.onError({error:"options format invalid",error_code:100});a&&a(Error("options format invalid"))}},
_downloadRemotes:function(d,b,c){b&&b.host&&b.port?(new m.aio.syncmanager.RemotesDownloader(b.host,b.port,b.username,b.password)).download(d,function(a){c&&c(a)}):c&&c(Error("options format invalid"))}};m.aio.syncmanager.Handler=m.aio.syncmanager.SyncManager;m.aio.syncmanager.Handler.prototype.RemotesDownloader=m.aio.syncmanager.RemotesDownloader;m.aio.syncmanager.Handler.prototype.RemoteDownloader=m.aio.syncmanager.RemoteDownloader;"undefined"!=typeof module&&null!=module&&(module.exports=new m.aio.syncmanager.Handler);
