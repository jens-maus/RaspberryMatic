...
$jscomp.Symbol=function(){function d(c){if(this instanceof d)throw new TypeError("Symbol is not a constructor");return new $jscomp.SymbolClass($jscomp.SYMBOL_PREFIX+(c||"")+"_"+b++,c)}var b=0;return d}();
...
$jscomp.iteratorFromArray=function(d,b){$jscomp.initSymbolIterator();d instanceof String&&(d+="");var c=0,a={next:function(){if(c<d.length){var e=c++;return{value:b(e,d[e]),done:!1}}a.next=function(){return{done:!0,value:void 0}};return a.next()}};a[Symbol.iterator]=function(){return a};return a};
$jscomp.polyfill=function(d,b,c,a){if(b){c=$jscomp.global;d=d.split(".");for(a=0;a<d.length-1;a++){var e=d[a];e in c||(c[e]={});c=c[e]}d=d[d.length-1];a=c[d];b=b(a);b!=a&&null!=b&&$jscomp.defineProperty(c,d,{configurable:!0,writable:!0,value:b})}};$jscomp.polyfill("Array.prototype.keys",function(d){return d?d:function(){return $jscomp.iteratorFromArray(this,function(b){return b})}},"es6","es3");var m=m||{};m.aio=m.aio||{};m.aio.syncmanager=m.aio.syncmanager||{};
var async=require("async"),crypto=require("crypto"),fs=require("fs"),path=require("path"),unorm=require("unorm");m.aio.syncmanager.PROTOCOL="https";m.aio.syncmanager.HOST="127.0.0.1";m.aio.syncmanager.PORT=8080;m.aio.syncmanager.PATH="";m.aio.syncmanager.HEADERS={"Content-Type":'application/json; charset="UTF-8"',"User-Agent":"mediola",Connection:"close"};
...
f.code=2,c(f));return}}var g=require("fs"),h=d+e.sep+b.name;try{var k=g.existsSync(h);if(k&&!this._override){c&&(f=Error("remote:"+b.name+" already exists"),f.code=3,c(f));return}if(k&&this._override){var l=(new Date).getTime();g.renameSync(h,h+"_"+l);this._deldir(h+"_"+l)}}catch(n){c&&c(n);return}if(this._listener&&this._listener.onProgress)this._listener.onDownloadProgress(1,"start to download files",{filesCount:b.resources.length});this._mkdir(d+e.sep+b.name,function(){var f=c,g=0,h=function(c){if(f&&
...
d,null,JSON.stringify(b),function(c,b){if(c)a&&a(c);else try{var e=JSON.parse(b);a&&a(null,e)}catch(p){a&&a(p)}})}})};
...
generateRemoteUploadReq:function(d,b,c){this.generateRemoteDownloadRsp(d,b,"from_upload_req",c)},_getRemoteInfo:function(d,b){var c=require("unorm"),a=d.getFolderPath(),e={name:c.nfc(d.id),resources:[]};this._getFilesInfos(a,function(a,c){a?b&&b(a):(c&&(e.resources=c),b&&b(null,e))})},_getRemoteDatabaseInfo:function(d,b,c){var a=this,e=d.getFolderPath(),f=require("fs-extra");async.parallel([function(c){var g=d.getDeviceFile();g?fs.exists(g,function(d){d?a._getFileInfos(g,e,function(a,e){if("from_download"==
b)try{f.removeSync(g)}catch(p){}c(a,e)}):c()}):c()},function(c){var b=d.getMacrosFile();b?fs.exists(b,function(d){d?a._getFileInfos(b,e,function(a,b){c(a,b)}):c()}):c()},function(c){var b=d.getDeviceinfoFile();b?fs.exists(b,function(d){d?a._getFileInfos(b,e,function(a,b){c(a,b)}):c()}):c()}],function(a,b){var e=[];b&&b.forEach(function(a){a&&e.push(a)});c&&c(a,e)})},_getTenantDatabaseInfo:function(d,b){var c=this,a=d.getFolderPath();async.parallel([function(b){var e=d.getDeviceFile();e?fs.exists(e,
function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()},function(b){var e=d.getMacrosFile();e?fs.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()},function(b){var e=d.getDeviceinfoFile();e?fs.exists(e,function(d){d?c._getFileInfos(e,a,function(a,c){b(a,c)}):b()}):b()}],function(a,c){var e=[];c&&c.forEach(function(a){a&&e.push(a)});b&&b(a,e)})},_listAllFiles:function(d,b){var c=[],a=this,e=function(f,g){if(g>=f.length)b(null,c);else{var h=path.join(d,f[g]);fs.stat(h,
function(d,l){d?b(d,c):l.isDirectory()?a._listAllFiles(h,function(a,d){a?b(a,c):(Array.isArray(d)&&(c=c.concat(d)),e(f,g+1))}):(c.push(h),e(f,g+1))})}};fs.readdir(d,function(a,d){a?b&&b(a,c):e(d,0)})},_getFilesInfos:function(d,b){var c=[],a=require("m.aio.configmanager.js").Page.FILE_EXT,e=this;this._listAllFiles(d,function(f,g){f?b&&b(f):async.eachLimit(g,50,function(b,f){if(0==path.basename(b).indexOf("."))f();else{var g=b.replace(d,""),h=path.extname(g);"controls"==g.substr(1,8)&&h!==a.PAGE&&h!==
a.POPUP?f():"resources_"==g.substr(1,10)?f():e._getFileInfos(b,d,function(a,b){a?f(a):(b&&c.push(b),f())})}},function(a){b&&b(a,c)})})},_getFileInfos:function(d,b,c){b=path.relative(b,d);b=unorm.nfc(b);d=unorm.nfc(d);"\\"==path.sep&&(b=b.replace(/\\/g,"/"));var a=path.basename(d),e={file:b,name:a},f=this;fs.stat(d,function(a,b){a?c(a):b.isFile()?(e.size=b.size,f._md5AndSha1(d,function(a,b){a?c(a):(e.md5=b.md5,e.sha1=b.sha1,c(null,e))})):c(Error(d+" is not a file"))})},_md5AndSha1:function(d,b){var c=
crypto.createHash("md5"),a=crypto.createHash("sha1"),e=fs.createReadStream(d);e.on("readable",function(){for(var b;null!==(b=e.read());)c.update(b),a.update(b)}).on("end",function(){var e={md5:c.digest("base64"),sha1:a.digest("base64")};b(null,e)})},_md5:function(d,b){var c=crypto.createHash("md5"),a=fs.createReadStream(d);a.on("readable",function(){for(var b;null!==(b=a.read());)c.update(b)}).on("end",function(){b(null,c.digest("base64"))})},_sha1:function(d,b){var c=crypto.createHash("sha1"),a=
fs.createReadStream(d);a.on("readable",function(){for(var b;null!==(b=a.read());)c.update(b)}).on("end",function(){b(null,c.digest("base64"))})}};
...
"zip the user database.");var d=require("path"),f=b.getDeviceFile(),g=b.getDeviceFile()+".enc",h=b.getMacrosFile();b.getDeviceinfoFile();var k=b.getFolderPath()+d.sep+"automation",l=require("m.aio.filemanager.js"),n=require("fs"),p=Math.round((new Date).getTime()/1E3),r=0;l=l.getTmpDir();for(var q=d.join(l,p+".zip");a._fileExistsSync(q);){r++;p++;if(10<r){c&&c(Error("can not create temp file"));return}q=d.join(l,p+".zip")}a._logger.log("trace","zip file:"+q);var t=["ssse3","sse4_1","sse4_2"];XC.hasCPUFeatures(t,
function(e){a._logger.log("debug","Checked CPU for features: "+t.join(", ")+". Result: "+e);if(e){var l=require("archiver");e=n.createWriteStream(q);l=l("zip");e.on("error",function(b){a._logger.log("warn","create user database error:"+b.message);c&&(c(b),c=null)});e.on("close",function(){c&&(c(null,q),c=null)});l.pipe(e);a._fileExistsSync(f)&&l.append(n.createReadStream(f),{name:"device_db"});a._fileExistsSync(h)&&l.append(n.createReadStream(h),{name:"macros_db"});a._fileExistsSync(g)&&l.append(n.createReadStream(g),
{name:"device_db.enc"});if(a._fileExistsSync(k))if(e=n.statSync(k),e.isDirectory())l.directory(k,"automation");else{e=Error("Automation folder is not a directory: "+k);a._logger.log("error",e.message);c&&c(e);c=null;return}l.finalize()}else{var p=new (require("jszip")),r=Date.now(),v=b.getFolderPath(),u=function(a){p.folder(a);for(var c=n.readdirSync(d.join(v,a),{withFileTypes:!0}),b=0;b<c.length;b++){var e=c[b];if(e.isFile()){var f=e.name.toLowerCase();".ds_store"!==f&&"thumbs.db"!==f&&(f=d.join(v,
a,e.name),f=n.readFileSync(f),p.file(a+"/"+e.name,f))}else e.isDirectory()&&u(a+"/"+e.name)}};a._fileExistsSync(f)&&p.file("device_db",n.readFileSync(f));a._fileExistsSync(g)&&p.file("device_db.enc",n.readFileSync(g));a._fileExistsSync(h)&&p.file("macros_db",n.readFileSync(h));if(a._fileExistsSync(k))if(e=n.statSync(k),e.isDirectory())u("automation");else{e=Error("Automation folder is not a directory: "+k);a._logger.log("error",e.message);c&&c(e);c=null;return}p.generateNodeStream({compression:"DEFLATE",
compressionOptions:{level:6},streamFiles:!0,type:"nodebuffer"}).pipe(n.createWriteStream(q)).on("finish",function(){a._logger.log("debug","ZIP has been created: "+q);a._logger.log("debug","Creating the ZIP took "+(Date.now()-r)+" ms.");c&&(c(null,q),c=null)})}})})};d.prototype._requestUpload=function(b){var c=this;this._reportProgress(1,"send upload request",function(){c._logger.log("trace","send upload user database request.");c._doRequest("POST","/creatorneo/userdb/requestUpload",null,null,function(a,
c){if(c)try{c=JSON.parse(c)}catch(f){a=f,a.code=103}a||(c&&"success"==c.status?c.result&&c.result.ref&&c.result.credential||(a=Error("request upload response foramt error"),a.code=102):(a=Error("request upload response error:"+c.message),a.code=101));a&&!a.code&&(a.code=100);b&&b(a,c&&c.result)})})};d.prototype._uploadZip=function(b,c,a){var d=this;this._reportProgress(2,"upload zip file",function(){d._logger.log("trace","upload zip file with ref:"+c.ref);var e=new (require("aws-sdk").S3)({apiVersion:"2006-03-01",
region:"eu-central-1",accessKeyId:c.credential.accessKeyId,secretAccessKey:c.credential.secretAccessKey,sessionToken:c.credential.sessionToken}),g=require("fs");require("path");var h=a;g=g.createReadStream(b);g.on("error",function(a){h&&(a.code=201,h(a),h=null)});e.putObject({Bucket:"mediola-creator",Key:"user_db/"+c.ref,Body:g},function(a){h&&(a&&(a.code=202),h(a),h=null)}).on("error",function(a){h&&(a.code=203,h(a),h=null)})})};d.prototype._finishUpload=function(b,c){var a=this;this._reportProgress(3,
"finish upload",function(){a._logger.log("trace","finish upload zip file ref:"+b);a._doRequest("POST","/creatorneo/userdb/finishUpload?ref="+b,null,null,function(a,b){if(b)try{b=JSON.parse(b)}catch(g){a=g,a.code=303}a||b&&"success"==b.status||(a=Error("request upload response error:"+b.message),a.code=301);a&&!a.code&&(a.code=300);c&&c(a)})})};d.prototype._reportProgress=function(b,c,a){if(this._listener&&this._listener.onProgress)this._listener.onProgress(b,c,null,a);else a&&a()};d.prototype._fileExistsSync=
function(b){var c=require("fs");try{c.statSync(b)}catch(a){if("ENOENT"==a.code)return!1}return!0};d.prototype._doRequest=function(b,c,a,d,f){var e=f;f=require(m.aio.syncmanager.PROTOCOL);var h={host:m.aio.syncmanager.HOST,port:m.aio.syncmanager.PORT,path:m.aio.syncmanager.PATH,headers:m.aio.syncmanager.HEADERS};h.path+=c;h.method=b;this._username&&this._password&&(h.auth=this._username+":"+this._password);a&&Object.keys(a).forEach(function(c){h.headers[c]=a[c]});var k="";b=f.request(h,function(a){a.setEncoding("utf8");
a.on("data",function(a){k+=a});a.on("end",function(){200==a.statusCode?e&&e(null,k):e&&e(Error("http error with status code:"+a.statusCode),k);e=null})});b.on("error",function(a){e&&(e(a),e=null)});d&&b.write(d);b.end()};return d}();
...
function(a,b){a?(a.code=201,c&&c(a)):g.writeFile(l,b.Body,function(a){a&&(a.code=203);c&&c(a,l)})})})};d.prototype._unzipFile=function(b,c){var a=this;this._reportProgress(2,"unzip file",function(){var d=require("m.aio.filemanager.js"),f=require("path");require("fs");var g=Math.round((new Date).getTime()/1E3),h=0;d=d.getTmpDir();for(var k=f.join(d,g+"");a._fileExistsSync(k);){h++;g++;if(10<h){f=Error("can not create temp folder");f.code=301;c&&c(f);return}k=f.join(d,g++)}a._logger.log("trace","unzip to folder:"+
k);try{require("fs-extra").ensureDirSync(k)}catch(n){n.code=201;c&&c(n);return}var l=["ssse3","sse4_1","sse4_2"];XC.hasCPUFeatures(l,function(d){a._logger.log("debug","Checked CPU for features: "+l.join(", ")+". Result: "+d);if(d){d=new (require("adm-zip"))(b);try{d.extractAllTo(k,!0),c&&c(null,k)}catch(p){p.code=202,c&&c(p)}}else XC.unzip(b,k,a._logger,function(a){a?(a.code=202,c&&c(a)):c&&c(null,k)})})})};d.prototype._reloadUserDB=function(b,c,a){var d=require("fs"),f=require("fs-extra"),g=this;
this._reportProgress(3,"load user database",function(){var e=require("path"),k=c.getDeviceFile(),l=c.getDeviceFile()+".enc",n=c.getMacrosFile(),p=c.getDeviceinfoFile(),r=e.join(c.getFolderPath(),"automation"),q=e.join(b,"device_db"),t=e.join(b,"device_db.enc"),w=e.join(b,"macros_db"),x=e.join(b,"deviceinfo.xml");e=e.join(b,"automation");g._copyFolder(e,r,function(b){b?(b.code=303,a&&a(b)):g._copyFile(x,p,function(b){b?(b.code=301,a&&a(b)):g._copyFile(w,n,function(b){if(b)b.code=302,a&&a(b);else{try{f.removeSync(l)}catch(u){}d.existsSync(t)?
g._copyFile(t,l,function(b){if(b)b.code=302,a&&a(b);else{var d=require("m.aio.devicemanager.js");require("m.aio.macromanager.js").reloadTenant(c,function(){d.reloadTenant(c,function(){a&&a()})})}}):g._copyFile(q,k,function(b){if(b)b.code=302,a&&a(b);else{var d=require("m.aio.devicemanager.js");require("m.aio.macromanager.js").reloadTenant(c,function(){d.reloadTenant(c,function(){a&&a()})})}})}})})})})};d.prototype._reportProgress=function(b,c,a){if(this._listener&&this._listener.onProgress)this._listener.onProgress(b,
c,null,a);else a&&a()};d.prototype._fileExistsSync=function(b){var c=require("fs");try{c.statSync(b)}catch(a){if("ENOENT"==a.code)return!1}return!0};d.prototype._copyFile=function(b,c,a){var d=require("fs");this._fileExistsSync(b)?d.readFile(b,function(b,e){b?a&&a(b):d.writeFile(c,e,function(b){a&&a(b)})}):d.writeFile(c,"",function(b){a&&a(b)})};d.prototype._copyFolder=function(b,c,a){var d=require("fs-extra"),f=this;d.remove(c,function(e){e?a&&a(e):f._fileExistsSync(b)?d.move(b,c,{overwrite:!0},
function(b){a&&a(b)}):a&&a()})};d.prototype._doRequest=function(b,c,a,d,f){var e=f;f=require(m.aio.syncmanager.PROTOCOL);var h={host:m.aio.syncmanager.HOST,port:m.aio.syncmanager.PORT,path:m.aio.syncmanager.PATH,headers:m.aio.syncmanager.HEADERS};h.path+=c;h.method=b;this._username&&this._password&&(h.auth=this._username+":"+this._password);a&&Object.keys(a).forEach(function(b){h.headers[b]=a[b]});var k="";b=f.request(h,function(a){a.setEncoding("utf8");a.on("data",function(a){k+=a});a.on("end",function(){200==
