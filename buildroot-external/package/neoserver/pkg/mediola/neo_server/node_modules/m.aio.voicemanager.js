...
d.index),f=require("xnm.aio.gateway.js").devicemanager.getSystem(e.info,d.info))if(f=f.buildQuery(d.info,e.info,{value:a.action.value,ext:a.action.ext})){var h={};if("SendGenericCmd"==f.XC_FNC){var g="/cmd?XC_FNC=SendGenericCmd&id="+f.id;if(f.data)for(var l in f.data)g+="&"+l+"="+(null==f.data[l]||void 0==f.data[l]?"":encodeURIComponent(f.data[l]))}else for(l in g="/cmd?",f)g+="&"+l+"="+(null==f[l]||void 0==f[l]?"":encodeURIComponent(f[l]));d.index==b._currentMacroGatewayIndex?h.C=g:h.A={host:d.info.ip,
cmd:g};c(null,h)}else c(Error("invalid device command"));else c(Error("unknown device type"));else c(Error("invalid gateway json"));else c(Error("no such gateway"))}):c(null,null):c(Error("invalid device json"))}else c(Error("no such device"))})};return{V5:b}}();
