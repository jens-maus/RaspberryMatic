...
function(b,c){if(b.cloud&&b.cloud.alias)return b.cloud.alias;b=b.name;c=c.name;return b?c?c+" "+b:b:c};a.prototype._genBaseUrl=function(b){var c=this._parseCCS(b.ccs);return c.protocol+"://"+c.host+"/rapi/"+b.token};a.prototype._getDevicesDetail=function(b,c){var a=this;this._getCloudEnabledDevices(b,function(d,f){d?c&&c(d):a._getGateways(b,function(d,g){d?c&&c(d):a._getRooms(b,function(b,a){c&&c(b,{devices:f,gateways:g,rooms:a})})})})};a.prototype._getGatewaysDetail=function(b,c){var a=this,d=0,
e=function(f,g,h,k){f?c&&c(f):(a._gatewayDetail[b[d].index]={sid:k,ccs:h,token:g},d++,d<b.length?a._getGatewayInfo(b[d].info,e):c&&c())};a._getGatewayInfo(b[d].info,e)};a.prototype._getMacrosConfig=function(b,a){var c=this;this._macromanager.execute(b,"read",null,null,null,function(b,f){if(b)a&&a(b);else if(f&&f.groups&&0!=f.groups.length){var d={};for(b=0;b<f.groups.length;b++){var g=f.groups[b];if(g&&g.macros&&0<g.macros.length){var h=0,e=function(b,a,f){!b&&a&&f&&(d[a]=f);h++;h<g.macros.length&&
...
c="heater thermostat wallthermo wallthermostat ip_heater ip_wallthermo ip_wallthermo2".split(" ");if(!a||!a.info)return!1;a=a.info;if("hm"==a.sys)return c=String(a.type).toLowerCase(),0<=b.indexOf(c);if("aio"==a.sys)if("HM"==a.type){if(-1!=c.indexOf(a.data))return!0}else{if("FHT80b"==a.type)return!0}else if("max"==a.sys){if(1==a.type||2==a.type||3==a.type)return!0}else if("tado"==a.sys||"netatmo"==a.sys&&"NATherm1"==a.type)return!0;return!1};e.prototype._isDeviceScene=function(a){return a&&a.info&&
"hm"==a.info.sys?"Prog"==a.info.type:!1};e.prototype._getDeviceCatagory=function(a){if(!a||!a.info)return null;if(this._isDeviceHeater(a))return"heater";if(this._isDeviceScene(a))return"scene";if(!a.info.command)return null;if("fibaro"==a.info.sys)return this._getFibaroHCDeviceCatagory(a);for(var b=a.info.command,c=!1,f=0;f<b.length;f++){var d=b[f];if(d&&d.name){d=d.name.toLowerCase();if(("hue"==a.info.sys||"osram"==a.info.sys)&&"brightness"==d)return"light";if("moveto"==d||"move to"==d||"position to"==
d||"positionto"==d)return"blind";if("dimto"==d||"dim to"==d||"levelto"==d||"level to"==d)return"light";"on"==d&&(c=!0)}}return c||c?"switch":null};e.prototype._getFibaroHCDeviceCatagory=function(a){a=a.info.command;for(var b=[],c=0;c<a.length;c++){var f=a[c];f&&f.name&&(f=f.name.toLowerCase(),-1==b.indexOf(f)&&b.push(f))}return-1!=b.indexOf("open")&&-1!=b.indexOf("close")?"blind":-1!=b.indexOf("up")&&-1!=b.indexOf("down")&&-1!=b.indexOf("set value")?"light":-1!=b.indexOf("on")&&-1!=b.indexOf("off")?
"switch":null};e.prototype._convertDeviceToCloudFormat=function(a,b,c,f,d){var e=a.info.sys,n=a.info.type,g=this._getDeviceCatagory(a);g&&(n=g);n={name:b,type:n,module:e,sys:e,baseurl:c,commands:{},states:{}};if("aio"==e&&"TASK"==a.info.type)return{name:b,type:"scene",module:e,sys:e,baseurl:c,commands:{on:{type:"POST",url:"/cmd",original:"run",data:{XC_FNC:"SendRM",device:a.index,command:{value:"run"}}}}};if("hm"==e&&"Prog"==a.info.type)return n={name:b,type:"scene",module:e,sys:e,baseurl:c,commands:{on:{type:"POST",
...
new m.aio.voicemanager.CloudConfigHandler.V5(this._devicemanager,this._macromanager);f.setTargetGateway(e);f.exportConfig(a,c,function(a,b){d&&d(a,b)})}else{var n=this;this.getCloudDeviceConfig(a,c,function(c,f){c?d&&d(c):n.getLocalDeviceConfig(a,e,b,function(a,b){if(a)d&&d(a);else{a={};var c;if(f)for(c in f)a[c]=f[c];if(b)for(c in b)a[c]=b[c];(b=Object.keys(a))&&0!=b.length?d&&d(null,a):(n._logger.log("warn","generate empty configuration"),a=Error("empty configuration"),a.code="50011",d&&d(a))}})})}else d&&
d(Error("username is null"))};e.prototype.exportAlexaConfig=function(a,b,c,e,d){if(c&&c.username)if(e){var f=this;this.previewAlexaConfig(a,b,c,e,function(b,e){if(b)d&&d(b);else{var h={};if(e)for(var g in e)b=e[g],delete b.sys,delete b.module,h[g]=b;f._configmanager.execute("read","/config/tenants/"+a,null,function(a,b){if(a)a.code="50013",d&&d(a);else{var e=null;b.license&&"normal"==b.license.type&&b.license.valid&&b.license.license&&(e=b.license.license);f._alexaHandler.uploadConfig(c,h,function(a){a?
(f._logger.log("warn","upload configuration error:"+a.message),a.code="000009"==a.code?"50014":"50012",d&&d(a)):e?f.bindLicense(c.username,c.password,e,function(a){a&&f._logger.log("warn","bind user license error:"+a.message);d&&d(null,h)}):d&&d(null,h)})}})}})}else d&&d(Error("gateway invalid"));else d&&d(Error("username is null"))};e.prototype._getDeviceWithCommandAndStatus=function(a,b){var c={filter:"prop","cloud.enabled":!0},e=this,d={};this._devicemanager.execute("read",a,"device",{filter:"ui",
f_ui_elem:"*",f_ui_type:"command"},function(f,n){if(f)b(f);else{if(n&&0<n.length)for(f=0;f<n.length;f++){var g=n[f];g&&g.cloud&&g.cloud.enabled&&g.info&&g.info.command&&0!=g.info.command.length&&(d[g.index]=g)}e._devicemanager.execute("read",a,"device",c,function(c,f){if(c)b(c);else if(f&&0!=f.length){var g=0,h=function(c,k){!c&&k&&(d[f[g].index]?d[f[g].index].info.status=k.states:(d[f[g].index]=f[g],f[g].info.status=k.states));g++;g<f.length?e._devicemanager.toGeneralDevice(a,f[g].index,!1,h):(c=
Object.values(d),b(null,c))};e._devicemanager.toGeneralDevice(a,f[g].index,!1,h)}else c=Object.values(d),b(null,c)})}})};e.prototype.bindLicense=function(a,b,c,e){b={host:this.CLOUD_HOST,port:this.CLOUD_PORT,path:this.CLOUD_PATH+"?license="+c,method:"GET",timeout:3E4,auth:a+":"+b,user:a,password:b,headers:{"Content-Type":'text/xml; charset="utf-8"',Authorization:"Basic "+(new Buffer(a+":"+b)).toString("base64"),Connection:"close"}};var d=this;this._logger.log("trace","bind license "+c+" to:"+a);a=
require(this.CLOUD_PROTOCOL).request(b,function(a){a.setEncoding("utf8");var b="";a.on("data",function(a){b+=a});a.on("end",function(){200==a.statusCode?(d._logger.log("trace","bind license success"),e&&e()):(d._logger.log("warn","bind license success +"+a.statusCode+":"+b),e&&e(Error("http response:"+a.statusCode)));e=null})});if(a.on)a.on("error",function(a){d._logger.log("warn","upload config error:"+a.message);e&&e(a);e=null});a.setTimeout(3E4,function(){e&&e(Error("timeout"));e=null});a.end()};
e.prototype.uploadDeviceDB=function(a,b,c,e){this._uploadFile(a,b,"device_db",c,e)};e.prototype.uploadMacroDB=function(a,b,c,e){this._uploadFile(a,b,"macros_db",c,e)};e.prototype._uploadFile=function(a,b,c,e,d){a="/xhub/xhubsync/upload2?gatewayid="+a+"&fileName="+c+"&auth=";b.password&&(a+=b.password);b={host:b.ip,port:b.port,path:a,method:"POST",timeout:3E4,headers:{"Content-Type":"text/plain",Connection:"close"}};var f=this;this._logger.log("trace","upload "+c+" to:"+JSON.stringify(b));b=require("http").request(b,
function(a){a.setEncoding("utf8");var b="";a.on("data",function(a){b+=a});a.on("end",function(){if(200==a.statusCode){f._logger.log("trace","upload "+c+" reponse:"+b);try{"success"==JSON.parse(b).status?d&&d():d&&d(Error("failed to upload "+c))}catch(h){d&&d(h)}}else f._logger.log("warn","upload "+c+" error +"+a.statusCode+":"+b),d&&d(Error("http response:"+a.statusCode))})});if(b.on)b.on("error",function(a){f._logger.log("warn","upload "+c+" error:"+a.message);d&&d(a);d=null});b.write(JSON.stringify(e));
b.end()};e.prototype.exportV5Config=function(a,b,c,e,d){if(c&&c.username)if(e){var f=this;(new m.aio.voicemanager.CloudConfigHandler.V5(this._devicemanager,this._macromanager)).exportConfig(a,c,function(b,e){b?(f._logger.log("error","generate configuration error:"+b.message),b.code="50012",d&&d(b)):f._configmanager.execute("read","/config/tenants/"+a,null,function(a,b){if(a)a.code="50013",d&&d(a);else{var g=null;b.license&&"normal"==b.license.type&&b.license.valid&&b.license.license&&(g=b.license.license);
f._alexaHandler.uploadConfig(c,e,function(a){a?(f._logger.log("warn","upload configuration error:"+a.message),a.code="000009"==a.code?"50014":"50012",d&&d(a)):g?f.bindLicense(c.username,c.password,g,function(a){a&&f._logger.log("warn","bind user license error:"+a.message);d&&d(null,e)}):d&&d(null,e)})}})})}else d&&d(Error("gateway invalid"));else d&&d(Error("username is null"))};return e}();"undefined"!=typeof module&&null!=module&&(module.exports=new m.aio.voicemanager.Handler);
