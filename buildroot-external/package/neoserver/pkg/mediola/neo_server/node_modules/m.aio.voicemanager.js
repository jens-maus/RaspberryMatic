...
$jscomp.iteratorFromArray=function(e,b){$jscomp.initSymbolIterator();e instanceof String&&(e+="");var a=0,c={next:function(){if(a<e.length){var f=a++;return{value:b(f,e[f]),done:!1}}c.next=function(){return{done:!0,value:void 0}};return c.next()}};c[Symbol.iterator]=function(){return c};return c};
...
l){h?(h.code=40001,b&&b(h)):l&&0!=l.length?(g={filter:"prop","info.sys":"cloudservice"},f._devicemanager.execute("read",a,"gateway",g,function(h,g){h?(h.code=40001,b&&b(h)):g&&0!=g.length?f._devicemanager.execute("read",a,"room",null,function(a,h){if(a)a.code=40001,b&&b(a);else if(h&&0!=h.length){a=[];for(var f=0;f<l.length;f++){var e=l[f];if(e&&e.info&&e.info.gateway){var n=d(e.info.gateway,g);if(n&&n.info&&(n=k.resolveGateway(n.info))){if(!n.equalsTo(c)){a=Error("cloud device "+e.index+" belongs to another account");
a.code=40002;b&&b(a);return}var q=d(e.room,h);q&&(n=e.name,q=q.name,n=(q?q+" ":"")+(n?n:""),e.cloud.alias&&(n=e.cloud.alias),e=JSON.parse(JSON.stringify(e)),e.__cloudref=n,a.push(e))}}}b&&b(null,a)}else b&&b(null,[])}):b&&b(null,[])})):b&&b(null,[])})};e.prototype._getCloudEnabledDevices=function(a,c){this._devicemanager.execute("read",a,"device",{filter:"prop","cloud.enabled":!0},function(a,b){c&&c(a,b)})};e.prototype._getGateways=function(a,c){this._devicemanager.execute("read",a,"gateway",null,
...
var g=this,h=require(a.protocol).request(c,function(a){a.setEncoding("utf8");var c="";a.on("data",function(a){c+=a});a.on("end",function(){if(200==a.statusCode){g._logger.log("trace","get access token reponse:"+c);try{var b=JSON.parse(c);b.XC_ERR?l&&l(Error(b.XC_ERR.msg)):b.XC_SUC?l&&l(null,b.XC_SUC.at):l&&l(Error("invalid response"))}catch(r){l&&l(r)}}else g._logger.log("warn","get access token error +"+a.statusCode+":"+c),l&&l(Error("http response:"+a.statusCode))})});if(h.on)h.on("error",function(a){g._logger.log("warn",
"get access token error:"+a.message);l&&l(a);l=null});h.setTimeout&&h.setTimeout(5E3,function(){h.abort();g._logger.log("warn","get access token timeout");l&&l(Error("http timeout"));l=null});h.end()};e.prototype._parseCCS=function(a){var c=a.indexOf(":");if(-1==c)return{protocol:"https",host:a,port:443};var b=a.substr(0,c);a=a.substr(c+1);8080==a?a=8443:80==a&&(a=443);return{protocol:"https",host:b,port:a}};var b=function(a,c){e.call(this,a,c);this._logger=require("x.logger.js").getLogger("m.aio.voicemanager.V5ConfigHandler");
this._targetGatewayJSON=null;this._gatewayDetail={};this._currentMacroGatewayIndex=this._tenantIdx=null};b.prototype=new e;b.prototype.constructor=b;b.prototype.setTargetGateway=function(a){this._targetGatewayJSON=a};b.prototype.exportConfig=function(a,c,b){var d=this;this._gatewayDetail={};this._tenantIdx=a;this._currentMacroGatewayIndex=null;this._getCloudDeviceConfig(a,c,function(c,f){c?b&&b(c):d._getGatewayDeviceConfig(a,function(c,h){c?b&&b(c):d._getMacrosConfig(a,function(a,c){a?b&&b(a):(a=
d._mergeConfig(f,h),a=d._mergeConfig(c,a),b&&b(null,a))})})})};b.prototype._isSameAsTargetGateway=function(a){return this._targetGatewayJSON?a.ip==this._targetGatewayJSON.ip&&a.sid==this._targetGatewayJSON.sid:!1};b.prototype._getGatewayDeviceConfig=function(a,c){var b=function(a,c){if(!c||0==c.length)return null;for(var b=c.length;b--;){var d=c[b];if(d&&d.index==a)return d}return null},d=this;this._getDevicesDetail(a,function(a,f){if(a)c&&c(a);else{var g=f.devices,h=f.gateways,l=f.rooms,e={};for(a=
...
l);f.push({device:a,gateway:n,room:q})}}if(0==f.length)c&&c(null,{});else{var p={};k=0;var x=function(a,b){!a&&b&&(p=d._mergeConfig(p,b));k++;k<f.length?d._genDeviceConfig(f[k].device,f[k].gateway,f[k].room,x):c&&c(null,p)};d._genDeviceConfig(f[k].device,f[k].gateway,f[k].room,x)}}else c&&c(null,{})})}})};b.prototype._genDeviceConfig=function(a,c,b,d){if(a&&a.info&&a.info.sys)if(c&&c.index){var f=this._gatewayDetail[c.index];if(f){var e=a.info.sys,g=this._devicemanager.getModule(e);g&&g.devicemanager&&
g.devicemanager.toGeneralDevice?(e=g.devicemanager.toGeneralDevice(a.info,null,"v5"))?(e.name=this._genDeviceName(a,b),e.local="http://"+c.info.ip+(c.info.port?":"+c.info.port:""),e.baseurl=this._genBaseUrl(f),e.data||(e.data={}),e.data.sid=f.sid,a={},a[e.name]=e,d&&d(null,a)):d&&d(null,null):d&&d(Error("sys "+e+" can not be convert to general device"))}else d&&d(Error("no gateway detail info"))}else d&&d(Error("invalid gateway json"));else d&&d(Error("invalid device json"))};b.prototype._genDeviceName=
...
h):(a=e._gatewayDetail[e._currentMacroGatewayIndex])?(f.commands.on.url=e._genBaseUrl(a)+"/macro",b&&b(null,d,f)):b&&b(Error("no main gateway"))};this._convertMacroCommand(c.commands[g],h)}}else b&&b(null,null,null)};b.prototype._convertMacroCommand=function(a,b){if(a&&a.classid)switch(a.classid){case "sleep":b(null,{P:a.time});break;case "command":if(!a.action||!a.action.device){b(Error("invalid macro device command"));break}this._convertMacroDeviceCommand(a,b);break;default:b(Error("unknown macro command"))}else b(Error("invalid macro command"))};
b.prototype._convertMacroDeviceCommand=function(a,b){var c=this;this._devicemanager.execute("read",this._tenantIdx,"device",{filter:"prop",index:a.action.device.index},function(d,f){if(d)b(d);else if(f&&0!=f.length){var e=f[0];e&&e.info&&e.info.sys?"aio"==e.info.sys?c._devicemanager.execute("read",c._tenantIdx,"gateway",{filter:"prop",index:e.info.gateway},function(d,f){if(d)b(d);else if(f&&0!=f.length)if((d=f[0])&&d.info&&d.index)if(c._gatewayDetail[d.index]&&!c._currentMacroGatewayIndex&&(c._currentMacroGatewayIndex=
...
f)g+="&"+l+"="+(null==f[l]||void 0==f[l]?"":encodeURIComponent(f[l]));d.index==c._currentMacroGatewayIndex?h.C=g:h.A={host:d.info.ip,cmd:g};b(null,h)}else b(Error("invalid device command"));else b(Error("system do not support command"))}else b(Error("unknown device type"));else b(Error("invalid gateway json"));else b(Error("no such gateway"))}):b(null,null):b(Error("invalid device json"))}else b(Error("no such device"))})};return{V5:b}}();
...
function(b,a,c,f,d){b={host:require("url").parse("https://"+b).hostname,port:443,path:"/getAccessToken?sid="+a,method:"GET",timeout:3E4,auth:"user:"+c,user:"user",password:c,headers:{Authorization:"Basic "+(new Buffer("user:"+c)).toString("base64")}};f&&(b.path+="&force=1");this._logger.log("trace","send get access token to:"+JSON.stringify(m.aio.voicemanager.Util.clearHttpOptionForLog(b)));var e=this,k=require("https").request(b,function(a){a.setEncoding("utf8");var b="";a.on("data",function(a){b+=
a});a.on("end",function(){if(200==a.statusCode){e._logger.log("trace","get access token reponse:"+b);try{var c=JSON.parse(b);c.XC_ERR?d&&d(Error(c.XC_ERR.msg)):c.XC_SUC?d&&d(null,c.XC_SUC.at):d&&d(Error("invalid response"))}catch(q){d&&d(q)}}else e._logger.log("warn","get access token error +"+a.statusCode+":"+b),d&&d(Error("http response:"+a.statusCode))})});if(k.on)k.on("error",function(a){e._logger.log("warn","get access token error:"+a.message);d&&d(a);d=null});k.setTimeout&&k.setTimeout(5E3,
...
function(d,e){d?(d=Error("get accesss token error:"+d.message),d.code="50003",a&&a(d)):e?a&&a(null,e,f.server,k):c._getAccessToken(f.server,k,b.password,!0,function(b,c){b?(b=Error("generate accesss token error:"+b.message),b.code="50004",a&&a(b)):c?a&&a(null,c,f.server,k):a&&a(Error("access token empty"))})}):(c._logger.log("warn","gateway has no cloud server"),d=Error("gateway has no cloud server"),d.code="50102",a&&a(d))}):(e=Error("gateway has no password"),e.code="50002",a&&a(e)):(e=Error("gateway has no sid"),
e.code="50002",a&&a(e))}):a&&a(Error("gateway json invalid"))}else a&&a(Error("gateway json is null"))};e.prototype.generateConfig=function(b,a,c,f,d,e,k,g,h,n){var l=require("url").parse("https://"+g),p=function(a){for(var b=a.length,c={};b--;){var d=a[b];c[d.index]=d.name}return c}(a),r=function(a){for(var b=a.length,c={};b--;){var d=a[b];c[d.index]=d.name}return c}(c);c={};g=this._resovleBaseUrl(g,e);a=h=null;for(var t=0;t<b.length;t++){var u=b[t];!u||!u.info||"cloudservice"==u.info.sys&&u.info.module||
...
function(b,a){var c=require("fs-extra");c.ensureFile(b,function(f){f?a&&a(null,{}):c.readFile(b,"utf8",function(b,c){if(b)a&&a(b);else if(c)try{var d=JSON.parse(c);a&&a(null,d)}catch(g){a&&a(null,{})}else a&&a(null,{})})})};e.prototype.saveCustomCommandMap=function(b,a,c){var f=require("fs-extra");f.ensureFile(b,function(d){if(d)c&&c(null,{});else try{var e=JSON.stringify(a,null,2);f.writeFile(b,e,function(a){c&&c(a)})}catch(k){c&&c(k)}})};return e}();
...
d._devicemanager.execute("read",b,"gateway",k,function(g,l){g?(g.code=40001,c&&c(g)):l&&0!=l.length?d._devicemanager.execute("read",b,"room",null,function(b,d){if(b)b.code=40001,c&&c(b);else if(d&&0!=d.length){b=[];for(var g=0;g<h.length;g++){var k=h[g];if(k&&k.info&&k.info.gateway){var n=f(k.info.gateway,l);if(n&&n.info&&(n=e.resolveGateway(n.info))){if(!n.equalsTo(a)){b=Error("cloud device "+k.index+" belongs to another account");b.code=40002;c&&c(b);return}var p=f(k.room,d);p&&(n=k.name,p=p.name,
n=(p?p+" ":"")+(n?n:""),k.cloud.alias&&(n=k.cloud.alias),k=JSON.parse(JSON.stringify(k)),k.__cloudref=n,b.push(k))}}}c&&c(null,b)}else c&&c(null,[])}):c&&c(null,[])})):c&&c(null,[])})};e.prototype.previewAlexaConfig=function(b,a,c,f,d){if(c&&c.username)if(f)if(f.version&&"C"==f.version.charAt(0)){var e=new m.aio.voicemanager.CloudConfigHandler.V5(this._devicemanager,this._macromanager);e.setTargetGateway(f);e.exportConfig(b,c,function(a,b){d&&d(a,b)})}else{var k=this;this.getCloudDeviceConfig(b,c,
...
a,c,e,d){if(c&&c.username)if(e){var f=this;this.previewAlexaConfig(b,a,c,e,function(a,e){if(a)d&&d(a);else{var h={};if(e)for(var g in e)a=e[g],delete a.sys,delete a.module,h[g]=a;f._configmanager.execute("read","/config/tenants/"+b,null,function(a,b){if(a)a.code="50013",d&&d(a);else{var e=null;b.license&&"normal"==b.license.type&&b.license.valid&&b.license.license&&(e=b.license.license);f._alexaHandler.uploadConfig(c,h,function(a){a?(f._logger.log("warn","upload configuration error:"+a.message),a.code=
"000009"==a.code?"50014":"50012",d&&d(a)):e?f.bindLicense(c.username,c.password,e,function(a){a&&f._logger.log("warn","bind user license error:"+a.message);d&&d(null,h)}):d&&d(null,h)})}})}})}else d&&d(Error("gateway invalid"));else d&&d(Error("username is null"))};e.prototype._getDeviceWithCommandAndStatus=function(b,a){var c={filter:"prop","cloud.enabled":!0},e=this,d={};this._devicemanager.execute("read",b,"device",{filter:"ui",f_ui_elem:"*",f_ui_type:"command"},function(f,k){if(f)a(f);else{if(k&&
0<k.length)for(f=0;f<k.length;f++){var g=k[f];g&&g.cloud&&g.cloud.enabled&&g.info&&g.info.command&&0!=g.info.command.length&&(d[g.index]=g)}e._devicemanager.execute("read",b,"device",c,function(c,f){if(c)a(c);else if(f&&0!=f.length){var g=0,h=function(c,k){!c&&k&&(d[f[g].index]?d[f[g].index].info.status=k.states:(d[f[g].index]=f[g],f[g].info.status=k.states));g++;g<f.length?e._devicemanager.toGeneralDevice(b,f[g].index,!1,h):(c=Object.values(d),a(null,c))};e._devicemanager.toGeneralDevice(b,f[g].index,
!1,h)}else c=Object.values(d),a(null,c)})}})};e.prototype.bindLicense=function(b,a,c,e){a={host:this.CLOUD_HOST,port:this.CLOUD_PORT,path:this.CLOUD_PATH+"?license="+c,method:"GET",timeout:3E4,auth:b+":"+a,user:b,password:a,headers:{"Content-Type":'text/xml; charset="utf-8"',Authorization:"Basic "+(new Buffer(b+":"+a)).toString("base64"),Connection:"close"}};var d=this;this._logger.log("trace","bind license "+c+" to:"+b);b=require(this.CLOUD_PROTOCOL).request(a,function(a){a.setEncoding("utf8");var b=
"";a.on("data",function(a){b+=a});a.on("end",function(){200==a.statusCode?(d._logger.log("trace","bind license success"),e&&e()):(d._logger.log("warn","bind license success +"+a.statusCode+":"+b),e&&e(Error("http response:"+a.statusCode)));e=null})});if(b.on)b.on("error",function(a){d._logger.log("warn","upload config error:"+a.message);e&&e(a);e=null});b.setTimeout(3E4,function(){e&&e(Error("timeout"));e=null});b.end()};e.prototype.uploadDeviceDB=function(b,a,c,e){this._uploadFile(b,a,"device_db",
c,e)};e.prototype.uploadMacroDB=function(b,a,c,e){this._uploadFile(b,a,"macros_db",c,e)};e.prototype._uploadFile=function(b,a,c,e,d){b="/xhub/xhubsync/upload2?gatewayid="+b+"&fileName="+c+"&auth=";a.password&&(b+=a.password);a={host:a.ip,port:a.port,path:b,method:"POST",timeout:3E4,headers:{"Content-Type":"text/plain",Connection:"close"}};var f=this;this._logger.log("trace","upload "+c+" to:"+JSON.stringify(a));a=require("http").request(a,function(a){a.setEncoding("utf8");var b="";a.on("data",function(a){b+=
a});a.on("end",function(){if(200==a.statusCode){f._logger.log("trace","upload "+c+" reponse:"+b);try{"success"==JSON.parse(b).status?d&&d():d&&d(Error("failed to upload "+c))}catch(h){d&&d(h)}}else f._logger.log("warn","upload "+c+" error +"+a.statusCode+":"+b),d&&d(Error("http response:"+a.statusCode))})});if(a.on)a.on("error",function(a){f._logger.log("warn","upload "+c+" error:"+a.message);d&&d(a);d=null});a.write(JSON.stringify(e));a.end()};e.prototype.exportV5Config=function(b,a,c,e,d){if(c&&
