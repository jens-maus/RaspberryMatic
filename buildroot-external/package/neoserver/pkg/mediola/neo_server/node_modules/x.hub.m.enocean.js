...
e.prototype.secureTeachIn=function(a,f,c){this._logger.log("trace","secure teach-in:"+a);a&&(f.key&&delete f.key,this._secInfo[a]=f);this._saveSec(this._secInfo,c)};e.prototype.resolve=function(a){if(!a)return null;switch(a.rorg){case 53:return this._resolveSecureTeachIn(a);case 49:return this._resolveSecureMessage(a);default:return null}};e.prototype._resolveSecureTeachIn=function(a){var f={};a=a.userData;var c=a.readUInt8(0);f.teachInInfo={};f.teachInInfo.idx=c>>6&3;f.teachInInfo.cnt=c>>4&3;f.teachInInfo.psk=
c>>3&1;f.teachInInfo.type=c>>2&1;f.teachInInfo.info=c&3;0==f.teachInInfo.idx?(c=a.readUInt8(1),f.slf={},f.slf.rlc_algo=c>>6&3,f.slf.rlc_tx=c>>5&1,f.slf.mac_alro=c>>3&3,f.slf.data_enc=c&7,c=0,1==f.slf.rlc_algo?c=2:2==f.slf.rlc_algo&&(c=3),c&&(f.rlc=a.slice(2,2+c)),f.key=a.slice(2+c),f.completeKey=null):f.key=a.slice(1);return f};e.prototype._resolveSecureMessage=function(a){var f=a.senderID,c=this._secInfo[f];if(!c||!c.completeKey)return null;var e=0,h=0;1==c.slf.mac_alro?e=3:2==c.slf.mac_alro&&(e=
4);1==c.slf.rlc_tx&&(1==c.slf.rlc_algo?h=2:2==c.slf.rlc_algo&&(h=3));var l=a.userData.length-e-h,p=a.userData.slice(0,l),r=null;h&&a.userData.slice(l,l+h);e&&(r=a.userData.slice(l+h));e=c.rlc;h=!0;r&&(r=this.checkCmac(a.rorg,p,e,c.completeKey,r),r.rlc&&e.toString("hex")!=r.rlc.toString("hex")&&(e=r.rlc,c.rlc=r.rlc),r&&r.ok?(this._logger.log("trace","receive from "+f+" with cmac rlc:"+e.toString("hex")),delete this._unsync[f]):(h=!1,this._logger.log("warn","receive from "+f+" with cmac out of snyc, last rlc:"+
e.toString("hex")),this._unsync[f]=e.toString("hex")));f=null;h&&3==c.slf.data_enc&&(p=this.decryptVAES(e,g,c.completeKey,p),f=a.convertToPacket(p).toRadioTelegram());c.rlc=this._incRLC(e);this._logger.log("trace","senderID next rlc:"+c.rlc.toString("hex"));this._saveSec(this._secInfo);return f};e.prototype.decryptVAES=function(a,f,c,e){for(var k=e.length,g=Math.ceil(k/16),p=[],r=null,v=0;v<g;v++){var y=16*v,C=y+16;C>k&&(C=k);y=e.slice(y,C);r=this._doVAESblock(a,f,c,r,y);p.push(r.data);r=r.enc}return Buffer.concat(p)};
e.prototype._doVAESblock=function(a,f,c,e,h){a=this._xor(a,f);e&&(a=this._xor(a,e));c=this._doAES128(c,null,a);e=this._xor(c,h);e=e.slice(0,h.length);return{data:e,enc:c}};e.prototype.checkCmac=function(a,f,c,e,h){if(c){for(var k=this._bufToInt(c),g=0;128>g;g++){if(0!=g){k+=1;var r=65535;3==c.length&&(r=16777215);k>r&&(k=0)}r=this._intToBuf(k,c.length);r=this.calcCmac(a,f,r,e,h.length);if(r.toString("hex")==h.toString("hex"))return{ok:!0,rlc:this._intToBuf(k,c.length)}}return{ok:!1,rlc:this._intToBuf(k,
c.length)}}r=this.calcCmac(a,f,null,e,h.length);return r.toString("hex")==h.toString("hex")?{ok:!0}:{ok:!1}};e.prototype.calcCmac=function(a,f,c,e,h){var k=this._getCmacSubkey(e),g=new Buffer(1);g.writeUInt8(a);g=Buffer.concat([g,f]);c&&(g=Buffer.concat([g,c]));a=g.length;f=Math.ceil(a/16);c=null;for(var r=0;r<f;r++){var v=16*r;var y=v+16;y>a&&(y=a);v=g.slice(v,y);c=v=this._doCmacBlock(e,k.k1,k.k2,v,c)}return v.slice(0,h)};e.prototype._doCmacBlock=function(a,f,c,e,h){for(var k=new Buffer(16),g=0;16>
g;g++)g<e.length?k.writeUInt8(e.readUInt8(g),g):g==e.length?k.writeUInt8(128,g):k.writeUInt8(0,g);h&&(k=this._xor(k,h));k=16>e.length?this._xor(k,c):this._xor(k,f);return this._doAES128(a,null,k)};e.prototype._getCmacSubkey=function(a){var f=new Buffer("00000000000000000000000000000000","hex");var c=new Buffer("00000000000000000000000000000087","hex");f=this._doAES128(a,null,f);a=new Buffer(f.toString("hex"),"hex");this._bufShiftLeft(a,1);f.readUInt8(0)&128&&(a=this._xor(a,c));f=new Buffer(a.toString("hex"),
"hex");this._bufShiftLeft(f,1);a.readUInt8(0)&128&&(f=this._xor(f,c));return{k1:a,k2:f}};e.prototype._doAES128=function(a,f,c){f||(f=new Buffer("00000000000000000000000000000000","hex"));a=crypto.createCipheriv("aes128",a,f);a.setAutoPadding(!1);c=a.update(c);a=a.final();return Buffer.concat([c,a])};e.prototype._xor=function(a,f){for(var c=Math.max(a.length,f.length),e=new Buffer(c),h=0;h<c;h++){var g=0,p=0;h<a.length&&(g=a.readUInt8(h));h<f.length&&(p=f.readUInt8(h));e.writeUInt8(g^p,h)}return e};
e.prototype._bufShiftLeft=function(a,f){if(f!==(f|0))throw Error("shiftBits must be a 32 bit int");var c=2*((f>>4)+(0!==(f&15)));f=8*c-f;for(var e=0,h=0;h-c<a.length;h+=2)if(e<<=16,h<a.length-1?e|=a.readUInt16BE(h):h<a.length&&(e|=a[h]<<8),0<=h-c)if(h-c<a.length-1)a.writeUInt16BE(e>>>f&65535,h-c);else{if(h-c!==a.length-1)throw Error();a[h-c]=e>>>f+8}else-1===h-c&&(a[0]=e>>>f)};e.prototype._bufToInt=function(a){for(var f=0,c=0;c<a.length;c++){var e=a.readUInt8(c);f=(f<<8*c)+e}return f};e.prototype._intToBuf=
function(a,f){for(a=a.toString(16);a.length<2*f;)a="0"+a;return new Buffer(a,"hex")};e.prototype._incRLC=function(a){var f=this._bufToInt(a);f+=1;var c=65535;3==a.length&&(c=16777215);f>c&&(f=0);return this._intToBuf(f,a.length)};e.prototype._getFile=function(){var a=require("x.hub.fileman.js").fileManager,f=require("path");require("fs");a=a.getUserRootDataPath();return f.resolve(a,this.SECURITY_FILE)};e.prototype._loadSec=function(a){var f=this._getFile();fs_extra.ensureFile(f,function(c){c?a&&a(c):
fs_extra.readFile(f,"utf8",function(c,f){if(c)a&&a(c);else{try{var e=JSON.parse(f);for(var k in e)if(e.hasOwnProperty(k)){var h=e[k];h.rlc&&(h.rlc=new Buffer(h.rlc,"hex"));h.completeKey&&(h.completeKey=new Buffer(h.completeKey,"hex"))}}catch(v){}a&&a(null,e)}})})};e.prototype._saveSec=function(a,f){var c=this._getFile(),e={},h;for(h in a)e[h]={},e[h].teachInInfo=a[h].teachInInfo,e[h].slf=a[h].slf,e[h].rlc=a[h].rlc.toString("hex"),e[h].completeKey=a[h].completeKey.toString("hex");fs_extra.writeFile(c,
JSON.stringify(e,null,2),function(a){f&&f(a)})};return e}();
...
this._learnProfile=this._learnCallback=null;this._secTeachInFrames={};this._teaching=!1;this._teachingTO=this._teachVendor=this._teachSenderID=this._teachID=this._teachProfile=this._teachCallback=null;var f=this;this._teachinTelegramCB={_onEOTelegram:function(a){f.onTeachInResponseTelegram(a)}};this._gpLearn=new g(this)};e.prototype.learn=function(a,f,c,e,h,g,p){if(this._learning)g&&g(Error("learning process running"));else if(this._teaching)g&&g(Error("teach-in process running"));else{if(!h||0>=
h)h=3E4;this._learning=!0;this._learnCallback=g;this._learnProfile=a?a.toLowerCase():null;this._learnAddress=f?f.toLowerCase():null;this._learnVendor=e?e:"general";this._learnSenderID=c?c.toLowerCase():null;this._secTeachInFrames={};this._saParams=null;!a||"bb-aa-cc"!=a.toLowerCase()&&"d2-06-10"!=a.toLowerCase()||(this._saParams={extended:1},p&&"undefined"!=typeof p.saLearnMode&&null!=p.saLearnMode&&(a=parseInt(p.saLearnMode),isNaN(a)&&(a=1),0>a&&(a=1),2<a&&(a=1),this._saParams.extended=a));var k=
...
17);this._controller._esp3.sendResponse(g,function(a){e._learnFinish(Error("eep not supported"))})}h=new h;var p=a.getPMCandidateId();p||(p=h.UNIQUE_GATEWAY_SENDERID?this._controller._deviceHandler.getNextFreeSenderId():x.hub.m.enocean.SENDER_ID);f={address:a.getSAClientId(),data:c,vendor:this._learnVendor,senderID:p,hopCount:a.getHopCount(),saLearnMode:this._saParams?this._saParams.extended:null,originalEEP:f};this._controller._deviceHandler.onLearnAddDeleteDevice(f,function(c,f,h){if(c)g=new x.hub.m.enocean.ESP3.Response.Response_OK_SA_CONFIRM_LEARN(500,
19),e._controller._esp3.sendResponse(g,function(a){e._learnFinish(a)});else{c=0;1==f&&(c=32);var k=a.getHopCount();g=new x.hub.m.enocean.ESP3.Response.Response_OK_SA_CONFIRM_LEARN(128*(k+1),c);e._controller._esp3.sendResponse(g,function(a){e._learnFinish(a,f,h)})}})}}};e.prototype.onTeachInResponseTelegram=function(a){if(a&&this._teaching){var f=this;if(this._teachProfile){var c=x.hub.m.enocean.profiles[this._teachProfile];if(c&&(c=new c,c.handleTeachinTelegram)){c.handleTeachinTelegram(this,a);return}}switch(a.rorg){case 165:if(0==
...
this.securityLvl])]));return new g(1,a,c)};f.prototype.toRadioTelegram=function(){return l.resolve(this)};f.resolve=function(a){var c=!1;if(a.optData){c=!0;var e=a.optData.readUInt8(0);var m=a.optData.slice(1,5).toString("hex");var g=a.optData.readUInt8(5);var h=a.optData.readUInt8(6)}return new f(a.data,c,e,m,g,h)};var c=function(a){this.data=a};c.prototype.toJSON=function(){return{data:this.data.toString("hex")}};c.prototype.toFrame=function(){return new g(6,this.data,null)};c.prototype.toSACommand=
function(){return N.resolve(this)};c.resolve=function(a){return new c(a.data)};var k=function(a){this.data=a};k.prototype.toJSON=function(){return{data:this.data.toString("hex")}};k.prototype.toFrame=function(){return new g(5,this.data,null)};k.prototype.toCommandCommand=function(){return oa.resolve(this)};k.resolve=function(a){return new k(a.data)};var h=function(a){this.data=a};h.prototype.toJSON=function(){return{data:this.data.toString("hex")}};h.prototype.toFrame=function(){return new g(4,this.data,
...
case 212:return S.resolve(a);case 53:return X.resolve(a);case 49:return T.resolve(a);case 64:return y.resolve(a);case 176:return p.resolve(a);case 178:return v.resolve(a);default:return null}};var p=function(){var a=function(a,c,e){8<=e&&(e=8);0>=e&&(e=1);var f=Math.floor(c/8),m=c%8;c=Math.pow(2,e)-1;if(8<m+e)return e=16-m-e,a.readUInt16BE(f)>>e&c;e=8-m-e;return a.readUInt8(f)>>e&c},c=function(a){this.type=a};c.prototype.getType=function(){return this.type};c.resolve=function(c,m){if(m+2>8*c.length)return null;
var h=a(c,m,2),H=null;switch(h){case 0:H=e.resovle(c,m);break;case 1:H={type:h,sigType:c.readUInt16BE(0)>>6&255,valueType:c.readUInt16BE(0)>>2&3,resolution:c.readUInt16BE(0)&15,engMin:c.readUInt8(2),scalingMin:c.readUInt16BE(3)>>12&15,engMax:c.readUInt16BE(3)>>4&255,scalingMax:c.readUInt16BE(3)&15};break;case 2:H=f.resovle(c,m);break;case 3:H=g.resovle(c,m)}return H};var e=function(a,c,e){this.type=0;this.sigType=a;this.dataLen=c;this.data=e};e.prototype=new c;e.prototype.constructor=e;e.prototype.getBitLength=
function(){return 2==this.sigType?50:18};e.prototype.getProductID=function(){return 2==this.sigType?this.data?this.data.readUInt32BE(0):null:null};e.resovle=function(c,f){if(f+18>8*c.legnth)return null;var m=a(c,f+2,8),g=a(c,f+10,8),h=null;if(2==m){if(f+50>8*c.legnth)return null;h=new Buffer(4);h.writeUInt8(a(c,f+18,8),0);h.writeUInt8(a(c,f+26,8),1);h.writeUInt8(a(c,f+34,8),2);h.writeUInt8(a(c,f+42,8),3)}else 1==m&&(h=null);return new e(m,g,h)};new c;var f=function(a,c){this.type=2;this.sigType=a;
this.valueType=c};f.prototype=new c;f.prototype.constructor=f;f.prototype.getBitLength=function(){return 12};f.resovle=function(c,e){if(e+12>8*c.length)return null;var m=a(c,e+2,8);c=a(c,e+10,2);return new f(m,c)};f.prototype.toJSON=function(){return{sigType:this.sigType,valueType:this.valueType}};var g=function(a,c,e){this.type=3;this.sigType=a;this.valueType=c;this.resolution=e};g.prototype=new c;g.prototype.constructor=g;g.prototype.getBitLength=function(){return 16};g.prototype.toJSON=function(){return{sigType:this.sigType,
valueType:this.valueType,resolution:this.resolution}};g.resovle=function(c,e){if(e+16>8*c.length)return null;var f=a(c,e+2,8),m=a(c,e+10,2);c=a(c,e+12,4);return new g(f,m,c)};var h=function(a,c,e){l.call(this,176);this.manufacturerID=a;this.dataDirection=c;this.purpose=e;this.productID=null;this.outboudChannels={};this.inboundChannels={};this.status=this.senderID=null};h.prototype=new l;h.prototype.constructor=h;h.prototype.getManufacturerId=function(){return this.manufacturerID};h.prototype.getProductId=
function(){return this.productID};h.prototype.toJSON=function(){var a={manufacturerID:this.manufacturerID,dataDirection:this.dataDirection,purpose:this.purpose,productID:this.productID,outboudChannels:{},inboundChannels:{}},c;for(c in this.outboudChannels)a.outboudChannels[c]=this.outboudChannels[c].toJSON();for(c in this.inboundChannels)a.inboundChannels[c]=this.inboundChannels[c].toJSON();return a};h.resolve=function(a){var e=a.data;a=e.readUInt16BE(1)>>5&2047;var f=e.readUInt16BE(1)>>4&1,m=e.readUInt16BE(1)>>
2&3;a=new h(a,f,m);f=e.slice(3,e.length-5);m=e.slice(e.length-5,e.length-1);e=e.readUInt8(e.length-1);a.senderID=m.toString("hex");a.status=e;for(var g=m=e=0;;){var H=c.resolve(f,m);if(!H)break;0==e?0==H.getType()?a.productID=H.getProductID():(a.outboudChannels[g]=H,g++,e=1):1==e?0==H.getType()?e=2:(a.outboudChannels[g]=H,g++,e=1):2==e&&(a.inboundChannels[g]=H,g++);m+=H.getBitLength()}return a};return h}(),r=function(){var a=function(a,c,e,f,m){l.call(this,177);this.manufacturerID=a;this.result=c;
this.chnAcknoledgement=e;this.senderID=f;this.status=m};a.prototype=new l;a.prototype.constructor=a;a.prototype.toPacket=function(){var a=new Buffer(3);a.writeUInt8(this.rorg,0);a.writeUInt16BE(this.manufacturerID<<5|this.result<<3,1);if(3==this.result){var c=[];if(this.chnAcknoledgement){for(var e=Math.ceil(this.chnAcknoledgement.length/8),m=0;m<e.length;m++)c.push(0);for(e=0;e<this.chnAcknoledgement.length;e++)m=Math.floor(e/8),c[m]|=chnAcknoledgement[e]<<8-e%8-1}a=Buffer.concat([a,new Buffer(c)])}a=
Buffer.concat([a,new Buffer(this.senderID,"hex")]);a=Buffer.concat([a,new Buffer([this.status])]);c=1;e="FFFFFFFF";m=255;var g=0;this.optData&&(c=this.optData.subTelNum,e=this.optData.dstId,m=this.optData.dBm,g=this.optData.securityLvl);return new f(a,!0,c,e,m,g)};a.prototype.toJSON=function(){return{rorg:this.rorg.toString(16),manufacturerID:this.manufacturerID.toString(16),result:this.result,chnAcknoledgement:this.chnAcknoledgement,senderID:this.senderID,receiverID:this.receiverID}};return a}(),
v=function(){var a=function(a,c,e){l.call(this,178);this.data=a;this.senderID=c;this.status=e};a.prototype=new l;a.prototype.constructor=a;a.prototype.toPacket=function(){var a=new Buffer(1);a.writeUInt8(this.rorg,0);this.data&&(a=Buffer.concat([a,this.data]));a=Buffer.concat([a,new Buffer(this.senderID,"hex")]);a=Buffer.concat([a,new Buffer([this.status])]);var c=1,e="FFFFFFFF",m=255,g=0;this.optData&&(c=this.optData.subTelNum,e=this.optData.dstId,m=this.optData.dBm,g=this.optData.securityLvl);return new f(a,
!0,c,e,m,g)};a.prototype.toJSON=function(){return{rorg:this.rorg.toString(16),data:this.data?this.data.toString("hex"):null,senderID:this.senderID,status:this.status}};a.resolve=function(c){var e=c.data;c=e.slice(1,e.length-5);var f=e.slice(e.length-5,e.length-1);e=e.readUInt8(e.length-1);return new a(c,f.toString("hex"),e)};return a}(),y=function(a,c,e,f,g,h,k,p){l.call(this,64,p);this.seq=a;this.idx=c;this.len=e;this.crorg=f;this.data=g;this.senderID=h;this.status=k};y.prototype=new l;y.prototype.constructor=
...
status:this.status}};y.resolve=function(a){var c=a.data,e=c.readUInt8(1)>>6&3,f=c.readUInt8(1)&63;if(0==f){var m=c.readUInt16BE(2);var g=c.readUInt8(4);var h=c.slice(5,c.length-5)}else g=m=null,h=c.slice(2,c.length-5);var k=c.slice(c.length-5,c.length-1).toString("hex");c=c.readUInt8(c.length-1);var l;a.hasOpt&&(l={subTelNum:a.subTelNum,dstId:a.dstId,dBm:a.dBm,securityLvl:a.securityLvl});return new y(e,f,m,g,h,k,c,l)};var C=function(a,c,e,f){l.call(this,208,f);this.messageIndex=a;this.userData=c;
...
f,c,g)};var U=function(a,c,e,f,g){l.call(this,166);this.rorg_en=a;this.data=c;this.dstID=e;this.senderID=f;this.status=g};U.prototype=new l;U.prototype.constructor=U;U.prototype.toPacket=function(){var a=new Buffer(2);a.writeUInt8(this.rorg,0);a.writeUInt8(this.rorg_en,1);a=Buffer.concat([a,this.data]);a=Buffer.concat([a,new Buffer(this.dstID,"hex")]);a=Buffer.concat([a,new Buffer(this.senderID,"hex")]);a=Buffer.concat([a,new Buffer([this.status])]);return new f(a,!1)};var S=function(a,c,e,f,g,h,
k,p,r,n,t){l.call(this,212);this.cmd=a;this.ute=c;this.rspExpected=e;this.subtype=f;this.numChannel=g;this.manufactureID=h;this.eepType=k;this.eepFunc=p;this.eepRorg=r;this.senderID=n;this.status=t};S.prototype=new l;S.prototype.constructor=S;S.prototype.toJSON=function(){return{rorg:this.rorg.toString(16),ute:this.ute,rspExpected:this.rspExpected,subtype:this.subtype,cmd:this.cmd,numChannel:this.numChannel,manufactureID:this.manufactureID,eepType:this.eepType.toString(16),eepFunc:this.eepFunc.toString(16),
eepRorg:this.eepRorg.toString(16),senderID:this.senderID,status:this.status}};S.prototype.toPacket=function(){var a=new Buffer(8);a.writeUInt8(this.rorg,0);a.writeUInt8(this.ute<<7|this.rspExpected<<6|this.subtype<<4|this.cmd,1);a.writeUInt8(this.numChannel,2);a.writeUInt16BE(parseInt(this.manufactureID,16),3);a.writeUInt8(this.eepType,5);a.writeUInt8(this.eepFunc,6);a.writeUInt8(this.eepRorg,7);a=Buffer.concat([a,new Buffer(this.senderID,"hex")]);a=Buffer.concat([a,new Buffer([this.status])]);return this.optData?
new f(a,!0,this.optData.subTelNum,this.optData.dstId,this.optData.dBm,this.optData.securityLvl):new f(a,!1)};S.resolve=function(a){var c=a.data.slice(1,8),e=a.data.slice(8,12).toString("hex"),f=a.data.readUInt8(12),g=c.readUInt8(0)>>7&1,h=c.readUInt8(0)&15,m=c.readUInt8(1),k=c.slice(2,4).toString("hex"),l=c.readUInt8(4),p=c.readUInt8(5),r=c.readUInt8(6),n=c.readUInt8(0)>>6&1;c=c.readUInt8(0)>>4&3;e=new S(h,g,n,c,m,k,l,p,r,e,f);a.hasOpt&&(e.optData={subTelNum:a.subTelNum,dstId:a.dstId,dBm:a.dBm,securityLvl:a.securityLvl});
return e};var X=function(a,c,e,f){l.call(this,53,f);this.userData=a;this.senderID=c;this.status=e};X.prototype=new l;X.prototype.constructor=X;X.prototype.toJSON=function(){return{rorg:this.rorg.toString(16),userData:this.userData.toString("hex"),senderID:this.senderID,status:this.status,optData:this.optData}};X.resolve=function(a){var c=a.data,e=c.slice(1,c.length-5),f=c.slice(c.length-5,c.length-1).toString("hex");c=c.readUInt8(c.length-1);var g;a.hasOpt&&(g={subTelNum:a.subTelNum,dstId:a.dstId,
dBm:a.dBm,securityLvl:a.securityLvl});return new X(e,f,c,g)};var T=function(a,c,e,f){l.call(this,49,f);this.userData=a;this.senderID=c;this.status=e};T.prototype=new l;T.prototype.constructor=T;T.prototype.toJSON=function(){return{rorg:this.rorg.toString(16),userData:this.userData.toString("hex"),senderID:this.senderID,status:this.status,optData:this.optData}};T.prototype.convertToPacket=function(a){a=Buffer.concat([a,new Buffer(this.senderID,"hex"),new Buffer([this.status])]);return this.optData?
new f(a,!0,this.optData.subTelNum,this.optData.dstId,this.optData.dBm,this.optData.securityLvl):new f(a,!1)};T.resolve=function(a){var c=a.data,e=c.slice(1,c.length-5),f=c.slice(c.length-5,c.length-1).toString("hex");c=c.readUInt8(c.length-1);var g;a.hasOpt&&(g={subTelNum:a.subTelNum,dstId:a.dstId,dBm:a.dBm,securityLvl:a.securityLvl});return new T(e,f,c,g)};l.SIG=C;l.RPS=I;l.BS1=J;l.BS4=z;l.VLD=w;l.ADT=U;l.UTE=S;l.TS=X;l.SM=T;l.GP_TI=p;l.GP_TR=r;l.GP_CD=v;var t=function(a,c){this._code=a;this._rspData=
c};t.CODE={0:"RET_OK",1:"RET_ERROR",2:"RET_NOT_SUPPORTED",3:"RET_WRONG_PARAM",4:"RET_OPERATION_DENIED",5:"RET_LOCK_SET",6:"RET_BUFFER_TO_SMALL",7:"RET_NO_FREE_BUFFER"};t.prototype.toJSON=function(){return{code:this._code,rspData:this._rspData?this._rspData.toString("hex"):null}};t.prototype.toPacket=function(){return new a(this._code,this._rspData)};t.resolve=function(a){return new t(a.retCode,a.resData)};r=function(a,c){t.call(this,0);this._responseTime=a;this._confirmCode=c};r.prototype.toPacket=
function(){var c=new Buffer(3);c.writeUInt16BE(this._responseTime,0);c.writeUInt8(this._confirmCode,2);return new a(this._code,c)};r.prototype.toJSON=function(){return{code:this._code,responseTime:this._responseTime,confirmCode:this._confirmCode}};t.Response_OK_SA_CONFIRM_LEARN=r;var n=function(a){this._code=a};n.resolve=function(a){if(!a.data)return null;switch(a.data.readUInt8(0)){case 1:return Y.resolve(a);case 2:return K.resolve(a);default:return null}};var Y=function(){this._code=1};Y.prototype=
new n;Y.prototype.constructor=Y;Y.prototype.toJSON=function(){return{code:this._code}};Y.prototype.toPacket=function(){var a=new Buffer(1);a.writeUInt8(this._code,0);return new h(a)};Y.resolve=function(a){return new Y};var K=function(a,c,e,f,g,h,k){this._code=2;this._priority=a;this._manufacturerId=c;this._eep=e;this._rssi=f;this._pm_candidateId=g;this._sa_clientId=h;this._hop_count=k};K.prototype=new n;K.prototype.constructor=K;K.prototype.getEEP=function(){return this._eep?this._eep.substr(0,2)+
...
7).toString("hex"),g=c.readUInt8(7),h=c.slice(8,12).toString("hex"),k=c.slice(12,16).toString("hex");c=c.readUInt8(16);return new K(a,e,f,g,h,k,c)};n.SA_RECLAIM_NOT_SUCCESSFUL=Y;n.SA_CONFIRM_LEARN=K;var oa=function(a){this._code=a};oa.resolve=function(a){if(!a.data)return null;switch(a.data.readUInt8(0)){case 9:return CO_WR_REPEATER.resolve(a);default:return null}};CO_WR_REPEATER=function(a,c){this._code=9;this._enable=a;this._level=c};CO_WR_REPEATER.prototype=new oa;CO_WR_REPEATER.prototype.constructor=
CO_WR_REPEATER;CO_WR_REPEATER.prototype.toJSON=function(){return{code:this._code,enable:this._enable,level:this._level}};CO_WR_REPEATER.prototype.toPacket=function(){var a=new Buffer(3);a.writeUInt8(this._code,0);a.writeUInt8(this._enable,1);a.writeUInt8(this._level,2);return new k(a)};CO_WR_REPEATER.resolve=function(a){a=a.data.readUInt8(1);var c=CO_WR_REPEATER.readUInt8(2);return new CO_WR_REPEATER(a,c)};oa.CO_WR_REPEATER=CO_WR_REPEATER;var N=function(a){this._code=a};N.resolve=function(a){if(!a.data)return null;
switch(a.data.readUInt8(0)){case 1:return O.resolve(a);case 6:return Z.resolve(a);case 8:return aa.resolve(a);default:return null}};var O=function(a,c,e){this._code=1;this._enable=a;this._extended=c;this._timeout=e};O.prototype=new N;O.prototype.constructor=O;O.prototype.toJSON=function(){return{code:this._code,enable:this._enable,extended:this._extended,timeout:this._timeout}};O.prototype.toPacket=function(){var a=new Buffer(7);a.writeUInt8(this._code,0);a.writeUInt8(this._enable,1);a.writeUInt8(this._extended,
2);a.writeUInt32BE(this._timeout,3);return new c(a)};O.resolve=function(a){var c=a.data;a=c.readUInt8(1);var e=c.readUInt8(2);c=c.readUInt32BE(3);return new O(a,e,c)};var ca=function(a){this._code=5;this._saClientId=a};ca.prototype=new N;ca.prototype.constructor=ca;ca.prototype.toJSON=function(){return{code:this._code,clientId:this._saClientId}};ca.prototype.toPacket=function(){var a=new Buffer(1);a.writeUInt8(this._code,0);a=Buffer.concat([a,new Buffer(this._saClientId,"hex")]);return new c(a)};
ca.resolve=function(a){a=a.data.slice(1,5).toString("hex");return new ca(a)};var Z=function(){this._code=6};Z.prototype=new N;Z.prototype.constructor=Z;Z.prototype.toJSON=function(){return{code:this._code}};Z.prototype.toPacket=function(){var a=new Buffer(1);a.writeUInt8(this._code,0);return new c(a)};Z.resolve=function(a){return new Z};var aa=function(a){this._code=8;this._mailBoxCount=a};aa.prototype=new N;aa.prototype.constructor=aa;aa.prototype.toJSON=function(){return{code:this._code,mailBoxCount:this._mailBoxCount}};
aa.prototype.toPacket=function(){var a=new Buffer(2);a.writeUInt8(this._code,0);a.writeUInt8(this._mailBoxCount,1);return new c(a)};aa.resolve=function(a){a=a.data.readUInt8(1);return new aa(a)};N.SA_WR_LEARNMODE=O;N.SA_WR_RESET=ca;N.SA_RD_LEARNEDCLIENTS=Z;N.SA_WR_POSTMASTER=aa;var Ma=function(){var a=function(a){this._esp3=a;this._buffer={}};a.prototype.receiveChainedTelegram=function(a){var c=a.getSenderID(),e=a.getSeq(),f=a.getIdx();this._buffer[c]||(this._buffer[c]={});var g=this._buffer[c];g[e]||
(g[e]=[]);g=g[e];for(var h=0;h<=f;h++)h!=f?g.length<f&&(g[h]=null):g[h]=a;this._checkChain(c,e)};a.prototype._checkChain=function(a,c){var e=this._buffer[a][c];if(e[0]){var g=e[0],h=g.getLen(),k=g.getCRORG();g=0;var m=new Buffer(1);m.writeUInt8(k);for(k=0;k<e.length;k++){var l=e[k];if(!l)return;l=l.getData();g+=l.length;m=Buffer.concat([m,l])}g>=h&&(delete this._buffer[a][c],m=Buffer.concat([m,new Buffer(a,"hex")]),m=Buffer.concat([m,new Buffer("00","hex")]),a=new f(m,!1),this._esp3._onEORadio_ERP1(a))}};
return a}(),u=function(a){this._logger=require("x.logger.js").getLogger("x.hub.m.enocean.ESP3");this._controller=a;this._listeners=[];a&&this._listeners.push(a);this._recomm=new x.hub.m.enocean.ReComm(this);this._baseID=this._serial=null;this._rdata=new Buffer([]);this._frameBuffer=[];this._learnHandler=null;this._learning=!1;this._learnTO=null;this._cdmHandler=new Ma(this)};u.RESPONSE_TO=1500;u.prototype.attachSerial=function(a,c){var e=this;this._serial=new x.hub.m.enocean.Serial(a);this._serial.setListener(this);
this._serial.open(function(a){a?(e._serial.setListener(null),e._serial=null,c&&c(a)):e.readBaseID(function(a,f){a?(e._serial.setListener(null),e._serial.close(),e._serial=null,c&&c(a)):(x.hub.m.enocean.SENDER_ID||(x.hub.m.enocean.SENDER_ID=f,localStorage&&localStorage.setItem("x.hub.m.enocean.SENDER_ID",f)),e._baseID=f,c&&c())})})};u.prototype.removeSerial=function(a,c){this._serial.setListener(null);this._serial.close();this._baseID=this._serial=null;c&&c()};u.prototype.getSerial=function(){return this._serial};
u.prototype.addListener=function(a){a&&-1==this._listeners.indexOf(a)&&this._listeners.push(a)};u.prototype.removeListener=function(a){a&&(a=this._listeners.indexOf(a),-1!=a&&this._listeners.splice(a,1))};u.prototype.readBaseID=function(a){var c=this,e=new g(5,new Buffer("08","hex"));this.sendFrame(e,!0,function(e,f){e?(c._logger.log("warn","read base id error:"+e.stack),a&&a(e)):f&&f.retCode?(e=Error("read BaseID error"),e.code=f.retCode,c._logger.log("warn","read base id error return code:"+f.retCode),
a&&a(e)):(e=f.resData.toString("hex"),a&&a(null,e.toLowerCase()))},8E3)};u.prototype.startLearn=function(a,c,e,f){if(this._serial){if(!c||0>=c)c=3E4;if(this._learning)f&&f(Error("learning another type right now"));else{this._learnHandler=a;this._learning=!0;var g=this;this._learnTO=setTimeout(function(){g._learning=!1;g._learnTO=null;var a=g._learnHandler;g._learnHandler=null;if(a&&a.onLearnTimeout)a.onLearnTimeout()},c);e?(a=new O(1,e.extended?e.extended:0,0),this.sendSACommand(a,!0,function(a){f&&
f(a)})):f&&f()}}else f&&f(Error("port not opened"))};u.prototype.stopLearn=function(a,c,e){this._learnTO&&clearTimeout(this._learnTO);this._learning=!1;this._learnTO=this._learnHandler=null;if(c){e&&e();var f=this;setTimeout(function(){var a=new O(0,c.extended?c.extended:0,0);f.sendSACommand(a,!0,function(a){e&&e(a)})},2E3)}else e&&e()};u.prototype.sendTelegram=function(a,c,e){this.sendPacket(a.toPacket(),c,e)};u.prototype.sendResponse=function(a,c){this._logger.log("trace","send RESPONSE:"+JSON.stringify(a.toJSON()));
...
u.ESP3_Frame=g;u.Telegram=l;u.RADIO_ERP1_Packet=f;u.Event=n;u.Response=t;u.COMMAND_COMMAND=oa;u.SA_COMMAND=N;return u}();
...
h.prototype=new c;h.prototype.constructor=h;h.prototype.evalTelegram=function(b){var d={};if(246==b.rorg){var a=b.userData>>5&7,c=b.userData>>1&7,e=b.userData&1,f=b.t21,g=b.nu;b=b.userData>>4&1?"pressed":"released";if(1==f&&1==g){switch(a){case 0:var h=b;break;case 1:var k=b;break;case 2:var l=b;break;case 3:var p=b}if(e)switch(c){case 0:h=b;break;case 1:k=b;break;case 2:l=b;break;case 3:p=b}}else if(1==f&&0==g)switch(a){case 0:p=l=k=h=b}h&&(d.buttonAI=h);k&&(d.buttonAO=k);l&&(d.buttonBI=l);p&&(d.buttonBO=
p)}return d};h.prototype.buildData=function(b,d,a,c){return b<<5|d<<4|a<<1|c};h.prototype.checkStateChange=function(b){var d=this._states,a=[],c=(new Date).getTime(),e;for(e in b){var f={};f.key=e;f.value=b[e];f.changed=!1;var g=d[e]?d[e].value:null,h=d[e]?d[e].time:null,k=b[e];g!=k&&(f.oldvalue=g,f.changed=!0);this._states[e]={value:b[e],time:c};f.changed&&a.push(f);"pressed"==g&&"released"==k&&(!h||c-h<this._longPressThreshold?(f={},f.key=e,f.value="shortPress"):(f={},f.key=e,f.value="longPress"),
f.changed=!0,a.push(f))}return a};h.prototype.executeCommand=function(b,d,a){if(this._senderID)if(d&&d.value){var c=this,e=100,q=d.value.substr(d.value.length-2),f={},g=0;switch(q){case "AI":g=0;break;case "AO":g=1;break;case "BI":g=2;break;case "BO":g=3}switch(d.value){case "longPressAI":case "longPressAO":case "longPressBI":case "longPressBO":e=this._longPressThreshold;case "shortPressAI":case "shortPressAO":case "shortPressBI":case "shortPressBO":var h=this.buildData(g,1,0,0);var k=new x.hub.m.enocean.ESP3.Telegram.RPS(h,
this._senderID,1,1,0,null);b.sendTelegram(k,!0,function(d){d?a&&a(d):(f["button"+q]="pressed",c.activeStateChange(f),setTimeout(function(){h=c.buildData(0,0,0,0);k=new x.hub.m.enocean.ESP3.Telegram.RPS(h,c._senderID,1,1,0,null);b.sendTelegram(k,!0,function(b){f["button"+q]="released";c.activeStateChange(f);a&&a(b)})},e))});break;case "pressAI":case "pressAO":case "pressBI":case "pressBO":h=this.buildData(g,1,0,0);k=new x.hub.m.enocean.ESP3.Telegram.RPS(h,this._senderID,1,1,0,null);b.sendTelegram(k,
!0,function(b){f["button"+q]="pressed";c.activeStateChange(f);a&&a(b)});break;case "releaseAI":case "releaseAO":case "releaseBI":case "releaseBO":h=c.buildData(0,0,0,0);k=new x.hub.m.enocean.ESP3.Telegram.RPS(h,this._senderID,1,1,0,null);b.sendTelegram(k,!0,function(b){f["button"+q]="released";c.activeStateChange(f);a&&a(b)});break;default:a&&a(Error("unknow command"))}}else a&&a(Error("command invalid"));else a&&a(Error("sender id is null"))};var l=function(b){h.call(this,b);this._data="f6-02-02"};
h.STATES=["buttonAI","buttonAO","buttonBI","buttonBO"];l.COMMANDS="shortPressAI longPressAI pressAI releaseAI shortPressAO longPressAO pressAO releaseAO shortPressBI longPressBI pressBI releaseBI shortPressBO longPressBO pressBO releaseBO".split(" ");l.prototype=new h;l.prototype.constructor=l;var p=function(b){c.call(this,b);this._data="f6-02-03"};p.STATES=["state"];p.COMMANDS=["auto","manu","dimUp","dimDown"];p.prototype=new c;p.prototype.constructor=p;p.prototype.evalTelegram=function(b){var d=
{};if(246==b.rorg)switch(b.userData){case 48:d.state="auto";break;case 16:d.state="manu";break;case 112:d.state="dimUp";break;case 80:d.state="dimDown"}return d};p.prototype.executeCommand=function(b,d,a){if(this._senderID)if(d&&d.value){switch(d.value){case "auto":var c=48;break;case "manu":c=16;break;case "dimUp":c=112;break;case "dimDown":c=80;break;default:a&&a(Error("unknow command"))}this.activeStateChange({state:d.value});d=new x.hub.m.enocean.ESP3.Telegram.RPS(c,this._senderID,1,1,0,null);
b.sendTelegram(d,!0,function(b){a&&a(b)})}else a&&a(Error("command invalid"));else a&&a(Error("sender id is null"))};var r=function(b){h.call(this,b);this._data="f6-02-04";this._longPressThreshold=1E3};r.STATES=["buttonAI","buttonAO","buttonBI","buttonBO","energyBow"];r.prototype=new h;r.prototype.constructor=r;r.prototype.evalTelegram=function(b){var d={};if(246==b.rorg){var a=b.userData>>3&1,c=b.userData>>2&1,e=b.userData>>1&1,f=b.userData&1;d.energyBow=b.userData>>7&1?"pressed":"released";d.buttonBI=
...
b);this._data="f6-05-02"};w.STATES=["smoke","lowbat"];w.prototype=new c;w.prototype.constructor=w;w.prototype.evalTelegram=function(b){var d={};if(246==b.rorg)switch(b.userData){case 0:d.smoke="ok";this._states.lowbat||(d.lowbat=!1);break;case 16:d.smoke="alarm";this._states.lowbat||(d.lowbat=!1);break;case 48:d.lowbat=!0}return d};var U=function(b){c.call(this,b);this._data="f6-10-00"};U.STATES=["window","handle_pos","handle_raw"];U.prototype=new c;U.prototype.constructor=U;U.prototype.evalTelegram=
function(b){var d={};246==b.rorg&&(b=b.userData,0==(b&208^192)?(d.window="open",d.handle_pos="middle"):0==(b&240^240)?(d.window="closed",d.handle_pos="down"):0==(b&240^208)&&(d.window="tilted",d.handle_pos="up"),d.handle_raw=b);return d};var S=function(b){c.call(this,b);this._data="f6-10-01"};S.STATES=["window","handle_pos","handle_raw"];S.prototype=new c;S.prototype.constructor=S;S.prototype.evalTelegram=function(b){var d={};246==b.rorg&&(b=b.userData&15,0==(b&13^12)?(d.window="open",d.handle_pos=
"middle"):0==(b&15^15)?(d.window="closed",d.handle_pos="down"):0==(b&15^13)&&(d.window="tilted",d.handle_pos="up"),d.handle_raw=b);return d};var X=function(b){g.call(this,b);this._data="d5-??-??"};X.prototype=new g;X.prototype.constructor=X;var T=function(b){X.call(this,b);this._data="d5-00-01"};T.STATES=["contact","contact_raw"];T.prototype=new X;T.prototype.constructor=T;T.prototype.evalTelegram=function(b){var d={};213==b.rorg&&(b=b.userData,1==(b>>3&1)&&(b&=1,d.contact=b?"closed":"open",d.contact_raw=
...
b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(2);1==(a>>3&1)&&(d.temperature=parseFloat(((255-b)/255*(this._tmpMax-this._tmpMin)+this._tmpMin).toFixed(1)),d.temperature_raw=b)}return d};var Y=function(b){n.call(this,b);this._data="a5-02-01";this._tmpMin=-40;this._tmpMax=0};Y.STATES=["temperature","temperature_raw"];Y.prototype=new n;Y.prototype.constructor=Y;var K=function(b){n.call(this,b);this._data="a5-02-02";this._tmpMin=-30;this._tmpMax=10};K.STATES=["temperature","temperature_raw"];
K.prototype=new n;K.prototype.constructor=K;var oa=function(b){n.call(this,b);this._data="a5-02-03";this._tmpMin=-20;this._tmpMax=20};oa.STATES=["temperature","temperature_raw"];oa.prototype=new n;oa.prototype.constructor=oa;var N=function(b){n.call(this,b);this._data="a5-02-04";this._tmpMin=-10;this._tmpMax=30};N.STATES=["temperature","temperature_raw"];N.prototype=new n;N.prototype.constructor=N;var O=function(b){n.call(this,b);this._data="a5-02-05";this._tmpMin=0;this._tmpMax=40};O.STATES=["temperature",
"temperature_raw"];O.prototype=new n;O.prototype.constructor=O;var ca=function(b){n.call(this,b);this._data="a5-02-06";this._tmpMin=10;this._tmpMax=50};ca.STATES=["temperature","temperature_raw"];ca.prototype=new n;ca.prototype.constructor=ca;var Z=function(b){n.call(this,b);this._data="a5-02-07";this._tmpMin=20;this._tmpMax=60};Z.STATES=["temperature","temperature_raw"];Z.prototype=new n;Z.prototype.constructor=Z;var aa=function(b){n.call(this,b);this._data="a5-02-08";this._tmpMin=30;this._tmpMax=
70};aa.STATES=["temperature","temperature_raw"];aa.prototype=new n;aa.prototype.constructor=aa;var Ma=function(b){n.call(this,b);this._data="a5-02-09";this._tmpMin=40;this._tmpMax=80};Ma.STATES=["temperature","temperature_raw"];Ma.prototype=new n;Ma.prototype.constructor=Ma;var u=function(b){n.call(this,b);this._data="a5-02-0a";this._tmpMin=50;this._tmpMax=90};u.STATES=["temperature","temperature_raw"];u.prototype=new n;u.prototype.constructor=u;var m=function(b){n.call(this,b);this._data="a5-02-0b";
this._tmpMin=60;this._tmpMax=100};m.STATES=["temperature","temperature_raw"];m.prototype=new n;m.prototype.constructor=m;var H=function(b){n.call(this,b);this._data="a5-02-10";this._tmpMin=-60;this._tmpMax=20};H.STATES=["temperature","temperature_raw"];H.prototype=new n;H.prototype.constructor=H;var ac=function(b){n.call(this,b);this._data="a5-02-11";this._tmpMin=-50;this._tmpMax=30};ac.STATES=["temperature","temperature_raw"];ac.prototype=new n;ac.prototype.constructor=ac;var La=function(b){n.call(this,
b);this._data="a5-02-12";this._tmpMin=-40;this._tmpMax=40};La.STATES=["temperature","temperature_raw"];La.prototype=new n;La.prototype.constructor=La;var bc=function(b){n.call(this,b);this._data="a5-02-13";this._tmpMin=-30;this._tmpMax=50};bc.STATES=["temperature","temperature_raw"];bc.prototype=new n;bc.prototype.constructor=bc;var cc=function(b){n.call(this,b);this._data="a5-02-14";this._tmpMin=-20;this._tmpMax=60};cc.STATES=["temperature","temperature_raw"];cc.prototype=new n;cc.prototype.constructor=
cc;var Ca=function(b){n.call(this,b);this._data="a5-02-15";this._tmpMin=-10;this._tmpMax=70};Ca.STATES=["temperature","temperature_raw"];Ca.prototype=new n;Ca.prototype.constructor=Ca;var dc=function(b){n.call(this,b);this._data="a5-02-16";this._tmpMin=0;this._tmpMax=80};dc.STATES=["temperature","temperature_raw"];dc.prototype=new n;dc.prototype.constructor=dc;var $a=function(b){n.call(this,b);this._data="a5-02-17";this._tmpMin=10;this._tmpMax=90};$a.STATES=["temperature","temperature_raw"];$a.prototype=
new n;$a.prototype.constructor=$a;var ec=function(b){n.call(this,b);this._data="a5-02-18";this._tmpMin=20;this._tmpMax=100};ec.STATES=["temperature","temperature_raw"];ec.prototype=new n;ec.prototype.constructor=ec;var fc=function(b){n.call(this,b);this._data="a5-02-19";this._tmpMin=30;this._tmpMax=110};fc.STATES=["temperature","temperature_raw"];fc.prototype=new n;fc.prototype.constructor=fc;var gc=function(b){n.call(this,b);this._data="a5-02-1a";this._tmpMin=40;this._tmpMax=120};gc.STATES=["temperature",
"temperature_raw"];gc.prototype=new n;gc.prototype.constructor=gc;var hc=function(b){n.call(this,b);this._data="a5-02-1b";this._tmpMin=50;this._tmpMax=130};hc.STATES=["temperature","temperature_raw"];hc.prototype=new n;hc.prototype.constructor=hc;var Da=function(b){t.call(this,b);this._data="a5-02-20";this._tmpMin=-10;this._tmpMax=41.2};Da.STATES=["temperature","temperature_raw"];Da.prototype=new t;Da.prototype.constructor=Da;Da.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);
b=b.userData.readUInt16BE(1);1==(a>>3&1)&&(a=b&1023,d.temperature=parseFloat(((1023-a)/1023*(this._tmpMax-this._tmpMin)+this._tmpMin).toFixed(1)),d.temperature_raw=a)}return d};var ic=function(b){Da.call(this,b);this._data="a5-02-30";this._tmpMin=-40;this._tmpMax=62.3};ic.STATES=["temperature","temperature_raw"];ic.prototype=new Da;ic.prototype.constructor=ic;var ha=function(b){t.call(this,b);this._data="a5-04-??";this._humMin=0;this._humMax=100;this._tempMin=0;this._tempMax=40};ha.STATES=["humidity",
"humidity_raw","temperature","temperature_raw","available"];ha.prototype=new t;ha.prototype.constructor=ha;ha.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2);b=b.userData.readUInt8(1);1==(a>>3&1)&&(d.humidity=parseFloat((b/250*(this._humMax-this._humMin)+this._humMin).toFixed(1)),d.humidity_raw=b,d.temperature=parseFloat((c/250*(this._tempMax-this._tempMin)+this._tempMin).toFixed(1)),d.temperature_raw=c,d.available=a>>1&1?!0:!1)}return d};
var jc=function(b){ha.call(this,b);this._data="a5-04-01"};jc.STATES=ha.STATES;jc.prototype=new ha;jc.prototype.constructor=jc;var Na=function(b){ha.call(this,b);this._data="a5-04-02";this._tempMin=-20;this._tempMax=60};Na.STATES=ha.STATES;Na.prototype=new ha;Na.prototype.constructor=Na;var ab=function(b){ha.call(this,b);this._data="a5-04-03";this._tempMin=-20;this._tempMax=60};ab.STATES=["humidity","humidity_raw","temperature","temperature_raw","type"];ab.prototype=new ha;ab.prototype.constructor=
ab;ab.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt16BE(1);b=b.userData.readUInt8(0);1==(a>>3&1)&&(d.humidity=parseFloat((b/255*(this._humMax-this._humMin)+this._humMin).toFixed(1)),d.humidity_raw=b,c&=1023,d.temperature=parseFloat((c/1023*(this._tempMax-this._tempMin)+this._tempMin).toFixed(1)),d.temperature_raw=c,d.type=a&1?"event":"heartbeat")}return d};var L=function(b){t.call(this,b);this._data="a5-06-??";this._svcMin=0;this._svcMax=
...
d,b.changed=!0);this._states.state={value:"off",time:(new Date).getTime()};require("x.hub.m.enocean.js")._onEOEvents(this,[b])};V.prototype.executeCommand=function(b,d,a){if(d&&d.value)switch(d.value){case "reset":this._reset();a&&a();break;default:a&&a(Error("no such command"))}else a&&a(Error("command invalid"))};V.prototype.fromJSON=function(b){if(!g.prototype.fromJSON.call(this,b))return null;this._params||(this._params={reset_timeout:30});return this};V.prototype.fromDBJSON=function(b){if(!g.prototype.fromDBJSON.call(this,
b))return null;this._params||(this._params={reset_timeout:30});return this};var ka=function(b){V.call(this,b);this._data="a5-07-01"};ka.COMMANDS=["reset"];ka.STATES=["voltage","voltage_raw","state","state_raw","voltage_avail"];ka.prototype=new V;ka.prototype.constructor=ka;ka.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2);b=b.userData.readUInt8(0);1==(a>>3&1)&&(d.voltage=parseFloat((b/250*5).toFixed(1)),d.voltage_raw=b,d.state=128<=
c?"on":"off",d.state_raw=c,d.voltage_avail=a&1?!0:!1,d.voltage_avail||delete d.voltage)}"on"==d.state?this._startPIROffTO():"off"==d.state&&this._stopPIROffTO();return d};var Pa=function(b){V.call(this,b);this._data="a5-07-02"};Pa.COMMANDS=["reset"];Pa.STATES=["voltage","voltage_raw","state","state_raw"];Pa.prototype=new V;Pa.prototype.constructor=Pa;Pa.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);1==(a>>3&1)&&(d.voltage=parseFloat((b/
250*5).toFixed(1)),d.voltage_raw=b,a=a>>7&1,d.state=a?"on":"off",d.state_raw=a)}"on"==d.state?this._startPIROffTO():"off"==d.state&&this._stopPIROffTO();return d};var Qa=function(b){V.call(this,b);this._data="a5-07-03"};Qa.COMMANDS=["reset"];Qa.STATES="voltage voltage_raw illumination illumination_raw state state_raw".split(" ");Qa.prototype=new V;Qa.prototype.constructor=Qa;Qa.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt16BE(1);b=
b.userData.readUInt8(0);1==(a>>3&1)&&(d.voltage=parseFloat((b/250*5).toFixed(1)),d.voltage_raw=b,c=c>>6&1023,d.illumination=c,d.illumination_raw=c,a=a>>7&1,d.state=a?"on":"off",d.state_raw=a)}"on"==d.state?this._startPIROffTO():"off"==d.state&&this._stopPIROffTO();return d};var va=function(b){t.call(this,b);this._data="a5-08-??";this._svcMin=0;this._svcMax=5.1;this._illMin=0;this._illMax=510;this._tmpMin=0;this._tmpMax=51};va.prototype=new t;va.prototype.constructor=va;va.prototype.evalTelegram=function(b){var d=
...
"off"}d.state_raw=c;a&=1;switch(a){case 0:d.button="pressed";break;case 1:d.button="released"}d.button_raw=a}}return d};var Ra=function(b){va.call(this,b);this._data="a5-08-01"};Ra.STATES="voltage voltage_raw illumination illumination_raw temperature temperature_raw state state_raw button button_raw".split(" ");Ra.prototype=new va;Ra.prototype.constructor=Ra;var lc=function(b){va.call(this,b);this._data="a5-08-02";this._illMax=1020};lc.STATES="voltage voltage_raw illumination illumination_raw temperature temperature_raw state state_raw button button_raw".split(" ");
lc.prototype=new va;lc.prototype.constructor=lc;var mc=function(b){va.call(this,b);this._data="a5-08-03";this._illMax=1530;this._tmpMin=-30;this._tmpMax=50};mc.STATES="voltage voltage_raw illumination illumination_raw temperature temperature_raw state state_raw button button_raw".split(" ");mc.prototype=new va;mc.prototype.constructor=mc;var D=function(b){t.call(this,b);this._data="a5-09-??"};D.prototype=new t;D.prototype.constructor=D;var db=function(b){D.call(this,b);this._data="a5-09-02";this._svcMin=
0;this._svcMax=5.1;this._gasMin=0;this._gasMax=1020;this._tmpMin=0;this._tmpMax=51};db.STATES="voltage voltage_raw gas gas_raw temperature temperature_raw temperature_available".split(" ");db.prototype=new D;db.prototype.constructor=db;db.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);1==(a>>3&1)&&(d.voltage=parseFloat((b/255*(this._svcMax-this._svcMin)+this._svcMin).toFixed(1)),
d.voltage_raw=b,d.gas=parseFloat((e/255*(this._gasMax-this._gasMin)+this._gasMin).toFixed(1)),d.gas_raw=e,d.temperature=parseFloat((c/255*(this._tmpMax-this._tmpMin)+this._tmpMin).toFixed(1)),d.temperature_raw=c,a=a>>1&1,d.temperature_available=a?!0:!1,a||delete d.temperature)}return d};var eb=function(b){D.call(this,b);this._data="a5-09-04";this._humMin=0;this._humMax=100;this._gasMin=0;this._gasMax=2550;this._tmpMin=0;this._tmpMax=51};eb.STATES="humidity humidity_raw gas gas_raw temperature temperature_raw temperature_available humidity_available".split(" ");
eb.prototype=new D;eb.prototype.constructor=eb;eb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);1==(a>>3&1)&&(d.humidity=parseFloat((b/255*(this._humMax-this._humMin)+this._humMin).toFixed(1)),d.humidity_raw=b,d.gas=parseFloat((e/255*(this._gasMax-this._gasMin)+this._gasMin).toFixed(1)),d.gas_raw=e,d.temperature=parseFloat((c/255*(this._tmpMax-this._tmpMin)+this._tmpMin).toFixed(1)),
d.temperature_raw=c,c=a>>2&1,d.humidity_available=c?!0:!1,c||delete d.humidity,a=a>>1&1,d.temperature_available=a?!0:!1,a||delete d.temperature)}return d};var fb=function(b){D.call(this,b);this._data="a5-09-05"};fb.STATES=["voc","voc_raw","voc_id","voc_id_raw","scale"];fb.prototype=new D;fb.prototype.constructor=fb;fb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2);b=b.userData.readUInt16BE(0);if(1==(a>>3&1)){a&=3;var e=1;switch(a){case 0:e=
...
"Diethyl ether";break;case 255:d.voc_id="ozone"}d.voc_id_raw=c}}return d};var gb=function(b){D.call(this,b);this._data="a5-09-06"};gb.STATES=["radon"];gb.prototype=new D;gb.prototype.constructor=gb;gb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt16BE(0);1==(a>>3&1)&&(d.radon=b>>6&1023)}return d};var hb=function(b){D.call(this,b);this._data="a5-09-07"};hb.STATES="pm10 pm25 pm1 pm10_active pm25_active pm25_active".split(" ");hb.prototype=
new D;hb.prototype.constructor=hb;hb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt16BE(2),e=b.userData.readUInt16BE(1);b=b.userData.readUInt16BE(0);1==(a>>3&1)&&(d.pm10=b>>7&511,d.pm25=e>>6&511,d.pm1=c>>5&511,c=a>>2&1,d.pm10_active=c?!0:!1,c||delete d.pm10,c=a>>1&1,d.pm25_active=c?!0:!1,c||delete d.pm25,a&=1,d.pm1_active=a?!0:!1,a||delete d.pm1)}return d};var ib=function(b){D.call(this,b);this._data="a5-09-08"};ib.STATES=["co2","co2_raw"];
ib.prototype=new D;ib.prototype.constructor=ib;ib.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(2);1==(a>>3&1)&&(d.co2=parseFloat((b/255*2E3).toFixed(1)),d.co2_raw=b)}return d};var jb=function(b){D.call(this,b);this._data="a5-09-09"};jb.STATES=["co2","co2_raw","power_failure_detected"];jb.prototype=new D;jb.prototype.constructor=jb;jb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(2);
1==(a>>3&1)&&(d.co2=parseFloat((b/255*2E3).toFixed(1)),d.co2_raw=b,d.power_failure_detected=a>>2&1?!0:!1)}return d};var kb=function(b){D.call(this,b);this._data="a5-09-0a";this._svcMin=2;this._svcMax=5;this._tmpMin=-20;this._tmpMax=60};kb.STATES="voltage voltage_raw gas gas_raw temperature temperature_raw temperature_available voltage_available".split(" ");kb.prototype=new D;kb.prototype.constructor=kb;kb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=
b.userData.readUInt8(2);b=b.userData.readUInt16BE(0);1==(a>>3&1)&&(d.gas=b,d.gas_raw=b,d.temperature=parseFloat((c/255*(this._tmpMax-this._tmpMin)+this._tmpMin).toFixed(1)),d.temperature_raw=c,c=a>>4&15,d.voltage=parseFloat((c/15*(this._svcMax-this._svcMin)+this._svcMin).toFixed(1)),d.voltage_raw=c,c=a>>1&1,d.temperature_available=c?!0:!1,c||delete d.temperature,a&=1,d.voltage_available=a?!0:!1,a||delete d.voltage)}return d};var lb=function(b){D.call(this,b);this._data="a5-09-0b";this._svcMin=2;this._svcMax=
5};lb.STATES="voltage voltage_raw radio radio_raw scale unit unit_raw voltage_available".split(" ");lb.prototype=new D;lb.prototype.constructor=lb;lb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt16BE(2);b=b.userData.readUInt8(0);if(1==(a>>3&1)){b=b>>4&15;d.voltage=parseFloat((b/15*(this._svcMax-this._svcMin)+this._svcMin).toFixed(1));d.voltage_raw=b;b=1;switch(a>>4&15){case 0:b=.001;break;case 1:b=.01;break;case 2:b=.1;break;case 3:b=
1;break;case 4:b=10;break;case 5:b=100;break;case 6:b=1E3;break;case 7:b=1E4;break;case 8:b=1E5}d.scale=b;d.radio=c/10*b;d.radio_raw=c;c=a>>1&3;switch(c){case 0:d.unit="\u03bcSv/h";break;case 1:d.unit="cpm";break;case 2:d.unit="Bq/L";break;case 3:d.unit="Bq/kg"}d.unit_raw=c;a&=1;d.voltage_available=a?!0:!1;a||delete d.voltage}}return d};var mb=function(b){D.call(this,b);this._data="a5-09-0c"};mb.STATES="voc voc_raw voc_id voc_id_raw unit unit_raw scale".split(" ");mb.prototype=new D;mb.prototype.constructor=
mb;mb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2);b=b.userData.readUInt16BE(0);if(1==(a>>3&1)){var e=a&3,f=1;switch(e){case 0:f=.01;break;case 1:f=.1;break;case 2:f=1;break;case 3:f=10}d.scale=f;a=a>>1&1;switch(a){case 0:d.unit="ppb";break;case 1:d.unit="\u03bcSv/m3"}d.unit_raw=a;d.voc=b*f;d.voc_raw=b;switch(e){case 0:d.voc_id="VOCT";break;case 1:d.voc_id="Formaldehyde";break;case 2:d.voc_id="Benzene";break;case 3:d.voc_id="Styrene";
...
break;case 32:d.voc_id="Indole";break;case 33:d.voc_id="Cadaverine";break;case 34:d.voc_id="Putrescine";break;case 35:d.voc_id="Caproic acid";break;case 255:d.voc_id="ozone"}d.voc_id_raw=c}}return d};var Sa=function(b){t.call(this,b);this._data="a5-10-??"};Sa.prototype=new t;Sa.prototype.constructor=Sa;var B=function(b){Sa.call(this,b);this._data="a5-10-01";this._params||(this._params={minSP:18,maxSP:26})};B.STATES="fan_speed fan_speed_raw setpoint setpoint_raw temperature temperature_raw button button_raw".split(" ");
B.prototype=new Sa;B.prototype.constructor=B;B.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.fan_speed=144>=b?"3":164>=b?"2":189>=b?"1":209>=b?"0":"auto";d.fan_speed_raw=b;d.setpoint=parseFloat((e/255*(this._params.maxSP-this._params.minSP)+this._params.minSP).toFixed(1));d.setpoint_raw=e;d.temperature=parseFloat(((255-c)/255*40).toFixed(1));d.temperature_raw=
...
la.prototype=new B;la.prototype.constructor=la;la.prototype.evalTelegram=function(b){var d=B.prototype.evalTelegram.call(this,b);if(0==Object.keys(d).length)return d;delete d.button;delete d.button_raw;b=b.userData.readUInt8(3)&1;d.slider=b?"day":"night";d.slider_raw=b;return d};var Fa=function(b){B.call(this,b);this._data="a5-10-03";this.TEACH_IN_VARIATION=1};Fa.STATES=["setpoint","setpoint_raw","temperature","temperature_raw"];Fa.prototype=new B;Fa.prototype.constructor=Fa;Fa.prototype.evalTelegram=
function(b){b=B.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.fan_speed;delete b.fan_speed_raw;delete b.button;delete b.button_raw;return b};var nb=function(b){B.call(this,b);this._data="a5-10-04"};nb.STATES="fan_speed fan_speed_raw setpoint setpoint_raw temperature temperature_raw".split(" ");nb.prototype=new B;nb.prototype.constructor=nb;nb.prototype.evalTelegram=function(b){b=B.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.button;
delete b.button_raw;return b};var ob=function(b){B.call(this,b);this._data="a5-10-05"};ob.STATES="setpoint setpoint_raw temperature temperature_raw button button_raw".split(" ");ob.prototype=new B;ob.prototype.constructor=ob;ob.prototype.evalTelegram=function(b){b=B.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.fan_speed;delete b.fan_speed_raw;return b};var Ga=function(b){la.call(this,b);this._data="a5-10-06"};Ga.STATES="setpoint setpoint_raw temperature temperature_raw slider slider_raw".split(" ");
Ga.prototype=new la;Ga.prototype.constructor=Ga;Ga.prototype.evalTelegram=function(b){b=la.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.fan_speed;delete b.fan_speed_raw;return b};var pb=function(b){B.call(this,b);this._data="a5-10-07"};pb.STATES=["fan_speed","fan_speed_raw","temperature","temperature_raw"];pb.prototype=new B;pb.prototype.constructor=pb;pb.prototype.evalTelegram=function(b){b=B.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;
...
b)};qa.prototype.fromDBJSON=function(b){return g.prototype.fromDBJSON.call(this,b)};var za=function(b){B.call(this,b);this._data="a5-10-0a"};za.STATES="setpoint setpoint_raw temperature temperature_raw contact contact_raw".split(" ");za.prototype=new B;za.prototype.constructor=za;za.prototype.evalTelegram=function(b){var d=B.prototype.evalTelegram.call(this,b);if(0==Object.keys(d).length)return d;delete d.fan_speed;delete d.fan_speed_raw;delete d.button;delete d.button_raw;b=b.userData.readUInt8(3)&
1;d.contact=b?"open":"closed";d.contact_raw=b;return d};var Ha=function(b){za.call(this,b);this._data="a5-10-0b";this._params=null};Ha.STATES=["temperature","temperature_raw","contact","contact_raw"];Ha.prototype=new za;Ha.prototype.constructor=Ha;Ha.prototype.evalTelegram=function(b){b=za.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.setpoint;delete b.setpoint_raw;return b};Ha.prototype.fromJSON=function(b){return g.prototype.fromJSON.call(this,b)};Ha.prototype.fromDBJSON=
function(b){return g.prototype.fromDBJSON.call(this,b)};var qb=function(b){pa.call(this,b);this._data="a5-10-0c"};qb.STATES=["temperature","temperature_raw","button","button_raw"];qb.prototype=new pa;qb.prototype.constructor=qb;qb.prototype.evalTelegram=function(b){b=pa.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.fan_speed;delete b.fan_speed_raw;return b};var rb=function(b){qa.call(this,b);this._data="a5-10-0d"};rb.STATES=["temperature","temperature_raw","slider",
"slider_raw"];rb.prototype=new qa;rb.prototype.constructor=rb;rb.prototype.evalTelegram=function(b){b=qa.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.fan_speed;delete b.fan_speed_raw;return b};var P=function(b){Sa.call(this,b);this._data="a5-10-10";this._params||(this._params={minSP:18,maxSP:26})};P.STATES="setpoint setpoint_raw humidity humidity_raw temperature temperature_raw button button_raw".split(" ");P.prototype=new Sa;P.prototype.constructor=P;P.prototype.evalTelegram=
...
P.prototype.fromJSON=function(b){if(!g.prototype.fromJSON.call(this,b))return null;this._params||(this._params={minSP:18,maxSP:26});return this};P.prototype.fromDBJSON=function(b){if(!g.prototype.fromDBJSON.call(this,b))return null;this._params||(this._params={minSP:18,maxSP:26});return this};var Aa=function(b){P.call(this,b);this._data="a5-10-11"};Aa.STATES="setpoint setpoint_raw humidity humidity_raw temperature temperature_raw slider slider_raw".split(" ");Aa.prototype=new P;Aa.prototype.constructor=
Aa;Aa.prototype.evalTelegram=function(b){var d=P.prototype.evalTelegram.call(this,b);if(0==Object.keys(d).length)return d;delete d.button;delete d.button_raw;b=b.userData.readUInt8(3)&1;d.slider=b?"day":"night";d.slider_raw=b;return d};var sb=function(b){P.call(this,b);this._data="a5-10-12"};sb.STATES="setpoint setpoint_raw humidity humidity_raw temperature temperature_raw".split(" ");sb.prototype=new P;sb.prototype.constructor=sb;sb.prototype.evalTelegram=function(b){b=P.prototype.evalTelegram.call(this,
b);if(0==Object.keys(b).length)return b;delete b.button;delete b.button_raw;return b};var tb=function(b){P.call(this,b);this._data="a5-10-13"};tb.STATES="humidity humidity_raw temperature temperature_raw button button_raw".split(" ");tb.prototype=new P;tb.prototype.constructor=tb;tb.prototype.evalTelegram=function(b){b=P.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.setpoint;delete b.setpoint_raw;return b};var ub=function(b){Aa.call(this,b);this._data="a5-10-14"};
ub.STATES="humidity humidity_raw temperature temperature_raw slider slider_raw".split(" ");ub.prototype=new Aa;ub.prototype.constructor=ub;ub.prototype.evalTelegram=function(b){b=Aa.prototype.evalTelegram.call(this,b);if(0==Object.keys(b).length)return b;delete b.setpoint;delete b.setpoint_raw;return b};var Ta=function(b){t.call(this,b);this._data="a5-11-??"};Ta.prototype=new t;Ta.prototype.constructor=Ta;var vb=function(b){Ta.call(this,b);this._data="a5-11-03"};vb.STATES="position angle position_flag position_flag_raw angle_flag angle_flag_raw error error_raw end_position end_position_raw state state_raw service_mode position_mode power energy current voltage power_raw power_unit energy_unit current_unit voltage_unit power_unit_raw".split(" ");
vb.prototype=new Ta;vb.prototype.constructor=vb;vb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.position=b;b=e&127;d.angle=2*(e>>7&1?0-b:b);e=c>>7&1;d.position_flag=e?!0:!1;d.position_flag_raw=e;e=c>>6&1;d.angle_flag=e?!0:!1;d.angle_flag_raw=e;e=c>>4&3;d.error_raw=e;switch(e){case 0:d.error="no_error";break;case 1:d.error="no_end_position";break;case 2:d.error=
"intern_failure"}e=c>>2&3;d.end_position_raw=e;switch(e){case 0:d.end_position="no_end_position";break;case 1:d.end_position="end_position_not_reach";break;case 2:d.end_position="fully_open";break;case 3:d.end_position="fully_closed"}c&=3;d.state_raw=c;switch(c){case 0:d.state="no_status";break;case 1:d.state="stopped";break;case 2:d.state="opens";break;case 3:d.state="closes"}d.service_mode=a>>7&1;d.position_mode=a>>6&1}}return d};var Ua=function(b){Ta.call(this,b);this._data="a5-11-04"};Ua.STATES=
"state state_raw level rgb power energy current voltage power_raw power_unit energy_unit current_unit voltage_unit power_unit_raw op_hour error error_raw service_mode op_hour_flag op_hour_flag_raw mode_raw".split(" ");Ua.prototype=new Ta;Ua.prototype.constructor=Ua;Ua.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.service_mode=a>>7&1;var f=a>>6&1;d.op_hour_flag=
...
b<<8|e,d.power_unit_raw=c,e="mW W kW MW Wh kWh MWh GWh mA 1/10A mV 1/10V".split(" "),3>=c?(d.power=d.power_raw,d.power_unit=e[c]):7>=c?(d.energy=d.power_raw,d.energy_unit=e[c]):9>=c?(d.current=d.power_raw,d.current_unit=e[c]):11>=c&&(d.voltage=d.power_raw,d.voltage_unit=e[c])}}}return d};var da=function(b){t.call(this,b);this._data="a5-12-??"};da.STATES="meter_raw channel_raw type_raw scale_raw scale meter current cumulative".split(" ");da.prototype=new t;da.prototype.constructor=da;da.prototype.evalTelegram=
function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt32BE(0);if(1==(a>>3&1)){b=b>>8&16777215;d.meter_raw=b;d.channel_raw=a>>4&15;var c=a>>2&1;d.type_raw=c;a&=3;d.scale_raw=a;switch(a){case 0:d.scale=1;d.meter=b;break;case 1:d.scale=.1;d.meter=.1*b;break;case 2:d.scale=.01;d.meter=.01*b;break;case 3:d.scale=.001,d.meter=.001*b}c?d.current=d.meter:d.cumulative=d.meter}}return d};var wb=function(b){da.call(this,b);this._data="a5-12-00"};wb.STATES=["current","counter",
"channel"];wb.prototype=new da;wb.prototype.constructor=wb;wb.prototype.evalTelegram=function(b){b=da.prototype.evalTelegram.call(this,b);b.counter=b.cumulative;b.channel=b.channel_raw;return b};var ma=function(b){da.call(this,b);this._data="a5-12-01"};ma.STATES=["power","energy","tariff"];ma.prototype=new da;ma.prototype.constructor=ma;ma.prototype.evalTelegram=function(b){b=da.prototype.evalTelegram.call(this,b);b.energy=b.cumulative;b.power=b.current;b.tariff=b.channel_raw;return b};var Ia=function(b){da.call(this,
b);this._data="a5-12-02"};Ia.STATES=["volume","flow","tariff"];Ia.prototype=new da;Ia.prototype.constructor=Ia;Ia.prototype.evalTelegram=function(b){b=da.prototype.evalTelegram.call(this,b);b.volume=b.cumulative;b.flow=b.current;b.tariff=b.channel_raw;return b};var nc=function(b){Ia.call(this,b);this._data="a5-12-03"};nc.STATES=["volume","flow","tariff"];nc.prototype=new Ia;nc.prototype.constructor=nc;var Q=function(b){t.call(this,b);this._data="a5-13-??"};Q.STATES="lux lux_raw temperature temperature_raw wind wind_raw day_night rain sun_west sun_west_raw sun_south sun_south_raw sun_east sun_east_raw hemisphere day month year date_src weekday hour min sec time_format am_pm time_src elv azm latitude longitude latitude_raw longitude_raw".split(" ");
Q.prototype=new t;Q.prototype.constructor=Q;Q.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),e=b.userData.readUInt8(1),f=b.userData.readUInt8(0);if(1==(a>>3&1))switch(a>>4&15){case 1:d.lux=parseFloat((f/255*1E3).toFixed(1));d.lux_raw=f;var g=e;d.temperature=parseFloat((g/255*120-40).toFixed(1));d.temperature_raw=g;d.wind=parseFloat((c/255*70).toFixed(1));d.wind_raw=c;d.day_night=a>>2&1?"night":"day";d.rain=a>>1&1?"true":"false";
...
f;c=b.userData.readUInt16BE(1)&511;d.azm=c;d.azm_raw=c;break;case 6:b=(f>>4&15)<<8|e,c|=(f&15)<<8,d.latitude=parseFloat((b/4095*180-90).toFixed(1)),d.latitude_raw=b,d.longitude=parseFloat((c/4095*360-180).toFixed(1)),d.longitude_raw=c}}return d};var oc=function(b){Q.call(this,b);this._data="a5-13-01"};oc.STATES="lux lux_raw temperature temperature_raw wind wind_raw day_night rain sun_west sun_west_raw sun_south sun_south_raw sun_east sun_east_raw hemisphere day month year date_src weekday hour min sec time_format am_pm time_src elv azm latitude longitude latitude_raw longitude_raw".split(" ");
oc.prototype=new Q;oc.prototype.constructor=oc;var pc=function(b){Q.call(this,b);this._data="a5-13-02"};pc.STATES="sun_west sun_west_raw sun_south sun_south_raw sun_east sun_east_raw hemisphere".split(" ");pc.prototype=new Q;pc.prototype.constructor=pc;var qc=function(b){Q.call(this,b);this._data="a5-13-03"};qc.STATES=["day","month","year","date_src"];qc.prototype=new Q;qc.prototype.constructor=qc;var rc=function(b){Q.call(this,b);this._data="a5-13-04"};rc.STATES="weekday hour min sec time_format am_pm time_src".split(" ");
rc.prototype=new Q;rc.prototype.constructor=rc;var sc=function(b){Q.call(this,b);this._data="a5-13-05"};sc.STATES=["elv","azm"];sc.prototype=new Q;sc.prototype.constructor=sc;var tc=function(b){Q.call(this,b);this._data="a5-13-06"};tc.STATES=["latitude","longitude"];tc.prototype=new Q;tc.prototype.constructor=tc;var xb=function(b){g.call(this,b);this._data="a5-14-01"};xb.STATES=["voltage","voltage_raw","state"];xb.prototype=new g;xb.prototype.constructor=xb;xb.prototype.evalTelegram=function(b){var d=
{};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.voltage=b/250*5;d.voltage_raw=b;a&=1;switch(a){case 0:d.state="closed";break;case 1:d.state="open"}d.state_raw=a}}return d};var yb=function(b){g.call(this,b);this._data="a5-14-02"};yb.STATES=["voltage","voltage_raw","illumination","illumination_raw","state"];yb.prototype=new g;yb.prototype.constructor=yb;yb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(1);
b=b.userData.readUInt8(0);if(1==(a>>3&1))switch(d.voltage=b/250*5,d.voltage_raw=b,d.illumination=c/250*1E3,d.illumination_raw=c,a&=1,d.state_raw=a,a){case 0:d.state="closed";break;case 1:d.state="open"}}return d};var zb=function(b){g.call(this,b);this._data="a5-14-03"};zb.STATES=["voltage","voltage_raw","state","viberation"];zb.prototype=new g;zb.prototype.constructor=zb;zb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);if(1==(a>>
3&1)){d.voltage=b/250*5;d.voltage_raw=b;switch(a&1){case 0:d.state="closed";break;case 1:d.state="open"}switch(a>>1&1){case 0:d.viberation=!1;break;case 1:d.viberation=!0}}}return d};var Ab=function(b){g.call(this,b);this._data="a5-14-04"};Ab.STATES="voltage voltage_raw illumination illumination_raw state viberation".split(" ");Ab.prototype=new g;Ab.prototype.constructor=Ab;Ab.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(1);b=b.userData.readUInt8(0);
if(1==(a>>3&1)){d.voltage=b/250*5;d.voltage_raw=b;d.illumination=c/250*1E3;d.illumination_raw=c;switch(a&1){case 0:d.state="closed";break;case 1:d.state="open"}switch(a>>1&1){case 0:d.viberation=!1;break;case 1:d.viberation=!0}}}return d};var Bb=function(b){g.call(this,b);this._data="a5-14-05"};Bb.STATES=["voltage","voltage_raw","viberation"];Bb.prototype=new g;Bb.prototype.constructor=Bb;Bb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);
if(1==(a>>3&1))switch(d.voltage=b/250*5,d.voltage_raw=b,a>>1&1){case 0:d.viberation=!1;break;case 1:d.viberation=!0}}return d};var Cb=function(b){g.call(this,b);this._data="a5-14-06"};Cb.STATES=["voltage","voltage_raw","illumination","illumination_raw","viberation"];Cb.prototype=new g;Cb.prototype.constructor=Cb;Cb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(a>>3&1))switch(d.voltage=b/250*5,d.voltage_raw=
b,d.illumination=c/250*1E3,d.illumination_raw=c,a>>1&1){case 0:d.viberation=!1;break;case 1:d.viberation=!0}}return d};var Db=function(b){g.call(this,b);this._data="a5-14-07"};Db.STATES=["voltage","voltage_raw","state","lock"];Db.prototype=new g;Db.prototype.constructor=Db;Db.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.voltage=b/250*5;d.voltage_raw=b;switch(a>>2&1){case 0:d.state="closed";break;case 1:d.state=
"open"}switch(a>>1&1){case 0:d.lock="locked";break;case 1:d.lock="unlocked"}}}return d};var Eb=function(b){g.call(this,b);this._data="a5-14-08"};Eb.STATES=["voltage","voltage_raw","state","lock","viberation"];Eb.prototype=new g;Eb.prototype.constructor=Eb;Eb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.voltage=b/250*5;d.voltage_raw=b;switch(a>>2&1){case 0:d.state="closed";break;case 1:d.state="open"}switch(a>>
1&1){case 0:d.lock="locked";break;case 1:d.lock="unlocked"}switch(a&1){case 0:d.viberation=!1;break;case 1:d.viberation=!0}}}return d};var Fb=function(b){g.call(this,b);this._data="a5-14-09"};Fb.STATES=["voltage","voltage_raw","state"];Fb.prototype=new g;Fb.prototype.constructor=Fb;Fb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);if(1==(a>>3&1))switch(d.voltage=b/250*5,d.voltage_raw=b,a>>1&3){case 0:d.state="closed";break;case 1:d.state=
"tilted";break;case 2:d.state="reserved";break;case 3:d.state="open"}}return d};var Gb=function(b){g.call(this,b);this._data="a5-14-0a"};Gb.STATES=["voltage","voltage_raw","state","viberation"];Gb.prototype=new g;Gb.prototype.constructor=Gb;Gb.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(0);if(1==(a>>3&1)){d.voltage=b/250*5;d.voltage_raw=b;switch(a>>1&3){case 0:d.state="closed";break;case 1:d.state="tilted";break;case 2:d.state="reserved";
break;case 3:d.state="open"}switch(a&1){case 0:d.viberation=!1;break;case 1:d.viberation=!0}}}return d};var ia=function(b){t.call(this,b);this._data="a5-20-04";this.TEACH_IN_VARIATION=3;this._defaultSetpoint=15;this._defaultWakeupCycle=3;this._defaultDisplay=0;this._states={pending:{value:!1,time:(new Date).getTime()}};this._service_command_new=this._button_lock_new=this._display_new=this._wakeup_cycle_new=this._measurement_new=this._setpoint_new=this._valve_new=null};ia.COMMANDS="setValve setTo up down measureOn measureOff setWakeup setDisplay buttonLock buttonUnlock openValve closeValve init".split(" ");
ia.STATES="valve measurement button_lock temperature_selection feed_temperature setpoint error temperature wakeup_cycle display pending".split(" ");ia.prototype=new t;ia.prototype.constructor=ia;ia.prototype.setDefaultParams=function(){this._params||(this._params={});if("undefined"==typeof this._params.wakeup_cycle||null==this._params.wakeup_cycle)this._params.wakeup_cycle=this._defaultWakeupCycle;if("undefined"==typeof this._params.display||null==this._params.display)this._params.display=this._defaultDisplay};
ia.prototype.getStatesBuffer=function(){var b=JSON.parse(JSON.stringify(this._states));b.wakeup_cycle={value:this._params.wakeup_cycle,time:-1};b.display={value:this._params.display,time:-1};"undefined"!=typeof this._wakeup_cycle_new&&null!=this._wakeup_cycle_new&&(b.wakeup_cycle={value:this._wakeup_cycle_new,time:-1});"undefined"!=typeof this._display_new&&null!=this._display_new&&(b.display={value:this._display_new,time:-1});"undefined"!=typeof this._valve_new&&null!=this._valve_new&&(b.valve={value:this._valve_new,
time:-1});"undefined"!=typeof this._setpoint_new&&null!=this._setpoint_new&&(b.setpoint={value:this._setpoint_new,time:-1});"undefined"!=typeof this._measurement_new&&null!=this._measurement_new&&(b.measurement={value:this._measurement_new,time:-1});"undefined"!=typeof this._button_lock_new&&null!=this._button_lock_new&&(b.button_lock={value:this._button_lock_new,time:-1});return b};ia.prototype.getCurrentStates=function(){var b={},d=this.getStatesBuffer(),a;for(a in d)d.hasOwnProperty(a)&&d[a]&&
(b[a]=d[a].value);return b};ia.prototype.executeCommand=function(b,d,a){var c=(new Date).getTime();if(d&&d.value){"setValve"==d.value.substr(0,8)&&8<d.value.length?(b=d.value.substr(8),d={value:"setValve",ext:b}):"setTo"==d.value.substr(0,5)&&5<d.value.length?(b=d.value.substr(5),d={value:"setTo",ext:b}):"up"==d.value.substr(0,2)&&2<d.value.length?(b=d.value.substr(2),d={value:"up",ext:b}):"down"==d.value.substr(0,4)&&4<d.value.length?(b=d.value.substr(4),d={value:"down",ext:b}):"setWakeup"==d.value.substr(0,
...
(l=this._display_new,this._params.display=this._display_new,f=!0,this._display_new=null);if("undefined"==typeof l||null==l)l=this._defaultDisplay,this._params.display=l;if("undefined"!=typeof this._button_lock_new&&null!=this._button_lock_new){var p="locked"==this._button_lock_new?1:0;this._states.button_lock={value:this._button_lock_new,time:c};this._button_lock_new=null;delete e.button_lock}else"undefined"!=typeof d.button_lock&&null!=d.button_lock?p="locked"==d.button_lock?1:0:this._states.button_lock&&
this._states.button_lock.value&&(p="locked"==this._states.button_lock.value?1:0);if("undefined"==typeof p||null==p)p=0,e.button_lock="unlocked";d=0;"undefined"!=typeof this._service_command_new&&null!=this._service_command_new&&(d=this._service_command_new,this._service_command_new=null);q=new Buffer([q,g,(h&1)<<6|k&63,(l&3)<<4|8|(p&1)<<2|d&3]);f&&require("x.hub.m.enocean.js")._deviceHandler.deviceParamsUpdated();f=this._senderID;f||(f=x.hub.m.enocean.SENDER_ID);f=new x.hub.m.enocean.ESP3.Telegram.ADT(165,
q,this._address,f,0);b.sendTelegram(f,!0,function(b){a&&a(b,e)})};var Va=function(b){t.call(this,b);this._data="a5-30-??"};Va.prototype=new t;Va.prototype.constructor=Va;var Ja=function(b){g.call(this,b);this._data="a5-30-01"};Ja.STATES=["battery_low","battery_raw","state","state_raw"];Ja.prototype=new Va;Ja.prototype.constructor=Ja;Ja.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2);b=b.userData.readUInt8(1);1==(a>>3&1)&&(d.battery_low=
120>=b?!0:!1,d.battery_raw=b,d.state=195>=c?"closed":"open",d.state_raw=c)}return d};var Hb=function(b){g.call(this,b);this._data="a5-30-01\u00b4"};Hb.STATES=["state"];Hb.prototype=new Va;Hb.prototype.constructor=Hb;Hb.prototype.evalTelegram=function(b){var d={};165==b.rorg&&(b=b.userData.readUInt8(3),1==(b>>3&1)&&(d.state=b&1?"open":"closed"));return d};var ea=function(b){g.call(this,b);this._data="a5-30-03"};ea.STATES="temperature temperature_raw wake input3 input2 input1 input0".split(" ");ea.prototype=
new Va;ea.prototype.constructor=ea;ea.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2);b=b.userData.readUInt8(1);1==(a>>3&1)&&(d.temperature=(255-b)/255*40,d.temperature_raw=b,d.wake=c>>4&1,d.input3=c>>3&1,d.input2=c>>2&1,d.input1=c>>1&1,d.input0=c&1)}return d};var Ib=function(b){g.call(this,b);this._data="a5-30-04"};Ib.STATES=["analog_input","input2","input1","input0"];Ib.prototype=new Va;Ib.prototype.constructor=Ib;Ib.prototype.evalTelegram=
function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3);b=b.userData.readUInt8(2);1==(a>>3&1)&&(d.analog_input=b,d.input2=a>>2&1,d.input1=a>>1&1,d.input0=a&1)}return d};var uc=function(b){t.call(this,b);this._data="a5-38-??"};uc.prototype=new t;uc.prototype.constructor=uc;var W=function(b){uc.call(this,b);this._data="a5-38-08"};W.prototype=new uc;W.prototype.constructor=W;W.prototype.evalTelegram=function(b){var d={};if(165==b.rorg){var a=b.userData.readUInt8(3),c=b.userData.readUInt8(2),
e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(a>>3&1))switch(b){case 2:d.level=Math.round(e/255*100),d.ramp=c,d.dim_range=a>>2&1?"relative":"absolute",d.store_final_value=a>>1&1,d.state=a&1?"on":"off"}}return d};W.prototype.switching=function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([1,d.TIM>>8&255,d.TIM&255,8|(d.LCK&1)<<2|(d.DEL&1)<<1|d.SW&1]);c=new x.hub.m.enocean.ESP3.Telegram.BS4(d,c,0);c.setDstId(this._address);b.sendTelegram(c,!0,function(b){a&&
a(b)})};W.prototype.dimming=function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([2,d.EDIM,d.RMP,8|(d.EDIMR&1)<<2|(d.STR&1)<<1|d.SW&1]);c=new x.hub.m.enocean.ESP3.Telegram.BS4(d,c,0);c.setDstId(this._address);b.sendTelegram(c,!0,function(b){a&&a(b)})};W.prototype.blind=function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([7,d.P1,d.P2,(d.FUNC&15)<<4|8|(d.SSF&1)<<2|(d.PAF&1)<<1|d.SMF&1]);c=new x.hub.m.enocean.ESP3.Telegram.BS4(d,
c,0);c.setDstId(this._address);b.sendTelegram(c,!0,function(b){a&&a(b)})};var vc=function(b){t.call(this,b);this._data="a5-3F-7F"};vc.prototype=new t;vc.prototype.constructor=vc;var fa=function(b){g.call(this,b);this._data="d2-??-??";this.UNIQUE_GATEWAY_SENDERID=!0};fa.prototype=new g;fa.prototype.constructor=fa;var E=function(b){fa.call(this,b);this._data="d2-01-??";this._lastQuery={};this._initCount=0};E.STATES="power_failure_detection power_failure over_current_switch_off error_level io_channel local_control output_value output_state".split(" ");
E.prototype=new fa;E.prototype.constructor=E;E.prototype.init=function(b){var d=this.getIOs();if(0<d.length){var a=require("x.hub.m.enocean.js")._esp3,c=0,e=this,f=function(){c++;c<d.length?e.statusQuery(a,{IO:d[c]},f):b&&b()};this.statusQuery(a,{IO:d[c]},f)}else b&&b()};E.prototype.getStatesBuffer=function(){0==Object.keys(this._states).length?(100>=this._initCount&&0==this._initCount%10?this.init():100<=this._initCount&&600>=this._initCount&&0==this._initCount%60?this.init():this._initCount=0,this._initCount++):
...
"off":"on"},a):this.init(function(){setTimeout(function(){c._states.output_state?c.executeCommand(b,d,a):a&&a(Error("unkown current status"))},200)})}}else a&&a(Error("command invalid"))};var Jb=function(b){na.call(this,b);this._data="d2-01-01"};Jb.COMMANDS=["on","off","toggle"];Jb.STATES="power_failure_detection power_failure over_current_switch_off error_level io_channel local_control output_value output_state".split(" ");Jb.prototype=new na;Jb.prototype.constructor=Jb;var Kb=function(b){na.call(this,
b);this._data="d2-01-03";this._lastOnLevel=-1};Kb.prototype=new na;Kb.prototype.constructor=Kb;Kb.prototype.evalData=function(b,d,a){(b=this.evalTelegram(d))?((0==b.output_value||"off"==b.output_state)&&this._states.output_value&&this._states.output_value.value&&(this._lastOnLevel=this._states.output_value.value),b=this.checkStateChange(b),a&&a(null,b)):a&&a(Error("data format invalid"))};Kb.prototype.executeCommand=function(b,d,a){if(d&&d.value){if("dimTo"==d.value.substr(0,5)&&5<d.value.length){var c=
...
a(Error("unknow command"))}}else a&&a(Error("command invalid"))};var Ba=function(b){E.call(this,b);this._data="d2-01-11"};Ba.COMMANDS="ch1On ch1Off ch1Toggle ch2On ch2Off ch2Toggle".split(" ");Ba.STATES="power_failure_detection power_failure over_current_switch_off error_level io_channel local_control output_value output_state ch1_output_value ch1_output_state ch2_output_value ch2_output_state".split(" ");Ba.prototype=new E;Ba.prototype.constructor=Ba;Ba.prototype.getIOs=function(){return[0,1]};Ba.prototype.evalTelegram=
function(b){if(b=E.prototype.evalTelegram.call(this,b))switch(b.io_channel){case 0:b.ch1_output_value=b.output_value;b.ch1_output_state=b.output_state;break;case 1:b.ch2_output_value=b.output_value,b.ch2_output_state=b.output_state}return b};Ba.prototype.executeCommand=function(b,d,a){if(d&&d.value){var c=this;switch(d.value){case "ch1On":this.activeStateChange({ch1_output_value:100,ch1_output_state:"on"});this.setOutput(b,{DV:0,IO:0,OV:100},a);break;case "ch1Off":this.activeStateChange({ch1_output_value:0,
...
3):null){case "ch1":var e=0;var f="ch1_output_value";var g="ch1_output_state";break;case "ch2":e=1;f="ch2_output_value";g="ch2_output_state";break;case "ch3":e=2;f="ch3_output_value";g="ch3_output_state";break;case "ch4":e=3;f="ch4_output_value";g="ch4_output_state";break;case "ch5":e=4;f="ch5_output_value";g="ch5_output_state";break;case "ch6":e=5;f="ch6_output_value";g="ch6_output_state";break;case "ch7":e=6;f="ch7_output_value";g="ch7_output_state";break;case "ch8":e=7,f="ch8_output_value",g="ch8_output_state"}switch(d.value){case "ch1On":case "ch2On":case "ch3On":case "ch4On":case "ch5On":case "ch6On":case "ch7On":case "ch8On":var q=
{};q[f]=100;q[g]="on";var h={DV:0,IO:e,OV:100};this.activeStateChange(q);this.setOutput(b,h,a);break;case "ch1Off":case "ch2Off":case "ch3Off":case "ch4Off":case "ch5Off":case "ch6Off":case "ch7Off":case "ch8Off":q={};q[f]=0;q[g]="off";h={DV:0,IO:e,OV:0};this.activeStateChange(q);this.setOutput(b,h,a);break;case "ch1Toggle":case "ch2Toggle":case "ch3Toggle":case "ch4Toggle":case "ch5Toggle":case "ch6Toggle":case "ch7Toggle":case "ch8Toggle":if(!this._states.output_state){this.init(function(){setTimeout(function(){c._states.output_state?
c.executeCommand(b,d,a):a&&a(Error("unkown current status"))},200)});break}if(!this._states[g]){a&&a(Error("state unknown"));break}h="on"==this._states[g].value?"off":"on";q={};q[f]="on"==h?100:0;q[g]=h;h={DV:0,IO:e,OV:"on"==h?100:0};this.activeStateChange(q);this.setOutput(b,h,a);break;default:a&&a(Error("unknown command"))}}else a&&a(Error("command invalid"))};var wc=function(b){fa.call(this,b);this._data="d2-05-??"};wc.prototype=new fa;wc.prototype.constructor=wc;var M=function(b){wc.call(this,
b);this._data="d2-05-00"};M.COMMANDS="moveUp moveDown moveTo stepUp stepDown lamellaUp lamellaDown lamellaTo stop setRunTime setRotationTime setParams[15.2s]:[10,20,30ms]:[0-3]".split(" ");M.STATES=["state","angle","lock","channel"];M.prototype=new wc;M.prototype.constructor=M;M.prototype.init=function(b){var d=require("x.hub.m.enocean.js")._esp3;this.query(d,{CHN:0},b)};M.prototype.getStatesBuffer=function(){0==Object.keys(this._states).length&&this.init();return this._states};M.prototype.goToPositionAngle=
function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([d.POS&127,d.ANG&127,(d.REPO&7)<<4|d.LOCK&7,(d.CHN&15)<<4|1]);c=new x.hub.m.enocean.ESP3.Telegram.VLD(d,c,0);c.setDstId(this._address);b.sendTelegram(c,!0,function(b){a&&a(b)})};M.prototype.stop=function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([(d.CHN&15)<<4|2]);c=new x.hub.m.enocean.ESP3.Telegram.VLD(d,c,0);c.setDstId(this._address);b.sendTelegram(c,!0,function(b){a&&
a(b)})};M.prototype.query=function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([(d.CHN&15)<<4|3]);c=new x.hub.m.enocean.ESP3.Telegram.VLD(d,c,0);c.setDstId(this._address);b.sendTelegram(c,!0,function(b){a&&a(b)})};M.prototype.setParameters=function(b,d,a){var c=this._senderID;this._senderID||(c=x.hub.m.enocean.SENDER_ID);d=new Buffer([d.VERT>>8&127,d.VERT&255,d.ROT&255,d.AA&7,(d.CHN&15)<<4|5]);c=new x.hub.m.enocean.ESP3.Telegram.VLD(d,c,0);c.setDstId(this._address);
b.sendTelegram(c,!0,function(b){a&&a(b)})};M.prototype.evalTelegram=function(b){var d={};if(210==b.rorg)switch(b.userData.readUInt8(b.userData.length-1)&15){case 4:if(4==b.userData.length){var a=b.userData.readUInt8(0);d.state=a&127;a=b.userData.readUInt8(1);d.angle=a&127;a=b.userData.readUInt8(2);switch(a&7){case 0:d.lock="normal";break;case 1:d.lock="blockage_mode";break;case 2:d.lock="alarm_mode"}a=b.userData.readUInt8(3);d.channel=a>>4&15}}return d};M.prototype.executeCommand=function(b,d,a){if(d&&
...
parseInt(g[2]):7;this.setParameters(b,{VERT:f,ROT:q,AA:g,CHN:c},a);break;default:a&&a(Error("unknow command"))}}else a&&a(Error("command invalid"))};var xc=function(b){fa.call(this,b);this._data="d2-06-??"};xc.prototype=new fa;xc.prototype.constructor=xc;var xa=function(b){fa.call(this,b);this._data="d2-06-01";this._states={pending:{value:!1,time:(new Date).getTime()}};this._vbir_new=this._suir_new=this._blcr_new=this._hccr_new=this._vacation_new=null};xa.COMMANDS="vacationOn vacationOff handleClosedClickOn handleClosedClickOff batteryLowClickOn batteryLowClickOff setSensorUpdateInterval setVacationBlinkInterval".split(" ");
xa.STATES="bal ppal state handle window br bl motion vacation temperature humidity illumination battery hccr blcr suir vbir".split(" ");xa.prototype=new xc;xa.prototype.constructor=xa;xa.prototype.executeCommand=function(b,d,a){b=(new Date).getTime();if(d&&d.value){"setSensorUpdateInterval"==d.value.substr(0,23)&&23<d.value.length?(d=d.value.substr(23),d={value:"setSensorUpdateInterval",ext:d}):"setVacationBlinkInterval"==d.value.substr(0,24)&&24<d.value.length&&(d=d.value.substr(24),d={value:"setVacationBlinkInterval",
...
null;l._vbir_new=null;var h={pending:!1};h.vacation=a?"on":"off";c&&(h.hccr=1==c?"off":"on");e&&(h.blcr=1==e?"off":"on");f&&(h.suir=f);g&&(h.vbir=g);l.activeStateChange(h);d&&d(b)})};var ya=function(b){fa.call(this,b);this._data="d2-06-02";this._states={pending:{value:!1,time:(new Date).getTime()}};this._vbis_new=this._suis_new=this._blcs_new=this._lacs_new=this._vacation_new=null};ya.COMMANDS="vacationOn vacationOff lockActiveClickOn lockActiveClickOff batteryLowClickOn batteryLowClickOff setSensorUpdateInterval setVacationBlinkInterval".split(" ");
ya.STATES="operation_mode handle window state temperature humidity battery vacation hccr blcr suir vbir".split(" ");ya.prototype=new xc;ya.prototype.constructor=ya;ya.prototype.executeCommand=function(b,d,a){b=(new Date).getTime();if(d&&d.value){"setSensorUpdateInterval"==d.value.substr(0,23)&&23<d.value.length?(d=d.value.substr(23),d={value:"setSensorUpdateInterval",ext:d}):"setVacationBlinkInterval"==d.value.substr(0,24)&&24<d.value.length&&(d=d.value.substr(24),d={value:"setVacationBlinkInterval",
...
5*b);if("tilted"==d.window)d.state="tilted";else switch(d.lock){case "unlocked":d.state="open";break;case "locked":d.state="closed"}break;case 16:a=b.userData.readUInt8(1),c=b.userData.readUInt16BE(2),e=b=b.userData.readUInt8(4),d.vacation=a>>7&1?"on":"off",d.hccr=a>>6&1?"on":"off",d.blcr=a>>5&1?"on":"off",5<=c&&(d.suir=c),3<=e&&(d.vbir=e)}return d};ya.prototype._sendResponse=function(b,a){var d=0;this._vacation_new&&"on"==this._vacation_new?d=1:this._vacation_new&&"off"==this._vacation_new&&(d=2);
var c=0;this._lacs_new&&"off"==this._lacs_new?c=1:this._lacs_new&&"on"==this._lacs_new&&(c=2);var e=0;this._blcs_new&&"off"==this._blcs_new?e=1:this._blcs_new&&"on"==this._blcs_new&&(e=2);var f=0;this._suis_new&&(f=this._suis_new);var g=0;this._vbis_new&&(g=this._vbis_new);var h=new Buffer([128,128|(d&3)<<4|(c&3)<<2|e&3,f>>8&255,f&255,g]),k=this._senderID;k||(k=x.hub.m.enocean.SENDER_ID);h=new x.hub.m.enocean.ESP3.Telegram.VLD(h,k,0);var l=this;b.sendTelegram(h,!0,function(b){l._vacation_new=null;
l._lacs_new=null;l._blcs_new=null;l._suis_new=null;l._vbis_new=null;var h={pending:!1};h.vacation=d?"on":"off";c&&(h.hccr=1==c?"off":"on");e&&(h.blcr=1==e?"off":"on");f&&(h.suir=f);g&&(h.vbir=g);l.activeStateChange(h);a&&a(b)})};var yc=function(b){fa.call(this,b);this._data="d2-20-??"};yc.prototype=new fa;yc.prototype.constructor=yc;var ba=function(b){yc.call(this,b);this._data="d2-20-00";this._poller=null;this._pollInterval=6E4};ba.COMMANDS="setFanLevel fanLevelUp fanLevelDown requestStatus setHumLevel setCo2Level".split(" ");
ba.STATES="filter_remaining_time operating_time temperature message_code anti_freeze current_fan_level set_fan_level".split(" ");ba.prototype=new yc;ba.prototype.constructor=ba;ba.prototype.init=function(b){var a=this;setTimeout(function(){var d=require("x.hub.m.enocean.js")._esp3;a.executeCommand(d,{value:"requestStatus"},b);a._restartPoller(d)},Math.floor(6E3*Math.random())+1E3)};ba.prototype.finalize=function(b){this._poller&&clearInterval(this._poller);b&&b()};ba.prototype._restartPoller=function(b){this._poller&&
clearInterval(this._poller);var a=this;this._poller=setInterval(function(){a.executeCommand(b,{value:"requestStatus"})},this._pollInterval)};ba.prototype.executeCommand=function(b,a,c){(new Date).getTime();if(a&&a.value){if("setFanLevel"==a.value.substr(0,11)&&11<a.value.length)a=a.value.substr(11),a={value:"setFanLevel",ext:a};else if("fanLevelUp"==a.value){var d=this.getBufferedState("set_fan_level");if("undefined"==typeof d||null==d){c&&c(Error("fanLevelUp unknown current fan level"));return}if(6==
...
c(Error("setCo2Level value invalid"));break}f=parseInt(a.ext);if(0>f||4<f){c&&c(Error("setCo2Level value invalid"));break}this._sendResponse(b,{LS:f,RS:1},function(a){e._restartPoller(b);c&&c(a)});break;case "requestStatus":this._sendResponse(b,{LS:5,RS:1},function(b){c&&c(b)});break;default:c&&c(Error("unknown command"))}}else c&&c(Error("command invalid"))};ba.prototype.evalData=function(b,a,c){b=this.evalTelegram(a);this._states&&this._states.set_fan_level||(b.set_fan_level=b.current_fan_level);
this._states&&this._states.set_fan_level&&5!=this._states.set_fan_level.value&&6!=this._states.set_fan_level.value&&(b.set_fan_level=b.current_fan_level);b=this.checkStateChange(b);c&&c(null,b)};ba.prototype.evalTelegram=function(b){var a={};if(210==b.rorg)if(1==b.userData.length){var c=b.userData.readUInt8(0)&7;a.current_fan_level=c}else{c=b.userData.readUInt8(0);var e=b.userData.readUInt8(1),f=b.userData.readUInt16LE(2),g=b.userData.readUInt8(4),h=b.userData.readUInt8(5),k=b.userData.readUInt8(6),
l=b.userData.readUInt16LE(7),p=b.userData.readUInt8(9),r=b.userData.readUInt8(10),n=b.userData.readInt16BE(11);b=b.userData.readUInt8(13);a.filter_remaining_time=g+":"+f+":"+e+":"+c;a.operating_time=p+":"+l+":"+k+":"+h;a.temperature=n/10;a.message_code=r;a.anti_freeze=b>>4&1?"on":"off";a.current_fan_level=b&7}return a};ba.prototype._sendResponse=function(b,a,c){var d=this._senderID;this._senderID||(d=x.hub.m.enocean.SENDER_ID);a=new Buffer([a.LS,a.RS]);d=new x.hub.m.enocean.ESP3.Telegram.VLD(a,d,
0);this._senderID!=this._address&&d.setDstId(this._address);b.sendTelegram(d,!0,function(b){c&&c(b)})};ba.prototype._activateSensorRegulation=function(b){var a=require("x.hub.m.enocean.js").getRehauHandler();a&&a.activateSensorRegulation(this.getAddress(),b)};ba.prototype._deactivateSensorRegulation=function(){var b=require("x.hub.m.enocean.js").getRehauHandler();b&&b.deactivateSensorRegulation(this.getAddress())};var Lb=function(b){U.call(this,b);this._data="eltako_contact";this._vendor="eltako";
this._eepAlias="f6-10-00"};Lb.STATES=["contact","contact_raw"];Lb.prototype=new U;Lb.prototype.constructor=Lb;Lb.prototype.evalTelegram=function(b){var a={};246==b.rorg&&(b=b.userData,240==b?a.contact="closed":224==b&&(a.contact="closed"),a.contact_raw=b);return a};var Mb=function(b){Ra.call(this,b);this._data="eltako_motion";this._vendor="eltako";this._eepAlias="a5-08-01"};Mb.STATES="voltage voltage_raw illumination illumination_raw state state_raw".split(" ");Mb.prototype=new Ra;Mb.prototype.constructor=
Mb;Mb.prototype.evalTelegram=function(b){var a={};if(165==b.rorg){var c=b.userData.readUInt8(3);b.userData.readUInt8(2);var e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(c>>3&1)){a.voltage=parseFloat((b/255*5.1).toFixed(1));a.voltage_raw=b;a.illumination=parseFloat((e/255*2048).toFixed(1));a.illumination_raw=e;c=c>>1&1;switch(c){case 0:a.state="on";break;case 1:a.state="off"}a.state_raw=c}}return a};var ja=function(b){W.call(this,b);this._data="eltako_switch";this._vendor="eltako";this._eepAlias=
"f6-02-01"};ja.COMMANDS=["on","off","toggle"];ja.STATES=["state"];ja.prototype=new W;ja.prototype.constructor=ja;ja.prototype.getTeachinType=function(){return"4BS"};ja.prototype.handleLearnTelegram=function(b,a){if(a&&246==a.rorg)require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._learnAddress,senderID:b._learnSenderID,data:b._learnProfile,vendor:b._learnVendor},{address:a.senderID,senderID:b._learnSenderID,data:this._data,vendor:this._vendor},function(a,d){b._learnFinish(a,0,d)})};
ja.prototype.getTeachinTelegram=function(b,a,c){b=new Buffer([224,64,13,128]);return new x.hub.m.enocean.ESP3.Telegram.BS4(b,a,0)};ja.prototype.evalTelegram=function(b){var a={};if(246==b.rorg)switch(b.userData){case 112:a.state="on";break;case 80:a.state="off"}return a};ja.prototype.executeCommand=function(b,a,c){if(a&&a.value){(new Date).getTime();var d=this;switch(a.value){case "on":this.switching(b,{TIM:0,LCK:0,DEL:0,SW:1},function(b){b||d.activeStateChange({state:"on"});c&&c(b)});break;case "off":this.switching(b,
{TIM:0,LCK:0,DEL:0,SW:0},function(b){b||d.activeStateChange({state:"off"});c&&c(b)});break;case "toggle":if(!this._states.state){c&&c(Error("unknow previous switch state"));break}"on"==this._states.state.value?this.executeCommand(b,{value:"off"},c):this.executeCommand(b,{value:"on"},c);break;default:c&&c(Error("unknow command"))}}else c&&c(Error("command invalid"))};var Lc=function(){var b=function(b){ja.call(this,b);this.TEACH_IN_VARIATION=3;this._data="eltako_tf_switch"};b.prototype=new ja;b.prototype.constructor=
b;b.prototype.handleTeachinTelegram=function(b,a){if(a&&165==a.rorg&&a.userData){var d=a.userData.toString("hex"),c=b._teachSenderID;if(d&&c&&c.substr(2)==d.substr(0,6))require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._teachID,senderID:b._teachSenderID,data:b._teachProfile,vendor:b._teachVendor},{address:a.senderID,senderID:b._teachSenderID,data:b._teachProfile,vendor:b._teachVendor},function(a,d){b._teachFinish(a,d)})}};return b}(),F=function(b){W.call(this,b);this._data="eltako_dimmer";
this._vendor="eltako";this._eepAlias="a5-38-08";this._lastOnLevel=-1};F.COMMANDS="on off toggle dimTo dimUp dimDown toggleTo setRampTime".split(" ");F.STATES=["state","level","ramp_time"];F.prototype=new W;F.prototype.constructor=F;F.prototype.setDefaultParams=function(){this._params||(this._params={});if("undefined"==typeof this._params.ramp_time||null==this._params.ramp_time)this._params.ramp_time=0};F.prototype.handleLearnTelegram=function(b,a){if(a&&165==a.rorg)require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._learnAddress,
senderID:b._learnSenderID,data:b._learnProfile,vendor:b._learnVendor},{address:a.senderID,senderID:b._learnSenderID,data:this._data,vendor:this._vendor},function(a,d){b._learnFinish(a,0,d)})};F.prototype.getTeachinTelegram=function(b,a,c){b=new Buffer([224,64,13,128]);return new x.hub.m.enocean.ESP3.Telegram.BS4(b,a,0)};F.prototype.getStatesBuffer=function(){var b=JSON.parse(JSON.stringify(this._states));b.ramp_time={value:this._params.ramp_time,time:-1};return b};F.prototype.evalData=function(b,
a,c){(b=this.evalTelegram(a))?((0==b.level||"off"==b.state)&&this._states.level&&this._states.level.value&&(this._lastOnLevel=this._states.level.value),b=this.checkStateChange(b),c&&c(null,b)):c&&c(Error("data format invalid"))};F.prototype.evalTelegram=function(b){var a={};if(165==b.rorg){var c=b.userData.readUInt8(3),e=b.userData.readUInt8(2),f=b.userData.readUInt8(1);b=b.userData.readUInt8(0);if(1==(c>>3&1))switch(b){case 2:a.level=f,a.ramp=e,a.dim_range=c>>2&1?"relative":"absolute",a.store_final_value=
c>>1&1,a.state=c&1?"on":"off"}}return a};F.prototype.executeCommand=function(b,a,c){if(a&&a.value){if("dimTo"==a.value.substr(0,5)&&5<a.value.length){var d=a.value.substr(5);a={value:"dimTo",ext:d}}else"toggleTo"==a.value.substr(0,8)&&8<a.value.length?(d=a.value.substr(8),a={value:"toggleTo",ext:d}):"setRampTime"==a.value.substr(0,11)&&11<a.value.length&&(d=a.value.substr(11),a={value:"setRampTime",ext:d});(new Date).getTime();var e=this;switch(a.value){case "on":d=this._lastOnLevel;if(-1==d||0==
d)d=100;this.dimming(b,{EDIM:d,RMP:this._params.ramp_time,EDIMR:0,STR:0,SW:1},function(b){b||e.activeStateChange({level:d,state:"on"});c&&c(b)});break;case "off":this._states.level&&this._states.level.value&&(this._lastOnLevel=this._states.level.value);this.dimming(b,{EDIM:0,RMP:this._params.ramp_time,EDIMR:0,STR:0,SW:0},function(b){b||e.activeStateChange({level:0,state:"off"});c&&c(b)});break;case "toggle":if(!this._states.state){c&&c(Error("unknow previous dimmer state"));break}"on"==this._states.state.value?
this.executeCommand(b,{value:"off"},c):this.executeCommand(b,{value:"on"},c);break;case "toggleTo":if(!this._states.state){c&&c(Error("unknow previous dimmer state"));break}"on"==this._states.state.value?this.executeCommand(b,{value:"off"},c):this.executeCommand(b,{value:"dimTo",ext:a.ext},c);break;case "dimTo":if("undefined"==typeof a.ext||null==a.ext){c&&c(Error("dim to value invalid"));break}d=parseFloat(a.ext);0>d&&(d=0);100<d&&(d=100);this.dimming(b,{EDIM:d,RMP:this._params.ramp_time,EDIMR:0,
STR:0,SW:1},function(b){b||e.activeStateChange({level:d,state:"on"});c&&c(b)});break;case "dimUp":if(!this._states.level){c&&c(Error("unknow previous dimmer level"));break}d=parseInt(this._states.level.value);d+=5;100<d&&(d=100);this.dimming(b,{EDIM:d,RMP:this._params.ramp_time,EDIMR:0,STR:0,SW:1},function(b){b||e.activeStateChange({level:d,state:"on"});c&&c(b)});break;case "dimDown":if(!this._states.level){c&&c(Error("unknow previous dimmer level"));break}d=parseInt(this._states.level.value);d-=
5;0>d&&(d=0);if(0==d){this.executeCommand(b,{value:"off"},c);break}this.dimming(b,{EDIM:d,RMP:this._params.ramp_time,EDIMR:0,STR:0,SW:1},function(b){b||e.activeStateChange({level:d,state:"on"});c&&c(b)});break;case "setRampTime":if(!a.ext){c&&c(Error("parameters is null"));break}d=parseInt(d);255<d&&(d=255);0>d&&(d=0);this._params.ramp_time=d;require("x.hub.m.enocean.js")._deviceHandler.deviceParamsUpdated(this,function(b){c&&c(b)});break;default:c&&c(Error("unknow command"))}}else c&&c(Error("command invalid"))};
var Mc=function(){var b=function(b){F.call(this,b);this.TEACH_IN_VARIATION=3;this._data="eltako_tf_dimmer"};b.prototype=new F;b.prototype.constructor=b;b.prototype.handleTeachinTelegram=function(b,a){if(a&&165==a.rorg&&a.userData){var d=a.userData.toString("hex"),c=b._teachSenderID;if(d&&c&&c.substr(2)==d.substr(0,6))require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._teachID,senderID:b._teachSenderID,data:b._teachProfile,vendor:b._teachVendor},{address:a.senderID,senderID:b._teachSenderID,
data:b._teachProfile,vendor:b._teachVendor},function(a,d){b._teachFinish(a,d)})}};return b}(),Wa=function(b){F.call(this,b);this._data="eltako_dimmer2";this._vendor="eltako";this._eepAlias="f6-02-01"};Wa.COMMANDS="on off toggle dimTo dimUp dimDown toggleTo setRampTime".split(" ");Wa.STATES=["state","ramp_time"];Wa.prototype=new F;Wa.prototype.constructor=F;Wa.prototype.handleLearnTelegram=function(b,a){if(a&&246==a.rorg)require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._learnAddress,
senderID:b._learnSenderID,data:b._learnProfile,vendor:b._learnVendor},{address:a.senderID,senderID:b._learnSenderID,data:this._data,vendor:this._vendor},function(a,d){b._learnFinish(a,0,d)})};Wa.prototype.evalTelegram=function(b){var a={};if(246==b.rorg)switch(b.userData){case 112:a.state="on";break;case 80:a.state="off"}return a};var Nb=function(b){F.call(this,b);this._data="eltako_fsud"};Nb.COMMANDS="on off toggle dimTo dimUp dimDown toggleTo setRampTime".split(" ");Nb.STATES=["state","ramp_time"];
Nb.prototype=new F;Nb.prototype.constructor=F;Nb.prototype.getTeachinTelegram=function(b,a,c){b=new Buffer([2,0,0,0]);return new x.hub.m.enocean.ESP3.Telegram.BS4(b,a,0)};var R=function(b){vc.call(this,b);this._data="eltako_blind";this._vendor="eltako";this._eepAlias="f6-02-01";this._syncRunning=!1;this._syncCB=this._syncTO=null};R.COMMANDS="up down stop stepUp stepDown setRV setRunTime moveTo sync".split(" ");R.STATES=["pos","sync"];R.prototype=new vc;R.prototype.constructor=R;R.prototype.setDefaultParams=
function(){this._params||(this._params={});if("undefined"==typeof this._params.rv||null==this._params.rv)this._params.rv=200;if("undefined"==typeof this._params.rt||null==this._params.rt)this._params.rt=200};R.prototype.handleLearnTelegram=function(b,a){if(a&&246==a.rorg)require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._learnAddress,senderID:b._learnSenderID,data:b._learnProfile,vendor:b._learnVendor},{address:a.senderID,senderID:b._learnSenderID,data:this._data,vendor:this._vendor},
function(a,d){b._learnFinish(a,0,d)})};R.prototype.getTeachinTelegram=function(b,a,c){b=new Buffer([255,248,13,128]);return new x.hub.m.enocean.ESP3.Telegram.BS4(b,a,0)};R.prototype.getStatesBuffer=function(){var b=JSON.parse(JSON.stringify(this._states));b.sync||(b.sync={value:!1,time:-1});b.rv={value:this._params.rv,time:-1};b.rt={value:this._params.rt,time:-1};return b};R.prototype.evalData=function(b,a,c){(b=this.evalTelegram(a))?(a=this.checkStateChange(b),b.sync&&this._syncRunning&&(this._syncRunning=
!1,this._syncTO&&(clearTimeout(this._syncTO),this._syncTO=null),this._syncCB&&(this._syncCB(),this._syncCB=null)),c&&c(null,a)):c&&c(Error("data format invalid"))};R.prototype.evalTelegram=function(b){var a={};if(246==b.rorg)switch(b.userData){case 112:case 1:a.sync=!0;a.pos=0;break;case 80:case 2:a.sync=!0,a.pos=100}else if(165==b.rorg){var c=b.userData.readUInt16BE(0);b=b.userData.readUInt8(2);var e=this._params.rt;e||(e=200);var f=this._states.pos?this._states.pos.value:null;if("undefined"!=typeof f&&
null!=f)switch(c=10*c/e,b){case 1:a.pos=Math.max(0,Math.round(f-c));break;case 2:a.pos=Math.min(100,Math.round(f+c))}}return a};R.prototype.executeCommand=function(b,a,c){if(a&&a.value){if("stepUp"==a.value.substr(0,6)&&6<a.value.length){var d=a.value.substr(6);a={value:"stepUp",ext:d}}else"stepDown"==a.value.substr(0,8)&&8<a.value.length?(d=a.value.substr(8),a={value:"stepDown",ext:d}):"setRV"==a.value.substr(0,5)&&5<a.value.length?(d=a.value.substr(5),a={value:"setRV",ext:d}):"setRunTime"==a.value.substr(0,
10)&&10<a.value.length?(d=a.value.substr(10),a={value:"setRunTime",ext:d}):"moveTo"==a.value.substr(0,6)&&6<a.value.length&&(d=a.value.substr(6),a={value:"moveTo",ext:d});(new Date).getTime();var e=this,f=this._params.rv;f||(f=200);var g=this._params.rt;g||(g=200);switch(a.value){case "up":this.doReq(b,{TIME:f,CMD:1,LCK:0,TYPE:0},function(b){c&&c(b)});break;case "down":this.doReq(b,{TIME:f,CMD:2,LCK:0,TYPE:0},function(b){c&&c(b)});break;case "stop":this.doReq(b,{TIME:0,CMD:0,LCK:0,TYPE:0},function(b){c&&
c(b)});break;case "stepUp":g="undefined"==typeof a.ext||null==a.ext?g/100:parseFloat(a.ext)/100*g;1>g&&(g=1);this.doReq(b,{TIME:g,CMD:1,LCK:0,TYPE:0},function(b){c&&c(b)});break;case "stepDown":g="undefined"==typeof a.ext||null==a.ext?g/100:parseFloat(a.ext)/100*g;1>g&&(g=1);this.doReq(b,{TIME:g,CMD:2,LCK:0,TYPE:0},function(b){c&&c(b)});break;case "moveTo":if("undefined"==typeof a.ext||null==a.ext){c&&c(Error("value invalid"));break}d=parseInt(a.ext);if(this._states.sync&&this._states.sync.value&&
this._states.pos&&"undefined"!=typeof this._states.pos.value&&null!=this._states.pos.value){g*=(d-this._states.pos.value)/100;if(0<g)var h=2;else if(0>g)g=0-g,h=1;else if(0==g){c&&c();break}this.doReq(b,{TIME:g,CMD:h,LCK:0,TYPE:1},function(b){c&&c(b)})}else this.snyc(b,function(d){d?c&&c(d):e.executeCommand(b,a,c)});break;case "sync":this.snyc(b,function(b){c&&c(b)});break;case "setRV":if(!a.ext){c&&c(Error("parameters is null"));break}d=parseInt(d);200<d&&(d=200);0>d&&(d=0);this._params.rv=d;h=require("x.hub.m.enocean.js")._deviceHandler;
h.deviceParamsUpdated(this,function(b){c&&c(b)});break;case "setRunTime":if(!a.ext){c&&c(Error("parameters is null"));break}d=parseInt(d);200<d&&(d=200);0>d&&(d=0);this._params.rt=d;h=require("x.hub.m.enocean.js")._deviceHandler;h.deviceParamsUpdated(this,function(b){c&&c(b)});break;default:c&&c(Error("unknow command"))}}else c&&c(Error("command invalid"))};R.prototype.snyc=function(b,a){if(this._syncRunning)a&&a(Error("sync running"));else{var d=this,c=this._params.rv;c||(c=200);this._syncRunning=
!0;this._syncTO=setTimeout(function(){d._syncRunning=!1;d._syncTO=null;d._syncCB&&d._syncCB(Error("sync timeout"));d._syncCB=null},1E3*(c+2));this._syncCB=a;this.doReq(b,{TIME:c,CMD:1,LCK:0,TYPE:0})}};R.prototype.doReq=function(b,a,c){var d=this._senderID;this._senderID||(d=x.hub.m.enocean.SENDER_ID);if(a.TYPE){var e=Math.round(10*a.TIME)>>8&255;var f=Math.round(10*a.TIME)&255}else e=0,f=Math.round(a.TIME)&255;a=new Buffer([e,f,a.CMD,8|(a.LCK&1)<<2|(a.TYPE&1)<<1]);d=new x.hub.m.enocean.ESP3.Telegram.BS4(a,
d,0);d.setDstId(this._address);b.sendTelegram(d,!0,function(b){c&&c(b)})};var Nc=function(){var b=function(b){R.call(this,b);this.TEACH_IN_VARIATION=3;this._data="eltako_tf_blind"};b.prototype=new R;b.prototype.constructor=b;b.prototype.handleTeachinTelegram=function(b,a){if(a&&165==a.rorg&&a.userData){var d=a.userData.toString("hex"),c=b._teachSenderID;if(d&&c&&c.substr(2)==d.substr(0,6))require("x.hub.m.enocean.js")._deviceHandler.onLearnDevice({address:b._teachID,senderID:b._teachSenderID,data:b._teachProfile,
vendor:b._teachVendor},{address:a.senderID,senderID:b._teachSenderID,data:b._teachProfile,vendor:b._teachVendor},function(a,d){b._teachFinish(a,d)})}};return b}(),Ob=function(b){Na.call(this,b);this._data="eltako_weather";this._vendor="eltako";this._eepAlias="a5-04-02";this._engMax=7.2;this._engMin=0};Ob.STATES="energy energy_raw humidity humidity_raw temperature temperature_raw available".split(" ");Ob.prototype=new Na;Ob.prototype.constructor=Ob;Ob.prototype.evalTelegram=function(b){var a={};if(165==
b.rorg){var c=b.userData.readUInt8(3),e=b.userData.readUInt8(2),f=b.userData.readUInt8(1);b=b.userData.readUInt8(0);1==(c>>3&1)&&(a.energy=parseFloat((b/250*(this._engMax-this._engMin)+this._engMin).toFixed(1)),a.energy_raw=b,a.humidity=parseFloat((f/250*(this._humMax-this._humMin)+this._humMin).toFixed(1)),a.humidity_raw=f,a.temperature=parseFloat((e/250*(this._tempMax-this._tempMin)+this._tempMin).toFixed(1)),a.temperature_raw=e,a.available=c>>1&1?!0:!1)}return a};var Pb=function(b){Oa.call(this,
b);this._data="eltako_lux";this._vendor="eltako";this._eepAlias="a5-06-01";this._ill1Max=3E4;this._ill1Min=300};Pb.STATES="illumination2 illumination2_raw illumination1 illumination1_raw illumination illumination_raw".split(" ");Pb.prototype=new Oa;Pb.prototype.constructor=Pb;Pb.prototype.evalTelegram=function(b){var a={};if(165==b.rorg){var c=b.userData.readUInt8(3);b.userData.readUInt8(2);var e=b.userData.readUInt8(1);b=b.userData.readUInt8(0);1==(c>>3&1)&&(a.illumination2=b,a.illumination2_raw=
b,a.illumination1=parseFloat((e/255*(this._ill1Max-this._ill1Min)+this._ill1Min).toFixed(1)),a.illumination1_raw=e,a.illumination=e?a.illumination1:a.illumination2,a.illumination_raw=e?a.illumination1_raw:a.illumination2_raw)}return a};var zc=function(b){Ea.call(this,b);this._data="eltako_lux2";this._vendor="eltako";this._eepAlias="a5-06-02";this._ill2Min=0;this._ill2Max=1024};zc.STATES=Ea.STATES;zc.prototype=new Ea;zc.prototype.constructor=zc;var G=function(b){c.call(this,b);this._data="eltako_buttons";
this._vendor="eltako";this._eepAlias="f6-02-01";this._longPressThreshold=1E3;this.BUTTONS=["button1","button2","button3","button4"]};G.STATES=["button1","button2","button3","button4"];G.EVENTS=["shortPress","longPress","press","release"];G.prototype=new c;G.prototype.constructor=G;G.prototype.evalTelegram=function(b){var a={};if(246==b.rorg)switch(b.userData){case 112:this.BUTTONS[0]&&(a[this.BUTTONS[0]]="pressed");break;case 80:this.BUTTONS[1]&&(a[this.BUTTONS[1]]="pressed");break;case 48:this.BUTTONS[2]&&
(a[this.BUTTONS[2]]="pressed");break;case 16:this.BUTTONS[3]&&(a[this.BUTTONS[3]]="pressed");break;case 0:for(b=0;b<this.BUTTONS.length;b++)this.BUTTONS[b]&&(a[this.BUTTONS[b]]="released")}return a};G.prototype.checkStateChange=function(b){var a=this._states,c=[],e=(new Date).getTime(),f;for(f in b){var g={};g.key=f;g.value=b[f];g.changed=!1;var h=a[f]?a[f].value:null,k=a[f]?a[f].time:null,l=b[f];h!=l&&(g.oldvalue=h,g.changed=!0);this._states[f]={value:b[f],time:e};g.changed&&c.push(g);"pressed"==
h&&"released"==l&&(!k||e-k<this._longPressThreshold?(g={},g.key=f,g.value="shortPress"):(g={},g.key=f,g.value="longPress"),g.changed=!0,c.push(g))}return c};var Ac=function(a){G.call(this,a);this._data="eltako_button1";this.BUTTONS=[null,null,null,"button"]};Ac.STATES=["button"];Ac.prototype=new G;Ac.prototype.constructor=Ac;var Bc=function(a){G.call(this,a);this._data="eltako_sender2";this.BUTTONS=["A1","A3"]};Bc.STATES=["A1","A3"];Bc.prototype=new G;Bc.prototype.constructor=Bc;var Cc=function(a){G.call(this,
a);this._data="eltako_sender4";this.BUTTONS=["E1","E2","E3","E4"]};Cc.STATES=["E1","E2","E3","E4"];Cc.prototype=new G;Cc.prototype.constructor=Cc;var Dc=function(a){G.call(this,a);this._data="eltako_button2";this.BUTTONS=["buttonO","buttonI"]};Dc.STATES=["buttonO","buttonI"];Dc.prototype=new G;Dc.prototype.constructor=Dc;var Ec=function(a){G.call(this,a);this._data="eltako_button4";this.BUTTONS=["BO","BI","AO","AI"]};Ec.STATES=["BO","BI","AO","AI"];Ec.prototype=new G;Ec.prototype.constructor=Ec;var Qb=
function(a){G.call(this,a);this._data="eltako_fsm60b"};Qb.STATES=["button"];Qb.prototype=new G;Qb.prototype.constructor=Qb;Qb.prototype.evalTelegram=function(a){var b={};if(246==a.rorg)switch(a.userData){case 112:case 80:case 16:b.button="pressed";break;case 0:b.button="released"}return b};var Rb=function(a){c.call(this,a);this._data="eltako_fkf";this._vendor="eltako";this._eepAlias="f6-04-01"};Rb.STATES=["keycard"];Rb.prototype=new c;Rb.prototype.constructor=Rb;Rb.prototype.evalTelegram=function(a){var b=
{};if(246==a.rorg)switch(a.userData){case 16:b.keycard="inserted";case 0:b.keycard="takeout"}return b};var Sb=function(a){c.call(this,a);this._data="eltako_fkc";this._vendor="eltako";this._eepAlias="f6-04-01"};Sb.STATES=["keycard"];Sb.prototype=new c;Sb.prototype.constructor=Sb;Sb.prototype.evalTelegram=function(a){var b={};if(246==a.rorg)switch(a.userData){case 48:b.keycard="guest_card";case 32:b.keycard="service_card";case 16:b.keycard="inserted";case 0:b.keycard="takeout"}return b};var Ka=function(a){Ga.call(this,
a);this._data="eltako_thermo";this._vendor="eltako";this._eepAlias="a5-10-06"};Ka.STATES="setpoint setpoint_raw temperature temperature_raw night_reduce night_reduce_raw".split(" ");Ka.prototype=new Ga;Ka.prototype.constructor=Ka;Ka.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3),e=a.userData.readUInt8(2),f=a.userData.readUInt8(1);a=a.userData.readUInt8(0);1==(c>>3&1)&&(b.night_reduce=6>=a?0:12>=a?1:19>=a?2:25>=a?3:31>=a?4:5,b.night_reduce_raw=a,b.setpoint=
parseFloat((f/255*40).toFixed(1)),b.setpoint_raw=f,b.temperature=parseFloat(((255-e)/255*40).toFixed(1)),b.temperature_raw=e)}return b};var Fc=function(a){Ka.call(this,a);this._data="eltako_futh65d";this._vendor="eltako";this._eepAlias="a5-10-12"};Fc.STATES="setpoint setpoint_raw temperature temperature_raw night_reduce night_reduce_raw".split(" ");Fc.prototype=new Ka;Fc.prototype.constructor=Fc;var Tb=function(a){Fa.call(this,a);this._data="eltako_ftr78s";this._vendor="eltako";this._eepAlias="a5-10-03"};
Tb.STATES=["setpoint","setpoint_raw","temperature","temperature_raw"];Tb.prototype=new Fa;Tb.prototype.constructor=Tb;Tb.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3),e=a.userData.readUInt8(2);a=a.userData.readUInt8(1);1==(c>>3&1)&&(b.setpoint=parseFloat((a/255*22+8).toFixed(1)),b.setpoint_raw=a,b.temperature=parseFloat(((255-e)/255*40).toFixed(1)),b.temperature_raw=e)}return b};var Xa=function(a){ma.call(this,a);this._data="eltako_powermeter";this._vendor=
"eltako";this._eepAlias="a5-12-01"};Xa.STATES=["power","energy","tariff"];Xa.prototype=new ma;Xa.prototype.constructor=Xa;Xa.prototype.evalTelegram=function(a){var b=ma.prototype.evalTelegram.call(this,a);165==a.rorg&&143==a.userData.readUInt8(3)&&(b={});return b};var Gc=function(a){ma.call(this,a);this._data="eltako_powermeter2";this._vendor="eltako";this._eepAlias="a5-12-01"};Gc.STATES=["power","tariff"];Gc.prototype=new ma;Gc.prototype.constructor=Xa;Gc.prototype.evalTelegram=function(a){a=ma.prototype.evalTelegram.call(this,
a);delete a.energy;delete a.tariff;return a};var Ya=function(a){G.call(this,a);this._data="eltako_fzs"};Ya.STATES=["button"];Ya.prototype=new G;Ya.prototype.constructor=Ya;Ya.prototype.evalTelegram=function(a){var b={};if(246==a.rorg)switch(a.userData){case 48:b.button="pressed";break;case 0:b.button="released"}return b};Ya.prototype.checkStateChange=function(a){var b=this._states,c=[],e=(new Date).getTime(),f;for(f in a){var g={};g.key=f;g.value=a[f];g.changed=!1;var h=b[f]?b[f].value:null,k=b[f]?
b[f].time:null,l=a[f];h!=l&&(g.oldvalue=h,g.changed=!0);this._states[f]={value:a[f],time:e};g.changed&&c.push(g);"pressed"==h&&"released"==l&&(!k||e-k<this._longPressThreshold?(g={},g.key=f,g.value="shortPress"):(g={},g.key=f,g.value="longPress"),g.changed=!0,c.push(g))}return c};var Ub=function(a){h.call(this,a);this._data="eltako_relay2";this._vendor="eltako";this._eepAlias="f6-02-01"};Ub.STATES=["ch1","ch2"];Ub.prototype=new h;Ub.prototype.constructor=Ub;Ub.prototype.evalTelegram=function(a){var b=
{};if(246==a.rorg)switch(a.userData){case 112:b.ch1="on";break;case 80:b.ch1="off";break;case 48:b.ch2="on";break;case 16:b.ch2="off"}return b};var Vb=function(a){h.call(this,a);this._data="eltako_relay";this._vendor="eltako";this._eepAlias="f6-02-01"};Vb.STATES=["state"];Vb.prototype=new h;Vb.prototype.constructor=Vb;Vb.prototype.evalTelegram=function(a){var b={};if(246==a.rorg)switch(a.userData){case 112:b.state="on";break;case 80:b.state="off"}return b};var Wb=function(a){ea.call(this,a);this._data=
"eltako_heat";this._vendor="eltako";this._eepAlias="a5-30-03"};Wb.STATES=["temperature","temperature_raw","alarm","alarm_raw"];Wb.prototype=new ea;Wb.prototype.constructor=Wb;Wb.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3),e=a.userData.readUInt8(2),f=a.userData.readUInt8(1);a.userData.readUInt8(0);1==(c>>3&1)&&(b.temperature=parseFloat(((255-f)/255*40).toFixed(1)),b.temperature_raw=f,b.alarm=15==e?"alarm":"ok",b.alarm_raw=e)}return b};var Xb=function(a){ea.call(this,
a);this._data="eltako_water_m3";this._vendor="eltako";this._eepAlias="a5-30-03"};Xb.STATES=["water","water_raw"];Xb.prototype=new ea;Xb.prototype.constructor=Xb;Xb.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3);a=a.userData.readUInt8(2);1==(c>>3&1)&&(b.water=15==a,b.water_raw=a)}return b};var Yb=function(a){Ja.call(this,a);this._data="eltako_water_m4";this._vendor="eltako";this._eepAlias="a5-30-01"};Yb.STATES=["water","water_raw"];Yb.prototype=new Ja;Yb.prototype.constructor=
Yb;Yb.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3);a=a.userData.readUInt8(2);1==(c>>3&1)&&(b.water=0==a,b.water_raw=a)}return b};var ta=function(a){W.call(this,a);this.TEACH_IN_VARIATION=3;this._data="peha_powerswitch";this._vendor="peha";this._eepAlias="a5-38-08"};ta.COMMANDS=["on","off","toggle"];ta.STATES="state state_raw level rgb power energy current voltage power_raw power_unit energy_unit current_unit voltage_unit power_unit_raw op_hour error error_raw service_mode op_hour_flag op_hour_flag_raw mode_raw".split(" ");
ta.prototype=new W;ta.prototype.constructor=ta;ta.prototype.getTeachinType=function(){return"4BS"};ta.prototype.evalTelegram=function(a){return Ua.prototype.evalTelegram.call(this,a)};ta.prototype.executeCommand=function(a,c,e){if(c&&c.value){(new Date).getTime();var b=this;switch(c.value){case "on":this.switching(a,{TIM:0,LCK:0,DEL:0,SW:1},function(a){a||b.activeStateChange({state:"on"});e&&e(a)});break;case "off":this.switching(a,{TIM:0,LCK:0,DEL:0,SW:0},function(a){a||b.activeStateChange({state:"off"});
e&&e(a)});break;case "toggle":if(!this._states.state){e&&e(Error("unknow previous switch state"));break}"on"==this._states.state.value?this.executeCommand(a,{value:"off"},e):this.executeCommand(a,{value:"on"},e);break;default:e&&e(Error("unknow command"))}}else e&&e(Error("command invalid"))};var Za=function(a){ta.call(this,a);this.TEACH_IN_VARIATION=3;this._data="peha_switch";this._vendor="peha";this._eepAlias="a5-38-08"};Za.COMMANDS=["on","off","toggle"];Za.STATES="state state_raw level rgb power energy current voltage power_raw power_unit energy_unit current_unit voltage_unit power_unit_raw op_hour error error_raw service_mode op_hour_flag op_hour_flag_raw mode_raw".split(" ");
Za.prototype=new ta;Za.prototype.constructor=Za;Za.prototype.evalTelegram=function(a){a=Ua.prototype.evalTelegram.call(this,a);delete a.power;delete a.energy;delete a.current;delete a.voltage;delete a.power_unit;delete a.energy_unit;delete a.current_unit;delete a.voltage_unit;return a};var ua=function(a){W.call(this,a);this.TEACH_IN_VARIATION=3;this._data="peha_blind";this._vendor="peha";this._eepAlias="a5-38-08";this._initCount=0};ua.COMMANDS="up down stop stepUp stepDown moveTo lamellaUp lamellaDown lamellaStepUp lamellaStepDown lamellaTo setRunTime setLamellaRuntime".split(" ");
ua.STATES="position angle position_flag position_flag_raw angle_flag angle_flag_raw error error_raw end_position end_position_raw state state_raw service_mode position_mode".split(" ");ua.prototype=new W;ua.prototype.constructor=ua;ua.prototype.init=function(a){var b=require("x.hub.m.enocean.js")._esp3;this.blind(b,{P1:0,P2:0,FUNC:0,SSF:0,PAF:0,SMF:0},function(b){a&&a()})};ua.prototype.getTeachinType=function(){return"4BS"};ua.prototype.getStatesBuffer=function(){0==Object.keys(this._states).length?
(100>=this._initCount&&0==this._initCount%10?this.init():100<=this._initCount&&600>=this._initCount&&0==this._initCount%60?this.init():this._initCount=0,this._initCount++):this._initCount=0;return this._states};ua.prototype.evalTelegram=function(a){return vb.prototype.evalTelegram.call(this,a)};ua.prototype.executeCommand=function(a,c,e){if(c&&c.value){(new Date).getTime();var b=this;if("stepUp"==c.value.substr(0,6)&&6<c.value.length){var d=c.value.substr(6);c={value:"stepUp",ext:d}}else"stepDown"==
...
originalEEP:this._originalEEP}};A.prototype.fromDBJSON=function(a){if(!a||!a.id||!a.eep||a.eep!=this._data)return null;this._address=a.id;this._senderID=a.senderID;this._vendor=a.vendor?a.vendor:this._vendor;this._virtual=a.virtual?!0:!1;this._params=a.params;this._hopCount=a.hopCount;this._saLearnMode=a.saLearnMode;a.originalEEP&&(this._originalEEP=a.originalEEP);this.setDefaultParams();return this};var Jc=function(a){A.call(this,a);this._data="d2-06-10"};Jc.prototype=new A;Jc.prototype.constructor=
Jc;var Hc=function(a){ea.call(this,a);this._data="afriso_water_con";this._vendor="afriso";this._eepAlias="a5-30-03"};Hc.prototype=new ea;Hc.prototype.constructor=Hc;Hc.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3),e=a.userData.readUInt8(2);a=a.userData.readUInt8(1);1==(c>>3&1)&&(b.temperature=(255-a)/255*40,b.temperature_raw=a,b.water=0==(e>>4&1)?!0:!1)}return b};var Zb=function(a){z.call(this,a);this._data="afriso_water_eco";this._vendor="afriso";this._eepAlias=
"f6-05-01"};Zb.prototype=new z;Zb.prototype.constructor=Zb;Zb.prototype.evalTelegram=function(a){var b={};246==a.rorg&&(a=a.nu,1==a?b.water="true":0==a&&(b.water="false"));return b};Zb.prototype.onLearn=function(){this._states.water={value:!1,time:(new Date).getTime()}};var Ic=function(a){ea.call(this,a);this._data="afriso_asd_10";this._vendor="afriso";this._eepAlias="a5-30-03"};Ic.prototype=new ea;Ic.prototype.constructor=Ic;Ic.prototype.evalTelegram=function(a){var b={};if(165==a.rorg){var c=a.userData.readUInt8(3),
e=a.userData.readUInt8(2);a=a.userData.readUInt8(1);1==(c>>3&1)&&(b.temperature=(255-a)/255*40,b.temperature_raw=a,b.smoke=0==(e>>4&1)?!0:!1)}return b};var $b=function(a){U.call(this,a);this._data="hoppe_door";this._vendor="hoppe";this._eepAlias="f6-10-00"};$b.STATES=["door"];$b.prototype=new U;$b.prototype.constructor=$b;$b.prototype.evalTelegram=function(a){var b={};246==a.rorg&&(a=a.userData,0==(a&208^192)?b.door="open":0==(a&240^240)&&(b.door="closed"),b.handle_raw=a);return b};var Kc=function(){var a=
...
a))return null;a&&a.gpInfo&&(this._gpInfo=a.gpInfo);return this};return a}(),Oc=function(){var a=function(a){Kc.call(this,a);this._data="afriso_ais_10";this._vendor="afriso";this._eepAlias="gp"};a.prototype=new Kc;a.prototype.constructor=a;a.prototype.isGPMatch=function(a,b){return 45!=a?!1:8==b};a.prototype.evalTelegram=function(a){var b={};if(178==a.rorg){var c=a.data.readUInt8(0)>>7&1,d=a.data.readUInt8(0)>>6&1;a=a.data.readUInt8(0)>>5&1;b.alarm=1==c?"on":"off";b.active=1==d?"on":"off";b.powerAlarm=
...
this._senderID||(d=x.hub.m.enocean.SENDER_ID);var f=0;b&&(f|=128);c&&(f|=64);b=new Buffer(1);b.writeUInt8(f,0);d=new x.hub.m.enocean.ESP3.Telegram.GP_CD(b,d,0);d.setDstId(this._address);a.sendTelegram(d,!0,function(a){e&&e(a)})};return a}(),Pc=function(){var a=function(a){ka.call(this,a);this._data="eltako_tf_ttb";this._vendor="eltako";this._eepAlias="a5-07-01";this._params={reset_timeout:300}};a.prototype=new ka;a.prototype.constructor=a;a.prototype.evalTelegram=function(a){return 246==a.rorg?k.prototype.evalTelegram.call(this,
a):165==a.rorg?ka.prototype.evalTelegram.call(this,a):{}};a.prototype.checkStateChange=function(a){return a.button?k.prototype.checkStateChange.call(this,a):ka.prototype.checkStateChange.call(this,a)};a.prototype.fromJSON=function(a){ka.prototype.fromJSON.call(this,a);this._params={reset_timeout:300};return this};return{TF_TTB:a}}();return{"f6-??-??":c,"f6-01-01":k,"f6-02-01":h,"f6-02-02":l,"f6-02-03":p,"f6-02-04":r,"f6-03-01":v,"f6-03-02":y,"f6-04-01":C,"f6-04-02":I,"f6-05-00":J,"f6-05-01":z,"f6-05-02":w,
"f6-10-00":U,"f6-10-01":S,"d5-xx-xx":a,"d5-??-??":X,"d5-00-01":T,"f6-so-da":f,"a5-??-??":t,"a5-02-01":Y,"a5-02-02":K,"a5-02-03":oa,"a5-02-04":N,"a5-02-05":O,"a5-02-06":ca,"a5-02-07":Z,"a5-02-08":aa,"a5-02-09":Ma,"a5-02-0a":u,"a5-02-0b":m,"a5-02-10":H,"a5-02-11":ac,"a5-02-12":La,"a5-02-13":bc,"a5-02-14":cc,"a5-02-15":Ca,"a5-02-16":dc,"a5-02-17":$a,"a5-02-18":ec,"a5-02-19":fc,"a5-02-1a":gc,"a5-02-1b":hc,"a5-02-20":Da,"a5-02-30":ic,"a5-04-01":jc,"a5-04-02":Na,"a5-04-03":ab,"a5-06-01":Oa,"a5-06-02":Ea,
"a5-06-03":bb,"a5-06-04":cb,"a5-06-05":kc,"a5-07-01":ka,"a5-07-02":Pa,"a5-07-03":Qa,"a5-08-01":Ra,"a5-08-02":lc,"a5-08-03":mc,"a5-09-02":db,"a5-09-04":eb,"a5-09-05":fb,"a5-09-06":gb,"a5-09-07":hb,"a5-09-08":ib,"a5-09-09":jb,"a5-09-0a":kb,"a5-09-0b":lb,"a5-09-0c":mb,"a5-10-01":B,"a5-10-02":la,"a5-10-03":Fa,"a5-10-04":nb,"a5-10-05":ob,"a5-10-06":Ga,"a5-10-07":pb,"a5-10-08":pa,"a5-10-09":qa,"a5-10-0a":za,"a5-10-0b":Ha,"a5-10-0c":qb,"a5-10-0d":rb,"a5-10-10":P,"a5-10-11":Aa,"a5-10-12":sb,"a5-10-13":tb,
"a5-10-14":ub,"a5-12-00":wb,"a5-12-01":ma,"a5-12-02":Ia,"a5-12-03":nc,"a5-13-01":oc,"a5-13-02":pc,"a5-13-03":qc,"a5-13-04":rc,"a5-13-05":sc,"a5-13-06":tc,"a5-14-01":xb,"a5-14-02":yb,"a5-14-03":zb,"a5-14-04":Ab,"a5-14-05":Bb,"a5-14-06":Cb,"a5-14-07":Db,"a5-14-08":Eb,"a5-14-09":Fb,"a5-14-0a":Gb,"a5-20-04":ia,"a5-30-01":Ja,"a5-30-02":Hb,"a5-30-03":ea,"a5-30-04":Ib,"a5-38-08":W,"d2-??-??":fa,"d2-01-00":na,"d2-01-01":Jb,"d2-01-03":Kb,"d2-01-11":Ba,"d2-01-13":ra,"d2-01-14":sa,"d2-05-00":M,"d2-05-01":wa,
"d2-06-01":xa,"d2-06-02":ya,"d2-06-10":Jc,"d2-06-50":e,"d2-20-00":ba,eltako_dimmer:F,eltako_dimmer2:Wa,eltako_fsud:Nb,eltako_motion:Mb,eltako_contact:Lb,eltako_weather:Ob,eltako_lux:Pb,eltako_lux2:zc,eltako_sender2:Bc,eltako_sender4:Cc,eltako_button1:Ac,eltako_button2:Dc,eltako_button4:Ec,eltako_blind:R,eltako_fsm60b:Qb,eltako_fkf:Rb,eltako_fkc:Sb,eltako_thermo:Ka,eltako_futh65d:Fc,eltako_ftr78s:Tb,eltako_powermeter:Xa,eltako_powermeter2:Gc,eltako_fzs:Ya,eltako_switch:ja,eltako_relay:Vb,eltako_relay2:Ub,
eltako_heat:Wb,eltako_water_m3:Xb,eltako_water_m4:Yb,eltako_tf_switch:Lc,eltako_tf_dimmer:Mc,eltako_tf_blind:Nc,peha_switch:Za,peha_powerswitch:ta,peha_blind:ua,"bb-aa-cc":A,afriso_water_con:Hc,afriso_water_eco:Zb,afriso_asd_10:Ic,hoppe_door:$b,gp:Kc,afriso_ais_10:Oc,eltako_tf_ttb:Pc.TF_TTB,parseDBJson:function(a){if(!a||!a.eep)return null;var b=this[a.eep];if(!b)return null;b=new b;return b=b.fromDBJSON(a)},parseJson:function(a){if(!a||!a.data)return null;var b=this[a.data];if(!b)return null;b=new b;
...
this._logger.log("info","config siegenia window:"+JSON.stringify(e));this._logger.log("info","window id:"+a+" sender id:"+c);var g=parseInt(e.alive),h=parseInt(e.sync),l=parseInt(e.eep),p=parseInt(e.type),r=parseInt(e.alarm);e=parseInt(e.alarm_sens);var v=[128,2,127,240,1,140,57,45,245],y=[192,13,127,242,49,0,0,2,g>>8&255],C=[193,g&255,0,1,2,h>>8&255,h&255,0,2],I=[194,1,l,0,3,1,p,0,4],J=[195,1,r,0,5,1,e,0,0],z=[128,2,127,240,2,140,57,45,245],w=this;this._logger.log("info","send config siegenia window (data1):"+
...
!a.inovent)return{};var c={},e;for(e in a.inovent){var f=a.inovent[e];if(f){var p=new g(e);p.fromJSON(f);c[e]=p}}return c};f.prototype._jsonToSmartGuardSettingsMap=function(c){if(!c||!c.smartguard)return{};var e={},f;for(f in c.smartguard){var g=c.smartguard[f];if(g){var p=new a(f);p.fromJSON(g);e[f]=p}}return e};f.prototype._loadFile=function(a){var c=this._getFilePath();fs_extra.ensureFile(c,function(e){e?a&&a(e):fs_extra.readFile(c,"utf8",function(c,e){if(c)a&&a(c);else if(e)try{var f=JSON.parse(e);
...
{value:f},function(a){c&&c({error:a?a.error:null})}):c&&c({error:Error("address invalid")}):c&&c({error:Error("not inited")})};g.prototype.getAllStatesLegacy=function(e,a){var f=[],c=this.getAllStates(),g=this._secHandler.getUnsyncDevice();c.forEach(function(c){if(c&&c.device&&c.states){var e=c.device.address,h={type:"ENOCEAN",adr:e,data:c.device.data,vendor:c.device.vendor,state:{}},k;for(k in c.states)h.state[k]=c.states[k].value;a&&"1"==a.config&&(h.senderID=c.device.senderID,h.virtual=c.device.virtual,
h.params=c.device.params,e&&g[e]&&(h.rlc_unsync=!0));f.push(h)}});e&&e({data:f})};g.prototype.getEnoceanInfo=function(){return{baseID:x.hub.m.enocean.SENDER_ID,usb_connected:null!=this._esp3._serial,usb_baseID:this._esp3._baseID,num_free_senderID:this._deviceHandler.getFreeSenderIdCount()}};g.prototype.learnDevice=function(e,a){var f=e?e.senderID:null,c=e?e.data:null,g=e?e.vendor:null,h=e?e.address:null;if(h){c=this._deviceHandler.findDevice(e);if(!c){a&&a({error:Error("device not found")});return}var l=
c.toJSON();c=l.data;g=l.vendor;f||(sednerID=l.senderID)}l={};l.saLearnMode=e?e.saLearnMode:null;this._learnHandler.learn(c,h,f,g,null,function(c,e,f){c?a&&a({error:c}):(c={},1==e?c.op=1:(c=f.toJSON(),c.op=0),a&&a({data:c}))},l)};g.prototype.learnSoda=function(e){if(this._esp3._serial){var a=this,f=setTimeout(function(){a._esp3.removeListener(c);e&&e({error:Error("learn timeout")})},2E4),c={_onEOTelegram:function(g){if(246==g.rorg&&(192==g.userData||208==g.userData||240==g.userData)){clearTimeout(f);
a._esp3.removeListener(c);var h={address:g.senderID,data:"f6-so-da",vendor:"soda"};a.addSensor(g.senderID,"f6-so-da","soda",null,function(a){a.error?e&&e(a):e&&e({data:h})})}}};this._esp3.addListener(c)}else e&&e({error:Error("port not opened")})};g.prototype.teachinDevice=function(e,a){if(e)if(e.data){var f=this._deviceHandler.findDevice(e);!f&&e.address?a&&a({error:Error("device not found")}):(f=f?f.toJSON():e,e.data!=f.data?a&&a({error:Error("device of another type with the same address exists")}):
e.vendor&&e.vendor!=f.vendor?a&&a({error:Error("device of another vendor with the same address exists")}):this._learnHandler.teachin(f.data,f.address,f.senderID,f.vendor,function(c,e,f){c?a&&a({error:c}):(c={},e&&(c=e.toJSON()),f&&(c.op=f),a&&a({data:c}))}))}else a&&a({error:Error("data is null")});else a&&a({error:Error("query is null")})};g.prototype.generateVirtualDevice=function(e,a){this._deviceHandler.generateVirtual(e,function(e,c){a&&a({error:e,data:c?c.toJSON():null})})};g.prototype._onEOFrame=
function(e,a){this._trackers.forEach(function(f){f.onFrame(e,a)})};g.prototype._onEOTelegram=function(e){this._trackers.forEach(function(a){a.onTelegram(e)});var a=this._deviceHandler.getDevice(e.senderID);if(a){var f=this;a.evalData(this._esp3,e,function(c,e){c||f._onEOEvents(a,e)})}};g.prototype._onEOEvents=function(e,a){var f=require("x.hub.eventbus.js");this._trackers.forEach(function(c){c.onResolveEvents(e._address,a)});this._rehauHandler.onDeviceEvent(e,a);var c=e.toJSON();c.sys="aio";c.type=
"ENOCEAN";for(var g=0;g<a.length;g++){var h=a[g];if(h&&("shortPress"==h.value||"longPress"==h.value)){var l={type:"ENOCEAN",adr:c.address,data:c.data,vendor:c.vendor,state:{}};l.state[h.key]=h.value;f.emit("gatewayevent",l,{address:"ENOCEAN"},"BTN")}}if(c=e.getStatesLegacy())f.emit("gatewayevent",c,{address:"ENOCEAN"},"STA"),f.emit("udpevent",c,"STA")};g.prototype._saveEnoceanEvents=function(e,a){var f=require("x.hub.datastore.js").dataStore;f&&f.openDB(function(c,f){if(!c){c=e._address;for(var g=
[],k=(new Date).toISOString(),p=0;p<a.length;p++){var r=a[p];if(r&&r.changed){r=r.key+":"+r.value;var v=null;v=0==g.length?"SELECT '"+c+"' AS 'window', '"+r+"' AS 'state', '"+k+"' AS 'time'":"UNION ALL SELECT '"+c+"', '"+r+"', '"+k+"'";g.push(v)}}0<g.length&&(c=g.join(" "),f.run("INSERT INTO windowstate(window, state, time) "+c+";",function(a){f.close()}))}})};g.prototype.addSensor=function(e,a){e?e.data?"siegenia"!=e.vendor||e.params?this._deviceHandler.addDevice(e,function(e,c){a&&a({error:e,data:c?
c.toJSON():null})}):a&&a({error:Error("siegenia windows params is null")}):a&&a({error:Error("enocean profile is null")}):a&&a({error:Error("enocean device is null")})};g.prototype.updateSensor=function(e,a,f){e?a?this._deviceHandler.updateDevice(e,a,function(a,e){f&&f({error:a,data:e?e.toJSON():null})}):f&&f({error:Error("device info is null")}):f&&f({error:Error("device address is null")})};g.prototype.delSensor=function(e,a){e?this._deviceHandler.deleteDevice({address:e},function(e){a&&a({error:e})}):
a&&a({error:Error("enocean id is null")})};g.prototype.resetDefaultSenderID=function(e){x.hub.m.enocean.SENDER_ID=null;localStorage&&localStorage.setItem("x.hub.m.enocean.SENDER_ID","");e&&e({})};g.prototype.setRehauAlarm=function(e,a){this._rehauHandler.setAlarm(e,a)};g.prototype.isRehauWindowOpen=function(){return this._rehauHandler.isWindowOpen()};g.prototype.setInoventHumidityAccessory=function(e,a,f){this._rehauHandler.setHumidityAccessory(e,a,function(a){f&&f({error:a})})};g.prototype.setInoventCo2Accessory=
function(e,a,f){this._rehauHandler.setCo2Accessory(e,a,function(a){f&&f({error:a})})};g.prototype.getInoventAccessories=function(e,a){e=this._rehauHandler.getInoventAccessory(e);a&&a({data:e?e.toJSON():{}})};g.prototype.__addTelegramTracker=function(e){e&&e.onTelegram&&e.onResolveEvents&&-1==this._trackers.indexOf(e)&&this._trackers.push(e)};g.prototype.__removeTelegramTracker=function(e){e&&(e=this._trackers.indexOf(e),-1!=e&&this._trackers.splice(e,1))};g.prototype.__sendEspFrame=function(e,a,f,
c,g){a=a?new Buffer(a,"hex"):null;f=f?new Buffer(f,"hex"):null;c=c?!0:!1;e=new x.hub.m.enocean.ESP3.ESP3_Frame(e,a,f);this._esp3.sendFrame(e,c,function(a,c){g&&g({error:a,data:c?c.toJSON():null})})};g.prototype.__simulateRawEnoceanPacket=function(e,a){this._esp3.onSerialData(new Buffer(e,"hex"));a&&a()};g.prototype._doSDK=function(e,a){if(e&&e.method){var f="send_"+e.method;if(this._sdk[f])this._sdk[f](e.params,function(c){a&&a({error:c})});else a&&a({error:Error("no such method")})}else a&&a({error:Error("data invalid")})};
