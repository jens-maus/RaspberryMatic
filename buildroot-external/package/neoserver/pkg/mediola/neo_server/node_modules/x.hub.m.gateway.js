...
x.hub.m.gateway.GatewayInternalHandler=function(){var c=function(){this._devices={};this._meta={};this._states={}};c.prototype.addStateDevice=function(a,b,e){if(a&&a.type&&a.address){a=JSON.parse(JSON.stringify(a));delete a.gateway;var d=this._getDeviceID(a);this._devices[d]||(this._devices[d]=a);this._meta[d]||(this._meta[d]=[]);this._states[d]||(this._states[d]={});e&&this._meta[d].push(e);b&&b()}else b&&b(Error("device json invalid"))};c.prototype.removeStateDevice=function(a,b,e){if(!a||!a.type||
!a.address)b&&b(Error("device json invalid"));else if(e){a=JSON.parse(JSON.stringify(a));delete a.gateway;a=this._getDeviceID(a);var d=this._meta[a];if(d){for(var c=-1,f=0;f<d.length;f++){var g=d[f];if(g&&g.src==e.src&&g.taskID==e.taskID&&g.statusKey==e.statusKey){c=f;break}}0<=c&&d.splice(c,1);0==d.length&&(delete this._devices[a],delete this._meta[a],delete this._states[a])}b&&b()}};c.prototype.onEvent=function(a,b,e){if(a&&a.type){b=null;var d=this._getSystem(a);if(d){var c=null,f;for(f in this._devices)if(this._devices.hasOwnProperty(f)){var g=
this._devices[f];if("EVT"==e&&d._handleUdpState){var h=d._handleUdpState(g,a);if(h&&0<Object.keys(h).length){c=g;this._getDeviceID(c);b=h;break}}else if("STA"==e&&d._handleState){if((h=d._handleState(g,a))&&0<Object.keys(h).length){c=g;this._getDeviceID(c);b=h;break}}else if("BTN"==e&&f==a.type+"@"+a.adr){c=g;b=a.state;break}}b&&this._updateState(c,b)}}};c.prototype._getDeviceID=function(a){return a&&"ZWAVE2"==a.type?a.type+"@"+a.address+"."+a.instance:a.type+"@"+a.address};c.prototype._getSystem=
function(a){return x.hub.m.gateway.AIOMODULE.devicemanager.getSystem(a)};c.prototype._updateState=function(a,b){var e=this._getDeviceID(a),d=this._meta[e],c=this._states[e]?this._states[e]:{};this._states[e]=b;if(d&&0<d.length)for(var f in b)if(b.hasOwnProperty(f)){e=b[f];var g=c[f];e={key:f,value:e,changed:g!=e,oldValue:g};g=require("x.hub.eventbus.js");require("x.hub.taskman.js");for(var h=0;h<d.length;h++){var k=d[h];!k||k.statusKey!=f&&"*"!=k.statusKey||g.emit("status",{module:"aio",device:a,
gateway:null,data:e},k)}}};return c}();
...
d.executeCommand(a,b,e):e&&e({error:Error("no module for type ENOCEAN")});else{var c=x.hub.m.gateway.AIOMODULE.getGatewayFunctionForType(a.type);if(c){if("CODE"==a.type){var f=x.hub.m.gateway.AIOMODULE.Systems.Code._rebuildDevice(JSON.parse(JSON.stringify(a)));a=x.hub.m.gateway.AIOMODULE.Systems.Code._findIRCode(b,a);a=x.hub.m.gateway.AIOMODULE.Systems.Code._buildParam(a,f)}else a="BG"==a.type?x.hub.m.gateway.AIOMODULE.Systems.BG._buildParam(null,a,b):x.hub.m.gateway.AIOMODULE.getGatewayParams(a,
b.value);c="http://localhost/command?XC_FNC="+c;if(a)for(d in a)c+="&"+d+"="+encodeURIComponent(a[d]);try{var g=require("x.hub.js").server;g?g._doSTMCommandRequest(c,!1,function(a){e&&e({error:a})}):e&&e({error:Error("server not initialized")})}catch(h){e&&e({error:h})}}else e&&e({error:Error("unknow device type")})}}else e&&e({error:Error("device type is null")})};c.prototype._executeCommandLocal=function(a,b,e,d){var c=x.hub.m.gateway.AIOMODULE.devicemanager.getSystem(b,a);if(c&&c.buildQuery&&(c=
c.buildQuery(a,b,e))){try{var f=require("x.hub.js").server;if(!f){d&&d({error:Error("server not initialized")});return}var g=require("querystring"),h=g.stringify(c);c=g.parse(h);a="/cmd?"+h;this._logger.log("trace","execute command local directly "+a);var k=this;f.doRequest("/cmd",{url:a,query:c},function(a){k._logger.log("trace","execute command local directly return:"+JSON.stringify(a));var b=null;if(a)try{(b=JSON.parse(a))&&b.XC_SUC?d&&d({data:b}):b&&b.XC_ERR?d&&d({error:Error(a)}):d&&d({error:Error("/cmd request return invalid format")})}catch(n){d&&
...
function(a,b,d){if(a.error)return{error:a.error};a=d._handleGeneralStatesRsp(a.data,b);b=null;a&&a[0]&&(b={value:a[0].status,time:(new Date).getTime()});return{error:null,data:b}},f=x.hub.m.gateway.AIOMODULE.resolveGateway(a);if(f){var g=[{id:"hub",device:b,status:{value:c}}],h=this;if(b&&b.type)switch(b.type){case "HM":require("x.hub.m.hm.js").getAllStatesLegacy(function(a){a=e(a,g,f);d&&d(a)});break;case "ENOCEAN":require("x.hub.m.enocean.js").getAllStatesLegacy(function(a){a=e(a,g,f);d&&d(a)});
break;case "ZWAVE2":require("x.hub.m.zway.js").getAllStatesLegacy(function(a){a=e(a,g,f);d&&d(a)});break;case "KNX":require("x.hub.m.knx.js").getAllStatesLegacy(function(a){a=e(a,g,f);d&&d(a)});break;case "ZIGBEE":require("x.hub.m.zigbee.js").getAllStatesLegacy(function(a){a=e(a,g,f);d&&d(a)});break;case "ONOFF":case "FLOAT":case "INT":case "STRING":require("x.hub.sysvarman.js").getSysvarStates(function(a){a=e(a,g,f);d&&d(a)});break;case "TASK":require("x.hub.taskman.js").readAllTasks(function(a){if(a.error)a=
e(a,g,f);else{var b=[];a.data.forEach(function(a){a&&b.push({type:"TASK",id:a.id,active:a.active})});a=e({data:b},g,f)}d&&d(a)});break;default:(a=require("x.hub.js").server)?(this._logger.log("trace","get states from ST directly"),a.doSTMCommand("XC_FNC=getStates",function(a){h._logger.log("trace","get states from ST return:"+JSON.stringify(a));if(a)if(a.XC_SUC){var b={data:null};Array.isArray(a.XC_SUC)?b.data=a.XC_SUC:0<Object.keys(a.XC_SUC).length&&(b.data=[a.XC_SUC]);a=e(b,g,f);d&&d(a)}else d&&
d({error:Error("ST get states return error:"+JSON.stringify(a))});else d&&d({error:Error("ST get states return null")})})):d&&d({error:Error("server not initialized")})}else x.hub.m.gateway.AIOMODULE.doStatus("hub",a,b,{value:c},function(a,b){var c=null;b&&(c={value:b.status,time:(new Date).getTime()});d&&d({error:a,data:c})})}else d&&d({error:Error("gateway json format invalid")})};c.prototype._processPassword=function(a){var b=this._localIpBuffer,c="";localStorage&&(c=localStorage.getItem("x.hub.Server.pwd"));
"localhost"==a.ip||"127.0.0.1"==a.ip||-1!=b.indexOf(a.ip)?(a.__isself=!0,c&&(a.at=c)):a.__neoInfo&&(1==a.__neoInfo.db&&a.__neoInfo.index==this._db1GatewayNeoIndex?(a.__isself=!0,c&&(a.at=c)):2==a.__neoInfo.db&&a.__neoInfo.index==this._db2GatewayNeoIndex&&(a.__isself=!0,c&&(a.at=c)));return a};c.prototype._updateLocalIPBuffer=function(){(new Date).getTime();var a=[],b=require("os").networkInterfaces(),c;for(c in b)for(var d=b[c],l=0;l<d.length;l++)a.push(d[l].address);this._localIpBuffer=a};return c}();
