...
x.hub.m.gateway.GatewayInternalHandler=function(){var b=function(){this._devices={};this._meta={};this._states={}};b.prototype.addStateDevice=function(a,c,e){if(a&&a.type&&a.address){a=JSON.parse(JSON.stringify(a));delete a.gateway;var d=this._getDeviceID(a);this._devices[d]||(this._devices[d]=a);this._meta[d]||(this._meta[d]=[]);this._states[d]||(this._states[d]={});if(e){a=!1;for(var b=0;b<this._meta[d].length;b++){var f=this._meta[d][b];if(f&&f.statusKey==e.statusKey&&f.src==e.src&&f.taskID==e.taskID){a=
!0;break}}a||this._meta[d].push(e)}c&&c()}else c&&c(Error("device json invalid"))};b.prototype.removeStateDevice=function(a,c,e){if(!a||!a.type||!a.address)c&&c(Error("device json invalid"));else if(e){a=JSON.parse(JSON.stringify(a));delete a.gateway;a=this._getDeviceID(a);var d=this._meta[a];if(d){for(var b=-1,f=0;f<d.length;f++){var g=d[f];if(g&&g.src==e.src&&g.taskID==e.taskID&&g.statusKey==e.statusKey){b=f;break}}0<=b&&d.splice(b,1);0==d.length&&(delete this._devices[a],delete this._meta[a],delete this._states[a])}c&&
c()}};b.prototype.onEvent=function(a,c,e){if(a){var d,b=null;if(a.sid)for(d in this._devices){var f=this._devices[d];if(f.sid==a.sid){var g=JSON.parse(JSON.stringify(a));f.type&&!g.type&&(g.type=f.type);f.address&&!g.address&&(g.address=f.address);if(c=this._getSystem(g)){if(a.changed&&a.changed.length){g.state={};a.changed.forEach(function(c){g.state[c]=a.state[c]});var h=c._handleState(f,g)}if(h&&0<Object.keys(h).length){var k=f;this._getDeviceID(k);b=h;break}}}}else{if(!a.type)return;c=this._getSystem(a);
if(!c)return;for(d in this._devices)if(this._devices.hasOwnProperty(d))if(f=this._devices[d],"EVT"==e&&c._handleUdpState){if((h=c._handleUdpState(f,a))&&0<Object.keys(h).length){k=f;this._getDeviceID(k);b=h;break}}else if("STA"==e&&c._handleState){if((h=c._handleState(f,a))&&0<Object.keys(h).length){k=f;this._getDeviceID(k);b=h;break}}else if("BTN"==e&&d==a.type+"@"+a.adr){k=f;b=a.state;break}}b&&this._updateState(k,b,a)}};b.prototype._getDeviceID=function(a){if(a&&a.sid){var c="sid@"+a.sid;a.type&&
a.address&&(c+="@"+a.type+"@"+a.address);return c}return a&&"ZWAVE2"==a.type?a.type+"@"+a.address+"."+a.instance:a.type+"@"+a.address};b.prototype._getSystem=function(a){return x.hub.m.gateway.AIOMODULE.devicemanager.getSystem(a)};b.prototype._updateState=function(a,c,e){var d=this._getDeviceID(a),b=this._meta[d],f=this._states[d]?this._states[d]:{};this._states[d]=c;if(b&&0<b.length)for(var g in c)if(c.hasOwnProperty(g)){d=c[g];var h=f[g];d={key:g,value:d,changed:h!=d,oldValue:h};e&&e.sid&&e.changed&&
e.changed.length&&(d.changed=!0,delete d.oldValue);h=require("x.hub.eventbus.js");require("x.hub.taskman.js");for(var k=0;k<b.length;k++){var l=b[k];!l||l.statusKey!=g&&"*"!=l.statusKey||h.emit("status",{module:"aio",device:a,gateway:null,data:d},l)}}};return b}();
...
if(a.type&&"IR"==a.type&&a.data)d.emit("irevt",{code:a.data});else this._intHandler.onEvent(a,c,b)}};b.prototype._executeCommandIQ=function(a,c,b){if(a.type){var d;if("HM"==a.type.toUpperCase())"ip_siren"==a.data&&(c.value=x.hub.m.gateway.AIOMODULE.Systems.HM_X._buildData(null,a,c)),(d=require("x.hub.moduleman.js").modulemanager.getModule("hm"))?d.executeCommand(a,c,b):b&&b({error:Error("no module for type HM")});else if("ENOCEAN"==a.type.toUpperCase())(d=require("x.hub.moduleman.js").modulemanager.getModule("enocean"))?
d.executeCommand(a,c,b):b&&b({error:Error("no module for type ENOCEAN")});else{var e=x.hub.m.gateway.AIOMODULE.getGatewayFunctionForType(a.type);if(e){if("CODE"==a.type){var f=x.hub.m.gateway.AIOMODULE.Systems.Code._rebuildDevice(JSON.parse(JSON.stringify(a)));a=x.hub.m.gateway.AIOMODULE.Systems.Code._findIRCode(c,a);a=x.hub.m.gateway.AIOMODULE.Systems.Code._buildParam(a,f)}else a="BG"==a.type?x.hub.m.gateway.AIOMODULE.Systems.BG._buildParam(null,a,c):x.hub.m.gateway.AIOMODULE.getGatewayParams(a,
c.value);e="http://localhost/command?XC_FNC="+e;if(a)for(d in a)e+="&"+d+"="+encodeURIComponent(a[d]);try{var g=require("x.hub.js").server;g?g._doSTMCommandRequest(e,!1,function(a){b&&b({error:a})}):b&&b({error:Error("server not initialized")})}catch(h){b&&b({error:h})}}else b&&b({error:Error("unknow device type")})}}else b&&b({error:Error("device type is null")})};b.prototype._executeCommandLocal=function(a,b,e,d){var c=x.hub.m.gateway.AIOMODULE.devicemanager.getSystem(b,a);if(c&&c.buildQuery){var f=
x.hub.m.gateway.AIOMODULE.resolveGateway(a);if(c=c.buildQuery(f?f:a,b,e)){try{var g=require("x.hub.js").server;if(!g){d&&d({error:Error("server not initialized")});return}var h=require("querystring"),k,l;if(c.fnc&&0==c.fnc.indexOf("/api")){(l=k=c.fnc)&&delete c.fnc;if(c.data)c=c.data;else{var m=h.stringify(c);c=h.parse(m);l=k+"?"+m}var p={url:l,query:c}}else"SendGenericCmd"==c.XC_FNC?(k="/cmd",p={method:"POST",url:l,query:c,json:c}):(k="/cmd",m=h.stringify(c),c=h.parse(m),l=k+"?"+m,p={url:l,query:c});
this._logger.log("trace","execute command local directly "+l);var q=this;g.doRequest(k,p,function(a){q._logger.log("trace","execute command local directly return:"+JSON.stringify(a));var b=null;if(a)try{(b=JSON.parse(a))&&b.XC_SUC?d&&d({data:b}):b&&b.XC_ERR?d&&d({error:Error(a)}):d&&d({error:Error("/cmd request return invalid format")})}catch(t){d&&d({error:t})}else d&&d({error:Error("/cmd request return null")})})}catch(r){d&&d({error:r})}return}}this._logger.log("trace","execute command via localhost:"+
...
data:b}},f=x.hub.m.gateway.AIOMODULE.resolveGateway(a);if(f){var g=[{id:"hub",device:b,status:{value:e}}],h=this;b&&"dev"==b.common_type?b.id?require("x.hub.commandman.js").commandHandlerV6.getDeviceStatesWithId(b.id,function(a,b){a?d&&d({error:a}):(a={value:b?b[e]:null,time:(new Date).getTime()},d&&d({error:null,data:a}))}):d&&d({error:Error("device json invalid")}):b&&b.type?"HM"==b.type&&"CA"!=global.HARDWARE_VERSION?require("x.hub.m.hm.js").getAllStatesLegacy(function(a){a=c(a,g,f);d&&d(a)}):
"ENOCEAN"==b.type&&"CA"!=global.HARDWARE_VERSION?require("x.hub.m.enocean.js").getAllStatesLegacy(function(a){a=c(a,g,f);d&&d(a)}):"ZWAVE2"==b.type?require("x.hub.m.zway.js").getAllStatesLegacy(function(a){a=c(a,g,f);d&&d(a)}):"KNX"==b.type?require("x.hub.m.knx.js").getAllStatesLegacy(function(a){a=c(a,g,f);d&&d(a)}):"ZIGBEE"==b.type?require("x.hub.m.zigbee.js").getAllStatesLegacy(function(a){a=c(a,g,f);d&&d(a)}):"ONOFF"==b.type||"FLOAT"==b.type||"INT"==b.type||"STRING"==b.type?require("x.hub.sysvarman.js").getSysvarStates(function(a){a=
c(a,g,f);d&&d(a)}):"TASK"==b.type?require("x.hub.taskman.js").readAllTasks(function(a){if(a.error)a=c(a,g,f);else{var b=[];a.data.forEach(function(a){a&&b.push({type:"TASK",id:a.id,active:a.active})});a=c({data:b},g,f)}d&&d(a)}):(a=require("x.hub.js").server)?(this._logger.log("trace","get states from ST directly"),a.doSTMCommand("XC_FNC=getStates",function(a){h._logger.log("trace","get states from ST return:"+JSON.stringify(a));if(a)if(a.XC_SUC){var b={data:null};Array.isArray(a.XC_SUC)?b.data=a.XC_SUC:
0<Object.keys(a.XC_SUC).length&&(b.data=[a.XC_SUC]);a=c(b,g,f);d&&d(a)}else d&&d({error:Error("ST get states return error:"+JSON.stringify(a))});else d&&d({error:Error("ST get states return null")})})):d&&d({error:Error("server not initialized")}):x.hub.m.gateway.AIOMODULE.doStatus("hub",a,b,{value:e},function(a,b){var c=null;b&&(c={value:b.status,time:(new Date).getTime()});d&&d({error:a,data:c})})}else d&&d({error:Error("gateway json format invalid")})};b.prototype._processPassword=function(a){var b=
this._localIpBuffer,e="";localStorage&&(e=localStorage.getItem("x.hub.Server.pwd"));"localhost"==a.ip||"127.0.0.1"==a.ip||-1!=b.indexOf(a.ip)?(a.ip="127.0.0.1",a.port=require("x.hub.js").getServer()._port,a.__isself=!0,e&&(a.at=e)):a.__neoInfo&&(1==a.__neoInfo.db&&a.__neoInfo.index==this._db1GatewayNeoIndex?(a.ip="127.0.0.1",a.port=require("x.hub.js").getServer()._port,a.__isself=!0,e&&(a.at=e)):2==a.__neoInfo.db&&a.__neoInfo.index==this._db2GatewayNeoIndex&&(a.ip="127.0.0.1",a.port=require("x.hub.js").getServer()._port,
a.__isself=!0,e&&(a.at=e)));return a};b.prototype._updateLocalIPBuffer=function(){(new Date).getTime();var a=[],b=require("os").networkInterfaces(),e;for(e in b)for(var d=b[e],n=0;n<d.length;n++)a.push(d[n].address);this._localIpBuffer=a};return b}();"undefined"!=typeof exports&&"undefined"!=typeof module&&module.exports&&(exports=module.exports=new x.hub.m.gateway.Handler);
