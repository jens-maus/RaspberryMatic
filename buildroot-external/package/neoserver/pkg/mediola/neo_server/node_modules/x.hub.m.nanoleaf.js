...
a.statusCode),this._logger.log("error","[_handleEventStream] "+a.message),c&&c({error:a});else{this._logger.log("debug","[_handleEventStream] Connection for "+b.ip+" has been opened.");var e=this,d="";this._connections[b.ip].stream=a;this._connections[b.ip].status="connected";a.on("data",function(g){d+=String(g);0<=d.indexOf("\n\n")?(g=d.split("\n\n"),d="",e._parseIncomingMessage(b,g)):2E4<d.length&&(e._logger.log("warn","[_handleEventStream] Incoming message string has become too long ("+d.length+
...
status:"connecting",stream:null});b&&this.addCallback(a,b);this._logger.log("debug","[addStateDevice] Opening connection for event stream to device "+a.ip+"...");x.hub.m.nanoleaf.Monitor.addDevice(a);x.hub.m.nanoleaf.Monitor.start();var d=this;b=require("http").request({host:a.ip,port:a.port||16021,path:"/api/v1/"+a.token+"/events?id=1,3,4",method:"GET",headers:{Accept:"text/event-stream","Cache-Control":"no-cache"}},function(g){d._handleEventStream(g,c,a)});"function"===typeof b.setEncoding&&b.setEncoding("utf8");
b.on("error",function(g){d._logger.log("error","[addStateDevice] Request error: "+g.message);d._retryWithTimeout(a)});b.end()}else e=Error("Nanoleaf device has no IP."),this._logger.log("error","[addStateDevice] "+e.message),c&&c({error:e});else e=Error("Nanoleaf device has no auth token."),this._logger.log("error","[addStateDevice] "+e.message),c&&c({error:e})},checkDeviceMatch:function(a,c){return c&&c.device&&a&&a.device?c.device.ip===a.device.ip:!1},removeCallback:function(a,c){a&&c&&c.statusKey&&
