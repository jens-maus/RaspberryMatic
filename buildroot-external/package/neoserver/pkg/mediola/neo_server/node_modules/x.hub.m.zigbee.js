...
x.hub.m.zigbee.Device=function(){var f=function(b,a){this._ieeeAdr=b;this._endpointID=a;this._deviceID=this._profileID=null;this._states={}};f.prototype.getIeeeAdr=function(){return this._ieeeAdr};f.prototype.getEndpointID=function(){return this._endpointID};f.prototype.setBufferedState=function(b,a){this._states[b]=a};f.prototype.getBufferedState=function(b){return this._states[b]};f.prototype.getBufferedStates=function(){return this._states};f.prototype.getConvertedStates=function(b){b={};"undefined"!=
...
null,function(a){a||d.setBufferedState("onoff",e);c&&c(a)})}else if("toggle"==a){var g=this.getBufferedState("onoff");e="on"==g?"off":"on";b._doCommand(this._ieeeAdr,this._endpointID,e,null,function(a){a||d.setBufferedState("onoff",e);c&&c(a)})}else if("white"==a)if("0200"==this._deviceID&&0==this._ieeeAdr.indexOf("000B57")){a=x.hub.m.zigbee.Util.rgbTocie(255,255,255);var h=Math.round(65536*a[0]);65279<h&&(h=65279);a=Math.round(65536*a[1]);65279<a&&(a=65279);b._doCommand(this._ieeeAdr,this._endpointID,
...
x.hub.m.zigbee.Zigbee2=function(){function f(){evt_string="";evt_string+=arguments[0]+":";for(var a=1;a<arguments.length;a++)evt_string+=":",evt_string+=arguments[a];return evt_string}var b=require("nodejs_zb_gateway/zigbee_library/zb_lib.js"),a=require("nodejs_zb_gateway/zb_gateway/zb-gateway.js"),c=function(c){this._logger=require("x.logger.js").getLogger("x.hub.m.zigbee.Zigbee2");this._ip=c;this._zigbeeGateway=new a(this._ip);this._state=0;this._cronJob=null;this._cronPattern="0 0 2 * * *";this._zigbeeGateway.on("zb-gateway:newDev",
function(a,c,b){a&&a.startPollDeviceStates&&a.startPollDeviceStates(-1)}).on("zb-gateway:deviceList",function(a,c,b){console.log("Webserver: Received zb-gateway:deviceList",a,"The unbindedList: ",c,"The BindedList: ",b)}).on("zb-gateway:removeDeviceCard",function(a){console.log("Webserver: Received Remove Device Card")}).on("zb-gateway:network:ready",function(){console.log("Webserver: Received Network Ready Ind")}).on("zb-gateway:network:info",function(a){console.log("Webserver: Received Network Info Ind")}).on("zb-gateway:binding:event",
function(a,c,b){console.log("Webserver: Received zb-gateway:binding:event, unbindedList: ",c,"The BindedList: ",b)}).on("zb-gateway:binding:failed",function(){console.log("Webserver: Received zb-gateway:binding:failed")}).on("zb-gateway:unbinding:failed",function(){console.log("Webserver: Received zb-gateway:unbinding:failed")}).on("zb-gateway:light_device:state",function(a){}).on("zb-gateway:light_device:level",function(a){}).on("zb-gateway:light_device:color",function(a){console.log("Webserver: Got light device color update: ",
a)}).on("zb-gateway:light_device:color_temp",function(a){console.log("Webserver: Got light device color temperature update: ",a)}).on("zb-gateway:light_device:color_xy",function(a){console.log("Webserver: Got light device color xy update: ",a)}).on("zb-gateway:temp_device:temp",function(a){console.log("Webserver: Got temp device state update: ",a)}).on("zb-gateway:doorlock_device:state",function(a){console.log("Webserver: Got doorlock device state update: ",a)}).on("zb-gateway:doorlock_device:set_rsp",
function(a){console.log("Webserver: Got doorlock device set response: ",a)}).on("zb-gateway:thermostat:attribute",function(a){console.log("Webserver: Got thermostat device attribute response: ",a)}).on(f("zb-gateway",b.HA_CLUSTER_ID.LEVEL_CONTROL,b.STATUS.UNSUPPORTED_ATTRIBUTE),function(a){console.log("Webserver: Got level control, on/off transition time, unsupported device attribute response: ",a)})};c.prototype.init=function(){if(0==this._state){this._state=1;this._zigbeeGateway.init();var a=this,
c=require("x.hub.taskman.js").Util._getTimeZone();this._cronJob=new (require("cron").CronJob)(this._cronPattern,function(){a._logger.log("info","do soft system reset");console.log("zigbee do soft system reset:"+(new Date).toString());a._zigbeeGateway.softSystemReset()},null,!0,c)}};c.prototype.terminate=function(){1==this._state&&(this._state=0,this._zigbeeGateway.terminate(),this._cronJob.stop())};c.prototype.getDevices=function(){return this._zigbeeGateway.getDevices()};c.prototype.getDevice=function(a,
c){a=a.substring(0,4)+a.substring(10,16);c=parseInt(c).toString(16);2>c.length&&(c="0"+c);a=(a+c).toLowerCase();return this._zigbeeGateway.getDevice(a)};c.prototype.getDeviceStates=function(a,c){return(a=this.getDevice(a,c))&&a.getStates?a.getStates():null};c.prototype.executeCommand=function(a,c,b){if(a)if(c){var d=a.indexOf(":");if(-1==d)b&&b(Error("address format invalid"));else{var e=a.substr(0,d);a=a.substr(d+1);(e=this.getDevice(e,a))?e.executeCommand(c,function(a){b&&b(a)}):b&&b(Error("no such device"))}}else b&&
b(Error("command invalid"));else b&&b(Error("address is empty"))};return c}();
...
b+" error:"+d.message),a&&a(d)):f?(c._logger.log("debug","remove device "+b+" error:"+f),a&&a(Error(f))):a&&a()})};f.prototype._doCommand=function(b,a,c,d,e){var g=[b,a,c,d];this._logger.log("debug","do command ["+g.join(",")+"]");g=["/srv/Apps/zigbee/SYSTEM/servers/zb_con_action.bin"];g.push(c);g.push(b);g.push(a);"undefined"!=typeof d&&null!=d&&(Array.isArray(d)?g=g.concat(d):g.push(d));var f=this;b=g.join(" ");this._exec(b,function(a,c,b){if(a)f._logger.log("debug","do command error:"+a.message),
e&&e(a);else if(b)f._logger.log("debug","do command error:"+b),e&&e(Error(b));else try{if(c){var d=JSON.parse(c);e&&e(null,d)}else e&&e()}catch(l){f._logger.log("debug","do command error (not json):"+c),e&&e(Error(c))}})};f.prototype._exec=function(b,a){this._logger.log("trace","execute native:"+b);var c=this,d=require("child_process").exec;b=d(b);var e,g="",f="";b.stderr.on("data",function(a){e=String(a);f+=e});b.stdout.on("data",function(a){e=String(a);g+=e});b.on("error",function(b){c._logger.log("warn",
...
x.hub.m.zigbee.Handler=function(){var f=function(){this._logger=require("x.logger.js").getLogger("x.hub.m.zigbee.Handler");this._zigbee2=this._zigbee=null;this._zigbeeStickAvaliable=!1};util.inherits(f,EventEmitter);f.prototype.Zigbee=x.hub.m.zigbee.Zigbee;f.prototype.Zigbee2=x.hub.m.zigbee.Zigbee2;f.prototype.getSystems=function(){return["zigbee"]};f.prototype.init=function(b){var a=this,c=require("x.hub.eventbus.js");c.on("x.hub.peripheralman.ADD_ZIGBEE",function(b){a._logger.log("debug","insert zigbee usb stick");
a._zigbeeStickAvaliable=!0;a._zigbee2.init()});c.on("x.hub.peripheralman.REMOVE_ZIGBEE",function(b){a._logger.log("debug","eject zigbee usb stick");a._zigbeeStickAvaliable=!1;a._zigbee2.terminate()});this._zigbee=new this.Zigbee;this._zigbee.init();global.ZIGBEE_URL&&"A20"!=global.PLATFORM&&"PI2"!=global.PLATFORM?(this._zigbeeStickAvaliable=!0,this._zigbee2=new this.Zigbee2(global.ZIGBEE_URL),this._zigbee2.init()):this._zigbee2=new this.Zigbee2("127.0.0.1");b&&b({})};f.prototype.executeCommand=function(b,
a,c){c&&c()};f.prototype.learnDevice=function(b,a){this._zigbeeStickAvaliable?this._zigbee.learnDevice(b.timeout,function(b,d){a&&a({error:b,data:d})}):a&&a({error:Error("zigbee usb stick not avaliable")})};f.prototype.removeDevice=function(b,a){this._zigbeeStickAvaliable?this._zigbee.removeDevice(b,function(b){a&&a({error:b})}):a&&a({error:Error("zigbee usb stick not avaliable")})};f.prototype.listDevices=function(b,a){if(b=this._zigbee.getDevices()){for(var c=[],d=b.length;d--;){var e=b[d].toJSON();
c.push(e)}a&&a({data:c})}else a&&a({data:[]})};f.prototype.getAllStatesLegacy=function(b){var a=[],c;var d=this._zigbee2.getDevices();for(c=d.length;c--;){var e=d[c];var g=e.getStates();g&&(e={type:"ZIGBEE",adr:e.getGUID().toUpperCase(),state:g},a.push(e))}d=this._zigbee.getDevices();for(c=d.length;c--;)if(e=d[c],!this._zigbee2.getDevice(e.getIeeeAdr(),e.getEndpointID())&&(g=e.getConvertedStates(this)))e={type:"ZIGBEE",adr:e.getIeeeAdr()+":"+e.getEndpointID(),state:g},a.push(e);b&&b({data:a})};f.prototype.executeCommandLegacy=
function(b,a,c){if(this._zigbeeStickAvaliable)if(b)if(a){var d=b.indexOf(":");if(-1==d)c&&c({error:Error("address format invalid")});else{var e=b.substr(0,d);d=b.substr(d+1);this._zigbee2.getDevice(e,d)?this._zigbee2.executeCommand(b,a,function(a){c&&c({error:a})},this):this._zigbee.executeCommand(b,a,function(a){c&&c({error:a})})}}else c&&c({error:Error("command invalid")});else c&&c({error:Error("address invalid")});else c&&c({error:Error("zigbee usb stick not avaliable")})};f.prototype.executeCommandNativeRaw=
function(b,a){if(this._zigbeeStickAvaliable){var c=b.command;(b=b.value)&&(b=JSON.parse(b));var d=["/srv/Apps/zigbee/SYSTEM/servers/zb_con_action.bin"];d.push(c);"undefined"!=typeof b&&null!=b&&(Array.isArray(b)?d=d.concat(b):d.push(b));this._zigbee._exec(d.join(" "),function(b,c,d){if(b)a&&a({error:b});else if(d)a&&a({error:Error(d)});else try{if(c){var e=JSON.parse(c);a&&a({data:e})}else a&&a({})}catch(k){a&&a({error:Error(c)})}})}else a&&a({error:Error("zigbee usb stick not avaliable")})};return f}();
