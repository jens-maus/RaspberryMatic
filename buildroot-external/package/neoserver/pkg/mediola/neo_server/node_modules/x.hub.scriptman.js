...
a))}else b.onError(Error("script id is null"))};e.prototype.stopScript=function(a,b){if(a){for(var c=[],d=0;d<this._scripts.length;d++){var f=this._scripts[d];if(f.id==a){c.push(f);break}}if(0==c.length)b&&b(Error("no such script with id:"+a));else{var e=this,h=0,l=function(){h++;h>=c.length?b&&b():e._service.stopScript(c[h],l)};this._service2.stopScript(c[h],l)}}else b&&b(Error("script id is null"))};e.prototype.deleteAllScripts=function(a){for(var b=0;b<this._scripts.length;b++)this.stopScript(this._scripts[b].id);
