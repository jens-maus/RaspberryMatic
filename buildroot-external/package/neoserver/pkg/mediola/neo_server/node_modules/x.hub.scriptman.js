var util=require("util"),EventEmitter=require("events");require("x.logger.js").setLevel("info","x.hub.scriptman");var x=x||{};x.hub=x.hub||{};x.hub.scriptman=x.hub.scriptman||{};x.hub.scriptman.Error=function(e,a){Error.call(this,e);this.code=a};util.inherits(x.hub.scriptman.Error,Error);
...
x.hub.scriptman.ScriptExecutorService2=function(){var e=function(a,c){this._executor=a;this._request=c};e.prototype.execute=function(){if(this._request&&this._request.method){var a=this,c=require("x.hub.scriptman.js");switch(this._request.method){case "executeCommand":case "getStatus":case "executeMacro":c._doCallback(this._request.params,function(b,c){var d=null;b&&(d={},d.message=b instanceof Error?b.message:b,d.code=b.code?b.code:0);a._finishWithResponse(d,c)});break;default:this._finishWithResponse({code:-32601,
...
g.stdout.setEncoding("utf8");g.stderr.setEncoding("utf8");g.stdout.on("data",function(a){f._logger.log("trace","stdout data:"+a);f._logger.log("debug",a);process.stdout.write(a)});g.stdout.on("error",function(a){f._logger.log("trace","stdout error:"+a.stack);console.error("child process stdout error",a)});g.stderr.on("data",function(a){f._logger.log("trace","stderr data:"+a);f._logger.log("debug",a);process.stderr.write(a)});g.stderr.on("error",function(a){f._logger.log("trace","stderr error:"+a.stack);
console.error("child process stderr error",a)});g.on("message",function(a){f._onChildprocessMessage(a)});g.on("error",function(a){f._logger.log("error","child process error:"+a.stack);console.error("child process error",a);g.__closed||(g.__closed=!0,f._onProcessExit(a))});g.on("exit",function(a,b){f._logger.log("trace","child process exit for script executor");console.log("child process exit",a,b);g.__closed||(g.__closed=!0,f._onProcessExit())});this._childProcess=g;a&&a()};a.prototype._onChildprocessMessage=
...
d&&d()})}else d&&d(Error("no such script with id:"+a))}else d&&d(Error("script id is null"))};e.prototype.deleteScript=function(a,b){if(a){for(var c=-1,d=null,f=0;f<this._scripts.length;f++){var e=this._scripts[f];if(e.id==a){c=f;d=e;break}}var h=this;this.stopScript(a);this._deleteScript(a,function(a){a?b&&b(a):(h._scripts.splice(c,1),h._writeData(function(a){a&&h._scripts.splice(c,0,d);b&&b(a)}))})}else b&&b(Error("script id is null"))};e.prototype.runScript=function(a,b,c){if(a){for(var d=null,
f=0;f<this._scripts.length;f++){var e=this._scripts[f];if(e.id==a){d=e;break}}d?(d=require("path"),a+="",f=this._getScriptDir(),a=d.resolve(f,a),this._service2.execute2(a,b,c)):b&&b(Error("no such script with id:"+a))}else b&&b(Error("script id is null"))};e.prototype.debugScript=function(a,b,c){if(a){for(var d=null,f=0;f<this._scripts.length;f++){var e=this._scripts[f];if(e.id==a){d=e;break}}if(d){var h=this;this.readScriptContent(a,function(a,f){if(a)b.onError(a);else h._service.debug(d,f,b,c)})}else b.onError(Error("no such script with id:"+
a))}else b.onError(Error("script id is null"))};e.prototype.stopScript=function(a,b){if(a){for(var c=[],d=0;d<this._scripts.length;d++){var f=this._scripts[d];if(f.id==a){c.push(f);break}}if(0==c.length)b&&b(Error("no such script with id:"+a));else{var e=this,h=0,l=function(){h++;h>=c.length?b&&b():e._service.stopScript(c[h],l)};this._service2.stopScript(c[h],l)}}else b&&b(Error("script id is null"))};e.prototype._getScriptDir=function(){var a=require("path"),b=require("x.hub.fileman.js").fileManager.getUserRootDataPath();
return a.resolve(b,e.FOLDER_NAME)};e.prototype._getScriptIndexFile=function(){var a=require("path"),b=this._getScriptDir();return a.resolve(b,e.FILE_NAME)};e.prototype._writeData=function(a){var b=this._scripts,c=require("fs"),d=this._getScriptIndexFile();c.writeFile(d,JSON.stringify(b,null,2),function(b){a&&a(b)})};e.prototype._loadData=function(a){var b=require("path"),c=require("fs"),d=this._getScriptDir();b=b.resolve(d,e.FILE_NAME);c.readFile(b,"utf8",function(b,c){if(b)a&&a(b);else if(c)try{a&&
a(null,JSON.parse(c))}catch(h){a&&a(h)}else a&&a(null,[])})};e.prototype._writeScript=function(a,b,c){var d=require("fs-extra"),f=require("path");a+="";var e=this._getScriptDir();a=f.resolve(e,a);d.writeFile(a,b,function(a){c&&c(a)})};e.prototype._readScript=function(a,b){var c=require("fs-extra"),d=require("path");a+="";var f=this._getScriptDir();a=d.resolve(f,a);c.readFile(a,"utf8",function(a,c){b&&b(a,c)})};e.prototype._deleteScript=function(a,b){var c=require("fs-extra"),d=require("path");a+=
"";var f=this._getScriptDir();a=d.resolve(f,a);c.remove(a,function(a){b&&b(a)})};e.prototype._sort=function(){var a=this._scripts.length-1;do{var b=!1;for(var c=0;c<a;++c)this._scripts[c].id>this._scripts[c+1].id&&(b=this._scripts[c],this._scripts[c]=this._scripts[c+1],this._scripts[c+1]=b,b=!0)}while(b)};return e}();
x.hub.scriptman.Handler=function(){var e=function(){this._logger=require("x.logger.js").getLogger("x.hub.scriptman.Handler");this.scriptManager=new x.hub.scriptman.ScriptManager;this.httpServer=null};e.prototype.ScriptManager=x.hub.scriptman.ScriptManager;e.prototype.init=function(a,b,c){this._config(a,b);this.scriptManager.init(c)};e.prototype._config=function(a,b){var c=this;this.httpServer=b;var d=require("url");b=new (require("ws").Server)({server:b,path:"/xhub/script/debug"});b.on("connection",
function(a){var b=d.parse(a.upgradeReq.url,!0);b.query&&b.query.id?(a=new x.hub.scriptman.Debugger(a),c.scriptManager.debugScript(b.query.id,a)):(a.send(JSON.stringify({type:"error",message:"no script id"})),a.close())});b.on("error",function(a){c._logger.log("warn","script web socket server error:"+a.stack)});b=require("body-parser");a.use("/xhub/script/callback",b.json({limit:"50mb"}));a.use("/xhub/script",b.text({limit:"50mb"}));a.post("/xhub/script",function(a,b){var d=a.query.name;d?c.scriptManager.createScript(d,
a.body,function(a,d){b.send(c._toRestfulResponse(a,d))}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no script name")))}.bind(this));a.get("/xhub/script",function(a,b){(a=a.query.id)?c.scriptManager.readScriptContent(a,function(a,d){a?b.send(c._toRestfulResponse(a)):b.send(d)}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no script id")))}.bind(this));a.get("/xhub/scripts",function(a,b){c.scriptManager.readAllScripts(function(a,d){a?b.send(c._toRestfulResponse(a)):b.send(JSON.stringify(d))})}.bind(this));
a.put("/xhub/script",function(a,b){var d=a.query.id;d?c.scriptManager.updateScript(d,a.query.name,a.body,function(a){b.send(c._toRestfulResponse(a,"ok"))}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no script id")))}.bind(this));a.delete("/xhub/script",function(a,b){(a=a.query.id)?c.scriptManager.deleteScript(a,function(a){b.send(c._toRestfulResponse(a,"ok"))}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no script id")))}.bind(this));a.get("/xhub/script/do",function(a,b){(a=
a.query.id)?c.scriptManager.runScript(a,function(a){b.send(c._toRestfulResponse(a,"ok"))}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no script id")))}.bind(this));a.get("/xhub/script/stop",function(a,b){(a=a.query.id)?x.hub.scriptman.ScriptExecutorService.stop(a,function(a){b.send(c._toRestfulResponse(a,"ok"))}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no executor id")))}.bind(this));a.get("/xhub/script/running",function(a,b){a=x.hub.scriptman.ScriptExecutorService.getAllRunning();
b.send(c._toRestfulResponse(null,a))}.bind(this));a.post("/xhub/script/callback",function(a,b){(a=a.body)?c._doCallback(a,function(a,d){b.send(c._toRestfulResponse(a,d))}):b.send(c._toRestfulResponse(new x.hub.scriptman.Error("no callback body")))}.bind(this))};e.prototype._toRestfulResponse=function(a,b){return a?{status:"fail",message:a.message,code:a.code?a.code:0}:{status:"success",data:b}};e.prototype._doCallback=function(a,b){this._logger.log("trace","do callback:"+JSON.stringify(a));var c=
...
a.func?a.groupName&&a.macroName?require("x.hub.macroman.js").execute(a.groupName,a.macroName,function(a){b&&b(a.error,a.data)}):b&&b(Error("execute macro argument invalid")):b&&b()};return e}();"undefined"!=typeof exports&&"undefined"!=typeof module&&module.exports&&(exports=module.exports=new x.hub.scriptman.Handler);
