...
b&&b(a):h._readData(function(a,c){!a&&c&&(h._sysvars=c);b&&b(a)})})};d.prototype.setVar=function(a,b){this._logger.log("trace","setVar:"+(a?JSON.stringify(a):null));if(a&&a.id&&a.type){var c=parseInt(a.id,16);if(0>=c)b&&b(Error("sysvar out of range"));else{c=c.toString(16).toUpperCase();2>c.length&&(c="0"+c);a={id:c,type:a.type,value:a.value};c=this._sysvars[a.id];switch(a.type){case "ONOFF":a.value||(a.value="OFF");a.value=a.value.toUpperCase();if("ON"!=a.value&&"OFF"!=a.value&&"TOGGLE"!=a.value){b&&
b(Error("ONOFF sysvar value invalid"));return}if("TOGGLE"==a.value)if(c){if(c.type!=a.type){b&&b(Error("sysvar is not a ONOFF type"));return}a.value="ON"==c.value?"OFF":"ON"}else a.value="OFF";break;case "FLOAT":case "INT":a.value||(a.value="00000000");var d=parseInt(a.value,16);if(isNaN(d)){b&&b(Error("FLOAT/INT sysvar value invalid"));return}break;case "STRING":a.value||(a.value=""),a.value=decodeURIComponent(a.value+"")}a.time=(new Date).getTime();this._sysvars[a.id]=a;d=require("x.hub.eventbus.js");
c&&c.value==a.value||d.emit("gatewayevent",this.getSysvarStateLegacy(a.id),{address:"Sysvar"},"STA");d.emit("udpevent",this.getSysvarStateLegacy(a.id),"STA");this._postWrite(function(a){b&&b(a)})}}else b&&b(Error("sysvar format invalid"))};d.prototype.delVar=function(a,b){a&&this._sysvars[a]?(delete this._sysvars[a],this._postWrite(function(a){b&&b(a)})):b&&b()};d.prototype.getVar=function(a){return a?this._sysvars[a]:null};d.prototype.getSysvars=function(){return this._sysvars};d.prototype.getSysvarStateLegacy=
function(a){if(!a||!this._sysvars[a])return null;a=this._sysvars[a];return{type:a.type,adr:a.id,state:a.value}};d.prototype.getSysvarStatesLegacy=function(){var a=[],b;for(b in this._sysvars)if(this._sysvars.hasOwnProperty(b)){var c=this.getSysvarStateLegacy(b);c&&a.push(c)}return a};d.prototype._readData=function(a){require("fs").readFile(this._sysvarFile,"utf8",function(b,c){if(b)a&&a(b);else if(c)try{var d=JSON.parse(c);a&&a(null,d?d:{})}catch(h){a&&a(h)}else a&&a(null,{})})};d.prototype._postWrite=
function(a){this._writeQueue++;this._writing||this._doWrite();a&&a()};d.prototype._doWrite=function(){if(0<this._writeQueue){var a=this;this._writeQueue=0;this._writing=!0;this._writeData(function(b){b&&a._logger.log("warn","write file failed:"+b.message);a._writing=!1;a._doWrite()})}};d.prototype._writeData=function(a){require("fs").writeFile(this._sysvarFile,JSON.stringify(this._sysvars,null,2),function(b){a&&a(b)})};return d}();
