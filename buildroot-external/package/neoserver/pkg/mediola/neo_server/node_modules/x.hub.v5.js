...
x.hub.v5.Handler.prototype.getNeoServerLogs=function(b){var a=require("path"),c=require("x.hub.fileman.js").fileManager,d=c.getUserRootDataPath(),e=a.dirname(c.getLogFile()),f=c.getTmpDir(),g=require("fs-extra"),c=require("archiver"),h="logs_"+(new Date).toISOString().substr(0,16).replace(/T/g,"").replace(/-/g,"").replace(/:/g,"")+".zip",k=f+a.sep+h,a=g.createWriteStream(k),c=c("zip");a.on("close",function(){b.setHeader("Content-disposition","attachment; filename="+h);b.sendFile(k,null,function(a){g.removeSync(k);
a&&b.status(500).end()})});a.on("error",function(a){b.status(500).end()});c.on("error",function(a){b.status(500).end()});c.pipe(a);c.directory(d);c.directory(e);c.finalize()};x.hub.v5.Handler.prototype.clearLogs=function(b){require("fs-extra").emptyDir("/var/log",function(a){b&&b()})};
