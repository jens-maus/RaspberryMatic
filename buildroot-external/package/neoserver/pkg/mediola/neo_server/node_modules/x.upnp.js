...
d.embeddedDevices.push(a)});return d};b.prototype._parseDescXml=function(a){if(!a)return null;var c=null,d=a.find("friendlyName");0<d.length&&(this.name=$(d[0]).text(),d=String(this.name).toLowerCase(),0==d.indexOf("philips hue")?this.type="hue":0==d.indexOf("wemo switch")||0==d.indexOf("wemo insight")?(this.type="wemo",0==d.indexOf("wemo insight")&&(this.subtype="insight")):0<d.indexOf("sonos")&&(this.type="sonos"));(d=a.find("deviceType"))&&0<d.length&&(this.deviceType=$(d[0]).text());(d=a.find("manufacturer"))&&
0<d.length&&(this.manufacturer=$(d[0]).text());(d=a.find("modelName"))&&0<d.length&&(this.modelName=$(d[0]).text());(d=a.find("modelDescription"))&&0<d.length&&(this.modelDescription=$(d[0]).text());(d=a.find("serialNumber"))&&0<d.length&&(this.serialNumber=$(d[0]).text());d=a.find("UDN");0<d.length&&(c=$(d[0]).text());d=a.find("X_DLNACAP");0<d.length&&(this.X_DLNACAP=$(d[0]).text());if(!this.name||!this.deviceType||!c)return null;6<c.length&&"uuid:"==c.substr(0,5)&&!this.uuid&&(this.uuid=c.substr(5));
c=x.upnp.Util.parseURN(this.deviceType);if(!c)return null;c.device&&"MediaServer"==c.device.type?a=this.parseServer(a):c.device&&"MediaRenderer"==c.device.type?a=this.parseRenderer(a):(a=new x.upnp.Device,a.init(this.ip,this.port,this.uuid,{name:this.name,type:this.type,subtype:this.subtype,modelName:this.modelName,manufacturer:this.manufacturer,modelDescription:this.modelDescription,deviceType:this.deviceType,dlnaCap:this.X_DLNACAP,serialNumber:this.serialNumber}));return a};b.prototype.parseServer=
function(a){var c=null,d=null;a.find("serviceList service").each(function(){var a=$(this),b=a.find("serviceType");a=a.find("controlURL");if(0<b.length&&0<a.length)if(b=$(b[0]).text(),(a=$(a[0]).text())&&"/"==a.charAt(0)||(a="/"+a),-1!=b.indexOf("ContentDirectory"))c={type:b,url:a};else if("urn:schemas-upnp-org:service:ConnectionManager:1"==b||"urn:schemas-upnp-org:service:ConnectionManager:2"==b)d={type:b,url:a}});return c&&d?(a=new x.upnp.MediaServer,a.init(this.ip,this.port,this.uuid,{name:this.name,
type:this.type,subtype:this.subtype,modelName:this.modelName,manufacturer:this.manufacturer,modelDescription:this.modelDescription,deviceType:this.deviceType,dlnaCap:this.X_DLNACAP,serialNumber:this.serialNumber}),a.contentDirectory=c,a.connectionManager=d,a):null};b.prototype.parseRenderer=function(a){var c=null,d=null,b=null,f=null,g=null;a.find("serviceList service").each(function(){var a=$(this),e=a.find("serviceType");a=a.find("controlURL");0<e.length&&0<a.length&&(e=$(e[0]).text(),(a=$(a[0]).text())&&
"/"==a.charAt(0)||(a="/"+a),-1!=e.indexOf("GroupRenderingControl")?f={type:e,url:a}:-1!=e.indexOf("RenderingControl")?c={type:e,url:a}:-1!=e.indexOf("ConnectionManager")?d={type:e,url:a}:-1!=e.indexOf("AVTransport")?b={type:e,url:a}:-1!=e.indexOf("Queue")&&(g={type:e,url:a}))});return c&&b?(a=new x.upnp.MediaRenderer,a.init(this.ip,this.port,this.uuid,{name:this.name,type:this.type,subtype:this.subtype,modelName:this.modelName,manufacturer:this.manufacturer,modelDescription:this.modelDescription,
...
x.upnp._DeviceCache.removeDevicesWithTarget(a);this._udpListener.sendMessage(this.M_SEARCH+"ST: "+a+"\r\n\r\n","239.255.255.250",1900)};b.prototype._onReceiveSSDP=function(a,c){if("RESULT"==c.type){var d=c.getUUID(),e=c.st,b=x.upnp._DeviceCache.getDeviceCache(d);if(b&&b.device)b.targets&&-1!=b.targets.indexOf(e)||(x.upnp._DeviceCache.addTarget(d,e),this._findDevice(e,b.device));else if(!b||!b.loaded){var g=this;x.upnp._DeviceCache.setDescLoaded(d,!0);(new x.upnp.DescLoader(a,d)).load(c.location,function(a,
c){a||!c?x.upnp._DeviceCache.setDescLoaded(d,!1):(x.upnp._DeviceCache.putDevice(d,c,e),g._findDevice(e,c))})}}};return b}();
...
"<");a=a.replace(/&gt;/g,">");a=a.replace(/&amp;/g,"&");a=a.replace(/&quot;/g,'"');return a=a.replace(/&apos;/g,"'")};b.prototype._parseSOAPResponse=function(a,c){if(!a)return null;try{var d=$($.parseXML(a))}catch(f){return null}if(!d)return null;try{var b=x.upnp.Util.findTag(d,c,"u");return b&&0<b.length?$(b[0]):null}catch(f){return null}};b.prototype.getURL=function(a){var c="http://"+this.ip;this.port&&80!=this.port&&(c+=":"+this.port);return c+a};b.prototype._getDuration=function(a){var c=a.split(":");
3==c.length&&(a=3600*parseInt(c[0])+60*parseInt(c[1])+parseInt(c[2]));return a};b.prototype._doSOAPRequest=function(a,c,d,b,f){var e="<u:"+d+' xmlns:u="'+c+'">',h;for(h in b)if(b.hasOwnProperty(h)){var k=b[h]+"";e+="<"+h+">"+(k?this._maskMetaData(k):"")+"</"+h+">"}var l=this;this._doRequest(a,'"'+c+"#"+d+'"',e+("</u:"+d+">"),function(a,c,b){a?f&&f(a):200!=b?f&&f(Error("http response code:"+b)):(a=l._parseSOAPResponse(c,d+"Response"))?f&&f(null,a):f&&f(Error(d+" reponse format invalid"))})};b.prototype._doRequest=
function(a,c,d,b){(new x.upnp.SOAPClient).doRequest(a,c,d,function(a,c,d){b&&b(null,c,d)})};return b}();
x.upnp.MediaServer=function(){var b=function(){this.connectionManager=this.contentDirectory=null};b.prototype=new x.upnp.Device;b.prototype.constructor=b;b.prototype.search=function(a,c,b,e){var d=this;c||(c="dc:title,upnp:artist,upnp:album,upnp:albumArtURI,res,res@protocolInfo,res@duration,res@resolution,res@size");var g='<u:Search xmlns:u="urn:schemas-upnp-org:service:ContentDirectory:1"><ContainerID>0</ContainerID><SearchCriteria>'+(a+"</SearchCriteria><Filter>")+(c+"</Filter><StartingIndex>")+
(b+"</StartingIndex>");g+="<RequestedCount>"+this.chunkSize+"</RequestedCount>";g+="<SortCriteria></SortCriteria>";g+="</u:Search>";(new XCUPNPSOAPRequest).doRequest(this.getURL(this.contentDirectory),'"urn:schemas-upnp-org:service:ContentDirectory:1#Search"',g,function(f){if(f=d._parseSOAPResponse(f,!0)){var g=0,h=[];try{var m=$($.parseXML(f));h=d._getItems(m);g=$(m.children()[0]).children().length}catch(n){}g==d.chunkSize?(setTimeout(function(){d.search(a,c,b+d.chunkSize,e)},0),e(h,!1)):e(h,!0)}})};
