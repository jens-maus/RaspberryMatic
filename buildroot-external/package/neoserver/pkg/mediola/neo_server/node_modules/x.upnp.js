...
x.upnp.DescLoader=function(){var e=function(a,b){this.ip=a;this.port=80;this.uuid=b;this.subtype=this.type=this.name=null;this._logger="undefined"!=typeof require&&require?require("x.logger.js").getLogger("x.upnp.DescLoader"):{log:{}}};e.prototype.load=function(a,b){var c=require("url").parse(a);c.port&&(this.port=c.port);var d=this;this._loadDeviceDesc2(a,function(f,g){f?b&&b(f):(f=d._parseDesc(g))?b&&b(null,f):b&&b(Error("device desc parse error"))})};e.prototype._loadDeviceDesc=function(a,b){var c=
this;this._logger.log("trace","load device description from URL: "+a);var d=b;$.ajax({url:a,type:"get",timeout:5E3,dataType:"text",cache:!0,success:function(f){c._logger.log("trace","http request success for: "+a);d&&(d(null,f),d=null)},error:function(f,g){f="http request error:"+(f?f.status:"0")+"-"+g;c._logger.log("trace",f);d&&(d(Error(f)),d=null)}})};e.prototype._loadDeviceDesc2=function(a,b){this._logger.log("trace","load device description from URL: "+a);var c=require("url").parse(a);c={host:c.hostname,
port:c.port,path:c.path,method:"GET"};var d=this,f="",g=require("http"),h=b;try{var k=g.request(c,function(l){l.on("data",function(m){f+=m});l.on("end",function(){if(200==l.statusCode)d._logger.log("trace","http request success for: "+a),h&&h(null,f);else{var m="http request error: "+l.statusCode+"-"+f;d._logger.log("trace",m);h&&h(Error(m))}h=null})});k.setTimeout&&k.setTimeout(2E3,function(){k.abort()});if(k.on&&"function"==typeof k.on)k.on("error",function(l){l||(l=Error("http request error"));
h&&h(l);h=null});k.end()}catch(l){h&&h(l),h=null}};e.prototype._parseDesc=function(a){-1!=a.indexOf('xmlns:ms=" urn:microsoft-com:wmc-1-0"')&&(a=a.replace('xmlns:ms=" urn:microsoft-com:wmc-1-0"','xmlns:ms="urn:microsoft-com:wmc-1-0"'));var b=null;try{b=$($.parseXML(a))}catch(f){}if(!b)return null;a=b.find("device");if(!a||0==a.length)return null;a=$(a[0]);var c=this._parseDescXml(a);if(!c)return null;c.embeddedDevices=[];var d=this;b.find("deviceList device").each(function(){var f=$(this);(f=d._parseDescXml(f))&&
c.embeddedDevices.push(f)});return c};e.prototype._parseDescXml=function(a){if(!a)return null;var b=null,c=a.find("friendlyName");0<c.length&&(this.name=$(c[0]).text(),c=String(this.name).toLowerCase(),c.startsWith("wemo switch")||c.startsWith("wemo insight")?(this.type="wemo",c.startsWith("wemo insight")&&(this.subtype="insight")):c.includes("sonos")&&(this.type="sonos"));(c=a.find("deviceType"))&&0<c.length&&(this.deviceType=$(c[0]).text());(c=a.find("manufacturer"))&&0<c.length&&(this.manufacturer=
$(c[0]).text());(c=a.find("modelName"))&&0<c.length&&(this.modelName=$(c[0]).text(),c=this.modelName.toLowerCase(),c.startsWith("philips hue bridge")?this.type="hue":c.startsWith("sonos")&&(this.type="sonos"));(c=a.find("modelDescription"))&&0<c.length&&(this.modelDescription=$(c[0]).text());(c=a.find("serialNumber"))&&0<c.length&&(this.serialNumber=$(c[0]).text());c=a.find("UDN");0<c.length&&(b=$(c[0]).text());c=a.find("X_DLNACAP");0<c.length&&(this.X_DLNACAP=$(c[0]).text());if(!this.name||!this.deviceType||
!b)return null;6<b.length&&"uuid:"===b.substr(0,5)&&!this.uuid&&(this.uuid=b.substr(5));b=x.upnp.Util.parseURN(this.deviceType);if(!b)return null;b.device&&"MediaServer"===b.device.type?a=this.parseServer(a):b.device&&"MediaRenderer"===b.device.type?a=this.parseRenderer(a):(a=new x.upnp.Device,a.init(this.ip,this.port,this.uuid,{name:this.name,type:this.type,subtype:this.subtype,modelName:this.modelName,manufacturer:this.manufacturer,modelDescription:this.modelDescription,deviceType:this.deviceType,
dlnaCap:this.X_DLNACAP,serialNumber:this.serialNumber}));return a};e.prototype.parseServer=function(a){var b=null,c=null;a.find("serviceList service").each(function(){var d=$(this),f=d.find("serviceType");d=d.find("controlURL");if(0<f.length&&0<d.length)if(f=$(f[0]).text(),(d=$(d[0]).text())&&"/"==d.charAt(0)||(d="/"+d),-1!=f.indexOf("ContentDirectory"))b={type:f,url:d};else if("urn:schemas-upnp-org:service:ConnectionManager:1"==f||"urn:schemas-upnp-org:service:ConnectionManager:2"==f)c={type:f,url:d}});
return b&&c?(a=new x.upnp.MediaServer,a.init(this.ip,this.port,this.uuid,{name:this.name,type:this.type,subtype:this.subtype,modelName:this.modelName,manufacturer:this.manufacturer,modelDescription:this.modelDescription,deviceType:this.deviceType,dlnaCap:this.X_DLNACAP,serialNumber:this.serialNumber}),a.contentDirectory=b,a.connectionManager=c,a):null};e.prototype.parseRenderer=function(a){var b=null,c=null,d=null,f=null,g=null;a.find("serviceList service").each(function(){var h=$(this),k=h.find("serviceType");
h=h.find("controlURL");0<k.length&&0<h.length&&(k=$(k[0]).text(),(h=$(h[0]).text())&&"/"==h.charAt(0)||(h="/"+h),-1!=k.indexOf("GroupRenderingControl")?f={type:k,url:h}:-1!=k.indexOf("RenderingControl")?b={type:k,url:h}:-1!=k.indexOf("ConnectionManager")?c={type:k,url:h}:-1!=k.indexOf("AVTransport")?d={type:k,url:h}:-1!=k.indexOf("Queue")&&(g={type:k,url:h}))});return b&&d?(a=new x.upnp.MediaRenderer,a.init(this.ip,this.port,this.uuid,{name:this.name,type:this.type,subtype:this.subtype,modelName:this.modelName,
manufacturer:this.manufacturer,modelDescription:this.modelDescription,deviceType:this.deviceType,dlnaCap:this.X_DLNACAP,serialNumber:this.serialNumber}),a.renderingControl=b,a.connectionManager=c,a.avTransport=d,a.groupRenderingControl=f,a.queue=g,a):null};return e}();
