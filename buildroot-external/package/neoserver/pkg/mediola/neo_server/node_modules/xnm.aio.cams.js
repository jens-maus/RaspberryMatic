...
xnm.aio.cams._moveFromTmp=function(b,a,c){var d=require("fs"),e=d.createReadStream(b);a=d.createWriteStream(a);a.on("error",function(a){XC.debugf(a.message,"error");c&&c(a)});e.on("error",function(a){XC.debugf(a.message,"error");c&&c(a)});e.on("end",function(){try{d.unlinkSync(b)}catch(f){}c&&c()});e.pipe(a)};
...
xnm.aio.cams.JPEGCam.prototype._loadImage=function(b){var a=require("fs"),c=this;this._loadImage2Tmp(function(d){if(d&&a.existsSync(d)){var e=a.dataPath()+"/cams/",f="image.jpg";c._options&&c._options.imageName&&(f=c._options.imageName);a.mkdirRecursiveSync(e);if(a.existsSync(e+f)){var g=e+f+"_"+(new Date).getTime();xnm.aio.cams.rename(e+f,g,function(){a.existsSync(g)&&a.unlinkSync(g);xnm.aio.cams.rename(d,e+f,function(){b&&b("file://"+e+f+"?"+(new Date).getTime())})})}else xnm.aio.cams.rename(d,
e+f,function(){b&&b("file://"+e+f+"?"+(new Date).getTime())})}else b&&b()})};xnm.aio.cams.JPEGCam.prototype._loadImage2Tmp=function(b){if(this._url)if(this._prevDone){var a=require("http");require("fs");var c=this;this._prevDone=!1;a.getFile(this._url,this._options,function(a){a?b&&b(a):(XC.debugf("failed to load "+c._url),b&&b());c._prevDone=!0},null,{encodeURL:!1})}else b&&b();else XC.debugf("[xnm.aio.cams.JPEGCam._loadImage] No URL set!")};
...
Connection:"close"}};f?h.headers.Authorization=f:b.user&&(b.password||(b.password=""),h.user=b.user,h.password=b.password,h.headers.Authorization="Basic "+(new Buffer(b.user+":"+b.password)).toString("base64"));d&&(h.method="POST",h.headers["Content-Length"]=d.length);var n=this;c=k.request(h,function(c){c.setEncoding("utf-8");var k="";c.on("data",function(a){k+=a});c.on("end",function(){XC.debugf("response:"+c.statusCode);if(401==c.statusCode&&!f&&c.headers&&c.headers["www-authenticate"]&&"Digest "==
c.headers["www-authenticate"].substr(0,7)){f=null;if(f=require("x.crypt.js").DigestAuth(c.headers["www-authenticate"],h.path,b.user,b.password,h.method)){f+=', opaque=""';XC.debugf("do digest auth:"+f);n._doRequest(a,b,d,e,f);return}var g=Error("http response:"+c.statusCode)}else 200!=c.statusCode&&(g=Error("http response:"+c.statusCode));e&&e(g,k)})});if(c.on)c.on("error",function(a){e&&e(a)});d&&c.write(d);c.end()};b.prototype.snapshot=function(a){this.getVideoInfo(function(b,d){b?a&&a(b):(b={user:d.username,
...
xnm.aio.cams.DoorBird.prototype.getStatesCreator=function(b,a,c){var d=require("xnm.aio.cams.js").getDoorBirdHandler(),e=d.getCamInfo(this._options);if(e)this._supportUdpMonitor(e)?this._getStatesFromUdpEvents(a,c):this._getStates(a,c);else{var f=this;this.getInfo(function(e,k){e?c&&c(e):k?(d.setCamInfo(f._options,k),f.getStatesCreator(b,a,c)):c&&c(Error("get info response invalid"))})}};
xnm.aio.cams.DoorBird.prototype.getInfo=function(b){this._doRequest2("/bha-api/info.cgi",function(a,c){if(a)b&&b(a);else try{var d=JSON.parse(c);d&&d.BHA&&d.BHA.VERSION&&1==d.BHA.RETURNCODE?b&&b(null,d.BHA):b&&b(Error("info response invaild"))}catch(e){b&&b(e)}})};
xnm.aio.cams.DoorBird.prototype._getStates=function(b,a){a||(a=function(){});var c=this._statusCBs.length;-1==this._statusCBs.indexOf(a)&&this._statusCBs.push(a);if(!(0<c)){var d=this;this._doRequest2("/bha-api/monitor.cgi?check=doorbell,motionsensor",function(a,c,g){var e={};!a&&g&&(console.log(g.currenttime,g.lastringtime,c),e=d._parseStatus(g.currenttime,g.lastringtime,c),console.log("status",e));a=[];for(c=0;c<b.length;c++)b[c]&&b[c].id&&b[c].status&&b[c].status.value&&(g=b[c].status.value,null!=
e[g]&&"undefined"!=typeof e[g]&&(g=e[g],a.push({id:b[c].id,status:g})));e=d._statusCBs;d._statusCBs=[];for(c=0;c<e.length;c++)e[c]&&e[c](null,a)})}};
xnm.aio.cams.DoorBird.prototype._getStatesFromUdpEvents=function(b,a){var c=require("xnm.aio.cams.js").getDoorBirdHandler().getDoorBirdEventsOrAddToMonitor(this._options),d=[],e={ring:!1,motion:!1,ring1:!1,ring2:!1,ring3:!1};if(c){var f=(new Date).getTime();c["103"]&&(e.ringT=1E3*c["103"][0],e.ringTStr=(new Date(1E3*c["103"][0])).toLocaleString(),1E4>f-c["103"][1]&&(e.ring=!0,e.ring3=!0));c["102"]&&(e.ringT=1E3*c["102"][0],e.ringTStr=(new Date(1E3*c["102"][0])).toLocaleString(),1E4>f-c["102"][1]&&
(e.ring=!0,e.ring2=!0));c["101"]&&(e.ringT=1E3*c["101"][0],e.ringTStr=(new Date(1E3*c["101"][0])).toLocaleString(),1E4>f-c["101"][1]&&(e.ring=!0,e.ring1=!0));c.doorbell&&(e.ringT=1E3*c.doorbell[0],e.ringTStr=(new Date(1E3*c.doorbell[0])).toLocaleString(),1E4>f-c.doorbell[1]&&(e.ring=!0));c.motionsensor?(e.motionT=1E3*c.motionsensor[0],e.motionTStr=(new Date(1E3*c.motionsensor[0])).toLocaleString(),1E4>f-c.motionsensor[1]&&(e.motion=!0)):c.motion&&(e.motionT=1E3*c.motion[0],e.motionTStr=(new Date(1E3*
c.motion[0])).toLocaleString(),1E4>f-c.motion[1]&&(e.motion=!0))}for(c=0;c<b.length;c++)b[c]&&b[c].id&&b[c].status&&b[c].status.value&&(f=b[c].status.value,null!=e[f]&&"undefined"!=typeof e[f]&&(f=e[f],d.push({id:b[c].id,status:f})));a&&a(null,d)};
xnm.aio.cams.DoorBird.prototype._parseStatus=function(b,a,c){var d={ring:!1,motion:!1},e=(new Date).getTime();b&&(e=parseInt(b));if(c&&(b=c.split("\r\n"),2<=b.length))for(c=0;c<b.length;c++)b[c]&&"doormotion="==b[c].substr(0,11)&&"1"==b[c].substr(11)&&(d.motion=!0);if(a)for(b=a.split(","),c=0;c<b.length;c++)if(b[c]){var f=b[c].indexOf(":");a=b[c].substr(0,f);f=b[c].substr(f+1);var g=parseInt(f);f&&("doorbell"==a?(d.ringT=f,d.ringTStr=(new Date(g)).toLocaleString(),1E4>=e-g&&(d.ring=!0)):"motionsensor"==
a&&(d.motionT=f,d.motionTStr=(new Date(g)).toLocaleString(),1E4>=e-g&&(d.motion=!0)))}return d};
xnm.aio.cams.DoorBird.prototype._doRequest2=function(b,a){var c=this._options.ip,d=this._options.port;d||(d=80);this._logger.log("trace","send to url:"+("http://"+c+":"+d+b));b={host:c,port:d,path:b,method:"GET",headers:{Connection:"close"}};this._options.username&&(this._options.password||(this._options.password=""),b.headers.Authorization="Basic "+(new Buffer(this._options.username+":"+this._options.password)).toString("base64"));var e=require("http").request(b,function(b){b.setEncoding("utf-8");
var c="";b.on("data",function(a){c+=a});b.on("end",function(){var e;200!=b.statusCode&&(e=Error("http response:"+b.statusCode));a&&a(e,c,b.headers)})});e.setTimeout&&e.setTimeout(2E3,function(){e.abort()});if(e.on)e.on("error",function(b){a&&a(b)});e.end()};
...
6)).toString("utf8");var n=Buffer.from(a.slice(6,14)).toString("utf8").trim();a=Buffer.from(a.slice(14,18)).readInt32BE();a={intercom_id:h,event:n,timestamp:a};e._logger.log("trace","(libsodium-wrappers) decrypt result:"+JSON.stringify(a));c&&c(null,a)})};var d=function(a){this._logger=require("x.logger.js").getLogger("xnm.aio.cams.DoorBird.Handler.UdpMonitor");this._sockets=[];this._port=35344;this._running=!1;this._listener=a};d.prototype.start=function(){this._running||(this._running=!0,this._addDiscoverSocket(this._port,
null))};d.prototype.isRunning=function(){return this._running};d.prototype._addDiscoverSocket=function(a,b){var c=require("dgram"),e=this;this._logger.log("trace","bind udp monitor on port "+a+(b?" ip "+b.address:""));if(!b){try{var d=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(p){d=c.createSocket("udp4")}d.on("message",function(a,b){e._receivedData(b.address,a)});d.on("error",function(a){e._logger.log("trace","bind udp monitor error "+a.message)});d.bind(a);this._sockets.push(d)}else if("IPv4"==
b.family&&0==b.internal){try{d=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(p){d=c.createSocket("udp4")}d.on("listening",function(){d.setBroadcast(!0)});d.on("message",function(a,b){e._receivedData(b.address,a)});d.on("error",function(a){e._logger.log("trace","bind udp monitor error "+a.message)});d.bind(a,b.address);this._sockets.push(d)}};d.prototype._receivedData=function(a,b){b=b.toString("hex");this._logger.log("trace","receive data from "+a+" data "+b);if(this._listener&&this._listener.onData)this._listener.onData(a,
b)};var e=function(){this._logger=require("x.logger.js").getLogger("xnm.aio.cams.DoorBird.Handler");this._udpMonitor=new d(this);this._doorbirds=[];this._doorbirdInfo={}};e.prototype.startUdpMonitor=function(){this._udpMonitor.start()};e.prototype.addDoorBird=function(a,b){a&&this._doorbirds.push({doorBird:a,events:{},listener:b})};e.prototype.removeDoorBird=function(a,b){for(var c=this._doorbirds.length-1;0<=c;c--){var d=this._doorbirds[c];d&&d.doorBird==a&&d.listener==b&&this._doorbirds.splice(c,
1)}};e.prototype.getCamInfo=function(a){return a&&a.ip?this._doorbirdInfo[a.ip]:null};e.prototype.setCamInfo=function(a,b){a&&a.ip&&(this._doorbirdInfo[a.ip]=b)};e.prototype.getDoorBirdEventsOrAddToMonitor=function(a){if(a){this._udpMonitor.isRunning()||this.startUdpMonitor();for(var b=-1,c,d=0;d<this._doorbirds.length;d++)if((c=this._doorbirds[d])&&c.doorBird&&c.doorBird.ip==a.ip){b=d;break}if(-1==b)return this.addDoorBird(a),null;c=this._doorbirds[d];return c.events}};e.prototype.onData=function(a,
...
this._doorbirds[c].doorBird,e)}};e.prototype._decryptData=function(a,b,c){if(b&&b.password){var d=this._getDecrypter();d?d.decrypt(a,b.password,function(a,b){c&&c(a,b)}):c&&c(Error("can not find decrypter"))}else c&&c(Error("no password"))};e.prototype._getDecrypter=function(){var d=null;window&&!window.XCHost&&(d="node-webkit");window&&window.XCHost&&window.XCHost.getType&&(d=window.XCHost.getType());return"undefined"!==typeof nw?new c:"node-webkit"==d||"undefined"!=typeof process&&null!=process&&
"node"==process.title?new a:new b};return e}();xnm.aio.cams.Foscam=function(b,a){xnm.aio.cams.Cam.call(this,b,a);this._logger=require("x.logger.js").getLogger("xnm.aio.cams.Foscam")};xnm.aio.cams.Foscam.prototype=new xnm.aio.cams.Cam;xnm.aio.cams.Foscam.prototype.constructor=xnm.aio.cams.Foscam;xnm.aio.cams.Foscam.prototype.executeCommandCreator=function(b,a,c){a?"gotoPreset"==a.value?this._gotoPreset(b,a,c):this.executeCommand(a.value,c):c&&c(Error("command invalid"))};
...
a.manufacturer);var g=e(a);!g&&b&&(g=f(a,b));return g},getCommandStatusMap:function(a,b){a=this.getCamInfo(a,b);if(!a||!a.pantilt||"false"==a.pantilt)return null;b={command:[]};if(a.keys)for(var c=0;c<a.keys.length;c++){var e=a.keys[c];b.command.push({type:"enum",name:e.id,ref:{value:e.id}})}a.command&&(b.command=b.command.concat(a.command));a.status&&(b.status=a.status);return b},getNeoServerCommandStatusMap:function(a,b){b={command:[]};if(a&&"system"==a.type&&("doorbird"==a.manufacturer||"doorbird"==
