...
xnm.aio.cloud.RemotesServer.prototype._downloadFile=function(b,c,a,d){var e=require("http"),f=require("fs"),g={fileData:a,url:{username:this.user,password:this.password}},h=this,l=function(b,e){b&&f.existsSync(b)?(e=c.substring(0,c.lastIndexOf("/")),f.mkdirRecursiveSync(e),f.createReadStream&&f.createWriteStream?h._moveFromTmp(b,c,function(b){b?d(b):(f.writeFile(c+".md5",a.md5),d())}):(f.renameSync(b,c),d())):d(e?e:Error("file from mediola server error"))},m=window.XCHost&&window.XCHost.getType&&
("iOS"==window.XCHost.getType()||"Android"==window.XCHost.getType());if(this._aws3Client&&a&&a.file.match(/^resources.*/))b={Bucket:"mediola-creator",Key:"resources/"+a.ref},e=null,m||(e=require("os").tmpdir()+require("path").sep+"xcdl"+require("path").sep+Math.random().toString(36).substring(2)),this._aws3Client.getObject2File(b,e,function(c,b){c?(XC.debugf("[xnm.aio.cloud.RemoteServer._downloadFile] failed to download file from S3: "+a.file),XC.debugf("[xnm.aio.cloud.RemoteServer._downloadFile] "+
c.message),l(null,Error("S3:"+c.message))):l(b)});else{var n=e.getFileAIOCloud;m&&(g={},b+="?username="+encodeURIComponent(this.user)+"&password="+encodeURIComponent(this.password),n=e.getFileUriEncoded);n(b,g,l)}};
xnm.aio.cloud.RemotesServer.prototype._checkFile=function(b,c,a,d,e){for(var f=this._remotesPath+"_new/"+b+"/"+d.file,g=this.secure?"https://":"http://",h=d.ref.split("/"),l="",m=0;m<h.length;m++)l+=encodeURIComponent(h[m]),m<h.length-1&&(l+="/");g+=this.ip+":"+this.port+"/creatorneo/remote/downloadFile/"+l;var n=!1,k=require("fs"),p=this._remotesPath+"/"+b+"/"+d.file,q=this;b=d.file.indexOf(".DS_Store");-1<b&&b+9==d.file.length?e():k.readFile(p+".md5","utf8",function(b,h){!b&&h&&h==d.md5&&(k.copyFileSync(p,
f),k.copyFileSync(p+".md5",f+".md5"),n=!0);n?e():q._downloadFile(g,f,d,function(b){var g=!0;"device_db"==d.file&&c&&(g=!1,k.readFile(f,"utf8",function(a,b){!a&&b?(a=require("x.crypt.js").AES.decodeString(b,c),k.writeFile(f,a,function(a){e()})):e("crypt")}));"device_db"==d.file&&a&&(g=!1,k.readFile(f,"utf8",function(a,b){if(!a&&b){var c=!1;require("xnm.aio.hm.js").discoverCCUs(function(a){if(!c){c=!0;var d=JSON.parse(b);if(d&&d.gateways)for(var g=0;g<d.gateways.length;g++){var h=d.gateways[g];h.info&&
"hm"==h.info.sys&&(h.info.uuid=a.uuid,h.info.ip=a.ip)}a=JSON.stringify(d);k.writeFile(f,a,function(a){k.existsSync(f+".md5")&&k.unlinkSync(f+".md5");e()})}});setTimeout(function(){c||e("noCCU")},1E3)}else e("file")}));g&&(b?e(Error("[File] "+b.message)):e())})})};
...
xnm.aio.cloud.RemotesServer.prototype._renameRemoteDir=function(b){XC.debugf("[xnm.aio.cloud] rename remote dir ...");if(b){var c=require("fs"),a=this._remotesPath+"_"+(new Date).getTime();c.existsSync(this._remotesPath)&&c.renameSync(this._remotesPath,a);try{c.renameSync(this._remotesPath+"_new",this._remotesPath)}catch(d){XC.debugf(d.message),c.renameSync(a,this._remotesPath)}c.existsSync(a)&&c.rmdirRecursiveSync(a);b.sort(function(a,b){return a.localeCompare(b)})}XC.debugf("[xnm.aio.cloud] ... renamed remote dir.");
