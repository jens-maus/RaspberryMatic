...
xnm.aio.cloudTaskmanager.CloudTaskmanager.prototype.addVxProCloudData=function(a,b){a.cloudTaskIds=[];a.root.if=a.root.if.filter(function(a){return!a.cloud&&(!a.gateway||"cloudservice"!==a.gateway.sys)});a.root.then[0].then=a.root.then[0].then.filter(function(a){return!a.cloud});this._cloudService.getRules(function(c,d){XC.debugf(d);if(c)return b(c,d);var e=[],f=[];xnm.aio.cloudTaskmanager.extractRules(d).forEach(function(b){var c=!1;b.action&&(c=b.action.some(function(b){return b.data&&b.data.data&&
b.data.data.id==a.id&&b.data.token===a.aio._token}));c&&(a.cloudTaskIds.push(b.ruleId),e=e.concat(xnm.aio.cloudTaskmanager.mapCloudTriggersToPro(b)),f=f.concat(xnm.aio.cloudTaskmanager.mapCloudActionsToPro(b)))});a.root.if=a.root.if.concat(e);a.root.then[0].then=a.root.then[0].then.concat(f);return b(c,a)})};
