...
xnm.aio.contronics.ExecEngine.prototype._doRequeset2=function(a,b){if(a&&"string"==typeof a){this._logger.log("trace","do request:"+a);var c={host:this.ip,port:this.port,path:"/RPC2",method:"POST",headers:{"Content-Type":"text/xml","Content-length":a.length,"User-Agent":"mediola",Connection:"keep-alive"}},d=b,e=this,f=require("http").request(c,function(g){g.setEncoding("utf-8");var h=g.statusCode,l="";g.on("data",function(m){l+=m});g.on("end",function(){e._logger.log("trace","response status code:"+
h);e._logger.log("trace","response data:"+l);200==h?d&&(d(null,l),d=null):d&&(d(Error("http request error:"+h),l),d=null)})});f.setTimeout(3E4,function(){e._logger.log("warn","http request timeout");f.abort()});f.on("error",function(g){e._logger.log("warn","http request error:"+g.message);d&&(d(g),d=null)});a&&f.write(a);f.end()}else b&&b(Error("data invalid"))};
xnm.aio.contronics.ExecEngine.prototype._doRequeset=function(a,b){if(a&&"string"==typeof a){var c=this,d="http://"+this.ip+":"+this.port+"/RP2";this._logger.log("trace","send POST to url:"+d+" data:"+a);var e=b;$.ajax({url:d,type:"POST",timeout:3E4,headers:{"Content-Type":"text/xml","Content-length":a.length,"User-Agent":"mediola",Connection:"keep-alive"},data:a,dataType:"text",cache:!0,success:function(f){c._logger.log("trace","http request success:"+f);e&&(e(null,f),e=null)},error:function(f,g){f=
"http request error:"+(f?f.status:"0")+"-"+g;c._logger.log("trace",f);e&&(e(Error(f)),e=null)}})}else b&&b(Error("data invalid"))};
xnm.aio.contronics.ExecEngine.prototype._doXmlRpcReq=function(a,b,c){var d=require("x.xmlrpc.js"),e=this;try{var f=d.serializeReq(a,b);this._doRequeset(f,function(g,h){if(g)xnm.aio.contronics.SessionBuffer.sessionHttpError(e.ip),c&&c(g);else{xnm.aio.contronics.SessionBuffer.sessionHttpOK(e.ip);try{var l=d.deserializeRsp(h);l.error?c&&c(Error("xmlrpc fault "+l.error.faultCode+":"+l.error.faultString)):l.params&&l.params[0]?c&&c(null,l.params[0]):c&&c(Error("xmlrpc response error"))}catch(m){c&&c(m)}}})}catch(g){c&&
c(g)}};
...
xnm.aio.contronics.ExecEngine.prototype._getDeviceList=function(a,b){var c=this;this._logger.log("debug","getDeviceList:"+a);this._doExecReq([a,8],function(d,e){if(d)b&&b(d);else if(e){var f=[];e.split("%06").forEach(function(g){if(g&&(g=g.split("%07"),5<=g.length)){for(var h={name:g[0],systemLabel:g[1],moduleLabel:g[2],moduleID:g[3],objects:[]},l=4;l<g.length;l++)if(g[l]){var m=g[l].split("%08");10==m.length&&(4!=l||h.name||(h.name=m[0]),h.objects.push({name:m[0],label:m[1],group:m[2],art:m[3],type:m[4],
min:m[5],max:m[6],valueInfo:m[7],ioInfo:m[8],deviceCode:m[9]}))}f.push(h)}});c._logger.log("debug","finish getDeviceList:"+a);b&&b(null,f)}else b&&b(null,[])})};xnm.aio.contronics.ExecEngine.prototype._getDeviceGroup=function(a,b){this._doExecReq([a,9],function(c,d){c?b&&b(c):b&&b(null,d)})};
...
xnm.aio.contronics.ExecEngine.prototype.getStatesCreator=function(a,b,c){b?this._getStates(function(d,e){if(d)c&&c(d);else{d=[];for(var f=0;f<b.length;f++)if(b[f].id&&b[f].device&&b[f].status){var g=b[f].device.name,h=b[f].device.art,l=b[f].device.valueInfo;g&&(g=e[g],null!=g&&"undefined"!=typeof g&&("N"==h?"0"==l?d.push({id:b[f].id,status:parseInt(g)}):d.push({id:b[f].id,status:parseFloat(g)}):"U"==h&&l?(h=l.split(";"),l=parseInt(g),h[l]?d.push({id:b[f].id,status:h[l]}):d.push({id:b[f].id,status:g})):
...
c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else c(new d(e.INVALID,"gateway is invalid"));else c(new d(e.INVALID,"catagory is invalid"));else c(new d(e.INVALID,"info is invalid"))},deleteDevice:function(a,b,c){c()},applyUIFilter:function(a,b){var c=function(h,l){if("*"==h)return!0;for(var m=!1,r=0;r<h.length;r++)if(h[r]==l){m=!0;break}return m},d=function(h,l,m){var r=[];switch(m.catagory){case "command":h.command&&h.command.forEach(function(k){c(m.elems,k.type)&&(k=JSON.parse(JSON.stringify(k)),
r.push(k))});break;case "status":h.status&&h.status.forEach(function(k){c(m.elems,k.type)&&(k=JSON.parse(JSON.stringify(k)),r.push(k))})}return r},e=function(h,l){var m=[];if(!h.art||"string"!=typeof h.art)return null;var r=!0,k=!0;if(h.ioInfo)switch(parseInt(h.ioInfo)){case 1:k=!1;break;case 2:r=!1}var q;switch(h.art.toLowerCase()){case "u":var n={};var p;h.valueInfo&&(p=h.valueInfo.split(";"));if(k&&p)for(n.command=[],q=0;q<p.length;q++)p[q]&&n.command.push({type:"enum",name:p[q],ref:{value:p[q]}});
...
n.command=[k])}n&&(k=d(n,h,l),m=m.concat(k));return m};if(!a.art)return null;var f=JSON.parse(JSON.stringify(a)),g=null;switch(b.catagory){case "command":g=e(a,b);f.command=g;break;case "status":g=e(a,b);f.status=g;break;default:return null}return g&&0!=g.length?f:null}};
