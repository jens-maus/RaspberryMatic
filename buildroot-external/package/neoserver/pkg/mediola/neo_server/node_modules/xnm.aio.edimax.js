...
$jscomp.global.Symbol(e):$jscomp.POLYFILL_PREFIX+c+"$"+e),$jscomp.defineProperty(d,$jscomp.propertyToPolyfillSymbol[e],{configurable:!0,writable:!0,value:b})))};$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(b,c){return $jscomp.findInternal(this,b,c).v}},"es6","es3");var xnm=xnm||{};xnm.aio=xnm.aio||{};xnm.aio.edimax=xnm.aio.edimax||{};
xnm.aio.edimax.SmartPlug=function(a,b){this.ip=a;this.port=1E4;this.uuid=null;this.user="admin";this.password="1234";b&&(this.password=b);this._state="?";this.data="switch";this.CommandHandler=this.model=null};
...
xnm.aio.edimax.SmartPlug.prototype.executeCommand=function(a,b,c){var d="OFF";if("on"==b)d="ON";else if("toggle"==b){var e=this;this.getState(function(g){"off"==g.state&&(d="ON");e._doRequest('<?xml version="1.0" encoding="UTF8"?><SMARTPLUG id="edimax"><CMD id="setup"><Device.System.Power.State>'+d+"</Device.System.Power.State></CMD></SMARTPLUG>",function(f){c&&c()})});return}this._doRequest('<?xml version="1.0" encoding="UTF8"?><SMARTPLUG id="edimax"><CMD id="setup"><Device.System.Power.State>'+
d+"</Device.System.Power.State></CMD></SMARTPLUG>",function(g){c&&c()})};xnm.aio.edimax.SmartPlug.prototype.getStates=function(a,b,c){var d=this;this.CommandHandler=a;this.getState(function(e){e&&b&&d.CommandHandler.setState?(d.CommandHandler.setState(b.id,e.state),d.CommandHandler.setState(b.id+":current",e.current),d.CommandHandler.setState(b.id+":power",e.power)):d.CommandHandler.receivedState&&d.CommandHandler.receivedState("edimax",d.ip,e);c&&c(e)})};
xnm.aio.edimax.SmartPlug.prototype.getStateValues=function(a){return a};xnm.aio.edimax.SmartPlug.prototype.executeCommandCreator=function(a,b,c){console.error("!!!!!",a,b);b&&b.value?this.executeCommand(a,b.value,function(d){c&&c(d)}):c&&c(Error("command value is empty"))};
xnm.aio.edimax.SmartPlug.prototype.getStatesCreator=function(a,b,c){this.getState(function(d){if(d){for(var e=[],g=0;g<b.length;g++){var f=b[g];if(f&&f.id&&f.status)switch(f.status.value){case "state":d.state&&"?"!=typeof d.state&&e.push({id:f.id,status:d.state});break;case "current":null!=d.current&&"undefined"!=typeof d.current&&"?"!=d.current&&e.push({id:f.id,status:d.current});break;case "power":null!=d.power&&"undefined"!=typeof d.power&&"?"!=d.power&&e.push({id:f.id,status:d.power})}}c&&c(null,
e)}else c&&c(Error("get state error"))})};xnm.aio.edimax.SmartPlug.prototype.toJSON=function(){return{sys:xnm.aio.edimax.DM.SYS,ip:this.ip,port:this.port,password:this.password,uuid:this.uuid,type:"smartplug",data:this.data,model:this.model}};xnm.aio.edimax.SmartPlug.prototype.equalsTo=function(a){return a&&a instanceof xnm.aio.edimax.SmartPlug?this.ip==a.ip&&this.port==a.port&&this.password==a.password&&this.data==a.data:!1};
...
xnm.aio.edimax.Discovery.prototype._addDiscoverSocket=function(a,b){var c=require("dgram"),d=this;if(b)"IPv4"==b.family&&0==b.internal&&(e=c.createSocket("udp4"),e.on("listening",function(){e.setBroadcast(!0)}),e.on("message",function(g,f){d._receivedData(f.address,g)}),e.bind(a,b.address),this._sockets.push(e));else{var e=c.createSocket("udp4");e.on("message",function(g,f){d._receivedData(f.address,g)});e.bind(a);this._sockets.push(e)}};
...
"info is invalid"))},updateDevice:function(a,b,c){b=xnm.aio.edimax.DMError;var d=b.ERROR_CODE;a?a.ip?a.password?a.type?a.data?c(null,a):c(new b(d.INVALID,"data is invalid")):c(new b(d.INVALID,"type is invalid")):c(new b(d.INVALID,"password is invalid")):c(new b(d.INVALID,"ip is invalid")):c(new b(d.INVALID,"info is invalid"))},deleteDevice:function(a,b,c){c()},getCommandStatusMap:function(a){return a.data?this.MAP[a.data]:null},applyUIFilter:function(a,b){var c=function(h,n){if("*"==h)return!0;for(var k=
!1,l=0;l<h.length;l++)if(h[l]==n){k=!0;break}return k},d=function(h,n,k){var l=[];switch(k.catagory){case "command":h.command&&h.command.forEach(function(m){c(k.elems,m.type)&&(m=JSON.parse(JSON.stringify(m)),l.push(m))});break;case "status":h.status&&h.status.forEach(function(m){c(k.elems,m.type)&&(m=JSON.parse(JSON.stringify(m)),l.push(m))})}return l},e=function(h,n){var k=[],l=xnm.aio.edimax.DM.getCommandStatusMap(h);l&&(h=d(l,h,n),k=k.concat(h));return k};if(!a.sys)return null;var g=JSON.parse(JSON.stringify(a)),
