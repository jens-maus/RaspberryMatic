...
$jscomp.global.Symbol(e):$jscomp.POLYFILL_PREFIX+c+"$"+e),$jscomp.defineProperty(d,$jscomp.propertyToPolyfillSymbol[e],{configurable:!0,writable:!0,value:b})))};$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(b,c){var d=$jscomp.checkStringArgs(this,b,"startsWith");b+="";var e=d.length,f=b.length;c=Math.max(0,Math.min(c|0,d.length));for(var h=0;h<f&&c<e;)if(d[c++]!=b[h++])return!1;return h>=f}},"es6","es3");var xnm=xnm||{};xnm.aio=xnm.aio||{};
xnm.aio.hcgw=xnm.aio.hcgw||{};xnm.aio.hcgw.Gateway=function(a){this.ip=a;this.port=49880;this.mode=1};
xnm.aio.hcgw.Gateway.prototype._doRequest=function(a,b,c){var d=require("dgram").createSocket("udp4");a=new Buffer(a);xnm.aio.hcgw._logger.log("debug","[Gateway._doRequest] Sending message to: "+this.ip+":"+this.port);d.send(a,0,a.length,this.port,this.ip,function(e,f){e?xnm.aio.hcgw._logger.log("error","[Gateway._doRequest] "+e.message):xnm.aio.hcgw._logger.log("debug","[Gateway._doRequest] Message sent.");d.close();c&&setTimeout(function(){c()},Math.floor(b+750))})};
...
xnm.aio.hcgw.Discovery.prototype._receivedData=function(a,b){var c=b.toString();xnm.aio.hcgw._logger.log("debug","[Discovery._receivedData] Received from "+a+": "+c);if(5<c.length&&c.startsWith("HCGW:")){b=new xnm.aio.hcgw.Gateway(a);c=c.substr(5);c=c.split(";");for(var d=0;d<c.length;d++){var e=c[d].split(":");2===e.length&&"VC"===e[0]&&"ITECHNO"===e[1]&&(b.mode=0)}this.Gateways[a]=b}};
xnm.aio.hcgw.Discovery.prototype._addDiscoverSocket=function(a,b){var c=require("dgram"),d=this,e=null;if(!b){try{e=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(f){xnm.aio.hcgw._logger.log("error","[Discovery._addDiscoverSocket] (no info) "+f.message),e=c.createSocket("udp4")}e.on("error",function(f){f&&("string"===typeof f&&(f=Error(f)),xnm.aio.hcgw._logger.log("error","[Discovery._addDiscoverSocket] (no info) socket error: "+f.message))});e.on("message",function(f,h){d._receivedData(h.address,
f.toString())});e.bind(a);this._sockets.push(e)}else if("IPv4"==b.family&&0==b.internal){try{e=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(f){xnm.aio.hcgw._logger.log("error","[Discovery._addDiscoverSocket] (IPv4, non-internal) "+f.message),e=c.createSocket("udp4")}e.on("error",function(f){f&&("string"===typeof f&&(f=Error(f)),xnm.aio.hcgw._logger.log("error","[Discovery._addDiscoverSocket] (IPv4, non-internal) socket error: "+f.message))});e.on("listening",function(){e.setBroadcast(!0)});e.on("message",
function(f,h){d._receivedData(h.address,f.toString())});e.bind(a,b.address);this._sockets.push(e)}};xnm.aio.hcgw.Discovery.prototype._sendDiscoveryMessages=function(){xnm.aio.hcgw._logger.log("debug","[Discovery._sendDiscoveryMessages] SEARCH HCGW");for(var a=new Buffer("SEARCH HCGW"),b=0;b<this._sockets.length;b++)this._sockets[b].send(a,0,a.length,49880,"255.255.255.255")};
xnm.aio.hcgw.Discovery.prototype.discoverGateways=function(){var a=this,b=0,c=setInterval(function(){a._sendDiscoveryMessages();1<=b++&&clearInterval(c)},250)};xnm.aio.hcgw.Handler=function(){xnm.aio.hcgw._logger||(xnm.aio.hcgw._logger=require("x.logger.js").getLogger("xnm.aio.hcgw"));this._Discovery=null};
xnm.aio.hcgw.Handler.prototype.discoverGateways=function(a){var b=this;this._Discovery||(this._Discovery=new xnm.aio.hcgw.Discovery);this._Discovery.discoverGateways();a&&setTimeout(function(){a(b._Discovery.Gateways)},5E3)};xnm.aio.hcgw.Handler.prototype.getDetectedGateways=function(){this._Discovery||(this._Discovery=new xnm.aio.hcgw.Discovery);return this._Discovery.Gateways};
...
d&=-2;"on"==b?d|=1:(0==b.indexOf("dimTo")&&(d|=1,b=b.replace(/dimTo/g,"")),b=b.replace(/%/g,""),"10"==b?c="00":"20"==b?c="10":"30"==b?c="20":"40"==b?c="30":"50"==b?c="50":"60"==b?c="70":"70"==b?c="90":"80"==b?c="B0":"90"==b?c="D0":"100"==b&&(c="F0"));a=null!=c?a.substr(0,6)+d+a.substr(7)+c:a.substr(0,6)+d+a.substr(7);c={data:[],timing:98,repeats:5,pause:10976};a=XC.getBytes(a,!0);c.data=c.data.concat([3,29]);for(b=0;4>b;b++)for(d=0;8>d;d++)c.data=a[b]&128>>d?c.data.concat([3,15,3,3]):c.data.concat([3,
3,3,15]);if(5==a.length)for(c.data[c.data.length-17]=c.data[c.data.length-19]=3,b=0;4>b;b++)c.data=a[4]&128>>b?c.data.concat([3,15,3,3]):c.data.concat([3,3,3,15]);c.data=c.data.concat([3,126])}return c};
xnm.aio.hcgw.Handler.prototype.getBrennenstuhlSignal=function(a,b,c){var d=null;if(6==a.length){a=XC.getBytes(a,!0);if("on"==b||"day"==b)a[2]|=32;else if("off"==b||"night"==b)a[2]&=-33;var e=[2,12,5,14,7,4,1,15,11,13,6,3,8,9,10,0];d=[10,0,4,12,2,14,7,5,1,15,11,13,9,6,3,8];a[0]&32&&(d=e);e=d[a[2]&15];var f=d[e^(a[2]&240)>>4],h=d[f^a[1]&15],g=d[h^(a[1]&240)>>4];a=[((9^(a[0]&240)>>4)<<4)+d[g^a[0]&15],(g<<4)+h,(f<<4)+e];d={data:[],timing:505,repeats:6,pause:0};e=[2,1];f=[1,2];"day"==b||"night"==b?(d.timing=
...
xnm.aio.hcgw.Gateway.prototype.learn=function(a,b){var c=function(k,m){if(!k)return null;for(var l=k.length;l--;)if(k[l]===m)return!0;return!1},d=function(k,m){for(k=k.toString(16).toUpperCase();k.length<m;)k="0"+k;return k};if(a&&a.type){var e=!0,f=[];if(a.devices)for(var h=a.devices.length;h--;)a.devices[h].address&&f.push(a.devices[h].address);var g=a.address;g&&(e=!1);switch(a.type){case "IT2":for(;e;)g=d(Math.floor(57070*Math.random())+4369,4)+"008.0",e=c(f,g);c={value:"on"};break;case "DY":case "DY2":for(;e;)g=
d(Math.floor(4294967295*Math.random()),8),e=c(f,g);c="DY"==a.type?{value:"learn"}:{value:"up"};break;case "BS":case "BS2":for(;e;)g=d(Math.floor(65535*Math.random()),4)+"01",e=c(f,g);c={value:"on"};"heater"==a.data&&(c={value:"day"});break;default:b&&b(Error("device type invalid"));return}this.executeCommandCreator({type:a.type,data:a.data?a.data:"switch",address:g},c,function(k){k?b&&b(k):b&&b(null,{address:g})})}else b&&b(Error("options invalid"))};
xnm.aio.hcgw.Gateway.prototype.executeCommandCreator=function(a,b,c){if(b&&b.value)if(a&&a.type&&a.address&&a.data){var d=a.type;"IT2"==d&&(d="IT");"DY2"==d&&(d="DY");a=a.address;var e=b.value;if("dimTo"==b.value){if(null==b.ext||"undefined"==typeof b.ext){c&&c(Error("invalid dimTo command format"));return}var f=b.ext;"string"==typeof b.ext&&(f=parseInt(b.ext));e="dimTo"+f;0==f&&(e="off")}b=require("xnm.aio.hcgw.js");if(d=b.getSignal(d,a,e)){var h;a=[];if(b._Discovery&&b._Discovery.Gateways)for(var g in b._Discovery.Gateways)b._Discovery.Gateways.hasOwnProperty(g)&&
(this.ip==g&&(h=b._Discovery.Gateways[g]),a.push(b._Discovery.Gateways[g]));h?h.sendSignal(d,c):0!=a.length?b.sendSignalToGateways(d,a,c):this.sendSignal(d,c);b.discover()}else c&&c(Error("invalid device address"))}else c&&c(Error("invalid device format"));else c&&c(Error("command json invalid"))};xnm.aio.hcgw.Gateway.prototype.toJSON=function(){return{sys:xnm.aio.hcgw.DM.SYS,ip:this.ip,port:this.port,mode:this.mode}};
...
b.data.gateways&&0!==b.data.gateways.length){b=b.data.gateways;var f;for(f=0;f<b.length;f++)if(b[f].index===a.gateway){var h=b[f];break}h?c(null,a):c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"));else c(new d(e.INVALID,"data is invalid"));else c(new d(e.INVALID,"address is invalid"));else c(new d(e.INVALID,"type is invalid"));else c(new d(e.INVALID,"gateway is invalid"));else c(new d(e.INVALID,"info is invalid"))},
updateDevice:function(a,b,c){var d=xnm.aio.hcgw.DM.DMError,e=xnm.aio.hcgw.DM.DMError.ERROR_CODE;if(a)if(a.address)if(a.type)if(a.data)if(b.data&&b.data.gateways&&0!==b.data.gateways.length){b=b.data.gateways;var f;for(f=0;f<b.length;f++)if(b[f].index===a.gateway){var h=b[f];break}h?c(null,a):c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"));else c(new d(e.INVALID,"data is invalid"));else c(new d(e.INVALID,"type is invalid"));
else c(new d(e.INVALID,"address is invalid"));else c(new d(e.INVALID,"info is invalid"))},deleteDevice:function(a,b,c){c()},applyUIFilter:function(a,b){var c=function(g,k){if("*"==g)return!0;for(var m=!1,l=0;l<g.length;l++)if(g[l]==k){m=!0;break}return m},d=function(g,k,m){var l=[];switch(m.catagory){case "command":g.command&&g.command.forEach(function(n){c(m.elems,n.type)&&(n=JSON.parse(JSON.stringify(n)),l.push(n))});break;case "status":g.status&&g.status.forEach(function(n){c(m.elems,n.type)&&
(n=JSON.parse(JSON.stringify(n)),l.push(n))})}return l},e=function(g,k){var m=[],l=null;switch(g.type){case "BS":l="heater"==g.data?xnm.aio.hcgw.DM.MAP.BS.heater:xnm.aio.hcgw.DM.MAP.BS.general;break;case "BS2":l=xnm.aio.hcgw.DM.MAP.BS2.general;break;case "ELRO":l=xnm.aio.hcgw.DM.MAP.ELRO.general;break;case "DY":case "DY2":l=xnm.aio.hcgw.DM.MAP.DY.shutter;break;case "IT":case "IT2":l="shutter"==g.data?xnm.aio.hcgw.DM.MAP.IT.shutter:"dimmer"==g.data?xnm.aio.hcgw.DM.MAP.IT.dimmer:xnm.aio.hcgw.DM.MAP.IT.general}l&&
(g=d(l,g,k),m=m.concat(g));return m};if(!a||null==a.type||"undefined"==typeof a.type)return null;var f=JSON.parse(JSON.stringify(a)),h=null;switch(b.catagory){case "command":h=e(a,b);f.command=h;break;case "status":h=e(a,b);f.status=h;break;default:return null}return h&&0!=h.length?f:null}};xnm.aio.hcgw.DM.DMError=function(a,b){this.code=a;this.message=b;this.stack=Error().stack};xnm.aio.hcgw.DM.DMError.prototype=Error();xnm.aio.hcgw.DM.DMError.prototype.name="HCGatewayDMError";
