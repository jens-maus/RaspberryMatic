...
String(g.value).startsWith("accessAuthState")&&(h=parseInt(String(g.value).replace("accessAuthState",""),10),k=xnm.aio.hm.Channel.buildStateScript(e,{address:h+1},"STATE"));return k},getCommandStatusMap:function(h){return{command:[{type:"enum",name:"lock",ref:{value:"lock"}},{type:"enum",name:"unlock",ref:{value:"unlock"}},{type:"enum",name:"open",ref:{value:"open"}},{type:"enum",name:"user 1 authorization",ref:{value:"accessAuthSet01",ext:"%e",extMeta:["on","off"]}},{type:"enum",name:"user 2 authorization",
...
e){if("?"===h)return h;var g=["unknown","locked","unlocked"];switch(e.value){case "maintBatBlBat":case "maintBatBlComm":case "maintBatBlDutyCycle":case "maintBatBlVoltage":return xnm.aio.hm.HmIPChannel.MAINTENANCE_BAT_BL.handleStatusResponse(h,e);case "lockState":return g[h]||"?"}if(String(e.value).startsWith("accessAuthState")){if("true"==h)return"on";if("false"==h)return"off"}return"?"},resolveState:function(h,e,g){if(!e||!e.channel)return null;var k=e.channel.indexOf(":");if(-1==k)return null;
k=e.channel.substr(k+1);var n=null,r=["unknown","locked","unlocked"];switch(k){case "0":n=xnm.aio.hm.HmIPChannel.MAINTENANCE_BAT_BL.resolveState(h,e,g);break;case "1":n={key:"lockState",value:r[g]||"?"};break;case "2":case "3":case "4":case "5":case "6":case "7":case "8":case "9":n={key:"accessAuthState0"+(parseInt(k,10)-1),value:g?"on":"off"}}return n}},resolveJSON:function(h){if(!h||!h.type)return null;if("HmIP-PDT"==h.type)return b.parseJSON(h);if("HMIP-BSM"==h.type.toUpperCase().substr(0,8))return d.parseJSON(h,
!0);if(this[h.type])return this[h.type];var e=null;switch(h.type){case "HmIP-WHS2":e=c.parseJSON(h,1,2);break;case "HmIP-DBB":case "HmIP-BRC2":e=a.parseJSON(h);break;case "HmIP-RCV-50":e=a.parseJSON(h)}if("HMIP-PS"==h.type.toUpperCase().substr(0,7))e="HMIP-PSM"==h.type.toUpperCase().substr(0,8)?d.parseJSON(h):c.parseJSON(h);else if("HmIPW-DRS"==h.type.substr(0,9))e=c.parseJSON(h,1,parseInt(h.type.substr(9)));else if("HmIPW-DRD"==h.type.substr(0,9))e=b.parseJSON(h);else if("HmIPW-DRBL"==h.type.substr(0,
