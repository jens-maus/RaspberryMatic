...
xnm.aio.hue.Util={_HSVToRGB:function(a,c,b){var d=Math.floor(6*a),e=6*a-d;a=b*(1-c);var f=b*(1-e*c);c=b*(1-(1-e)*c);switch(d%6){case 0:var g=b;var k=c;var h=a;break;case 1:g=f;k=b;h=a;break;case 2:g=a;k=b;h=c;break;case 3:g=a;k=f;h=b;break;case 4:g=c;k=a;h=b;break;case 5:g=b,k=a,h=f}return[255*g,255*k,255*h]},_RGBToHex:function(a,c,b){for(a=a.toString(16);2>a.length;)a="0"+a;for(c=c.toString(16);2>c.length;)c="0"+c;for(b=b.toString(16);2>b.length;)b="0"+b;return(a+c+b).toUpperCase()}};
...
xnm.aio.hue.Bridge.prototype._doRequest=function(a,c,b,d){var e="/api/"+this._USER+c;c||(e="/api");a={host:this.ip,port:this.port,path:e,method:a,headers:{"Content-Type":'application/json; charset="UTF-8"',"User-Agent":"mediola",Connection:"close"}};var f=this,g=d,k=require("http").request(a,function(a){if(200==a.statusCode){a.setEncoding("utf-8");var b="";a.on("data",function(a){b+=a});a.on("end",function(){var a={};try{var c=JSON.parse(b);c&&c[0]&&c[0].error?(a.error="unknown",1==c[0].error.type&&
(a.error="unauthorized"),101==c[0].error.type&&(a.error="unpressed")):a.data=c}catch(m){}g&&(g(a),g=null)})}else XC.debugf("Failed to post request to "+f.ip),g&&(g(null),g=null)});k.setTimeout(1E3,function(){k.abort()});k.on("error",function(a){g&&(g(null),g=null)});b&&k.write(b);k.end()};
xnm.aio.hue.Bridge.prototype._doRequestForAuth=function(a,c,b,d){var e="/api/"+this._USER+c;c||(e="/api");a={host:this.ip,port:this.port,path:e,method:a,headers:{"Content-Type":'application/json; charset="UTF-8"',"User-Agent":"mediola",Connection:"close"}};this._logger.log("debug","do auth:"+JSON.stringify(a));var f=this,g=d,k=null,h=require("http").request(a,function(a){f._logger.log("trace","do auth return status:"+a.statusCode);k=a.statusCode;if(200==a.statusCode){a.setEncoding("utf-8");var b=
"";a.on("data",function(a){b+=a});a.on("end",function(){f._logger.log("trace","do auth return:"+b);var a={};try{var c=JSON.parse(b);c&&c[0]&&c[0].error?(a.error="unknown",1==c[0].error.type&&(a.error="unauthorized"),101==c[0].error.type&&(a.error="unpressed")):a.data=c}catch(q){}g&&(g(a),g=null)})}else XC.debugf("Failed to post request to "+f.ip),g&&(g(null),g=null)});h.setTimeout(1E3,function(){f._logger.log("trace","do auth timeout");h.abort()});h.on("error",function(a){k&&"ECONNRESET"==a.code?
f._logger.log("trace","do auth connection reset"):(f._logger.log("trace","do auth error:"+a.message),g&&(g(null),g=null))});b&&h.write(b);h.end()};xnm.aio.hue.Bridge.prototype.getLights=function(a){this._doRequest("GET","/lights",null,function(c){a&&a(c)})};
...
"osram"==a.manufacturer.toLowerCase())c=c.data.state;else if(d&&c.data.action&&null!=c.data.action.bri&&"undefined"!=typeof c.data.action.bri){var e=c.data.action.bri;c=c.data.action}else!d&&c.data.state&&null!=c.data.state.bri&&"undefined"!=typeof c.data.state.bri?(e=c.data.state.bri,c=c.data.state):(e=0,c={on:!1});var g={on:!0};a.type&&0==a.type.indexOf("On/Off")&&a.manufacturer&&"osram"==a.manufacturer.toLowerCase()?c.on&&(g.on=!1):(c.on&&(g.on=!1,a.manufacturer&&"osram"==a.manufacturer.toLowerCase()&&
(g.transitiontime=0)),g.on&&0==e&&(g.bri=10));e=JSON.stringify(g);f._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",e,b)}else b&&b({error:Error("no data field")})})}else if("up"==c||"down"==c)f=this,this._doRequest("GET",d?"/groups/"+a.address:"/lights/"+a.address,null,function(e){if(e&&e.error)b&&b(e);else{if(e&&e.data){var g=-1;d&&e.data.action&&null!=e.data.action.bri&&"undefined"!=typeof e.data.action.bri?g=e.data.action.bri:!d&&e.data.state&&null!=e.data.state.bri&&
"undefined"!=typeof e.data.state.bri&&(g=e.data.state.bri);-1!=g&&(e=g,e="up"==c?Math.floor(e+25.4):Math.floor(e-25.4),254<e&&(e=254),0>e&&(e=0),g='{"bri": '+e+', "on": true}',0==e&&(g='{"bri": '+e+', "on": false, "transitiontime":0}'),f._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",g,b),b=null)}b&&b()}});else if("string"==typeof c&&9<c.length&&"transtime"==c.substr(0,9))e=c.substr(9),e=parseInt(e),this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+
a.address+"/state",'{"transitiontime": '+e+"}",b);else if(6==c.length){e=parseInt(c.substr(0,2),16);var g=parseInt(c.substr(2,2),16),k=parseInt(c.substr(4,2),16);e=this._RGBToHSV(e,g,k);var h=Math.floor(e[0]/360*65535+3E3);65535<h&&(h=65535);0>h&&(h=0);var l=Math.floor(254*e[1]);254<l&&(l=254);0>l&&(l=0);f=this;this._doRequest("GET",d?"/groups/"+a.address:"/lights/"+a.address,null,function(c){if(c&&c.error)b&&b(c);else{if(c&&c.data){var e=-1;d&&c.data.action&&null!=c.data.action.bri&&"undefined"!=
typeof c.data.action.bri?e=c.data.action.bri:!d&&c.data.state&&null!=c.data.state.bri&&"undefined"!=typeof c.data.state.bri&&(e=c.data.state.bri);-1!=e&&(c='{"hue": '+h+', "sat": '+l+', "on": true}',0==e&&(c='{"hue": '+h+', "sat": '+l+', "bri": 10, "on": true}'),f._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",c,b),b=null)}b&&b()}})}else 8==c.length?(h=Math.floor(parseInt(c.substr(0,4),16)),l=Math.floor(parseInt(c.substr(4,2),16)),k=Math.floor(parseInt(c.substr(6,
2),16)),this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",'{"hue": '+h+', "sat": '+l+', "bri": '+k+', "on": true}',b)):(g=Math.floor(2.54*parseInt(c)),254<g&&(g=254),0>g&&(g=0),e='{"bri": '+g+', "on": true}',0==g&&(e='{"bri": '+g+', "on": false, "transitiontime":0}'),this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",e,b))};
...
xnm.aio.hue.Bridge.prototype.getStates=function(a,c,b){var d=this;this.CommandHandler=a;this.getLights(function(a){if(a&&a.error&&"unauthorized"==a.error)b&&b("AUT");else if(a&&a.data&&a.data.XC_ERR)XC.debugf("[xnm.aio.hue.Bridge.getStates] "+JSON.stringify(a.data),"error"),b&&b("XC_ERR",a.data.XC_ERR);else{if(a&&a.data)if(c&&d.CommandHandler.setState)for(var e=0;e<c.length;e++){var g=c[e],k;if(a.data[g.address]&&(k=a.data[g.address].state)){var h=d.getStateValues(g.subtype,k),l;for(l in h)"state"==
l?d.CommandHandler.setState(g.id,h[l]):d.CommandHandler.setState(g.id+":"+l,h[l])}}else if(d.CommandHandler.receivedState)for(e in a.data)d.CommandHandler.receivedState("hue",e,a.data[e].state);b&&b("OK")}})};xnm.aio.hue.Bridge.prototype.getStateValues=function(a,c){a={};c.on?(a.state=""+Math.round(c.bri/2.54),1==c.bri&&(a.state="1"),a.power="on"):(a.state="0",a.power="off");c.ct&&(a.ct=""+c.ct);return a};
...
xnm.aio.hue.Handler.prototype.discoverBridges=function(a){var c=this;(function(a){var b=require("x.upnp.js");b&&b.discoverDevices(function(b){if("hue"==b.type){c._logger.log("trace","upnp find hue:"+b.ip+" uuid:"+b.uuid);var e=new xnm.aio.hue.Bridge(b.ip,80,b.uuid);c.detectedBridges[b.uuid]=e;a(e)}})})(a);(function(a){var b=function(a,c){var b="http://"+a+"/description.xml";(new (require("x.upnp.js").DescLoader)(a)).load(b,function(a,b){c&&c(a,b)})};(function(a){var b=a;$.ajax({url:"https://www.meethue.com/api/nupnp",
type:"GET",timeout:3E3,dataType:"text",cache:!0,success:function(a){c._logger.log("trace","nupnp request success:"+a);try{var e=JSON.parse(a);b&&(b(null,e),b=null)}catch(h){b&&(b(h),b=null)}},error:function(a,e){a="nupnp request error:"+(a?a.status:"0")+"-"+e;c._logger.log("trace",a);b&&(b(Error(a)),b=null)}})})(function(e,d){if(!e&&d&&0!=d.length)for(e=0;e<d.length;e++)d[e]&&d[e].internalipaddress&&b(d[e].internalipaddress,function(b,d){!b&&d&&"hue"==d.type&&(c._logger.log("trace","nupnp find hue:"+
d.ip+" uuid:"+d.uuid),b=new xnm.aio.hue.Bridge(d.ip,80,d.uuid),c.detectedBridges[d.uuid]=b,a(b))})})})(a)};xnm.aio.hue.Handler.prototype._NUPnP=function(a){};xnm.aio.hue.Handler.prototype.Bridge=xnm.aio.hue.Bridge;xnm.aio.hue.Handler.prototype.getStateValues=function(a,c){return(new xnm.aio.hue.Bridge).getStateValues(a.type,c)};
...
xnm.aio.hue.Bridge.prototype.getGroupStates=function(a,c,b){var d=this;this.CommandHandler=a;c&&0!=c.length?xnm.aio.hue.Group.getAllGroups(this,function(a,f){if(a&&1==a.error)b&&b("AUT");else{if(f)if(c&&d.CommandHandler.setState)for(a=0;a<c.length;a++)for(var e=c[a],k,h=0;h<f.length;h++){if(f[h]&&f[h]._meta&&f[h]._meta.action&&f[h]._address==e.address){k=f[h]._meta.action;k=d.getStateValues(e.subtype,k);for(var l in k)"state"==l?d.CommandHandler.setState(e.id,k[l]):d.CommandHandler.setState(e.id+
":"+l,k[l])}}else if(d.CommandHandler.receivedState)for(h=0;h<f.length;h++)f[h]&&f[h]._meta&&f[h]._meta.action&&f[h]._address&&d.CommandHandler.receivedState("hue",f[h]._address,f[h]._meta.action,!0);b&&b("OK")}}):b&&b("OK")};
...
c.ext,d){case "none":d="effectNone";break;case "colorloop":d="effectColorloop";break;default:b&&b(Error("effect command ext invalid"));return}else if("setScene"==d){d=c.ext;if(!d){b&&b(Error("setScene command ext invalid"));return}d="setscene@"+d}else if("transtime"==d){d=c.ext;if(null==d||"undefined"==typeof d){b&&b(Error("transtime command ext invalid"));return}d="transtime"+d}a.manufacturer=a.manufacturername;switch(a.category){case "light":this.executeCommand(a,d,function(a){if(a&&a.error){var c=
Error(a.error);"unauthorized"==a.error&&(c.code=1,c.module=xnm.aio.hue.DM.SYS)}b&&b(c)});break;case "group":this.executeCommandGroup(a,d,function(a){if(a&&a.error){var c=Error(a.error);"unauthorized"==a.error&&(c.code=1,c.module=xnm.aio.hue.DM.SYS)}b&&b(c)});break;default:b&&b(Error("no such category:"+a.category))}}else b&&b(Error("command is null"));else b&&b(Error("device is null"))};
xnm.aio.hue.Bridge.prototype.getStatesCreator=function(a,c,b){if(c){a=[];for(var d=[],e=0;e<c.length;e++){var f=c[e];"group"==f.device.category?d.push({id:f.id,address:f.device.address}):a.push({id:f.id,address:f.device.address})}var g=[];e={};var k=this;e.receivedState=function(a,b,d,e){d=k.getStateValues(0,d);for(var f=0;f<c.length;f++)c[f]&&c[f].device&&c[f].device.address==b&&c[f].status&&(!e||"group"==c[f].device.category)&&("power"==c[f].status.value&&null!==d.power&&"undefined"!=typeof d.power?
(a={id:c[f].id,status:d.power},g.push(a)):"brightness"==c[f].status.value&&null!==d.state&&"undefined"!=typeof d.state?(a={id:c[f].id,status:parseInt(d.state)},g.push(a)):"ct"==c[f].status.value&&null!==d.ct&&"undefined"!=typeof d.ct&&(d=parseInt(d.ct),153<=d||500>=d)&&(a={id:c[f].id,status:d},g.push(a)))};var h=3,l=b;this.getStates(e,a,function(a){h--;"AUT"==a?(a=Error(a),a.code=1,a.module=xnm.aio.hue.DM.SYS,l&&l(a)):0==h&&(l&&l(null,g),l=null)});this.getGroupStates(e,d,function(a){h--;"AUT"==a?
(a=Error(a),a.code=1,a.module=xnm.aio.hue.DM.SYS,l&&l(a),l=null):0==h&&(l&&l(null,g),l=null)});xnm.aio.hue.Sensor._getAllSensors(this,function(a,b){h--;if(a)l&&l(a),l=null;else{if(b)for(a=0;a<c.length;a++)if(c[a]){var d=c[a].id,e=c[a].device,f=c[a].status;if(d&&f&&f.value&&e&&e.address&&"sensor"==e.category){var k=b[e.address];if(k&&(e=xnm.aio.hue.Sensor._parseSensor(e.address,k))&&(e=e.getStates())){f=e[f.value];if("undefined"==typeof f||null==f)f="?";g.push({id:d,status:f})}}}0==h&&(l&&l(null,g),
l=null)}})}else b&&b(Error("deviceList is null"))};xnm.aio.hue.Bridge.prototype.executeCommandGroup=function(a,c,b){this.executeCommand(a,c,b,!0)};xnm.aio.hue.Bridge.prototype.getAllGroups=function(a){xnm.aio.hue.Group.getAllGroups(this,a)};xnm.aio.hue.Bridge.prototype.toJSON=function(){return{sys:xnm.aio.hue.DM.SYS,ip:this.ip,port:this.port,uuid:this.uuid,username:this._USER}};
...
xnm.aio.hue.Group=function(){var a=function(c,b){this._category=a.CATEGORY;this._address=c;this._meta=b};a.prototype.toJSON=function(){var a=JSON.parse(JSON.stringify(this._meta));a.sys=xnm.aio.hue.DM.SYS;a.category=this._category;a.address=this._address;return a};a.CATEGORY="group";a.getAllGroups=function(a,b){var c=this,e,f;this._getGroupWithID(a,0,function(d,k){d?b&&b(d):(f=k,c._getGroups(a,function(a,d){a?b&&b(a):(e=d?d:{},f&&(e["0"]=f),b&&b(null,c._parseGroupList(e)))}))})};a.createGroup=function(c,
...
xnm.aio.hue.MAP={onoff:{command:[{type:"enum",name:"toggle",ref:{value:"toggle"}},{type:"enum",name:"off",ref:{value:"off"}},{type:"enum",name:"on",ref:{value:"on"}},{type:"enum",name:"alert",ref:{value:"alert",ext:"%e",extMeta:["stop","one","repeat"]}},{type:"int",name:"transition time",ref:{value:"transtime",ext:"%d"}}],status:[{type:"enum",name:"power",ref:{value:"power",options:["on","off"]}}]},dimmable:{command:[{type:"enum",name:"up",ref:{value:"up"}},{type:"enum",name:"down",ref:{value:"down"}},
{type:"int",name:"brightness",ref:{value:"bri",ext:"%d",extMeta:"0-100"}}],status:[{type:"int",name:"brightness",ref:{value:"brightness",extMeta:"0-100"}}]},colortemp:{command:[{type:"int",name:"color temperature",ref:{value:"ct",ext:"%d",extMeta:"153-500",scale:"1"}}],status:[{type:"int",name:"color temperature",ref:{value:"ct",extMeta:"153-500",scale:"1"}}]},colorlight:{command:[{type:"string",name:"rgb color",ref:{value:"rgbS",ext:"%s"}},{type:"rgb",name:"rgb color",ref:{value:"rgb",ext:"%s"}},
{type:"enum",name:"effect",ref:{value:"effect",ext:"%e",extMeta:["none","colorloop"]}},{type:"string",name:"hsb color",ref:{value:"hue",ext:"%s"}}]},extendcolor:{},group:{command:[{type:"dynamic",name:"set scene",ref:{value:"setScene",ext:"%dynamic"}}]},sensor:{Daylight:{status:[{type:"enum",name:"day light",ref:{value:"daylight",options:["true","fasle"]}},{type:"string",name:"last event time",ref:{value:"ltime"}}],ipevt:[{type:"enum",name:"event",ref:{value:"event",options:["sunrise","sunset"]}}]},
ZGPSwitch:{status:[{type:"enum",name:"last event",ref:{value:"levent",options:["Press_Button1","Press_Button2","Press_Button3","Press_Button4"]}},{type:"string",name:"last event time",ref:{value:"ltime"}}],ipevt:[{type:"enum",name:"event",ref:{value:"levent",options:["Press_Button1","Press_Button2","Press_Button3","Press_Button4"]}}]},ZLLSwitch:{status:[{type:"enum",name:"last event",ref:{value:"levent",options:["ON","DIM_UP","DIM_DOWN","OFF"]}},{type:"string",name:"last event time",ref:{value:"ltime"}}],
ipevt:[{type:"enum",name:"event",ref:{value:"levent",options:["ON","DIM_UP","DIM_DOWN","OFF"]}}]},ZLLPresence:{status:[{type:"enum",name:"detect motion",ref:{value:"motion",options:["true","false"]}},{type:"string",name:"last event time",ref:{value:"ltime"}},{type:"int",name:"battery",ref:{value:"battery"}},{type:"enum",name:"detect motion (on/off)",ref:{value:"motion2",options:["on","off"]}}],ipevt:[{type:"enum",name:"detect motion",ref:{value:"motion",options:["true","false"]}}]}}};
xnm.aio.hue.DMError=function(a,c){this.code=a;this.message=c;this.stack=Error().stack};xnm.aio.hue.DMError.prototype=Error();xnm.aio.hue.DMError.prototype.name="HUEDeviceManagerError";xnm.aio.hue.DMError.prototype.code=0;xnm.aio.hue.DMError.ERROR_CODE={DEFAULT:0,INDEX_EXISTS:1,ID_EXISTS:2,INVALID:3,UNKNOW:4,NOT_FOUND:5,IP_EXISTS:6};
...
!1,d=0;d<a.length;d++)if(a[d]==b){c=!0;break}return c},e=function(a,b,c){var e=[];switch(c.catagory){case "command":a.command&&a.command.forEach(function(a){d(c.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))});break;case "status":a.status&&a.status.forEach(function(a){d(c.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))})}return e},f=function(a,b){if(!a.type)return null;a=xnm.aio.hue.MAP.sensor[a.type];if(!a)return null;switch(b.catagory){case "command":return a.command;case "status":return a.status;
default:return null}},g=function(a,c){if("sensor"==a.category)return f(a,c);var d=[];if(!a.type)return null;var g=a.type.toLowerCase(),h=[];if(a.state||a.action){var k=a.state;if("room"==g||"lightgroup"==g||"luminaire"==g||"lightsource"==g)k=a.action;k&&(null!=k.on&&"undefined"!=typeof k.on&&(h=b(h,xnm.aio.hue.MAP.onoff)),null!=k.bri&&"undefined"!=typeof k.bri&&(h=b(h,xnm.aio.hue.MAP.dimmable)),null!=k.ct&&"undefined"!=typeof k.ct&&(h=b(h,xnm.aio.hue.MAP.colortemp)),null!=k.hue&&"undefined"!=typeof k.hue&&
(h=b(h,xnm.aio.hue.MAP.colorlight)),"room"==g||"lightgroup"==g||"luminaire"==g||"lightsource"==g)&&(h=b(h,xnm.aio.hue.MAP.group))}else switch(a.type.toLowerCase()){case "on/off light":case "on/off plug-in unit":h=xnm.aio.hue.MAP.onoff;break;case "dimmable light":case "dimmable plug-in unit":h=b(xnm.aio.hue.MAP.onoff,xnm.aio.hue.MAP.dimmable);break;case "color temperature light":h=b(xnm.aio.hue.MAP.onoff,xnm.aio.hue.MAP.colortemp);break;case "color light":h=b(b(xnm.aio.hue.MAP.onoff,xnm.aio.hue.MAP.dimmable),
xnm.aio.hue.MAP.colorlight);break;case "extended color light":case "lightgroup":case "luminaire":case "lightsource":if(h=b(b(b(xnm.aio.hue.MAP.onoff,xnm.aio.hue.MAP.dimmable),xnm.aio.hue.MAP.colortemp),xnm.aio.hue.MAP.colorlight),"lightgroup"==g||"luminaire"==g||"lightsource"==g)h=b(h,xnm.aio.hue.MAP.group)}h&&(a=e(h,a,c),d=d.concat(a));return d};if(!a.category)return null;var k=JSON.parse(JSON.stringify(a)),h=null;switch(c.catagory){case "command":h=g(a,c);k.command=h;break;case "status":h=g(a,c);
k.status=h;break;default:return null}return h&&0!=h.length?k:null},applyIPEventFilter:function(a,c){if(!a.sys||"sensor"!=a.category)return null;c=xnm.aio.hue.MAP.sensor[a.type];if(!c)return null;a=JSON.parse(JSON.stringify(a));a.ipevt=c.ipevt;return a},supportSmartWidget:function(a){if(!a||!a.type)return!1;if("ZLLPresence"==a.type)return!0;a=this.applyUIFilter(a,{catagory:"command",elems:"*"});if(!a||!a.command||0==a.command.length)return!1;for(var c=0;c<a.command.length;c++){var b=a.command[c];if(b&&
b.ref&&("on"==b.ref.value||"up"==b.ref.value||"ct"==b.ref.value||"rgb"==b.ref.value))return!0}return!1},applySmartWidgetFilter:function(a,c,b){if(!a||!a.type)return!1;var d=this.applyUIFilter(a,{catagory:"command",elems:"*"});if(!d||!d.command||0==d.command.length)return!1;for(var e=!1,f=!1,g=!1,k=!1,h=0;h<d.command.length;h++){var l=d.command[h];l&&l.ref&&("on"==l.ref.value&&(e=!0),"up"==l.ref.value&&(f=!0),"ct"==l.ref.value&&(g=!0),"rgb"==l.ref.value&&(k=!0))}var n;if("ZLLPresence"==a.type){var p=
{"%motionState%":{value:"motion2",options:["on","off"],name:"detect motion (on/off)"}};var m=["motion"]}e&&(n||(n={}),n["%on%"]||(n["%on%"]={value:"on",name:"on"}),n["%off%"]||(n["%off%"]={value:"off",name:"off"}),n["%toggle%"]||(n["%toggle%"]={value:"toggle",name:"toggle"}),p||(p={}),p["%switchState%"]||(p["%switchState%"]={value:"power",options:["off","on"],name:"power"}),m||(m=[]),-1==m.indexOf("switch")&&m.push("switch"));f&&(n||(n={}),n["%dimOn%"]||(n["%dimOn%"]={value:"on",name:"on"}),n["%dimOff%"]||
(n["%dimOff%"]={value:"off",name:"off"}),n["%dimUp%"]||(n["%dimUp%"]={value:"up",name:"up"}),n["%dimDown%"]||(n["%dimDown%"]={value:"down",name:"down"}),n["%dimTo%"]||(n["%dimTo%"]={value:"bri",ext:"%d",extMeta:"0-100",name:"brightness"}),p||(p={}),p["%dimmerState%"]||(p["%dimmerState%"]={value:"brightness",name:"brightness"}),m||(m=[]),-1==m.indexOf("dimmer")&&m.push("dimmer"));g&&(n||(n={}),n["%colorTemp%"]||(n["%colorTemp%"]={value:"ct",ext:"%d",extMeta:"153-500",name:"color temperature"}),m||
(m=[]),-1==m.indexOf("color_temp")&&m.push("color_temp"));k&&(n||(n={}),n["%rgb%"]||(n["%rgb%"]={value:"rgb",ext:"%s",name:"rgb color"}),n["%white%"]||(n["%white%"]={value:"rgbS",ext:"FFFFFF",name:"rgb color"}),m||(m=[]),-1==m.indexOf("rgb")&&m.push("rgb"));return m&&0!=m.length?this._injectToSmartWidgetTemplate(c,m,b,n,p):null},_injectToSmartWidgetTemplate:function(a,c,b,d,e){var f=[];a||(d={});d||(d={});e||(e={});for(var g=0;g<a.length;g++){var k=a[g];if(k.meta&&-1!=c.indexOf(k.meta.type)&&k.elements){k=
JSON.parse(JSON.stringify(k));var h=!1,l;for(l in k.elements){var n=k.elements[l];if(n&&Array.isArray(n))for(var p=0;p<n.length;p++){var m=n[p];m&&(m.action&&"command"==m.action.type&&m.action.ref&&m.action.ref.value&&(d[m.action.ref.value]?(m.action.ref=d[m.action.ref.value],m.action.device=b,h=!0):m.action=null),m.status&&m.status.ref&&m.status.ref.value&&(e[m.status.ref.value]?(m.status.ref=e[m.status.ref.value],m.status.device=b,h=!0):m.status=null))}}h&&f.push(k)}}return f}};
