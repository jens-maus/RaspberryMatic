...
$jscomp.polyfill("Symbol",function(a){if(a)return a;var b=function(g,k){this.$jscomp$symbol$id_=g;$jscomp.defineProperty(this,"description",{configurable:!0,writable:!0,value:k})};b.prototype.toString=function(){return this.$jscomp$symbol$id_};var c="jscomp_symbol_"+(1E9*Math.random()>>>0)+"_",d=0,e=function(g){if(this instanceof e)throw new TypeError("Symbol is not a constructor");return new b(c+(g||"")+"_"+d++,g)};return e},"es6","es3");
...
xnm.aio.hue.Util={_HSVToRGB:function(a,b,c){var d=Math.floor(6*a),e=6*a-d;a=c*(1-b);var g=c*(1-e*b);b=c*(1-(1-e)*b);switch(d%6){case 0:var k=c;var f=b;var h=a;break;case 1:k=g;f=c;h=a;break;case 2:k=a;f=c;h=b;break;case 3:k=a;f=g;h=c;break;case 4:k=b;f=a;h=c;break;case 5:k=c,f=a,h=g}return[255*k,255*f,255*h]},_RGBToHex:function(a,b,c){for(a=a.toString(16);2>a.length;)a="0"+a;for(b=b.toString(16);2>b.length;)b="0"+b;for(c=c.toString(16);2>c.length;)c="0"+c;return(a+b+c).toUpperCase()}};
...
xnm.aio.hue.Bridge.prototype._doRequest=function(a,b,c,d){var e="/api/"+this._USER+b;b||(e="/api");e={host:this.ip,port:this.port,path:e,method:a,headers:{"Content-Type":'application/json; charset="UTF-8"',"User-Agent":"mediola",Connection:"close"}};var g=this,k=d;this._logger.log("trace","send '"+a+"' to "+this.ip+":"+this.port+"/api/xxxxxxxx"+(b?b:""));c&&this._logger.log("trace","with data: "+c);var f=require("http").request(e,function(h){if(200==h.statusCode){h.setEncoding("utf-8");var n="";h.on("data",
function(l){n+=l});h.on("end",function(){g._logger.log("trace","http response 200: "+n);var l={};try{var r=JSON.parse(n);r&&r[0]&&r[0].error?(l.error="unknown",1==r[0].error.type&&(l.error="unauthorized"),101==r[0].error.type&&(l.error="unpressed")):l.data=r}catch(q){}k&&(k(l),k=null)})}else g._logger.log("trace","http response "+h.statusCode),k&&(k(null),k=null)});f.setTimeout(1E3,function(){g._logger.log("trace","http request timeout");f.abort()});f.on("error",function(h){g._logger.log("trace",
"http request error:"+h.message);k&&(k(null),k=null)});c&&f.write(c);f.end()};
xnm.aio.hue.Bridge.prototype._doRequestForAuth=function(a,b,c,d){var e="/api/"+this._USER+b;b||(e="/api");a={host:this.ip,port:this.port,path:e,method:a,headers:{"Content-Type":'application/json; charset="UTF-8"',"User-Agent":"mediola",Connection:"close"}};this._logger.log("debug","do auth:"+JSON.stringify(a));var g=this,k=d,f=null,h=require("http").request(a,function(n){g._logger.log("trace","do auth return status:"+n.statusCode);f=n.statusCode;if(200==n.statusCode){n.setEncoding("utf-8");var l=
"";n.on("data",function(r){l+=r});n.on("end",function(){g._logger.log("trace","do auth return:"+l);var r={};try{var q=JSON.parse(l);q&&q[0]&&q[0].error?(r.error="unknown",1==q[0].error.type&&(r.error="unauthorized"),101==q[0].error.type&&(r.error="unpressed")):r.data=q}catch(p){}k&&(k(r),k=null)})}else XC.debugf("Failed to post request to "+g.ip),k&&(k(null),k=null)});h.setTimeout(1E3,function(){g._logger.log("trace","do auth timeout");h.abort()});h.on("error",function(n){f&&"ECONNRESET"==n.code?
g._logger.log("trace","do auth connection reset"):(g._logger.log("trace","do auth error:"+n.message),k&&(k(null),k=null))});c&&h.write(c);h.end()};xnm.aio.hue.Bridge.prototype.getLights=function(a){this._doRequest("GET","/lights",null,function(b){a&&a(b)})};
...
25.4),254<p&&(p=254),0>p&&(p=0),m='{"bri": '+p+', "on": true}',0==p&&(m='{"bri": '+p+', "on": false, "transitiontime":0}'),g._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",m,c),c=null)}c&&c()}});else if("string"==typeof b&&7<b.length&&"briRamp"==b.substr(0,7)){e=b.substr(7).split(":");var k=Math.floor(2.54*parseInt(e[0])),f=parseInt(e[1]);e=null;e=0==k?{on:!1,transitiontime:f}:{on:!0,bri:k,transitiontime:f};this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+
a.address+"/state",JSON.stringify(e),c)}else if(6==b.length){var h=parseInt(b.substr(0,2),16),n=parseInt(b.substr(2,2),16),l=parseInt(b.substr(4,2),16);e=this._RGBToHSV(h,n,l);var r=Math.floor(e[0]/360*65535+3E3),q=Math.floor(254*e[1]);"osram"===String(a.manufacturer).toLowerCase()&&(r-=3E3);65535<r?r=65535:0>r&&(r=0);254<q?q=254:0>q&&(q=0);g=this;this._doRequest("GET",d?"/groups/"+a.address:"/lights/"+a.address,null,function(p){if(p&&p.error)c&&c(p);else{if(p&&p.data){var m=-1;d&&p.data.action&&
null!=p.data.action.bri&&"undefined"!=typeof p.data.action.bri?m=p.data.action.bri:!d&&p.data.state&&null!=p.data.state.bri&&"undefined"!=typeof p.data.state.bri&&(m=p.data.state.bri);-1!=m&&(p.data.state&&"xy"==p.data.state.colormode?(p=g._RGBToXY(h,n,l),0==m&&(m=10),p=JSON.stringify({xy:p,on:!0,bri:m})):(p='{"hue": '+r+', "sat": '+q+', "on": true}',0==m&&(p='{"hue": '+r+', "sat": '+q+', "bri": 10, "on": true}')),g._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",p,
c),c=null)}c&&c()}})}else 8==b.length?(r=Math.floor(parseInt(b.substr(0,4),16)),q=Math.floor(parseInt(b.substr(4,2),16)),l=Math.floor(parseInt(b.substr(6,2),16)),this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",'{"hue": '+r+', "sat": '+q+', "bri": '+l+', "on": true}',c)):(k=Math.floor(2.54*parseInt(b)),254<k&&(k=254),0>k&&(k=0),e='{"bri": '+k+', "on": true}',0==k&&(e='{"bri": '+k+', "on": false, "transitiontime":0}'),this._doRequest("PUT",d?"/groups/"+a.address+
...
xnm.aio.hue.Bridge.prototype.getStates=function(a,b,c){var d=this;this.CommandHandler=a;this.getLights(function(e){if(e&&e.error&&"unauthorized"==e.error)c&&c("AUT");else if(e&&e.data&&e.data.XC_ERR)XC.debugf("[xnm.aio.hue.Bridge.getStates] "+JSON.stringify(e.data),"error"),c&&c("XC_ERR",e.data.XC_ERR);else{if(e&&e.data)if(b&&d.CommandHandler.setState)for(var g=0;g<b.length;g++){var k=b[g],f;if(e.data[k.address]&&(f=e.data[k.address].state)){var h=d.getStateValues(k.subtype,f),n;for(n in h)"state"==
n?d.CommandHandler.setState(k.id,h[n]):d.CommandHandler.setState(k.id+":"+n,h[n])}}else if(d.CommandHandler.receivedState)for(g in e.data)d.CommandHandler.receivedState("hue",g,e.data[g].state);c&&c("OK")}})};xnm.aio.hue.Bridge.prototype.getStateValues=function(a,b){a={};b.on?(a.state=""+Math.round(b.bri/2.54),1==b.bri&&(a.state="1"),a.power="on"):(a.state="0",a.power="off");b.ct&&(a.ct=""+b.ct);return a};
...
xnm.aio.hue.Handler.prototype.discoverBridges=function(a){var b=this;(function(c){require("x.upnp.js").discoverDevices(function(d){if("hue"===d.type){b._logger.log("trace","upnp found Hue: "+d.ip+" UUID:"+d.uuid);var e=new xnm.aio.hue.Bridge(d.ip,80,d.uuid);b.detectedBridges[d.uuid]=e;c(e)}})})(a);(function(c){var d=function(e,g){var k="http://"+e+"/description.xml";(new (require("x.upnp.js").DescLoader)(e)).load(k,function(f,h){g&&g(f,h)})};(function(e){$.ajax({url:"https://discovery.meethue.com",
type:"GET",timeout:3E3,dataType:"text",cache:!0,success:function(g){b._logger.log("trace","nupnp request success: "+g);try{var k=JSON.parse(g);e&&(e(null,k),e=null)}catch(f){e&&(e(f),e=null)}},error:function(g,k){g="nupnp request error: "+(g?g.status:"0")+"-"+k;b._logger.log("error",g);e&&(e(Error(g)),e=null)}})})(function(e,g){if(!e&&g&&0!==g.length)for(e=0;e<g.length;e++){var k=g[e];k&&k.internalipaddress&&d(k.internalipaddress,function(f,h){!f&&h&&"hue"===h.type&&(b._logger.log("trace","nupnp found Hue: "+
h.ip+" UUID: "+h.uuid),f=new xnm.aio.hue.Bridge(h.ip,80,h.uuid),b.detectedBridges[h.uuid]=f,c(f))})}})})(a)};xnm.aio.hue.Handler.prototype._NUPnP=function(a){};xnm.aio.hue.Handler.prototype.Bridge=xnm.aio.hue.Bridge;xnm.aio.hue.Handler.prototype.getStateValues=function(a,b){return(new xnm.aio.hue.Bridge).getStateValues(a.type,b)};
...
xnm.aio.hue.Bridge.prototype.getGroupStates=function(a,b,c){var d=this;this.CommandHandler=a;b&&0!=b.length?xnm.aio.hue.Group.getAllGroups(this,function(e,g){if(e&&1==e.error)c&&c("AUT");else{if(g)if(b&&d.CommandHandler.setState)for(e=0;e<b.length;e++)for(var k=b[e],f,h=0;h<g.length;h++){if(g[h]&&g[h]._meta&&g[h]._meta.action&&g[h]._address==k.address){f=g[h]._meta.action;f=d.getStateValues(k.subtype,f);for(var n in f)"state"==n?d.CommandHandler.setState(k.id,f[n]):d.CommandHandler.setState(k.id+
":"+n,f[n])}}else if(d.CommandHandler.receivedState)for(h=0;h<g.length;h++)g[h]&&g[h]._meta&&g[h]._meta.action&&g[h]._address&&d.CommandHandler.receivedState("hue",g[h]._address,g[h]._meta.action,!0);c&&c("OK")}}):c&&c("OK")};
...
b.ext,d){case "none":d="effectNone";break;case "colorloop":d="effectColorloop";break;default:c&&c(Error("effect command ext invalid"));return}else if("setScene"==d){d=b.ext;if(!d){c&&c(Error("setScene command ext invalid"));return}d="setscene@"+d}else if("briRamp"==d){if(!b.ext||0==b.ext.length)return c&&c(Error("briRamp command ext invalid")),null;for(d=0;d<b.ext.length;d++){var e=b.ext[d];if(e)switch(e.value){case "bri":var g=e.ext;break;case "transtime":var k=e.ext}}if("undefined"==typeof g||null==
g||"undefined"==typeof k||null==k){c&&c(Error("briRamp command ext invalid"));return}d="briRamp"+g+":"+k}a.manufacturer=a.manufacturername;switch(a.category){case "light":this.executeCommand(a,d,function(f){if(f&&f.error){var h=Error(f.error);"unauthorized"==f.error&&(h.code=1,h.module=xnm.aio.hue.DM.SYS)}c&&c(h)});break;case "group":this.executeCommandGroup(a,d,function(f){if(f&&f.error){var h=Error(f.error);"unauthorized"==f.error&&(h.code=1,h.module=xnm.aio.hue.DM.SYS)}c&&c(h)});break;default:c&&
c(Error("no such category:"+a.category))}}else c&&c(Error("command is null"));else c&&c(Error("device is null"))};
xnm.aio.hue.Bridge.prototype.getStatesCreator=function(a,b,c){if(b){a=[];for(var d=[],e=0;e<b.length;e++){var g=b[e];"group"==g.device.category?d.push({id:g.id,address:g.device.address}):a.push({id:g.id,address:g.device.address})}var k=[];e={};var f=this;e.receivedState=function(l,r,q,p){q=f.getStateValues(0,q);for(l=0;l<b.length;l++)if(b[l]&&b[l].device&&b[l].device.address==r&&b[l].status&&(!p||"group"==b[l].device.category))if("power"==b[l].status.value&&null!==q.power&&"undefined"!=typeof q.power){var m=
{id:b[l].id,status:q.power};k.push(m)}else"brightness"==b[l].status.value&&null!==q.state&&"undefined"!=typeof q.state?(m={id:b[l].id,status:parseInt(q.state)},k.push(m)):"ct"==b[l].status.value&&null!==q.ct&&"undefined"!=typeof q.ct&&(m=parseInt(q.ct),153<=m||500>=m)&&(m={id:b[l].id,status:m},k.push(m))};var h=3,n=c;this.getStates(e,a,function(l){h--;"AUT"==l?(l=Error(l),l.code=1,l.module=xnm.aio.hue.DM.SYS,n&&n(l)):0==h&&(n&&n(null,k),n=null)});this.getGroupStates(e,d,function(l){h--;"AUT"==l?(l=
Error(l),l.code=1,l.module=xnm.aio.hue.DM.SYS,n&&n(l),n=null):0==h&&(n&&n(null,k),n=null)});xnm.aio.hue.Sensor._getAllSensors(this,function(l,r){h--;if(l)n&&n(l),n=null;else{if(r)for(l=0;l<b.length;l++)if(b[l]){var q=b[l].id,p=b[l].device,m=b[l].status;if(q&&m&&m.value&&p&&p.address&&"sensor"==p.category){var t=r[p.address];if(t&&(p=xnm.aio.hue.Sensor._parseSensor(p.address,t))&&(p=p.getStates())){m=p[m.value];if("undefined"==typeof m||null==m)m="?";k.push({id:q,status:m})}}}0==h&&(n&&n(null,k),n=
...
xnm.aio.hue.Group=function(){var a=function(b,c){this._category=a.CATEGORY;this._address=b;this._meta=c};a.prototype.toJSON=function(){var b=JSON.parse(JSON.stringify(this._meta));b.sys=xnm.aio.hue.DM.SYS;b.category=this._category;b.address=this._address;return b};a.CATEGORY="group";a.getAllGroups=function(b,c){var d=this,e,g;this._getGroupWithID(b,0,function(k,f){k?c&&c(k):(g=f,d._getGroups(b,function(h,n){h?c&&c(h):(e=n?n:{},g&&(e["0"]=g),c&&c(null,d._parseGroupList(e)))}))})};a.createGroup=function(b,
c,d){if(c)if(c.name)if(c.lights)if(c.lights.length){var e=this;this._createGroup(b,c,function(g,k){g?d&&d(g):e._getGroupWithID(b,k,function(f,h){f?d&&d(f):(f=new a(k,h),d&&d(null,f))})})}else d&&d(Error("group lights is empty"));else d&&d(Error("group lights is invalid"));else d&&d(Error("group name is invalid"));else d&&d(Error("group is null"))};a._getGroups=function(b,c){b._doRequest("GET","/groups",null,function(d){if(d)if(d.error){var e=Error(d.error);"unauthorized"==d.error&&(e.code=1,e.module=
...
xnm.aio.hue.Sensor=function(){var a=function(f,h){this._category=a.CATEGORY;this._address=f;this._meta=h};a.CATEGORY="sensor";a.getAllSensors=function(f,h){var n=this;this._getAllSensors(f,function(l,r){l?h&&h(l):h&&h(null,n._parseSensors(r))})};a._parseSensors=function(f){for(var h=[],n=Object.keys(f),l=0;l<n.length;l++){var r=this._parseSensor(n[l],f[n[l]]);r&&h.push(r)}return h};a._parseSensor=function(f,h){switch(h.type){case "Daylight":return new b(f,h);case "ZGPSwitch":return new c(f,h);case "ZLLSwitch":return new d(f,
h);case "ZLLPresence":return new e(f,h);case "ZLLTemperature":return new g(f,h);case "ZLLLightLevel":return new k(f,h);default:return null}};a._getAllSensors=function(f,h){f._doRequest("GET","/sensors",null,function(n){if(n)if(n.error){var l=Error(n.error);"unauthorized"==n.error&&(l.code=1,l.module=xnm.aio.hue.DM.SYS);h&&h(l)}else n.data?h&&h(null,n.data):h&&h(Error("hue response format invalid"));else h&&h(Error("hue request error"))})};a.prototype.getType=function(){return this._meta?this._meta.type:
null};a.prototype.getAddress=function(){return this._address};a.prototype.getStates=function(){return null};a.prototype.toJSON=function(){var f=JSON.parse(JSON.stringify(this._meta));f.sys=xnm.aio.hue.DM.SYS;f.category=this._category;f.address=this._address;return f};var b=function(f,h){a.call(this,f,h)};b.prototype=new a;b.prototype.constructor=b;b.prototype.getStates=function(){if(!this._meta)return null;var f={};this._meta.state&&(f.daylight=this._meta.state.daylight,f.ltime=this._meta.state.lastupdated);
return f};var c=function(f,h){a.call(this,f,h)};c.prototype=new a;c.prototype.constructor=c;c.prototype.getStates=function(){if(!this._meta)return null;var f={};if("FOHSWITCH"==this._meta.modelid){if(this._meta.state){switch(this._meta.state.buttonevent){case 20:f.levent="Press_Button1";break;case 21:f.levent="Press_Button2";break;case 23:f.levent="Press_Button3";break;case 22:f.levent="Press_Button4"}f.ltime=this._meta.state.lastupdated}}else if(this._meta.state){switch(this._meta.state.buttonevent){case 34:f.levent=
"Press_Button1";break;case 16:f.levent="Press_Button2";break;case 17:f.levent="Press_Button3";break;case 18:f.levent="Press_Button4"}f.ltime=this._meta.state.lastupdated}return f};var d=function(f,h){a.call(this,f,h)};d.prototype=new a;d.prototype.constructor=d;d.prototype.getStates=function(){if(!this._meta)return null;var f={};if(this._meta.state){switch(this._meta.state.buttonevent){case 1E3:f.levent="ROM001"==this._meta.modelid?"INITIAL_PRESS":"ON";break;case 1001:f.levent="ROM001"==this._meta.modelid?
...
(f.reachable=this._meta.config.reachable))}return f};var e=function(f,h){a.call(this,f,h)};e.prototype=new a;e.prototype.constructor=e;e.prototype.getStates=function(){if(!this._meta)return null;var f={};this._meta.state&&(f.motion=this._meta.state.presence,f.motion2=this._meta.state.presence?"on":"off",f.ltime=this._meta.state.lastupdated);this._meta.config&&(f.battery=this._meta.config.battery);return f};var g=function(f,h){a.call(this,f,h)};g.prototype=new a;g.prototype.constructor=g;g.prototype.getStates=
function(){if(!this._meta)return null;var f={};this._meta.state&&(f.temperature=this._meta.state.temperature/100,f.ltime=this._meta.state.lastupdated);this._meta.config&&(f.battery=this._meta.config.battery);return f};var k=function(f,h){a.call(this,f,h)};k.prototype=new a;k.prototype.constructor=k;k.prototype.getStates=function(){if(!this._meta)return null;var f={};this._meta.state&&(f.lightlevel=this._meta.state.lightlevel,f.dark=this._meta.state.dark,f.daylight=this._meta.state.daylight,f.ltime=
...
name:"day light",ref:{value:"daylight",options:["true","false"]}},{type:"string",name:"last event time",ref:{value:"ltime"}}],ipevt:[{type:"enum",name:"event",ref:{value:"event",options:["sunrise","sunset"]}}]},ZGPSwitch:{status:[{type:"enum",name:"last event",ref:{value:"levent",options:["Press_Button1","Press_Button2","Press_Button3","Press_Button4"]}},{type:"string",name:"last event time",ref:{value:"ltime"}}],ipevt:[{type:"enum",name:"event",ref:{value:"levent",options:["Press_Button1","Press_Button2",
...
b,c){var d=xnm.aio.hue.DMError,e=xnm.aio.hue.DMError.ERROR_CODE;if(a)if(a.gateway)if(null==a.address||"undefined"==typeof a.address)c(new d(e.INVALID,"address is invalid"));else if(a.category)if(b.data&&b.data.gateways&&0!==b.data.gateways.length){b=b.data.gateways;var g;for(g=0;g<b.length;g++)if(b[g].index===a.gateway){var k=b[g];break}k?c(null,a):c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"));else c(new d(e.INVALID,
"catagory is invalid"));else c(new d(e.INVALID,"gateway is invalid"));else c(new d(e.INVALID,"info is invalid"))},updateDevice:function(a,b,c){var d=xnm.aio.hue.DMError,e=xnm.aio.hue.DMError.ERROR_CODE;if(a)if(null==a.address||"undefined"==typeof a.address)c(new d(e.INVALID,"address is invalid"));else if(a.category)if(a.gateway){b=b.data.gateways;var g;for(g=0;g<b.length;g++)if(b[g].index===a.gateway){var k=b[g];break}k?c(null,a):c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else c(new d(e.INVALID,
"gateway is invalid"));else c(new d(e.INVALID,"catagory is invalid"));else c(new d(e.INVALID,"info is invalid"))},deleteDevice:function(a,b,c){c()},applyUIFilter:function(a,b){var c=function(f,h){if("*"==f)return!0;for(var n=!1,l=0;l<f.length;l++)if(f[l]==h){n=!0;break}return n},d=function(f,h,n){var l=[];switch(n.catagory){case "command":f.command&&f.command.forEach(function(r){c(n.elems,r.type)&&(r=JSON.parse(JSON.stringify(r)),l.push(r))});break;case "status":f.status&&f.status.forEach(function(r){c(n.elems,
r.type)&&(r=JSON.parse(JSON.stringify(r)),l.push(r))})}return l},e=function(f,h){var n=[],l=xnm.aio.hue.DM.getCommandStatusMap(f);l&&(f=d(l,f,h),n=n.concat(f));return n};if(!a.category)return null;var g=JSON.parse(JSON.stringify(a)),k=null;switch(b.catagory){case "command":k=e(a,b);g.command=k;break;case "status":k=e(a,b);g.status=k;break;default:return null}return k&&0!=k.length?g:null},applyIPEventFilter:function(a,b){if(!a.sys||"sensor"!=a.category)return null;b=this.getCommandStatusMap(a);if(!b)return null;
a=JSON.parse(JSON.stringify(a));a.ipevt=b.ipevt;return a},getCommandStatusMap:function(a,b){b=function(g,k){var f=[],h=[];g&&g.command&&0<g.command.length&&(f=f.concat(g.command));k&&k.command&&0<k.command.length&&(f=f.concat(k.command));g&&g.status&&0<g.status.length&&(h=h.concat(g.status));k&&k.status&&0<k.status.length&&(h=h.concat(k.status));return{command:f,status:h}};if(!a.type)return null;if("sensor"==a.category)return"ROM001"==a.modelid?xnm.aio.hue.MAP.sensor.SmartButton:xnm.aio.hue.MAP.sensor[a.type];
...
c.ref.value||"rgb"==c.ref.value))return!0}return!1},applySmartWidgetFilter:function(a,b,c){if(!a||!a.type)return!1;var d=this.applyUIFilter(a,{catagory:"command",elems:"*"});if(!d||!d.command||0==d.command.length)return!1;for(var e=!1,g=!1,k=!1,f=!1,h=0;h<d.command.length;h++){var n=d.command[h];n&&n.ref&&("on"==n.ref.value&&(e=!0),"up"==n.ref.value&&(g=!0),"ct"==n.ref.value&&(k=!0),"rgb"==n.ref.value&&(f=!0))}var l;if("ZLLPresence"==a.type){var r={"%motionState%":{value:"motion2",options:["on","off"],
...
(l["%dimUp%"]={value:"up",name:"up"}),l["%dimDown%"]||(l["%dimDown%"]={value:"down",name:"down"}),l["%dimTo%"]||(l["%dimTo%"]={value:"bri",ext:"%d",extMeta:"0-100",name:"brightness"}),r||(r={}),r["%dimmerState%"]||(r["%dimmerState%"]={value:"brightness",name:"brightness"}),q||(q=[]),-1==q.indexOf("dimmer")&&q.push("dimmer"));k&&(l||(l={}),l["%colorTemp%"]||(l["%colorTemp%"]={value:"ct",ext:"%d",extMeta:"153-500",name:"color temperature"}),q||(q=[]),-1==q.indexOf("color_temp")&&q.push("color_temp"));
f&&(l||(l={}),l["%rgb%"]||(l["%rgb%"]={value:"rgb",ext:"%s",name:"rgb color"}),l["%white%"]||(l["%white%"]={value:"rgbS",ext:"FFFFFF",name:"rgb color"}),q||(q=[]),-1==q.indexOf("rgb")&&q.push("rgb"));return q&&0!=q.length?this._injectToSmartWidgetTemplate(b,q,c,l,r):null},_injectToSmartWidgetTemplate:function(a,b,c,d,e){var g=[];a||(d={});d||(d={});e||(e={});for(var k=0;k<a.length;k++){var f=a[k];if(f.meta&&-1!=b.indexOf(f.meta.type)&&f.elements){f=JSON.parse(JSON.stringify(f));var h=!1,n;for(n in f.elements){var l=
f.elements[n];if(l&&Array.isArray(l))for(var r=0;r<l.length;r++){var q=l[r];q&&(q.action&&"command"==q.action.type&&q.action.ref&&q.action.ref.value&&(d[q.action.ref.value]?(q.action.ref=d[q.action.ref.value],q.action.device=c,h=!0):q.action=null),q.status&&q.status.ref&&q.status.ref.value&&(e[q.status.ref.value]?(q.status.ref=e[q.status.ref.value],q.status.device=c,h=!0):q.status=null))}}h&&g.push(f)}}return g}};
...
xnm.aio.hue.Handler.prototype.discoverBridgesWithTimeout=function(a,b){this._discoverCallbacks||(this._discoverCallbacks=[]);for(var c=!1,d=0;d<this._discoverCallbacks.length;d++)if(this._discoverCallbacks[d]===b){c=!0;break}c||this._discoverCallbacks.push(b);var e=this,g=[];10>=a&&(a=10);setTimeout(function(){if(null!=e._discoverCallbacks)for(var k=0;k<e._discoverCallbacks.length;k++)e._discoverCallbacks[k](null,g);e._discoverCallbacks=null},1E3*a);this.discoverBridges(function(k){if(k){for(var f=
0;f<g.length;f++)if(g[f].uuid==k.uuid)return;g.push(k)}})};xnm.aio.hue.Handler.prototype.auth=function(a,b){a.ip?(a=this.resolveGateway(a))?a.authorizeUser2(function(c,d){b&&b(c,d)}):b&&b(Error("gateway json format invalid")):b&&b(Error("bridge format invalid"))};
