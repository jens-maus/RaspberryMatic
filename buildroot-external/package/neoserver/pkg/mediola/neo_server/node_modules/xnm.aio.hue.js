...
function(a){b+=a});a.on("end",function(){f._logger.log("trace","http response 200: "+b);var a={};try{var c=JSON.parse(b);c&&c[0]&&c[0].error?(a.error="unknown",1==c[0].error.type&&(a.error="unauthorized"),101==c[0].error.type&&(a.error="unpressed")):a.data=c}catch(m){}h&&(h(a),h=null)})}else f._logger.log("trace","http response "+a.statusCode),h&&(h(null),h=null)});g.setTimeout(1E3,function(){f._logger.log("trace","http request timeout");g.abort()});g.on("error",function(a){f._logger.log("trace",
...
"";a.on("data",function(a){b+=a});a.on("end",function(){f._logger.log("trace","do auth return:"+b);var a={};try{var c=JSON.parse(b);c&&c[0]&&c[0].error?(a.error="unknown",1==c[0].error.type&&(a.error="unauthorized"),101==c[0].error.type&&(a.error="unpressed")):a.data=c}catch(q){}h&&(h(a),h=null)})}else XC.debugf("Failed to post request to "+f.ip),h&&(h(null),h=null)});k.setTimeout(1E3,function(){f._logger.log("trace","do auth timeout");k.abort()});k.on("error",function(a){g&&"ECONNRESET"==a.code?
...
"osram"==a.manufacturer.toLowerCase())c=c.data.state;else if(e&&c.data.action&&null!=c.data.action.bri&&"undefined"!=typeof c.data.action.bri){var d=c.data.action.bri;c=c.data.action}else!e&&c.data.state&&null!=c.data.state.bri&&"undefined"!=typeof c.data.state.bri?(d=c.data.state.bri,c=c.data.state):(d=0,c={on:!1});var g={on:!0};a.type&&0==a.type.indexOf("On/Off")&&a.manufacturer&&"osram"==a.manufacturer.toLowerCase()?c.on&&(g.on=!1):(c.on&&(g.on=!1,a.manufacturer&&"osram"==a.manufacturer.toLowerCase()&&
(g.transitiontime=0)),g.on&&0==d&&(g.bri=10));d=JSON.stringify(g);f._doRequest("PUT",e?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",d,b)}else b&&b({error:Error("no data field")})})}else if("up"==c||"down"==c)f=this,this._doRequest("GET",e?"/groups/"+a.address:"/lights/"+a.address,null,function(d){if(d&&d.error)b&&b(d);else{if(d&&d.data){var g=-1;e&&d.data.action&&null!=d.data.action.bri&&"undefined"!=typeof d.data.action.bri?g=d.data.action.bri:!e&&d.data.state&&null!=d.data.state.bri&&
"undefined"!=typeof d.data.state.bri&&(g=d.data.state.bri);-1!=g&&(d=g,d="up"==c?Math.floor(d+25.4):Math.floor(d-25.4),254<d&&(d=254),0>d&&(d=0),g='{"bri": '+d+', "on": true}',0==d&&(g='{"bri": '+d+', "on": false, "transitiontime":0}'),f._doRequest("PUT",e?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",g,b),b=null)}b&&b()}});else if("string"==typeof c&&7<c.length&&"briRamp"==c.substr(0,7)){d=c.substr(7).split(":");var h=Math.floor(2.54*parseInt(d[0])),g=parseInt(d[1]);d=null;d=0==h?
{on:!1,transitiontime:g}:{on:!0,bri:h,transitiontime:g};this._doRequest("PUT",e?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",JSON.stringify(d),b)}else if(6==c.length){d=parseInt(c.substr(0,2),16);h=parseInt(c.substr(2,2),16);g=parseInt(c.substr(4,2),16);d=this._RGBToHSV(d,h,g);var k=Math.floor(d[0]/360*65535+3E3);65535<k&&(k=65535);0>k&&(k=0);var l=Math.floor(254*d[1]);254<l&&(l=254);0>l&&(l=0);f=this;this._doRequest("GET",e?"/groups/"+a.address:"/lights/"+a.address,null,function(c){if(c&&
c.error)b&&b(c);else{if(c&&c.data){var d=-1;e&&c.data.action&&null!=c.data.action.bri&&"undefined"!=typeof c.data.action.bri?d=c.data.action.bri:!e&&c.data.state&&null!=c.data.state.bri&&"undefined"!=typeof c.data.state.bri&&(d=c.data.state.bri);-1!=d&&(c='{"hue": '+k+', "sat": '+l+', "on": true}',0==d&&(c='{"hue": '+k+', "sat": '+l+', "bri": 10, "on": true}'),f._doRequest("PUT",e?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",c,b),b=null)}b&&b()}})}else 8==c.length?(k=Math.floor(parseInt(c.substr(0,
4),16)),l=Math.floor(parseInt(c.substr(4,2),16)),g=Math.floor(parseInt(c.substr(6,2),16)),this._doRequest("PUT",e?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",'{"hue": '+k+', "sat": '+l+', "bri": '+g+', "on": true}',b)):(h=Math.floor(2.54*parseInt(c)),254<h&&(h=254),0>h&&(h=0),d='{"bri": '+h+', "on": true}',0==h&&(d='{"bri": '+h+', "on": false, "transitiontime":0}'),this._doRequest("PUT",e?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",d,b))};
...
{id:c[a].id,status:d.power};h.push(f)}else"brightness"==c[a].status.value&&null!==d.state&&"undefined"!=typeof d.state?(f={id:c[a].id,status:parseInt(d.state)},h.push(f)):"ct"==c[a].status.value&&null!==d.ct&&"undefined"!=typeof d.ct&&(f=parseInt(d.ct),153<=f||500>=f)&&(f={id:c[a].id,status:f},h.push(f))};var k=3,l=b;this.getStates(d,a,function(a){k--;"AUT"==a?(a=Error(a),a.code=1,a.module=xnm.aio.hue.DM.SYS,l&&l(a)):0==k&&(l&&l(null,h),l=null)});this.getGroupStates(d,e,function(a){k--;"AUT"==a?(a=
Error(a),a.code=1,a.module=xnm.aio.hue.DM.SYS,l&&l(a),l=null):0==k&&(l&&l(null,h),l=null)});xnm.aio.hue.Sensor._getAllSensors(this,function(a,b){k--;if(a)l&&l(a),l=null;else{if(b)for(a=0;a<c.length;a++)if(c[a]){var d=c[a].id,e=c[a].device,g=c[a].status;if(d&&g&&g.value&&e&&e.address&&"sensor"==e.category){var f=b[e.address];if(f&&(e=xnm.aio.hue.Sensor._parseSensor(e.address,f))&&(e=e.getStates())){g=e[g.value];if("undefined"==typeof g||null==g)g="?";h.push({id:d,status:g})}}}0==k&&(l&&l(null,h),l=
...
(m=[]),-1==m.indexOf("color_temp")&&m.push("color_temp"));g&&(n||(n={}),n["%rgb%"]||(n["%rgb%"]={value:"rgb",ext:"%s",name:"rgb color"}),n["%white%"]||(n["%white%"]={value:"rgbS",ext:"FFFFFF",name:"rgb color"}),m||(m=[]),-1==m.indexOf("rgb")&&m.push("rgb"));return m&&0!=m.length?this._injectToSmartWidgetTemplate(c,m,b,n,p):null},_injectToSmartWidgetTemplate:function(a,c,b,e,d){var f=[];a||(e={});e||(e={});d||(d={});for(var h=0;h<a.length;h++){var g=a[h];if(g.meta&&-1!=c.indexOf(g.meta.type)&&g.elements){g=
JSON.parse(JSON.stringify(g));var k=!1,l;for(l in g.elements){var n=g.elements[l];if(n&&Array.isArray(n))for(var p=0;p<n.length;p++){var m=n[p];m&&(m.action&&"command"==m.action.type&&m.action.ref&&m.action.ref.value&&(e[m.action.ref.value]?(m.action.ref=e[m.action.ref.value],m.action.device=b,k=!0):m.action=null),m.status&&m.status.ref&&m.status.ref.value&&(d[m.status.ref.value]?(m.status.ref=d[m.status.ref.value],m.status.device=b,k=!0):m.status=null))}}k&&f.push(g)}}return f}};
