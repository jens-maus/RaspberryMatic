...
xnm.aio.hue.Bridge.prototype.executeCommand=function(a,c,b,d){"power"==c&&(c="toggle");if("ct"==a.channel||3<c.length&&"CT:"==c.substr(0,3)){var e=Math.floor(500-3.46*parseInt(c));3<c.length&&"CT:"==c.substr(0,3)&&(e=parseInt(c.substr(3)));500<e&&(e=500);153>e&&(e=153);this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",'{"ct": '+e+', "on": true}',b)}else if("on"==c)this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",JSON.stringify({on:!0}),
...
a.address+"/state",JSON.stringify(e),b)}else if(6==c.length){var k=parseInt(c.substr(0,2),16),m=parseInt(c.substr(2,2),16),n=parseInt(c.substr(4,2),16);e=this._RGBToHSV(k,m,n);var p=Math.floor(e[0]/360*65535+3E3),l=Math.floor(254*e[1]);"osram"===String(a.manufacturer).toLowerCase()&&(p-=3E3);65535<p?p=65535:0>p&&(p=0);254<l?l=254:0>l&&(l=0);f=this;this._doRequest("GET",d?"/groups/"+a.address:"/lights/"+a.address,null,function(c){if(c&&c.error)b&&b(c);else{if(c&&c.data){var e=-1;d&&c.data.action&&
null!=c.data.action.bri&&"undefined"!=typeof c.data.action.bri?e=c.data.action.bri:!d&&c.data.state&&null!=c.data.state.bri&&"undefined"!=typeof c.data.state.bri&&(e=c.data.state.bri);-1!=e&&(c.data.state&&"xy"==c.data.state.colormode?(c=f._RGBToXY(k,m,n),0==e&&(e=10),c=JSON.stringify({xy:c,on:!0,bri:e})):(c='{"hue": '+p+', "sat": '+l+', "on": true}',0==e&&(c='{"hue": '+p+', "sat": '+l+', "bri": 10, "on": true}')),f._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",c,
b),b=null)}b&&b()}})}else 8==c.length?(p=Math.floor(parseInt(c.substr(0,4),16)),l=Math.floor(parseInt(c.substr(4,2),16)),n=Math.floor(parseInt(c.substr(6,2),16)),this._doRequest("PUT",d?"/groups/"+a.address+"/action":"/lights/"+a.address+"/state",'{"hue": '+p+', "sat": '+l+', "bri": '+n+', "on": true}',b)):(g=Math.floor(2.54*parseInt(c)),254<g&&(g=254),0>g&&(g=0),e='{"bri": '+g+', "on": true}',0==g&&(e='{"bri": '+g+', "on": false, "transitiontime":0}'),this._doRequest("PUT",d?"/groups/"+a.address+
"/action":"/lights/"+a.address+"/state",e,b))};xnm.aio.hue.Bridge.prototype.authorizeUser=function(a){this._doRequest("POST",null,'{"username": "'+this._USER+'", "devicetype": "IQONTROL"}',a)};
