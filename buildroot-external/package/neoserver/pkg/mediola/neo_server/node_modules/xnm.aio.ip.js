...
$jscomp.polyfill("Array.prototype.keys",function(b){return b?b:function(){return $jscomp.iteratorFromArray(this,function(a){return a})}},"es6","es3");var xnm=xnm||{};xnm.aio=xnm.aio||{};xnm.aio.ip=xnm.aio.ip||{};
xnm.aio.ip.Http=function(){var b=function(a,c){this._options=a;this._deviceInfo=c};b.prototype.executeCommandCreator=function(a,c,d){this.executeCommand(c.value,function(a){d&&d(a)})};b.prototype.executeCommand=function(a,c){if(a)if(this._options&&this._options.ip){var d=this._getPort();if(d){var b=this._getMethod();if(b){var f=this._buildControlUrl(a);f?(a=this._buildPostData(a),this._doRequest({host:this._options.ip,port:d,path:f,method:b,username:this._options.username,password:this._options.password},
a,function(a,d){c&&c(a,d)})):c&&c(Error("build control url error"))}else c&&c(Error("build method error"))}else c&&c(Error("build port error"))}else c&&c(Error("ip is empty"));else c&&c(Error("command is empty"))};b.prototype._getPort=function(){if(this._options&&this._options.port&&0<this._options.port)return this._options.port;if(!this._deviceInfo||!this._deviceInfo.controlport)return 0;var a=parseInt(this._deviceInfo.controlport);return 0>=a?0:a};b.prototype._getMethod=function(){return this._deviceInfo?
this._deviceInfo.controlpostdata?"POST":this._deviceInfo.controlurl?"GET":null:null};b.prototype._buildControlUrl=function(a){if(!this._deviceInfo||!this._deviceInfo.controlurl)return null;var c=xnm.aio.ip.Util._decodeXml(this._deviceInfo.controlurl);a=this._getCommandCode(a);if(!a)return null;a=xnm.aio.ip.Util._decodeXml(a);return c.replace("%@",a)};b.prototype._buildPostData=function(a){if(!this._deviceInfo||!this._deviceInfo.controlpostdata)return null;var c=xnm.aio.ip.Util._decodeXml(this._deviceInfo.controlpostdata);
a=this._getCommandCode(a);if(!a)return null;a=xnm.aio.ip.Util._decodeXml(a);return c.replace("%@",a)};b.prototype._getCommandCode=function(a){if(!a||!this._deviceInfo||!this._deviceInfo.keys)return null;for(var c=0;c<this._deviceInfo.keys.length;c++){var d=this._deviceInfo.keys[c];if(d.id&&d.id==a)return d.code}return null};b.prototype._doRequest=function(a,c,d){var b={host:a.host,port:a.port,path:a.path,method:a.method,headers:{"Content-Type":'text/plain; charset="utf-8"',Connection:"close"}};a.username&&
(a.password||(a.password=""),b.user=a.username,b.password=a.password,b.headers.Authorization="Basic "+(new Buffer(a.username+":"+a.password)).toString("base64"));c&&(b.headers["Content-Length"]=c.length);a=require("http").request(b,function(a){a.setEncoding("utf-8");var c="";a.on("data",function(a){c+=a});a.on("end",function(){var b;200!=a.statusCode&&(b=Error("http response:"+a.statusCode));d&&d(b,c)})});if(a.on)a.on("error",function(a){d&&d(a)});c&&a.write(c);a.end()};return b}();
...
b.id==a)return b.code}return null};b.prototype._doRequest=function(a,c,b){var d=this,f={port:this._options.port,host:this._options.ip},l=!1,k=!1,g=b,h=require("net").connect(f);h.setTimeout(1E3);c&&"binary"!=c&&h.setEncoding(c);this._logger.log("trace","try to connect tcp to:"+d._options.ip);h.on("connect",function(){d._logger.log("trace","tcp connected to:"+d._options.ip);d._logger.log("trace","write data:"+a.toString());l=!0;h.write(a,c);k=!0});h.on("data",function(a){d._logger.log("trace","receive from tcp:"+
d._options.ip+" data:"+a);k&&h.end()});h.on("error",function(a){l?(d._logger.log("trace","tcp:"+d._options.ip+" error:"+a.message),h.end()):d._logger.log("trace","tcp:"+d._options.ip+" connection error:"+a.message);g&&(g(a),g=null)});h.on("timeout",function(){l?(d._logger.log("trace","tcp:"+d._options.ip+" data timeout"),h.end(),g&&(g(),g=null)):(d._logger.log("trace","tcp:"+d._options.ip+" connection timeout"),h.end(),g&&(g(Error("connection timeout")),g=null))});h.on("close",function(){d._logger.log("trace",
...
xnm.aio.ip.DM=function(){var b=function(a,c){this.code=a;this.message=c;this.stack=Error().stack};b.prototype=Error();b.prototype.name="IpDMError";b.prototype.code=0;b.ERROR_CODE={DEFAULT:0,INDEX_EXISTS:1,ID_EXISTS:2,INVALID:3,UNKNOW:4,NOT_FOUND:5,IP_EXISTS:6};return{SYS:"ip",getIPDeviceType:function(a,c){c=[];if(a&&(a=a.getDevices()))for(var b=0;b<a.length;b++){var e=a[b],f=$(e).getAttributes();if(f&&f.id&&f.name){var l=this._deviceinfoXml2Json($(e)),k=this.getDeviceInfoType(l);k&&(e={sys:this.SYS,
type:k,id:f.id,name:f.name,port:f.controlport},"proxy"==k&&!f.controlport&&(f=this._getProxyDeviceClass(l))&&(f=new f,e.port=f.port),c.push(e))}}return c},createDevice:function(a,c,d){c=b.ERROR_CODE;a?a.type?a.id?a.ip?d(null,a):d(new b(c.INVALID,"ip is invalid")):d(new b(c.INVALID,"id is invalid")):d(new b(c.INVALID,"type is invalid")):d(new b(c.INVALID,"info is invalid"))},updateDevice:function(a,c,d){c=b.ERROR_CODE;a?a.type?a.id?a.ip?d(null,a):d(new b(c.INVALID,"ip is invalid")):d(new b(c.INVALID,
"id is invalid")):d(new b(c.INVALID,"type is invalid")):d(new b(c.INVALID,"info is invalid"))},deleteDevice:function(a,c,b){b()},applyUIFilter:function(a,c,b){var d=function(a,b){if("*"==a)return!0;for(var c=!1,d=0;d<a.length;d++)if(a[d]==b){c=!0;break}return c},f=function(a,c,b){var e=[];switch(b.catagory){case "command":a.command&&a.command.forEach(function(a){d(b.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))});break;case "status":a.status&&a.status.forEach(function(a){d(b.elems,a.type)&&
(a=JSON.parse(JSON.stringify(a)),e.push(a))})}return e},l=function(a,c){var d=[],e=xnm.aio.ip.DM.getCommandStatusMap(a,b);e&&(a=f(e,a,c),d=d.concat(a));return d};if(!a.sys)return null;var k=JSON.parse(JSON.stringify(a)),g=null;switch(c.catagory){case "command":g=l(a,c);k.command=g;break;case "status":g=l(a,c);k.status=g;break;default:return null}return g&&0!=g.length?k:null},getDeviceInfo:function(a,b){if(!a.id||!b)return null;var c;(a=b.find('device[id="'+a.id+'"]'))&&0<a.length&&(c=a[0]);c=c?$(c):
null;return c?this._deviceinfoXml2Json(c):null},getDeviceInfoType:function(a){if("samsung_tv"==a.id||a.ircccontrolurl||a.pnccontrolurl)return"proxy";if(a.tcptype){if("text"==a.tcptype||"textnl"==a.tcptype||"pioneer"==a.tcptype||"binary"==a.tcptype)return"tcp";if("generic"!=a.tcptype)return"proxy"}return a.udptype?"udp":a.controlurl?"http":null},getCommandStatusMap:function(a,c){a=this.getDeviceInfo(a,c);if(!a)return null;c={command:[]};if(a.keys)for(var b=0;b<a.keys.length;b++){var e=a.keys[b];c.command.push({type:"enum",
name:e.id,ref:{value:e.id}})}return c},_getProxyDeviceClass:function(a){var b=null;"samsung_tv"==a.id?b=require("xnm.aio.samsung.js").SamsungTV:a.ircccontrolurl?b=require("xnm.aio.sony.js").IRCC:a.pnccontrolurl?b=require("xnm.aio.panasonic.js").TV:"onkyo"==a.tcptype?b=require("xnm.aio.onkyo.js").AVReceiver:"lgav"==a.tcptype?b=require("xnm.aio.lg.js").BDP:"panasonicbdp"==a.tcptype?b=require("xnm.aio.panasonic.js").BDP:"lgtv"==a.tcptype?b=require("xnm.aio.lg.js").TV:"philips2011"==a.tcptype&&(b=require("xnm.aio.philips.js").JointSpaceTV);
