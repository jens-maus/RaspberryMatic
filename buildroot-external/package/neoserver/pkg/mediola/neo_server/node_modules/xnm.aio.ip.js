...
$jscomp.initSymbol=function(){};$jscomp.polyfill("Symbol",function(d){if(d)return d;var a=function(f,h){this.$jscomp$symbol$id_=f;$jscomp.defineProperty(this,"description",{configurable:!0,writable:!0,value:h})};a.prototype.toString=function(){return this.$jscomp$symbol$id_};var b="jscomp_symbol_"+(1E9*Math.random()>>>0)+"_",c=0,e=function(f){if(this instanceof e)throw new TypeError("Symbol is not a constructor");return new a(b+(f||"")+"_"+c++,f)};return e},"es6","es3");
...
c.id==a)return c.code}return null};d.prototype._doRequest=function(a,b,c){var e=this,f={port:this._options.port,host:this._options.ip},h=!1,l=!1,k=c,g=require("net").connect(f);g.setTimeout(1E3);b&&"binary"!=b&&g.setEncoding(b);this._logger.log("trace","try to connect tcp to:"+e._options.ip);g.on("connect",function(){e._logger.log("trace","tcp connected to:"+e._options.ip);e._logger.log("trace","write data:"+a.toString());h=!0;g.write(a,b);l=!0});g.on("data",function(m){e._logger.log("trace","receive from tcp:"+
e._options.ip+" data:"+m);l&&g.end()});g.on("error",function(m){h?(e._logger.log("trace","tcp:"+e._options.ip+" error:"+m.message),g.end()):e._logger.log("trace","tcp:"+e._options.ip+" connection error:"+m.message);k&&(k(m),k=null)});g.on("timeout",function(){h?(e._logger.log("trace","tcp:"+e._options.ip+" data timeout"),g.end(),k&&(k(),k=null)):(e._logger.log("trace","tcp:"+e._options.ip+" connection timeout"),g.end(),k&&(k(Error("connection timeout")),k=null))});g.on("close",function(){e._logger.log("trace",
"tcp:"+e._options.ip+" close socket");k&&(k(),k=null)});g._doConnect&&"function"==typeof g._doConnect&&g._doConnect()};return d}();
...
xnm.aio.ip.DM=function(){var d=function(a,b){this.code=a;this.message=b;this.stack=Error().stack};d.prototype=Error();d.prototype.name="IpDMError";d.prototype.code=0;d.ERROR_CODE={DEFAULT:0,INDEX_EXISTS:1,ID_EXISTS:2,INVALID:3,UNKNOW:4,NOT_FOUND:5,IP_EXISTS:6};return{SYS:"ip",getIPDeviceType:function(a,b){b=[];if(a&&(a=a.getDevices()))for(var c=0;c<a.length;c++){var e=a[c],f=$(e).getAttributes();if(f&&f.id&&f.name){var h=this._deviceinfoXml2Json($(e)),l=this.getDeviceInfoType(h);l&&(e={sys:this.SYS,
type:l,id:f.id,name:f.name,port:f.controlport},"proxy"==l&&!f.controlport&&(f=this._getProxyDeviceClass(h))&&(f=new f,e.port=f.port),b.push(e))}}return b},createDevice:function(a,b,c){b=d.ERROR_CODE;a?a.type?a.id?a.ip?c(null,a):c(new d(b.INVALID,"ip is invalid")):c(new d(b.INVALID,"id is invalid")):c(new d(b.INVALID,"type is invalid")):c(new d(b.INVALID,"info is invalid"))},updateDevice:function(a,b,c){b=d.ERROR_CODE;a?a.type?a.id?a.ip?c(null,a):c(new d(b.INVALID,"ip is invalid")):c(new d(b.INVALID,
...
(q=JSON.parse(JSON.stringify(q)),p.push(q))})}return p},h=function(g,m){var n=[],p=xnm.aio.ip.DM.getCommandStatusMap(g,c);p&&(g=f(p,g,m),n=n.concat(g));return n};if(!a.sys)return null;var l=JSON.parse(JSON.stringify(a)),k=null;switch(b.catagory){case "command":k=h(a,b);l.command=k;break;case "status":k=h(a,b);l.status=k;break;default:return null}return k&&0!=k.length?l:null},getDeviceInfo:function(a,b){if(!a.id||!b)return null;var c;(a=b.find('device[id="'+a.id+'"]'))&&0<a.length&&(c=a[0]);c=c?$(c):
