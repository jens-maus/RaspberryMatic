...
xnm.aio.kt.Socket.prototype._sendMessage=function(b,a){for(var c=[1,0],e=this.uuid.replace(/-/g,""),d=0;6>d;d++)c.push(parseInt(e.substr(2*d,2),16));c.push(b.length+7);c.push(0);c.push(0);c.push(0);c.push(this._manuCode);c.push(this._authCode[0]);c.push(this._authCode[1]);c.push(this._deviceType);for(d=0;d<b.length;d++)c.push(b[d]);b=new Buffer(c);console.log("send packet",b.toString("hex"));var f=require("dgram").createSocket("udp4");f.send(b,0,b.length,this.port,this.ip,function(g,h){f.close();
a&&setTimeout(function(){a.call()},1E3)})};xnm.aio.kt.Socket.prototype.getState=function(b){this._sendMessage([2,0,0,0,0]);var a=this;setTimeout(function(){b(a._state)},3E3)};xnm.aio.kt.Socket.prototype.executeCommand=function(b,a,c){var e=this,d=[1,0,0,0,255];if("on"==a)d[3]=255;else if("toggle"==a){this.getState(function(f){"on"==f||"off"==f?("off"==f&&(d[3]=255),e._sendMessage(d,function(){e._sendMessage(d,c)})):c&&c.call()});return}e._sendMessage(d,function(){e._sendMessage(d,c)})};
xnm.aio.kt.Socket.prototype.getStates=function(b,a,c){var e=this;this.CommandHandler=b;this.getState(function(d){var f={state:d};a&&e.CommandHandler.setState?e.CommandHandler.setState(a.id,d):e.CommandHandler.receivedState&&e.CommandHandler.receivedState("kt",e.uuid,f);c&&c(f)})};xnm.aio.kt.Socket.prototype.getStateValues=function(b){return b};
xnm.aio.kt.Discovery=function(){this.Devices={};this.callback=null;this._sockets=[];var b=require("os");require("dgram");if(b&&b.networkInterfaces){b=b.networkInterfaces();for(var a in b)for(var c=b[a],e=0;e<c.length;e++)this._addDiscoverSocket(28530,c[e])}else this._addDiscoverSocket(28530,null)};xnm.aio.kt.Discovery.prototype._getMAC=function(b){for(var a="",c=0;5>c;c++)a+=XC.getHexVal(b[c],2).toUpperCase()+"-";return a+=XC.getHexVal(b[c],2).toUpperCase()};
...
xnm.aio.kt.Discovery.prototype._addDiscoverSocket=function(b,a){var c=require("dgram"),e=this;if(a)"IPv4"==a.family&&0==a.internal&&(d=c.createSocket("udp4"),d.on("listening",function(){d.setBroadcast(!0)}),d.on("message",function(f,g){e._receivedData(g.address,f)}),d.bind(b,a.address),this._sockets.push(d));else{var d=c.createSocket("udp4");d.on("message",function(f,g){e._receivedData(g.address,f)});d.bind(b);this._sockets.push(d)}};
