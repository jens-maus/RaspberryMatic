...
xnm.aio.m10T.STMUpdater.prototype._startDFUUpdate_DFUUtil=function(d,c,a){this._callback=c;this._bytesWritten=this._bytesToWrite=0;if(!require("fs").existsSync(this._file)&&c)c({error:"invalid dfu file",state:0}),this._callback=c=null;else{c='"'+d+'" -t 1024 -d 0483:df11 -a 0 -s 0x08000000:leave -D '+this._file;a&&"v6p"==a&&(c='"'+d+'" -d 0483:df11 -a 0 -s 0x08000000:leave -D '+this._file);d=require("child_process").exec;d=d(c);var b=0,e=this;d.stdout.on("data",function(a){a=""+a;if("."==a)e._bytesWritten+=
1024,e._callback&&(b=Math.floor(e._bytesWritten/e._bytesToWrite*100),e._callback({error:null,state:b}));else if(a&&1<a.length&&"-"==a.charAt(0))b=Number(a.substr(1)),e._callback&&100!=b&&e._callback({error:null,state:b});else if(a&&1===a.indexOf("Download"))a=a.split("%")[0].split(" "),b=Number(a[a.length-1]),e._callback&&!isNaN(b)&&100!=b&&e._callback({error:null,state:b});else if(a){var c=a.indexOf(", size = ");-1<c&&(e._bytesToWrite=parseInt(a.substr(c+9)))}});d.on("error",function(a){e._callback&&
(e._callback({error:"failed to start dfu-util",state:0}),e._callback=null)});d.on("close",function(a){0==a||74==a&&100==b?e._callback&&(e._callback({error:null,state:100}),e._callback=null):e._callback&&(e._callback({error:"failed to flash via dfu-util",state:0}),e._callback=null)})}};
...
xnm.aio.m10T.ESPUpdater.prototype.startSerialUpdate=function(d,c,a){this._callback=c;this._blockSize=1024;this._bytesSent=0;var b=this;require("fs").stat(this._file,function(e,f){if(e)c&&c({error:"invalid update file",state:0});else{b._filesize=b._fixedSize?b._fixedSize:f.size;var g=new xnm.aio.m10T.STMUSBSerial,h=!1,k=!1,l=0,n=[192,0,8,36,0,221,0,0,0,7,7,18,32];for(e=0;32>e;e++)n.push(85);n.push(192);var p=new Buffer([]);g.on("error",function(a){g.close();if(c){var b="serial port error";a&&a.message&&
(b+=" ("+a.message+")");c({error:b,state:0})}});g.on("data",function(d){var e=!1;p=Buffer.concat([p,d]);var f=-1,l=null;for(d=0;d<p.length;d++)if(192==p[d])if(0>f)f=d;else if(l=p.slice(f,f+d+1),12==l.length&&0==l[9]&&0==l[10])if(!h&&8==l[2]||!k&&2==l[2]||3==l[2]){p=new Buffer([]);g.flush();e=!0;break}else f=d;else f=d;b._connTimeoutTimer&&(clearTimeout(b._connTimeoutTimer),b._connTimeoutTimer=null);h||k||(b._connTimeoutTimer=setTimeout(function(){h||k||(g.close(),c&&c({error:"communication error",
state:0}))},2E3));if(e)if(h||k)if(h=!0,k?b._bytesSent+=b._blockSize:k=!0,b._bytesSent<b._filesize)b._callback&&b._callback({error:null,state:Math.floor(b._bytesSent/b._filesize*100)}),e=Math.round(b._bytesSent/b._blockSize),b._bytesSent+b._blockSize>b._filesize&&(b._blockSize=b._filesize-b._bytesSent),b._getNextSerialFlashData(e,function(a){a?(b._lastCRC=a[5],setTimeout(function(){g.write(a,500)},5)):b._callback&&b._callback({error:"failed to read update file",state:0})});else{var n=[192,0,4,4,0,
b._lastCRC,0,0,0,1,0,0,0,192];setTimeout(function(){g.write(n);setTimeout(function(){b._callback&&b._callback({error:null,state:100});g.close()},250)},50)}else{h=!0;var q=[192,0,2,16,0,26,0,0,0,0,240,3,0,0,2,0,0,0,4,0,0,0,0,0,0,192];q[11]=7;q[10]=0;1==a&&(q[11]=63);e=239;for(d=10;d<q.length-1;d++)e^=q[d];q[5]=e;setTimeout(function(){g.write(q,2E4)},20)}});g.on("timeout",function(){h?(c&&c({error:"serial timeout",state:0}),g.close()):5>l?(g.write(n,500),l++):(g.close(),c&&c({error:"failed to connect",
state:0}))});g.open(d,function(){g.write([31,48,0,4]);setTimeout(function(){g.write(n,500)},250)})}})};
...
xnm.aio.m10T.V5Updater.prototype.packFile=function(d,c,a,b,e,f,g){var h=require("fs"),k=this;k.getFileInfos(a,function(l,n){l?g&&g("invalid file"):k.getFileInfos(b,function(l,k){if(l)g&&g("invalid file");else{l="HEADER@esp_"+(e+"_"+d+"_0_"+n.size+"-"+n.md5)+"@st_"+(e+"_"+c+"_"+n.size+"_"+k.size+"-"+k.crc32)+"@HEADER\r\n";h.existsSync(f)&&h.unlinkSync(f);var p=h.createWriteStream(f);p.write(l);var m=h.createReadStream(a);m.on("end",function(){m=h.createReadStream(b);m.on("end",function(){p.end();g&&
g()});m.pipe(p,{end:!1})});m.pipe(p,{end:!1})}})})};
xnm.aio.m10T.V5Updater.prototype.packFileV5Plus=function(d,c,a,b,e,f,g){var h=require("fs");h.stat(a,function(k,l){k?g&&g("invalid file"):h.stat(b,function(k,p){if(k)g&&g("invalid file");else{k=0;var n="a20_"+e+"_"+d+"_"+k+"_"+l.size;k+=l.size;p="HEADER@"+n+"@st_"+(e+"_"+c+"_"+k+"_"+p.size)+"@HEADER\r\n";h.existsSync(f)&&h.unlinkSync(f);var r=h.createWriteStream(f);r.write(p);var m=h.createReadStream(a);m.on("end",function(){m=h.createReadStream(b);m.on("end",function(){r.end();g&&g()});m.pipe(r,
{end:!1})});m.pipe(r,{end:!1})}})})};
xnm.aio.m10T.V5Updater.prototype.updateFirmware=function(d,c,a,b){var e=require("fs"),f=this;e.existsSync(c)?e.stat(c,function(g,h){g||e.readFileLine(c,1,function(e,h){var g=!1;if(!e&&h){var k=f.parseHeader(h);if(k){e=null;f.useMD5&&(e=k.st[5]);var l=new xnm.aio.m10T.STMUpdater(c,k.offset+k.st[3],k.st[4],e);l.at=f.at;l.auth=f.auth;l.startNetUpdate(d,function(e){e.error&&a?a(e):(a&&a({error:null,state:Math.floor(e.state/2)}),100==e.state&&(e=null,f.useMD5&&(e=k.esp[5]),l=new xnm.aio.m10T.ESPUpdater(c,
k.offset,k.esp[4],e),l.startNetUpdate(d,8081,function(b){b.error&&a?a(b):a&&a({error:null,state:Math.floor(50+b.state/2)})},b)))});g=!0}}!g&&a&&a({error:"[04] invalid firmware file",state:0})})}):a&&a({error:"[02] file does not exist",state:0})};
xnm.aio.m10T.V5Updater.prototype.updateFirmwareInReverseOrder=function(d,c,a,b){var e=require("fs"),f=this;e.existsSync(c)?e.stat(c,function(g,h){g?a&&a({error:g.message,state:0}):e.readFileLine(c,1,function(e,h){var g=!1;if(!e&&h){var k=f.parseHeader(h);if(k){e=null;f.useMD5&&(e=k.esp[5]);var l=new xnm.aio.m10T.ESPUpdater(c,k.offset,k.esp[4],e);l.startNetUpdate(d,8081,function(b){if(b&&b.error)a&&a(b);else{var e=Math.floor(b.state/2);a&&a({error:null,state:e});if(100===b.state){var h=function(){var b=
null;f.useMD5&&(b=k.st[5]);l=new xnm.aio.m10T.STMUpdater(c,k.offset+k.st[3],k.st[4],b);l.at=f.at;l.auth=f.auth;l.startNetUpdate(d,function(b){b&&b.error?a&&a(b):(b=Math.floor(50+b.state/2),a&&a({error:null,state:b}))})};setTimeout(function(){h()},9E4)}}},b);g=!0}}g||a&&a({error:"[04] invalid firmware file",state:0})})}):a&&a({error:"[02] file does not exist",state:0})};
...
xnm.aio.m10T.V5Updater.prototype._isDFUConnected_STM32=function(d,c,a){var b=require("child_process").exec;d=b('"'+d+'" -l usb');var e="";Array.isArray(c)||(c=[c]);d.stdout.on("data",function(a){a&&(e+=a.toString())});d.on("error",function(a){XC.debugf("STM32 tool error: "+a,"error")});d.on("close",function(b){b=!1;var d=null,f=null;XC.debugf(e);for(var k=0;k<c.length;k++){var l=c[k];if(-1<e.indexOf("Device ID              : "+l)){b=!0;d=l;k=e.split("\n");for(l=0;l<k.length;l++){var n=k[l];if(0<=
n.indexOf("Device Index           : ")){f=n.split(":");f=f[f.length-1].trim();f=f.toLowerCase();break}}break}}a&&a(b,d,f)})};xnm.aio.m10T.V5Updater.prototype.waitForDFUConnect=function(d,c,a,b){var e=this;this.isDFUConnected(d,c,function(f,g,h){f?b&&b(!0,g,h):0<a?setTimeout(function(){e.waitForDFUConnect(d,c,a-.5,b)},500):b&&b(!1,g,h)})};
xnm.aio.m10T.V5Updater.prototype.waitForSerialConnection=function(d,c,a){if(this._useChromeAPI)this._chromeWaitForSerialConnection(d,c,a);else{var b=this,e=function(e,f){void 0==f&&(f=[]);if(a&&f&&a.length<f.length)for(e=0;e<f.length;e++){for(var h=!0,g=0;g<a.length;g++)if(f[e].comName==a[g].comName){h=!1;break}if(h){var n=f[e].comName;setTimeout(function(){c&&c(n)},1E3)}}else 0<d?setTimeout(function(){b.waitForSerialConnection(d-.5,c,f)},500):c&&c(null)},f=require("serialport");"undefined"!==typeof nw&&
8<=String(nw.App.manifest.dependencies.serialport).split(".")[0].replace(/[><=~^]*/,"")?f.list().then(function(a){e(null,a)},function(a){e(a,null)}):f.list(e)}};
...
xnm.aio.m10T.V6PUpdater=function(){var d=function(c,a,b,d){this._logger=null;try{this._logger=require("x.logger.js").getLogger("xnm.aio.m10T.V6PUpdater")}catch(g){var e=this;this._logger={log:function(){console.log.apply(e,arguments)}}}this._gatewayJSON=c;this._cloud=this._gateway=this._updateFileUrl=null;this._listeners={};this._state=0;this._gatewayRebootTimeout=a?a:3E5;this._updateFirmwareMessageTimeout=b?b:18E4;this._wsClz=d?d:require("ws")};d.prototype.on=function(c,a){c&&a&&(this._listeners[c]||
...
c;this._onStart();if(this._gateway=require("xnm.aio.gateway.js").resolveGateway(this._gatewayJSON)){this._gateway.timeout=2E3;var b=this;this._onStage(1,"get gateway info");this._logger.log("debug","get gateway info");this._getGatewayInfo(function(a){b._logger.log("debug","get gateway info"+(a?" failed:"+a.message:" success"));if(a)b._onError(Error("get gateway info failed:"+a.message));else{var c=b._gateway._mac;b._onStage(2,"submit update file url to gateway");b._logger.log("debug","submit update file url to gateway["+
c+"]");b._submitUpdateFileUrl(function(a){b._logger.log("debug","submit update file url to gateway["+c+"]"+(a?" failed:"+a.message:" success"));a?b._onError(Error("submit update file url to gateway failed:"+a.message)):(b._onStage(3,"gateway reboot to recovery mode"),b._logger.log("debug","wait gateway["+c+"] reboot to recovery mode"),b._gatewayRebootToRecovery(function(a){b._logger.log("debug","gateway["+c+"] reboot to recovery mode"+(a?" failed:"+a.message:" success"));a?b._onError(Error("gateway reboot to recovery mode failed:"+
a.message)):(b._onStage(4,"update firmware"),b._logger.log("debug","update firmware"),b._updateFirmware(function(a){b._logger.log("debug","update firmware"+(a?" failed:"+a.message:" success"));a?b._onError(Error("update firmware failed:"+a.message)):(b._onStage(5,"gateway reboot to normal mode"),b._logger.log("debug","wait gateway["+c+"] reboot to normal mode"),b._gatewayRebootToNormal(function(a){b._logger.log("debug","gateway["+c+"] reboot to normal mode"+(a?" failed:"+a.message:" success"));a?
b._onError(Error("gateway reboot to normal mode failed:"+a.message)):b._onEnd()}))}))}))})}})}else this._logger.log("debug","invalid gateway json"),this._onError(Error("invalid gateway json"))};d.prototype._getGatewayInfo=function(c){var a=this;this._gateway._getInfo(function(b,d){b?c(b):d&&d.mac?(a._gateway._mac=d.mac,d.mediola_mac&&(a._gateway._mac=d.mediola_mac),c()):c(Error("invalid gateway info"))})};d.prototype._submitUpdateFileUrl=function(c){this._gateway._executeRequest("/update",{url:this._updateFileUrl},
{timeout:3E4},function(a){c(a)})};d.prototype._gatewayRebootToRecovery=function(c){var a=!0,b=this,d=function(){a&&(b._logger.log("trace","start search gateway"),require("xnm.aio.gateway.js").discoverWithTimeout(5E3,null,function(c,e){if(a){if(!c&&e&&e.length)for(b._logger.log("trace","find gateways:"+JSON.stringify(e.map(function(a){return{ip:a._ip,mac:a._mac,mode:a._mode,hwv:a._hwv}}))),c=0;c<e.length;c++){var f=e[c];f&&f._mac==b._gateway._mac&&0==f._mode&&f._ip!=b._gateway._ip&&(b._gateway._ip=
f._ip,b._gateway._port=f._port)}d()}}))},f=function(c,d){a?c._getInfo(function(e,g){b._logger.log("trace","get gateway["+c._mac+"] - "+c._ip+" info "+(e?"failed:"+e.message:"success"));!e&&g&&b._logger.log("trace","get gateway["+c._mac+"] - "+c._ip+" info:"+JSON.stringify(g));e?setTimeout(function(){f(c,d)},500):g&&0==g.mode?(e=g.mac,g.mediola_mac&&(e=g.mediola_mac),e&&-1!=e.indexOf(":")&&(e=e.replace(/:/g,"-")),e!=c._mac?setTimeout(function(){f(c,d)},500):(b._logger.log("trace","gateway["+c._mac+
"] - "+c._ip+" in recovery mode"),a=!1,d())):setTimeout(function(){f(c,d)},500)}):d(Error("get info from gateway in recovery mode failed"))},g=setTimeout(function(){a=!1;g=null;c&&(c(Error("wait timeout")),c=null)},this._gatewayRebootTimeout);d();f(this._gateway,function(a){g&&(clearTimeout(g),g=null);c&&(c(a),c=null)})};d.prototype._updateFirmware=function(c){var a=!1,b=null,d=null,f=this,g=new this._wsClz("ws://"+this._gateway._ip+":"+this._gateway._port+"/ws"),h=function(){b&&(clearTimeout(b),
b=null);b=setTimeout(function(){f._logger.log("trace","websocket message timeout");var a=b=null;c&&(a=c,c=null);g.close();a&&a(Error("firmware update message timeout"))},f._updateFirmwareMessageTimeout)};g.on("open",function(){f._logger.log("trace","websocket open");a=!0;h()});g.on("message",function(a){f._logger.log("trace","websocket message:"+a);h();try{var e=JSON.parse(a);if(e)switch(e.type){case "status":switch(e.status){case "START":d=0;break;case "SUCCESS":d=100}break;case "step":var k=parseInt(e.number),
p=(parseInt(e.step)-1)/k*100,q=100/e.number,r=parseInt(e.percent);d=q*r/100+p;d=Math.round(d);f._logger.log("debug","update firmware progress:"+d+" name:"+e.name)}c&&f._onStage(4,"update firmware",d,e);if("status"==e.type&&"DONE"==e.status){b&&(clearTimeout(b),b=null);var m=null;c&&(m=c,c=null);g.close();m&&m()}else"status"==e.type&&"FAILURE"==e.status&&(b&&(clearTimeout(b),b=null),m=null,c&&(m=c,c=null),g.close(),m&&m(Error("firmware update failure")))}catch(t){}});g.on("close",function(b,d){f._logger.log("trace",
"websocket closed");!a&&c&&(c(Error("enable to connect websocket, directly closed")),c=null)});g.on("error",function(b){f._logger.log("trace","websocket error:"+b.message);!a&&c&&(c(b),c=null)});g.on("pong",function(){f._logger.log("trace","receive pong")});g.on("ping",function(){f._logger.log("trace","receive ping");g.pong()})};d.prototype._gatewayRebootToNormal=function(c){var a=!0,b=this,d=function(){a&&(b._logger.log("trace","start search gateway"),require("xnm.aio.gateway.js").discoverWithTimeout(5E3,
null,function(c,e){if(a){if(!c&&e&&e.length)for(b._logger.log("trace","find gateways:"+JSON.stringify(e.map(function(a){return{ip:a._ip,mac:a._mac,mode:a._mode,hwv:a._hwv}}))),c=0;c<e.length;c++){var f=e[c];f&&f._mac==b._gateway._mac&&0!=f._mode&&f._ip!=b._gateway._ip&&(b._gateway._ip=f._ip,b._gateway._port=f._port)}d()}}))},f=function(c,d){a?c._getInfo(function(e,g){b._logger.log("trace","get gateway["+c._mac+"] - "+c._ip+" info "+(e?"failed:"+e.message:"success"));!e&&g&&b._logger.log("trace","get gateway["+
c._mac+"] - "+c._ip+" info:"+JSON.stringify(g));e?setTimeout(function(){f(c,d)},500):g&&0!=g.mode?(e=g.mac,g.mediola_mac&&(e=g.mediola_mac),e&&-1!=e.indexOf(":")&&(e=e.replace(/:/g,"-")),e!=c._mac?setTimeout(function(){f(c,d)},500):(b._logger.log("trace","gateway["+c._mac+"] - "+c._ip+" in normal mode"),a=!1,d())):setTimeout(function(){f(c,d)},500)}):d(Error("get info from gateway in normal mode failed"))},g=setTimeout(function(){a=!1;g=null;c&&(c(Error("wait timeout")),c=null)},this._gatewayRebootTimeout);
d();f(this._gateway,function(a){g&&(clearTimeout(g),g=null);c&&(c(a),c=null)})};d.prototype._emit=function(){var c=arguments[0],a=Array.prototype.slice.call(arguments);a=a.slice(1);if(c=this._listeners[c])for(var b=0;b<c.length;b++){var d=c[b];try{d.apply(this,a)}catch(f){}}};d.prototype._onStart=function(){this._state=1;this._emit("start")};d.prototype._onStage=function(c,a,b,d){this._state=c;this._emit("stage",{stage:c,message:a,progress:b,info:d})};d.prototype._onError=function(c){this._emit("error",
