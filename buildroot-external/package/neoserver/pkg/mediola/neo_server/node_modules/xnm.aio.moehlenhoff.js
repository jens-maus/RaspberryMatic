...
!b.Devices)return null;if(b.Devices.Device)return b.Devices.Device.ID==a?b.Devices.Device:null;if(b.Devices.Devices){b=b.Devices.Devices;for(var c=0;c<b.length;c++){var e=b[c];if(e.ID==a)return e}}return null},f=this;b=function(a){for(var b=[],c=0;c<a.length;c++){var e=a[c],f=e.address.split(":");2<=f.length&&b.push({id:e.id,type:e.type,address:f[0],dpt:e.data,number:f[1]})}return b}(b);this._getInfo(function(d,g){if(d)c&&c("Error");else{var h,k;if(b&&a.setState)for(d=0;d<b.length;d++){var m=b[d];
if(h=e(m.address,g))switch(m.dpt){case "HEATAREA":var n;a:{if(h&&h.HEATAREAs)for(n=0;n<h.HEATAREAs.length;n++){var p=h.HEATAREAs[n];if(p.NUMBER==m.number){n=p;break a}}n=null}if(n)for(k in h=f._getStateValues(m,n,h),h)h.hasOwnProperty(k)&&a.setState(m.id+":"+k,h[k])}}c&&c("OK")}})};d.prototype._executeCommand=function(a,b,c){if(a)if(a.address)if(b){var e='<?xml version="1.0" encoding="UTF-8"?><Devices><Device><ID>'+a.address+"</ID>%subdev</Device></Devices>";"vac_off"==b&&(e=e.replace("%subdev","<VACATION><VACATION_STATE>0</VACATION_STATE><START_DATE>2000-00-00</START_DATE><END_DATE>2000-00-00</END_DATE></VACATION>"));
switch(a.dpt){case "HEATAREA":if(null==a.number||"undefined"==typeof a.number){c&&c(Error("device number invalid"));return}a='<HEATAREA nr="'+a.number+'">%cmd</HEATAREA>';e=e.replace("%subdev",a);switch(b){case "auto":b="<HEATAREA_MODE>0</HEATAREA_MODE>";break;case "day":b="<HEATAREA_MODE>1</HEATAREA_MODE>";break;case "night":b="<HEATAREA_MODE>2</HEATAREA_MODE>";break;default:if("string"==typeof b&&b.match(/^w_p/))b="<PROGRAM_WEEK>"+b.replace("w_p","")+"</PROGRAM_WEEK>";else if("string"==typeof b&&
b.match(/^we_p/))b="<PROGRAM_WEEKEND>"+b.replace("we_p","")+"</PROGRAM_WEEKEND>";else{var f;"number"==typeof b?f=b/5:b.match(/^[-+]?[0-9]*\.?[0-9]+$/)?f=parseFloat(b/5):c&&c(Error("no such command"));5>f&&(f=5);30<f&&(f=30);b="<T_TARGET>"+f+"</T_TARGET>"}}b&&(e=e.replace("%cmd",b))}this._doRequest({path:"/data/changes.xml",method:"POST"},e,function(a){c&&c(a)})}else c&&c(Error("command invalid"));else c&&c(Error("device address invalid"));else c&&c(Error("device address format invalid"))};d.prototype._getStates=
function(a){var b=this;this._getInfo(function(c,e){c?a&&a("Error"):(c=b._resolveInfo(e),a&&a(null,c))})};d.prototype._resolveInfo=function(a){if(!a||!a.Devices)return null;var b,c=[];a.Devices.Device&&(b=this._resolveDevice(a.Devices.Device))&&c.push(b);if(a.Devices.Devices){a=a.Devices.Devices;for(var e=0;e<a.length;e++)(b=this._resolveDevice(a[e]))&&c.push(b)}return c};d.prototype._resolveDevice=function(a){if(!a)return null;var b=this._getStateValues(null,null,a);if(!b)return null;b.id=a.ID;b.heatAreas=
[];if(a.HEATAREAs)for(var c=0;c<a.HEATAREAs.length;c++){var e=a.HEATAREAs[c],f=this._getStateValues(null,e,a);f&&(f.number=e.NUMBER,b.heatAreas.push(f))}return b};d.prototype.getDevices=function(a){this._getDevices(function(b,c){if(b)a&&a(b);else if(c){for(var e in c)if(c.hasOwnProperty(e)){b=c[e];for(var f=0;f<b.length;f++)delete b[f]._deviceName}a&&a(null,c)}else a&&a(null,{})})};d.prototype._getDevices=function(a){this._getInfo(function(b,c){if(b)a&&a(b);else if(b={},c.Devices&&c.Devices.Devices){c=
c.Devices.Devices;for(var e=0;e<c.length;e++){var f=c[e];if(f.ID){b[f.ID]=[];var d=f.NAME,g={};if(f.IODEVICEs)for(var h=0;h<f.IODEVICEs.length;h++){var l=f.IODEVICEs[h];l.HEATAREA_NR&&(g[l.HEATAREA_NR]=l)}if(f.HEATAREAs)for(h=0;h<f.HEATAREAs.length;h++)if(l=f.HEATAREAs[h],l.NUMBER){if(g[l.NUMBER]){var m=g[l.NUMBER].IODEVICE_TYPE;switch(m){case "1":case "3":case "4":m="analog";break;default:m="digital"}}b[f.ID].push({type:"alpha2",address:f.ID,dpt:"HEATAREA",name:l.HEATAREA_NAME,number:l.NUMBER,iodevice:m,
_deviceName:d})}}}a&&a(null,b)}else a&&a(Error("info format invalid"))})};d.prototype._getStateValues=function(a,b,c){a={};var e="\u00b0C",f={};if(c){if(c.VACATION){switch(c.VACATION.VACATION_STATE){case "0":a.vacation="off";break;default:a.vacation="on"}c.VACATION.START_DATE&&(a.vacation_start_date=c.VACATION.START_DATE);c.VACATION.START_TIME&&(a.vacation_start_time=c.VACATION.START_TIME);c.VACATION.END_DATE&&(a.vacation_end_date=c.VACATION.END_DATE);c.VACATION.END_TIME&&(a.vacation_end_time=c.VACATION.END_TIME)}c.TEMPERATUREUNIT&&
"0"!=c.TEMPERATUREUNIT&&(e="\u00b0F");if(c.PROGRAM&&c.PROGRAM.SHIFT_PROGRAMs)for(var d=c.PROGRAM.SHIFT_PROGRAMs,g=0;g<d.length;g++){var h=d[g],l=h.NUMBER,m=h.START;h=h.END;l&&m&&h&&(a["prog_"+l]=a["prog_"+l]?a["prog_"+l]+(","+m+"-"+h):m+"-"+h)}if(c.IODEVICEs)for(d=0;d<c.IODEVICEs.length;d++)g=c.IODEVICEs[d],g.HEATAREA_NR&&(f[g.HEATAREA_NR]=g)}if(b){switch(b.HEATAREA_MODE){case "0":a.mode="auto";break;case "1":a.mode="day";break;case "2":a.mode="night"}b.T_ACTUAL&&(a.actual_temp=parseFloat(b.T_ACTUAL),
a.actual_temp_text=parseFloat(b.T_ACTUAL)+e);b.T_TARGET&&(a.target_temp=parseFloat(5*b.T_TARGET),a.target_temp_text=parseFloat(b.T_TARGET)+e);b.PROGRAM_WEEK&&(a.prog_week_0="inactive",a.prog_week_1="inactive",a.prog_week_2="inactive",a.prog_week_3="inactive",a["prog_week_"+parseInt(b.PROGRAM_WEEK)]="active");b.PROGRAM_WEEKEND&&(a.prog_weekend_0="inactive",a.prog_weekend_1="inactive",a.prog_weekend_2="inactive",a.prog_weekend_3="inactive",a["prog_weekend_"+parseInt(b.PROGRAM_WEEKEND)]="active");if(f&&
f[b.NUMBER])switch(f[b.NUMBER].IODEVICE_TYPE){case "1":case "3":case "4":a.iodevice="analog";break;default:a.iodevice="digital"}}return a};d.prototype._getInfo=function(a){var b=this;this._doRequest({path:"/data/static.xml",method:"GET"},null,function(c,e){if(c)a&&a(c);else try{var f=$($.parseXML(e)),d=b._xml2json(f);a&&a(null,d)}catch(g){a&&a(g)}})};d.prototype._xml2json=function(a){var b=a.children();if(b&&0<b.length){a={};for(var c=0;c<b.length;c++){var e=$(b[c]),f=e.prop("tagName"),d=this._xml2json(e);
if("HEATAREA"==f||"HEATCTRL"==f||"IODEVICE"==f){var g=e.attr("nr");null!=g&&"undefined"!=typeof g&&(d.NUMBER=g)}"SHIFT_PROGRAM"==f&&(g=e.attr("nr"),e=e.attr("shiftingtime"),null!=g&&"undefined"!=typeof g&&(d.NUMBER=g),null!=e&&"undefined"!=typeof e&&(d.SHIFINGTIME=e));"Device"==f||"HEATAREA"==f||"HEATCTRL"==f||"IODEVICE"==f?(a[f+"s"]||(a[f+"s"]=[]),a[f+"s"].push(d)):a[f]&&!a[f+"s"]?(a[f+"s"]=[],a[f+"s"].push(a[f]),a[f+"s"].push(d),delete a[f]):!a[f]&&a[f+"s"]?a[f+"s"].push(d):a[f]=d}return a}return a.text()};
d.prototype._doRequest=function(a,b,c){var e=c;a.host=this._ip;a.port=this._port;a.headers={"Content-Type":'text/xml; charset="UTF-8"',"User-Agent":"mediola",Connection:"close"};"POST"==a.method&&b&&(a.headers["Content-Length"]=b.length);var f=this,d="";c=require("http").request(a,function(a){a.setEncoding("utf8");a.on("data",function(a){d+=a});a.on("end",function(){if(200!=a.statusCode){f._logger.log("warn","Failed to post request to "+f._ip+" with status code:"+a.statusCode);var b=Error("http request failed with status code:"+
a.statusCode)}e&&(e(b,d),e=null)})});if(c.on&&"function"==typeof c.on)c.on("error",function(a){e&&e(a);e=null});"POST"==a.method&&b&&c.write(b);c.end()};d.prototype.getDevicesCreator=function(a){this._getDevices(function(b,c){if(b)a&&a(b);else{b={};var e=[];if(c){for(var f in c)if(c.hasOwnProperty(f)){var d=c[f];if(d)for(var g=0;g<d.length;g++){var h=d[g];h&&(h.address&&!b[h.address]&&(b[h.address]=new xnm.aio.moehlenhoff.Alpha2Device(h),b[h.address]._name=h._deviceName,e.push(b[h.address])),h=new xnm.aio.moehlenhoff.Alpha2HeatArea(h),
e.push(h))}}a&&a(null,e)}}})};d.prototype.executeCommandCreator=function(a,b,c){if(a)if(b){var e=b.value;"target_temp"==e&&(e=5*b.ext);this._executeCommand(a,e,c)}else c&&c(Error("command in null"));else c&&c(Error("device in null"))};d.prototype.getStatesCreator=function(a,b,c){var e=function(a,b,c,e){a=function(a,b){if(!b.heatAreas)return null;for(var c=0;c<b.heatAreas.length;c++)if(b.heatAreas[c]&&b.heatAreas[c].number==a)return b.heatAreas[c];return null};if(!b.address||!c||!c.value)return null;
e=function(a,b){for(var c=0;c<b.length;c++)if(b[c]&&b[c].id==a)return b[c];return null}(b.address,e);if(!e)return null;switch(b.dpt){case "HEATAREA":b=a(b.number,e);if(!b)return null;b=b[c.value];return"target_temp"==c.value?b/5:b;case "DEVICE":return e[c.value]}};this._getStates(function(a,d){if(a)c&&c(a);else{a=[];for(var f=0;f<b.length;f++){var h=b[f],k=e(h.id,h.device,h.status,d);k&&a.push({id:h.id,status:k})}c&&c(null,a)}})};d.prototype.toJSON=function(){return{sys:xnm.aio.moehlenhoff.Alpha2.DM.SYS,
ip:this._ip,port:this._port}};d.prototype.equalsTo=function(a){return a&&a instanceof d?this._ip==a._ip&&this._port==a._port:!1};d.parseJSON=function(a){return a&&a.ip?new d(a.ip,a.port):null};return d}();
...
name:"M\u00f6hlenhoff Alpha2",port:80},{sys:this.NEA_SMART_SYS,name:"REHAU Nea Smart",port:80}]},createGateway:function(a,c,e){c=d.ERROR_CODE;a?a.ip?e(null,a):e(new d(c.INVALID,"ip is invalid")):e(new d(c.INVALID,"info is invalid"))},updateGateway:function(a,c,e,f){a=d.ERROR_CODE;c?c.ip?f(null,c):f(new d(a.INVALID,"ip is invalid")):f(new d(a.INVALID,"update is invalid"))},deleteGateway:function(a,c,e){e()},createDevice:function(a,c,e){var b=d.ERROR_CODE;if(a.gateway)if(c.data&&c.data.gateways&&0!==
c.data.gateways.length){c=c.data.gateways;var k;for(k=0;k<c.length;k++)if(c[k].index===a.gateway){var g=c[k];break}g?e(null,a):e(new d(b.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else e(new d(b.NOT_FOUND,"gateway with index "+a.gateway+" not exists"));else e(new d(b.INVALID,"gateway is invalid"))},updateDevice:function(a,c,e){var b=d.ERROR_CODE;if(a)if(a.gateway){c=c.data.gateways;var k;for(k=0;k<c.length;k++)if(c[k].index===a.gateway){var g=c[k];break}g?e(null,a):e(new d(b.NOT_FOUND,
"gateway with index "+a.gateway+" not exists"))}else e(new d(b.INVALID,"gateway is invalid"));else e(new d(b.INVALID,"info is invalid"))},deleteDevice:function(a,c,e){e()},getCommandStatusMap:function(b){if(!b||!b.sys||!b.dpt)return null;var c=b.sys;c==this.NEA_SMART_SYS&&(c=this.SYS);c=a[c];if(!c)return null;c=c[b.dpt];if("analog"==b.iodevice){b={command:[],status:c.status};for(var e=0;e<c.command.length;e++){var d=c.command[e];"target_temp"!=d.ref.value&&b.command.push(d)}c=b}return c}}}();
...
xnm.aio.rehau.NeaSmart.prototype.getDevicesCreator=function(d){this._getDevices(function(a,b){if(a)d&&d(a);else{a={};var c=[];if(b){for(var e in b)if(b.hasOwnProperty(e)){var f=b[e];if(f)for(var k=0;k<f.length;k++){var g=f[k];g&&(g.address&&!a[g.address]&&(a[g.address]=new xnm.aio.rehau.NeaSmartDevice(g),a[g.address]._name=g._deviceName,c.push(a[g.address])),g=new xnm.aio.rehau.NeaSmartHeatArea(g),c.push(g))}}d&&d(null,c)}}})};xnm.aio.rehau.NeaSmart.DM={SYS:xnm.aio.moehlenhoff.Alpha2.DM.NEA_SMART_SYS};
...
[];switch(e.catagory){case "command":a.command&&a.command.forEach(function(a){c(e.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),d.push(a))});break;case "status":a.status&&a.status.forEach(function(a){c(e.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),d.push(a))})}return d},d=function(a,c){var b=[],d;if(a.sys){var f=this._getSystem(a.sys);f&&f.DM&&f.DM.getCommandStatusMap&&(d=f.DM.getCommandStatusMap(a))}d&&(a=e(d,a,c),b=b.concat(a));return b};if(!a.sys)return null;var k=JSON.parse(JSON.stringify(a)),
g=null;switch(b.catagory){case "command":g=d.call(this,a,b);k.command=g;break;case "status":g=d.call(this,a,b);k.status=g;break;default:return null}return g&&0!=g.length?k:null}}}();
