...
xnm.aio.nanoleaf={API_BASE_V1:"/api/v1",PORT_DEFAULT:16021,_logger:{log:function(){}},_buildRequestOptions:function(a,b,c,d){var e=xnm.aio.nanoleaf.API_BASE_V1;a.token&&"new"!==c.path&&(e+="/"+a.token);e+="/"+c.path;a={host:a.ip,port:a.port,path:e,method:b,headers:{"Content-Type":'application/json; charset="utf-8"',Connection:"close"}};"string"===typeof d&&(a.headers["Content-Length"]=d.length);return a},_handleResponse:function(a,b,c,d){if(200===c.statusCode){var e="";c.on("data",function(g){e+=
String(g)});c.on("end",function(){if("json"===b.dataType)try{e=JSON.parse(e)}catch(g){xnm.aio.nanoleaf._logger.log("error","[_handleResponse] Failed to parse result as JSON: "+g.message)}d&&d(null,{statusCode:c.statusCode,response:e});d=null})}else 204===c.statusCode?d&&d(null,{statusCode:c.statusCode,response:null}):(a=Error(a+' request for "'+b.path+'" failed with status code '+c.statusCode),xnm.aio.nanoleaf._logger.log("error","[_handleResponse] "+a.message),d&&d(a,{statusCode:c.statusCode,response:null}))},
doRequest:function(a,b,c,d){var e=null;if("undefined"!==typeof c.data&&null!==c.data)try{e=JSON.stringify(c.data)}catch(f){xnm.aio.nanoleaf._logger.log("error","[doRequest] Failed to parse body. Error: "+f.message);d&&d(f,null);return}var g=require("http");a=xnm.aio.nanoleaf._buildRequestOptions(a,b,c,e);g=g.request(a,function(f){xnm.aio.nanoleaf._handleResponse(b,c,f,d)});"function"===typeof g.setEncoding&&g.setEncoding("utf8");if("function"===typeof g.on)g.on("error",function(f){f&&xnm.aio.nanoleaf._logger.log("error",
"[doRequest] Error: "+f.message);d&&d(f);d=null});"number"!==typeof c.timeout||isNaN(c.timeout)||"function"!==typeof g.setTimeout||g.setTimeout(c.timeout);e&&g.write(e);g.end()},parseJSON:function(a){!a.type&&a.model&&("NL22"===a.model?a.type="panel":"NL29"===a.model&&(a.type="canvas"));if(!a||!a.ip||!a.type)return xnm.aio.nanoleaf._logger.log("warn","[parseJSON] Missing IP and/or type."),null;if("panel"===a.type)return new xnm.aio.nanoleaf.Panel(a.ip,a);if("canvas"===a.type)return new xnm.aio.nanoleaf.Canvas(a.ip,
a);xnm.aio.nanoleaf._logger.log("warn","[parseJSON] Unknown type: "+a.type);return null},rgb2hsv:function(a){a=a.replace("#","");var b=0,c=0;var d=0;if(6===a.length){d=a.substring(0,2);var e=a.substring(2,4);a=a.substring(4,6)}else if(3===a.length)d=a[0],d+=d,e=a[1],e+=e,a=a[2],a+=a;else return[b,c,d];d=parseInt(d,16)/255;e=parseInt(e,16)/255;a=parseInt(a,16)/255;var g=Math.max(Math.max(d,e),a),f=g-Math.min(Math.min(d,e),a);b=-1;c=0;0!==g&&(b=0,c=f/g,0!==f&&(b=60*(d===g?(e-a)/f:e===g?2+(a-d)/f:4+
(d-e)/f),0>b&&(b+=360)));b=Math.round(b);c=Math.round(100*c);d=Math.round(100*g);return[b,c,d]}};
...
{type:"string",name:"serial number",ref:{value:"serialNo"}},{type:"string",name:"firmware version",ref:{value:"firmwareVersion"}},{type:"int",name:"number of panels",ref:{value:"numPanels"}}],ipevt:[{type:"enum",name:"is on",ref:{value:"on",options:["true","false"]}},{type:"int",name:"brightness",ref:{value:"brightness",extMeta:"0-100"}},{type:"enum",name:"color mode",ref:{value:"colorMode",options:["ct","effect","hs"]}},{type:"int",name:"color temperature",ref:{value:"ct",extMeta:"1200-6500"}},{type:"int",
name:"hue",ref:{value:"hue",extMeta:"0-360"}},{type:"int",name:"saturation",ref:{value:"sat",extMeta:"0-100"}},{type:"dynamic",name:"scene name",ref:{value:"effectSelected",ext:"%dynamic"}}]}},SYS:"nanoleaf",applyIPEventFilter:function(a,b){return a&&a.type?(a=xnm.aio.nanoleaf.DM.getIPevtMap(a))?{ipevt:a}:null:null},applyUIFilter:function(a,b,c){if(!a.sys)return null;var d=function(h,p){if("*"===h||"*"===h[0])return!0;for(var k=0;k<h.length;k++)if(h[k]==p)return!0;return!1},e=function(h,p,k){var m=
[];switch(k.catagory){case "command":h.command&&h.command.forEach(function(n){d(k.elems,n.type)&&(n=JSON.parse(JSON.stringify(n)),m.push(n))});break;case "status":h.status&&h.status.forEach(function(n){d(k.elems,n.type)&&(n=JSON.parse(JSON.stringify(n)),m.push(n))})}return m},g=function(h,p){var k=[],m=xnm.aio.nanoleaf.DM.getCommandStatusMap(h,c);m&&(h=e(m,h,p),k=k.concat(h));return k},f=JSON.parse(JSON.stringify(a)),l=null;switch(b.catagory){case "command":l=g(a,b);f.command=l;break;case "status":l=
g(a,b);f.status=l;break;default:return null}return l&&0!=l.length?f:null},createDevice:function(a,b,c){b=xnm.aio.nanoleaf.DM;a?a.type?a.ip?c(null,a):c(new b.Error(b.Error.ERROR_CODE.INVALID,"IP is missing")):c(new b.Error(b.Error.ERROR_CODE.INVALID,"type is missing")):c(new b.Error(b.Error.ERROR_CODE.INVALID,"info is missing"))},deleteDevice:function(a,b,c){a.ip&&a.token?(a=xnm.aio.nanoleaf.parseJSON(a))?a.deleteUser(function(d){c()}):c():c()},getCommandStatusMap:function(a,b){return a.type?xnm.aio.nanoleaf.DM.MAP[a.type]:
...
a),xnm.aio.nanoleaf._logger.log("error","[Device.cmdIncrementBrightness] "+a.message),b&&b(a,null)):(a={data:{brightness:{increment:a}},path:"state"},xnm.aio.nanoleaf._logger.log("trace","[Device.cmdIncrementBrightness] Sending request."),xnm.aio.nanoleaf.doRequest(this,"PUT",a,function(c,d){b&&b(c,null)}))},cmdNextScene:function(a){xnm.aio.nanoleaf._logger.log("trace","[Device.cmdNextScene] Requesting effectsList to find next scene.");var b=this;xnm.aio.nanoleaf.doRequest(this,"GET",{dataType:"json",
...
path:"state"},xnm.aio.nanoleaf._logger.log("trace","[Device.cmdSetBrightness] Sending request."),xnm.aio.nanoleaf.doRequest(this,"PUT",a,function(c,d){b&&b(c,null)}))},cmdSetColor:function(a,b){a=xnm.aio.nanoleaf.rgb2hsv(a);a={data:{hue:{value:a[0]},sat:{value:a[1]},brightness:{value:a[2]}},path:"state"};xnm.aio.nanoleaf._logger.log("trace","[Device.cmdSetColor] Sending request.");xnm.aio.nanoleaf.doRequest(this,"PUT",a,function(c,d){b&&b(c,null)})},cmdSetColorTemperature:function(a,b){"number"!==
typeof a||isNaN(a)||1200>a||6500<a?(a=Error("Invalid value for color temperature: "+a),xnm.aio.nanoleaf._logger.log("error","[Device.cmdSetColorTemperature] "+a.message),b&&b(a,null)):(a={data:{ct:{value:a}},path:"state"},xnm.aio.nanoleaf._logger.log("trace","[Device.cmdSetColorTemperature] Sending request."),xnm.aio.nanoleaf.doRequest(this,"PUT",a,function(c,d){b&&b(c,null)}))},cmdSetScene:function(a,b){a={data:{select:a},path:"effects"};xnm.aio.nanoleaf._logger.log("trace","[Device.cmdSetScene] Sending request.");
xnm.aio.nanoleaf.doRequest(this,"PUT",a,function(c,d){b&&b(c,null)})},cmdToggle:function(a){xnm.aio.nanoleaf._logger.log("trace","[Device.cmdToggle] Requesting state first, before deciding command.");var b=this;xnm.aio.nanoleaf.doRequest(this,"GET",{dataType:"json",path:"state/on"},function(c,d){c?a&&a(c,null):d.response.value?b.cmdOff(a):b.cmdOn(a)})},deleteUser:function(a){xnm.aio.nanoleaf._logger.log("debug","[Device.deleteUser] Sending request.");xnm.aio.nanoleaf.doRequest(this,"DELETE",{path:""},
...
c&&c(b,null)},executeCommandCreator:function(a,b,c){b&&b.value?this.executeCommand(a,b,function(d){c&&c(d)}):c&&c(Error("Command value is empty."))},getState:function(a){if(this.token)xnm.aio.nanoleaf._logger.log("trace","[Device.getState] Sending request."),xnm.aio.nanoleaf.doRequest(this,"GET",{dataType:"json",path:""},function(c,d){c?a&&a(c,null):a&&a(null,d.response)});else{var b=Error("Request not possible without a token.");a&&a(b,null)}},getStatesCreator:function(a,b,c){this.getState(function(d,
e){if(e){d=e.effects;for(var g=e.panelLayout,f=e.rhythm,l=e.state,h=[],p=0;p<b.length;p++){var k=b[p];if(k&&k.id&&k.status)switch(k.status.value){case "brightness":l.brightness&&h.push({id:k.id,status:l.brightness.value});break;case "colorMode":l.colorMode&&h.push({id:k.id,status:l.colorMode});break;case "ct":l.ct&&h.push({id:k.id,status:l.ct.value});break;case "hue":l.hue&&h.push({id:k.id,status:l.hue.value});break;case "on":l.on&&h.push({id:k.id,status:l.on.value?"on":"off"});break;case "sat":l.sat&&
h.push({id:k.id,status:l.sat.value});break;case "effectSelected":"string"===typeof d.select&&h.push({id:k.id,status:d.select});break;case "effectIndex":if("string"===typeof d.select&&Array.isArray(d.effectsList)){var m=d.effectsList.indexOf(d.select);h.push({id:k.id,status:m})}break;case "name":"string"===typeof e.name&&h.push({id:k.id,status:e.name});break;case "serialNo":"string"===typeof e.serialNo&&h.push({id:k.id,status:e.serialNo});break;case "model":"string"===typeof e.model&&h.push({id:k.id,
status:e.model});break;case "firmwareVersion":"string"===typeof e.firmwareVersion&&h.push({id:k.id,status:e.firmwareVersion});break;case "numPanels":g&&g.layout&&"number"===typeof g.layout.numPanels&&h.push({id:k.id,status:g.layout.numPanels});break;case "rhythmActive":f&&"boolean"===typeof f.rhythmActive&&h.push({id:k.id,status:f.rhythmActive?"on":"off"});break;case "rhythmConnected":f&&"boolean"===typeof f.rhythmConnected&&h.push({id:k.id,status:f.rhythmConnected?"true":"false"});break;case "rhythmMode":f&&
"number"===typeof f.rhythmMode&&(m="unknown",0===f.rhythmMode?m="microphone":1===f.rhythmMode&&(m="aux"),h.push({id:k.id,status:m}));break;case "auxAvailable":f&&"boolean"===typeof f.auxAvailable&&h.push({id:k.id,status:f.auxAvailable?"true":"false"})}}c&&c(null,h)}else c&&c(Error("Failed to get states."))})},toJSON:function(){return{sys:xnm.aio.nanoleaf.DM.SYS,type:this.type,ip:this.ip,id:this.id,firmware:this.firmware,model:this.model,token:this.token}}};
...
xnm.aio.nanoleaf.Panel.prototype.cmdSetRhythmMode=function(a,b){"aux"===a?a=1:"microphone"===a&&(a=0);"number"!==typeof a||isNaN(a)||0>a||1<a?(a=Error("Invalid value for Rhythm mode: "+a),xnm.aio.nanoleaf._logger.log("error","[Panel.cmdSetRhythmMode] "+a.message),b&&b(a,null)):(a={data:{rhythmMode:a},path:"rhythm/rhythmMode"},xnm.aio.nanoleaf._logger.log("trace","[Panel.cmdSetRhythmMode] Sending request."),xnm.aio.nanoleaf.doRequest(this,"PUT",a,function(c,d){b&&b(c,null)}))};
xnm.aio.nanoleaf.Handler=function(){var a=require("x.logger.js");a&&(xnm.aio.nanoleaf._logger=a.getLogger("xnm.aio.nanoleaf"))};
xnm.aio.nanoleaf.Handler.prototype={devicemanager:xnm.aio.nanoleaf.DM,discoverWithTimeout:function(a,b){var c=require("x.mdns.js");c.clearRecordBuffer();if("number"!==typeof a||isNaN(a))a=1E4;c.discoverServiceWithTimeout("_nanoleafapi._tcp.local",a,function(d,e){if(d)b&&b(d,null);else{for(var g=[],f=0;f<e.length;f++){var l=e[f];l.ip&&0<l.ip.length&&l.info&&g.push({firmware:l.info.srcvers,id:l.info.id,ip:l.ip[0],model:l.info.md,sys:xnm.aio.nanoleaf.DM.SYS})}b&&b(d,g)}})},doCommand:function(a,b,c){var d=
this.resolveDevice(a);d?d.executeCommandCreator(a,b,function(e){c&&c(e)}):c&&c(Error("device format invalid"))},doStatus:function(a,b,c,d){(b=this.resolveDevice(b))?b.getStatesCreator(null,[{id:a,status:c}],function(e,g){e?d&&d(e):d&&d(null,g[0])}):d&&d(Error("device format invalid"))},getDeviceCommands:function(a,b){a=(a=xnm.aio.nanoleaf.DM.applyUIFilter(a,{catagory:"command",elems:"*"}))?a.command:[];b&&b(null,a)},getDeviceStatus:function(a,b){a=(a=xnm.aio.nanoleaf.DM.applyUIFilter(a,{catagory:"status",
