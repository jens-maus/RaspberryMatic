...
C,w.statusCode),m=null)})});if(r.on)r.on("error",function(w){k._logger.log("trace","do request error:"+w.message);m&&(m(w),m=null)});r.setTimeout&&r.setTimeout(1E4,function(){k._logger.log("trace","do request timeout");r.abort();m&&(m(Error("timeout")),m=null)});l&&(h=l.replace(/(client_id|client_secret|username|password)=[a-zA-Z0-9%*!()._-]+/g,"$1=*****"),this._logger.log("trace","send body:"+h),r.write(l));r.end()};p.prototype._doUrlRequest=function(a,f,c,l){var h=this,d=l,k=null;try{k=JSON.parse(JSON.stringify(c)),
k.username&&(k.username="xxxxxx"),k.password&&(k.password="xxxxxx")}catch(m){k=c}this._logger.log("trace","send "+a+" req to:"+f+" data:"+JSON.stringify(k));$.ajax({url:f,type:a,timeout:1E4,dataType:"json",data:c,cache:!1,success:function(m){h._logger.log("trace","http request success:"+JSON.stringify(m));l&&l(null,m)},error:function(m,r,w){r="http request error:"+(m?m.status:"0")+"-"+r;h._logger.log("warn","http request error:"+r);r=Error(r);w&&(h._logger.log("warn","http request http error:"+w),
r.httpError=!0,r.statusCode=m?m.status:0);d&&(d(r,m?m.responseText:null),d=null)}})};return p}();
