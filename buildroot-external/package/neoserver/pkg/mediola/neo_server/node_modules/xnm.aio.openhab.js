...
"gateway with index "+a.gateway+" not exists"))}else c(new f(d.NOT_FOUND,"gateway with index "+a.gateway+" not exists"));else c(new f(d.INVALID,"address is invalid"));else c(new f(d.INVALID,"gateway is invalid"));else c(new f(d.INVALID,"info is invalid"))},deleteDevice:function(a,b,c){c()},applyUIFilter:function(a,b){var c=this,d=function(l,p){if("*"==l)return!0;for(var m=!1,n=0;n<l.length;n++)if(l[n]==p){m=!0;break}return m},e=function(l,p,m){var n=[];switch(m.catagory){case "command":l.command&&
l.command.forEach(function(q){d(m.elems,q.type)&&(q=JSON.parse(JSON.stringify(q)),n.push(q))});break;case "status":l.status&&l.status.forEach(function(q){d(m.elems,q.type)&&(q=JSON.parse(JSON.stringify(q)),n.push(q))})}return n},g=function(l,p){var m=[],n=c.getCommandStatusMap(l);n&&(l=e(n,l,p),m=m.concat(l));return m};if(!a||null==a.type||"undefined"==typeof a.type)return null;var h=JSON.parse(JSON.stringify(a)),k=null;switch(b.catagory){case "command":k=g(a,b);h.command=k;break;case "status":k=
...
d){b&&b(c,d)}):b&&b(Error("gateway json format invalid"))};f.prototype.discover=function(a,b){var c=this,d=require("x.mdns.js");d.clearRecordBuffer();var e={};d.discoverServiceWithTimeout("_openhab-server._tcp.local",a,function(g,h){if(g)b&&b(g);else{for(g=0;g<h.length;g++){var k=h[g];if(k.target&&k.ip&&0<k.ip.length){var l=k.ip[0];k=new c.Server({ip:k.ip[0]});e[l]||(e[l]=k)}}h=[];for(var p in e)h.push(e[p]);b&&b(null,h)}})};return f}();
