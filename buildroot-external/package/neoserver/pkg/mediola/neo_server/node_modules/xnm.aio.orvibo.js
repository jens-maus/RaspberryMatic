...
xnm.aio.orvibo.Device.prototype._sendMessage=function(a,b,c){var d=[104,100,0];d.push(b.length+12);d.push((a&65280)>>8);d.push(a&255);a=this.uuid.replace(/-/g,"");for(var e=0;6>e;e++)d.push(parseInt(a.substr(2*e,2),16));for(e=0;e<b.length;e++)d.push(b[e]);b=new Buffer(d);var f=require("dgram").createSocket("udp4");f.send(b,0,b.length,1E4,this.ip,function(g,h){f.close();c&&setTimeout(function(){c.call()},100)})};
...
xnm.aio.orvibo.Socket.prototype.executeCommand=function(a,b,c){var d=this;this._subscribe(function(){var e=[32,32,32,32,32,32,0,0,0,0];if("on"==b)e.push(1);else if("off"==b)e.push(0);else if("toggle"==b){setTimeout(function(){"off"==d._state?e.push(1):e.push(0);d._sendMessage(25699,e,function(){d._sendMessage(25699,e,c)})},100);return}d._sendMessage(25699,e,function(){d._sendMessage(25699,e,c)})})};
xnm.aio.orvibo.Socket.prototype.getStates=function(a,b,c){var d=this;this.CommandHandler=a;this.getState(function(e){var f={state:e};b&&d.CommandHandler&&d.CommandHandler.setState?d.CommandHandler.setState(b.id,e):d.CommandHandler&&d.CommandHandler.receivedState&&d.CommandHandler.receivedState("orvibo",d.uuid,f);c&&c(f)})};xnm.aio.orvibo.Socket.prototype.getStateValues=function(a){return a};xnm.aio.orvibo.AllOne=function(a,b){this.ip=a;this.uuid=b;this._learnedIRCallback=null};
xnm.aio.orvibo.AllOne.prototype=new xnm.aio.orvibo.Device;xnm.aio.orvibo.AllOne.prototype.constructor=xnm.aio.orvibo.AllOne;xnm.aio.orvibo.AllOne.prototype.learnIR=function(a){var b=this;this._learnedIRCallback=a;this._subscribe(function(){var c=[32,32,32,32,32,32,0,0,0,0,0];b._sendMessage(27763,c,function(){b._sendMessage(27763,c);setTimeout(function(){b._learnedIRCallback&&(b._learnedIRCallback(),b._learnedIRCallback=null)},1E4)})})};
...
xnm.aio.orvibo.Discovery.prototype._addDiscoverSocket=function(a,b){var c=require("dgram"),d=this;if(!b){try{var e=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(f){e=c.createSocket("udp4")}e.on("message",function(f,g){d._receivedData(g.address,f)});e.on("error",function(f){});e.bind(a);this._sockets.push(e)}else if("IPv4"==b.family&&0==b.internal){try{e=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(f){e=c.createSocket("udp4")}e.on("listening",function(){e.setBroadcast(!0)});e.on("message",function(f,
g){d._receivedData(g.address,f)});e.on("error",function(f){});e.bind(a,b.address);this._sockets.push(e)}};xnm.aio.orvibo.Discovery.prototype._sendDiscoveryMessages=function(a,b,c,d){a=new Buffer([104,100,0,6,113,97]);for(b=0;b<this._sockets.length;b++)this._sockets[b].send(a,0,a.length,1E4,"255.255.255.255")};xnm.aio.orvibo.Discovery.prototype.discoverDevices=function(){this._sendDiscoveryMessages()};xnm.aio.orvibo.Handler=function(){this._Discovery=new xnm.aio.orvibo.Discovery;this._Discovery.discoverDevices()};
...
xnm.aio.orvibo.DMError=function(){var a=function(b,c){this.code=b;this.message=c;this.stack=Error().stack};a.prototype=Error();a.prototype.name="OrviboDmError";a.prototype.code=0;a.ERROR_CODE={DEFAULT:0,INDEX_EXISTS:1,ID_EXISTS:2,INVALID:3,UNKNOW:4,NOT_FOUND:5,IP_EXISTS:6};return a}();
...
b,c):c&&c(new d(e.INVALID,"no such sys:"+a.sys))}else c&&c(new d(e.INVALID,"sys is invalid"));else c&&c(new d(e.INVALID,"info is invalid"))},deleteDevice:function(a,b,c){var d=xnm.aio.orvibo.DMError,e=xnm.aio.orvibo.DMError.ERROR_CODE;if(a)if(a.sys){var f=this._getSystem(a.sys);f&&f.DM&&f.DM.deleteDevice?f.DM.deleteDevice(a,b,c):c&&c(new d(e.INVALID,"no such sys:"+a.sys))}else c&&c(new d(e.INVALID,"sys is invalid"));else c&&c()},applyUIFilter:function(a,b){var c=function(h,n){if("*"==h)return!0;for(var m=
!1,l=0;l<h.length;l++)if(h[l]==n){m=!0;break}return m},d=function(h,n,m){var l=[];switch(m.catagory){case "command":h.command&&h.command.forEach(function(k){c(m.elems,k.type)&&(k=JSON.parse(JSON.stringify(k)),l.push(k))});break;case "status":h.status&&h.status.forEach(function(k){c(m.elems,k.type)&&(k=JSON.parse(JSON.stringify(k)),l.push(k))})}return l},e=function(h,n){var m=[],l;if(h.sys){var k=this._getSystem(h.sys);k&&k.DM&&k.DM.getCommandStatusMap&&(l=k.DM.getCommandStatusMap(h))}l&&(h=d(l,h,
n),m=m.concat(h));return m};if(!a.sys)return null;var f=JSON.parse(JSON.stringify(a)),g=null;switch(b.catagory){case "command":g=e.call(this,a,b);f.command=g;break;case "status":g=e.call(this,a,b);f.status=g;break;default:return null}return g&&0!=g.length?f:null}}}();xnm.aio.orvibo.Socket.prototype.executeCommandCreator=function(a,b,c){b&&b.value?this.executeCommand(null,b.value,function(){c&&c()}):c&&c(Error("command invalid"))};
xnm.aio.orvibo.Socket.prototype.getStatesCreator=function(a,b,c){this._getState(function(d){for(var e=[],f=0;f<b.length;f++)if(b[f]&&b[f].id){var g="?";b[f].status&&b[f].status.value&&d&&d[b[f].status.value]&&(g=d[b[f].status.value]);e.push({id:b[f].id,status:g})}c&&c(null,e)})};
...
xnm.aio.orvibo.Handler.prototype.getDeviceStatus=function(a,b){a=(a=this.devicemanager.applyUIFilter(a,{catagory:"status",elems:"*"}))?a.status:[];b&&b(null,a)};xnm.aio.orvibo.Handler.prototype.searchDeviceWithTimeout=function(a,b){var c=[];this._Discovery.callback=function(d){for(var e=0;e<c.length;e++)if(c[e]&&c[e].ip==d.ip)return;c.push(d)};this._Discovery.discoverDevices();setTimeout(function(){b&&b(null,c)},a)};
xnm.aio.orvibo.Handler.prototype.searchIRBlastWithTimeout=function(a,b){this.searchDeviceWithTimeout(a,function(c,d){if(c)b&&b(c);else{c=[];for(var e=0;e<d.length;e++)d[e]&&d[e]instanceof xnm.aio.orvibo.AllOne&&c.push(d[e]);b&&b(null,c)}})};xnm.aio.orvibo.Handler.prototype.learnIRCode=function(a,b,c){(a=this.resolveGateway(a))&&a instanceof xnm.aio.orvibo.AllOne?a.learnIR(function(d){var e=d?null:Error("learn ir code error");c&&c(e,d)}):c&&c(Error("gateway json format invalid"))};
xnm.aio.orvibo.Handler.prototype.doIRCommand=function(a,b,c,d){(a=this.resolveGateway(a))&&a instanceof xnm.aio.orvibo.AllOne?a.executeCommandCreator(b,c,d):d&&d(Error("gateway json format invalid"))};xnm.aio.orvibo.Handler.prototype.searchSocketWithTimeout=function(a,b){this.searchDeviceWithTimeout(a,function(c,d){if(c)b&&b(c);else{c=[];for(var e=0;e<d.length;e++)d[e]&&d[e]instanceof xnm.aio.orvibo.Socket&&c.push(d[e]);b&&b(null,c)}})};
xnm.aio.orvibo.Handler.prototype.doSocketCommand=function(a,b,c){var d=this.resolveDevice(a);d?d.executeCommandCreator(a,b,function(e){c&&c(e)}):c&&c(Error("device json format invalid"))};xnm.aio.orvibo.Handler.prototype.doSocketStatus=function(a,b,c,d){var e=this.resolveDevice(b);e?e.getStatesCreator(null,[{id:a,device:b,status:c}],function(f,g){f?d&&d(f):d&&d(null,g[0])}):d&&d(Error("device json format invalid"))};
