...
xnm.aio.orvibo.Socket.prototype.getStates=function(a,b,c){var d=this;this.CommandHandler=a;this.getState(function(a){var f={state:a};b&&d.CommandHandler&&d.CommandHandler.setState?d.CommandHandler.setState(b.id,a):d.CommandHandler&&d.CommandHandler.receivedState&&d.CommandHandler.receivedState("orvibo",d.uuid,f);c&&c(f)})};xnm.aio.orvibo.Socket.prototype.getStateValues=function(a){return a};xnm.aio.orvibo.AllOne=function(a,b){this.ip=a;this.uuid=b;this._learnedIRCallback=null};
...
xnm.aio.orvibo.Discovery.prototype._addDiscoverSocket=function(a,b){var c=require("dgram"),d=this;if(!b){var e;try{e=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(f){e=c.createSocket("udp4")}e.on("message",function(a,b){d._receivedData(b.address,a)});e.on("error",function(a){});e.bind(a);this._sockets.push(e)}else if("IPv4"==b.family&&0==b.internal){try{e=c.createSocket({reuseAddr:!0,type:"udp4"})}catch(g){e=c.createSocket("udp4")}e.on("listening",function(){e.setBroadcast(!0)});e.on("message",
function(a,b){d._receivedData(b.address,a)});e.on("error",function(a){});e.bind(a,b.address);this._sockets.push(e)}};xnm.aio.orvibo.Discovery.prototype._sendDiscoveryMessages=function(a,b,c,d){a=new Buffer([104,100,0,6,113,97]);for(b=0;b<this._sockets.length;b++)this._sockets[b].send(a,0,a.length,1E4,"255.255.255.255")};xnm.aio.orvibo.Discovery.prototype.discoverDevices=function(){this._sendDiscoveryMessages()};xnm.aio.orvibo.Handler=function(){this._Discovery=new xnm.aio.orvibo.Discovery;this._Discovery.discoverDevices()};
xnm.aio.orvibo.Handler.prototype.discoverDevices=function(a){a&&(this._Discovery.callback=a);this._Discovery.discoverDevices()};xnm.aio.orvibo.Handler.prototype.getStateValues=function(a,b){return(new xnm.aio.orvibo.Socket).getStateValues(b)};xnm.aio.orvibo.Handler.prototype.getDeviceCommandList=function(a,b){var c=[];c.push({id:window.XC_Text.on,cmd:"on"});c.push({id:window.XC_Text.off,cmd:"off"});return c};
...
updateGateway:function(a,b,c,d){a=xnm.aio.orvibo.DMError;c=xnm.aio.orvibo.DMError.ERROR_CODE;b?b.ip?d(null,b):d(new a(c.INVALID,"ip is invalid")):d(new a(c.INVALID,"update is invalid"))},deleteGateway:function(a,b,c){c()},createDevice:function(a,b,c){var d=xnm.aio.orvibo.DMError,e=xnm.aio.orvibo.DMError.ERROR_CODE;if(a.gateway)if(b.data&&b.data.gateways&&0!==b.data.gateways.length){b=b.data.gateways;var f,g;for(f=0;f<b.length;f++)if(b[f].index===a.gateway){g=b[f];break}g?c(null,a):c(new d(e.NOT_FOUND,
"gateway with index "+a.gateway+" not exists"))}else c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"));else c(new d(e.INVALID,"gateway is invalid"))},updateDevice:function(a,b,c){var d=xnm.aio.orvibo.DMError,e=xnm.aio.orvibo.DMError.ERROR_CODE;if(a)if(a.gateway){b=b.data.gateways;var f,g;for(f=0;f<b.length;f++)if(b[f].index===a.gateway){g=b[f];break}g?c(null,a):c(new d(e.NOT_FOUND,"gateway with index "+a.gateway+" not exists"))}else c(new d(e.INVALID,"gateway is invalid"));else c(new d(e.INVALID,
...
xnm.aio.orvibo.Handler.prototype.getDeviceCommands=function(a,b){var c=xnm.aio.orvibo.DM.applyUIFilter(a,{catagory:"command",elems:"*"}),c=c?c.command:[];b&&b(null,c)};xnm.aio.orvibo.Handler.prototype.getDeviceStatus=function(a,b){var c=this.devicemanager.applyUIFilter(a,{catagory:"status",elems:"*"}),c=c?c.status:[];b&&b(null,c)};
xnm.aio.orvibo.Handler.prototype.searchDeviceWithTimeout=function(a,b){var c=[];this._Discovery.callback=function(a){for(var b=0;b<c.length;b++)if(c[b]&&c[b].ip==a.ip)return;c.push(a)};this._Discovery.discoverDevices();setTimeout(function(){b&&b(null,c)},a)};xnm.aio.orvibo.Handler.prototype.searchIRBlastWithTimeout=function(a,b){this.searchDeviceWithTimeout(a,function(a,d){if(a)b&&b(a);else{for(var e=[],f=0;f<d.length;f++)d[f]&&d[f]instanceof xnm.aio.orvibo.AllOne&&e.push(d[f]);b&&b(null,e)}})};
xnm.aio.orvibo.Handler.prototype.learnIRCode=function(a,b,c){(a=this.resolveGateway(a))&&a instanceof xnm.aio.orvibo.AllOne?a.learnIR(function(a){var b=a?null:Error("learn ir code error");c&&c(b,a)}):c&&c(Error("gateway json format invalid"))};xnm.aio.orvibo.Handler.prototype.doIRCommand=function(a,b,c,d){(a=this.resolveGateway(a))&&a instanceof xnm.aio.orvibo.AllOne?a.executeCommandCreator(b,c,d):d&&d(Error("gateway json format invalid"))};
xnm.aio.orvibo.Handler.prototype.searchSocketWithTimeout=function(a,b){this.searchDeviceWithTimeout(a,function(a,d){if(a)b&&b(a);else{for(var e=[],f=0;f<d.length;f++)d[f]&&d[f]instanceof xnm.aio.orvibo.Socket&&e.push(d[f]);b&&b(null,e)}})};xnm.aio.orvibo.Handler.prototype.doSocketCommand=function(a,b,c){var d=this.resolveDevice(a);d?d.executeCommandCreator(a,b,function(a){c&&c(a)}):c&&c(Error("device json format invalid"))};
xnm.aio.orvibo.Handler.prototype.doSocketStatus=function(a,b,c,d){var e=this.resolveDevice(b);e?e.getStatesCreator(null,[{id:a,device:b,status:c}],function(a,b){a?d&&d(a):d&&d(null,b[0])}):d&&d(Error("device json format invalid"))};"undefined"!=typeof exports&&"undefined"!=typeof module&&module.exports&&(exports=module.exports=new xnm.aio.orvibo.Handler);
