...
xnm.aio.rademacher.Util={clearHttpOptionForLog:function(a){try{var b=JSON.parse(JSON.stringify(a));b.auth&&(b.auth="xxxxxx:xxxxxx");if(b.headers)for(var c in headers)if(c&&"authorization"==c.toLocaleLowerCase()){var d=headers[c];if(d){var e=d.indexOf(" ");headers[c]=-1==e?"xxxxxx":d.substr(0,e)+" xxxxxx"}}return b}catch(f){return a}}};
...
this._inRemoteMode&&this._user&&this._password&&(a.headers.Authorization="Basic "+(new Buffer(this._user+":"+this._password)).toString("base64"));var h=this;this._logger.log("trace","send http req:"+JSON.stringify(xnm.aio.rademacher.Util.clearHttpOptionForLog(a)));var g=f.request(a,function(a){h._logger.log("trace","receive http rsp status code:"+a.statusCode);if(200==a.statusCode){a.setEncoding("utf-8");var c="";a.on("data",function(a){c+=a});a.on("end",function(){h._logger.log("trace","receive http rsp:"+
c);var a=JSON.parse(c);b&&b(a)})}else 403==a.statusCode?XC.debugf("UNAUTHORIZED"):XC.debugf("Failed to post request"),b&&b(null)});g.setTimeout(6E3,function(){h._logger.log("trace","send http req timeout");g.abort()});g.on("error",function(a){h._logger.log("trace","send http req err:"+a.message);b&&b(null)});g.end()};
