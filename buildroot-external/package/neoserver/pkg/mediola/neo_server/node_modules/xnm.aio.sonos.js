...
this._players[a];5<=b.missCount?delete this._players[a]:b.missCount++}this._mediaServer={};this._discovery.search(null)};d.prototype.findPlayer=function(a){var b=a.uuid;if(b){var c=this._players[b];c||(this._players[b]={missCount:0,dummy:!1},c=this._players[b]);if(c.player)c.player.setIP(a.ip),c.player.setPort(a.port);else{b=new xnm.aio.sonos.Player(a.ip,a.port,a.uuid,a.name);var e,d;if(a.embeddedDevices)for(var g=0;g<a.embeddedDevices.length;g++){var h=a.embeddedDevices[g];h&&"urn:schemas-upnp-org:device:MediaServer:1"==
h.deviceType?d=h:h&&"urn:schemas-upnp-org:device:MediaRenderer:1"==h.deviceType&&(e=h)}b._device=a;b._renderer=e;b._server=d;c.player=b}c.dummy=!1;c.missCount=0;var k=this;c.player.getZoneGroupState(function(a,c,b){!a&&c&&b&&k._updateTopology(c)})}};d.prototype.findMediaServer=function(a){var b=a.uuid;b&&(this._mediaServer[b]=a)};d.prototype.getCurrentZone=function(a){if(this._currentZone)a&&a(null,this._currentZone);else{var b=this;this.search(function(c){c?a&&a(c):this._currentZone?b.getCurrentZone(a):
a&&a(Error("no current zone"))})}};d.prototype.setCurrentZone=function(a,b){if(this._topology){var c=null,e;for(e in this._topology){var d=this._topology[e];if(d&&d.id==a){c=d;break}}c&&c.coordinator?(a=this._players[c.coordinator])?(this._currentZone=JSON.parse(JSON.stringify(c)),this._currentZone.player=a.player,b&&b()):b&&b(Error("no such player")):b&&b(Error("no such zone"))}else{var g=this;this.search(function(a){a?b&&b(a):this._topology?g.setCurrentZone(b):b&&b(Error("no player found"))})}};
d.prototype.getZones=function(a){var b=this,c=this.getBufferedPlayers();c&&0!=c.length?a&&a(null,this._topology):this.search(function(c,d){c?a&&a(c):d&&0!=d.length?b.getZones(a):a&&a(Error("find no sonos player"))})};d.prototype.getPlayer=function(a){if("master"==a.type)return this;if(!a.uuid){if(!a.ip)return null;for(c in this._players)if(this._players.hasOwnProperty(c)){var b=this._players[c];if(b&&b.player&&b.player.getIP()==a.ip)return b.player}if(this._tmpPlayers[a.ip])return this._tmpPlayers[a.ip];
var c=new xnm.aio.sonos.Player(a.ip,a.port,a.uuid,a.name);return this._tmpPlayers[a.ip]=c}if((c=this._players[a.uuid])&&c.player)return c.player;if(!a.ip)return null;c=new xnm.aio.sonos.Player(a.ip,a.port,a.uuid,a.name);this._players[a.uuid]={missCount:0,dummy:!0,player:c};return c};d.prototype._updateTopology=function(a){this._topology||(this._topology={});var b=this;a.forEach(function(a){a&&a.id&&(b._topology[a.id]=a)});for(var c,e=0;e<a.length;e++)if((c=a[e])&&c.coordinator){var d=this._players[c.coordinator];
if(d&&d.player&&!this._currentZone){this._currentZone=JSON.parse(JSON.stringify(c));this._currentZone.player=d.player;return}}for(var g=0;g<a.length;g++)if((c=a[e])&&c.coordinator&&(d=this._players[c.coordinator])&&d.player){this._currentZone=JSON.parse(JSON.stringify(c));this._currentZone.player=d.player;break}};d.prototype.executeCommandCreator=function(a,b,c){if(this._currentZone)this._currentZone.player.executeCommandCreator(a,b,c);else{var e=this;this.search(function(d){d?c&&c(d):e._currentZone?
e._currentZone.player.executeCommandCreator(a,b,c):c&&c(d)})}};d.prototype.getStatesCreator=function(a,b,c){if(this._currentZone)this._currentZone.player.getStatesCreator(a,b,c);else{var e=this;this.search(function(d){d?c&&c(d):e._currentZone?e._currentZone.player.getStatesCreator(a,b,c):c&&c(null,[])})}};d.prototype.play=function(a,b,c){if(this._currentZone)this._currentZone.player.play(a,b,c);else{var e=this;this.search(function(d){d?c&&c(d):e._currentZone?e._currentZone.player.play(a,b,c):c&&c(d)})}};
d.prototype.addToQueueAndPlay=function(a,b,c){if(this._currentZone)this._currentZone.player.addToQueueAndPlay(a,b,c);else{var e=this;this.search(function(d){d?c&&c(d):e._currentZone?e._currentZone.player.addToQueueAndPlay(a,b,c):c&&c(d)})}};d.prototype.addToQueue=function(a,b,c){if(this._currentZone)this._currentZone.player.addToQueue(a,b,c);else{var e=this;this.search(function(d){d?c&&c(d):e._currentZone?e._currentZone.player.addToQueue(a,b,c):c&&c(d)})}};d.prototype.replaceQueueAndPlay=function(a,
b,c){if(this._currentZone)this._currentZone.player.replaceQueueAndPlay(a,b,c);else{var e=this;this.search(function(d){d?c&&c(d):e._currentZone?e._currentZone.player.replaceQueueAndPlay(a,b,c):c&&c(d)})}};d.prototype.browser=function(a,b,c,e,d){if(this._currentZone)this._currentZone.player.browser(a,b,c,e,d);else{var f=this;this.search(function(h){h?d&&d(h):f._currentZone?f._currentZone.player.browser(a,b,c,e,d):d&&d(h)})}};d.prototype.clearQueue=function(a,b){if(this._currentZone)this._currentZone.player.clearQueue(b);
...
d.prototype.getInfo=function(a){this._getRenderer().getInfo(a)};d.prototype.playItem=function(a,b){this._getRenderer().playItem(a,b)};d.prototype.playPlaylist=function(){var a=require("x.upnp.js"),b=this._getRenderer();var c='<u:SetAVTransportURI xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID>'+("<CurrentURI>x-rincon-queue:"+b.uuid+"#0</CurrentURI><CurrentURIMetaData></CurrentURIMetaData>");c+="</u:SetAVTransportURI>";(new a.SOAPRequest).doRequest(b.getURL(b.AVTransport),
...
c);break;case "balanceLeft":this.balanceToLeft(c);break;case "balanceRight":this.balanceToRight(c);break;case "seekTo":if("undefined"==typeof b||null==b){c&&c(Error("seek value invalid"));break}this._renderer.seekRelTimeSec(b,c);break;case "playLineIn":this.playLineIn(c);break;case "playUrl":if("undefined"==typeof b||null==b){c&&c(Error("playUrl url invalid"));break}this.playUrl(b,c);break;case "playUrlwithTimeout":if("undefined"==typeof b||null==b){c&&c(Error("playUrl url invalid"));break}this.addUrlToQueueAndPlayThenDelete(b,
d?d.timeout:0,c);break;default:c&&c(Error("unknown command"))}else c&&c(Error("renderer not initialized"))};d.prototype._getStates=function(a,b){if(this._renderer)if(a&&0!=a.length){var c=function(){f--;0==f&&b&&b(null,d)},d={},f=a.length;-1!=a.indexOf("PLAY_STATE")&&this._renderer.getPlayState(function(a,b){a||(d.transportState=b);c()});-1!=a.indexOf("PLAY_INFO")&&this.getPlayInfo(function(a,b){a||(d.playInfo=b);c()});-1!=a.indexOf("MUTE")&&this._renderer.isMuted(function(a,b){a||(d.muted=b);c()});
-1!=a.indexOf("VOLUME")&&this._renderer.getVolume(function(a,b){a||(d.volume=b);c()});-1!=a.indexOf("PLAY_MODE")&&this.getPlayMode(function(a,b){a||(d.playMode=b);c()});-1!=a.indexOf("GROUP_MUTE")&&this.isGroupMuted(function(a,b){a||(d.groupMuted=b);c()});-1!=a.indexOf("GROUP_VOLUME")&&this.getGroupVolume(function(a,b){a||(d.groupVolume=b);c()});-1!=a.indexOf("BALANCE")&&this.getBalance(function(a,b){a||(d.balance=b);c()});-1!=a.indexOf("BASS")&&this.getBass(function(a,b){a||(d.bass=b);c()});-1!=
a.indexOf("TREBLE")&&this.getTeble(function(a,b){a||(d.treble=b);c()});-1!=a.indexOf("LOUDNESS")&&this.isLoudnessActive(function(a,b){a||(d.loudness=b?"on":"off");c()})}else b&&b(null,{});else b&&b(Error("renderer not initialized"))};d.prototype.playUrl=function(a,b){if(a){var c=this;this._renderer._setAVTransportURI(0,a,null,function(a){a?b&&b(a):c._renderer.play(b)})}else b&&b(Error("url is null"))};d.prototype.addUrlToQueueAndPlayThenDelete=function(a,b,c){if(a){b||(b=30);var d=this;this._addURItoQueue(0,
a,null,0,!0,function(a,e){a?c&&c(a):e&&"undefined"!=typeof e.firstTrackNumberEnqueued&&null!=e.firstTrackNumberEnqueued?d._renderer.seekTrack(e.firstTrackNumberEnqueued,function(a){a?c&&c(a):d._renderer.play(function(a){a?c&&c(a):setTimeout(function(){d._removeAllTracks(0,0,function(a){c&&c(a)})},1E3*b)})}):c&&c(Error("result format invalid"))})}else c&&c(Error("url is null"))};d.prototype.play=function(a,b,c){if(a&&b&&b["class"]&&b.id){var d=this;a.browseMetaData(b.id,function(a,e){a?c&&c(a):b.res&&
b.res.url?d._renderer.playResource({url:b.res.url},e.result,c):c&&c(Error("resource url invalid"))})}else c&&c(Error("play obj invalid"))};d.prototype.addToQueueAndPlay=function(a,b,c){if(a&&b&&b.res&&b.res.url&&b.id){var d=this;b.parentID&&"Q:"==b.parentID.substr(0,2)?this.playFromQueue(a,b,c):a.browseMetaData(b.id,function(a,e){a?c&&c(a):e&&e.meta&&e.meta.res&&"x-sonosapi-stream"==e.meta.res.substr(0,17)?d._renderer.playResource({url:e.meta.res},e.meta.resMD,c):d._addURI(0,0,b.res.url,e.result,
0,!0,function(a,b){a?c&&c(a):b&&"undefined"!=typeof b.firstTrackNumberEnqueued&&null!=b.firstTrackNumberEnqueued?d._renderer.seekTrack(b.firstTrackNumberEnqueued,function(a){a?d.playQueue(c):d._renderer.play(c)}):c&&c(Error("result format invalid"))})})}else c&&c(Error("play obj invalid"))};d.prototype.playQueue=function(a){var b=this;this._renderer._setAVTransportURI(0,"x-rincon-queue:"+this._renderer.uuid+"#0",null,function(c){c?a&&a(c):b._renderer.play(a)})};d.prototype.playFromQueue=function(a,
b,c){if(b.id)if(a=b.id.indexOf("/"),-1==a)c&&c(Error("queue object id is invalid"));else{var d=this;b=b.id.substr(a+1);this._renderer.seekTrack(b,function(a){a?d.playQueue(c):d._renderer.play(c)})}else c&&c(Error("queue object id is null"))};d.prototype.addToQueue=function(a,b,c){if(a&&b&&b.res&&b.res.url&&b.id){var d=this;a.browseMetaData(b.id,function(a,e){a?c&&c(a):d._addURI(0,0,b.res.url,e.result,0,!0,c)})}else c&&c(Error("play obj invalid"))};d.prototype.replaceQueueAndPlay=function(a,b,c){if(a&&
b&&b.res&&b.res.url&&b.id){var d=this;a.browseMetaData(b.id,function(a,e){a?c&&c(a):d._removeAllTracks(0,0,function(a){a?c&&c(a):d._addURI(0,0,b.res.url,e.result,0,!0,function(a,b){a?c&&c(a):b&&"undefined"!=typeof b.firstTrackNumberEnqueued&&null!=b.firstTrackNumberEnqueued?d._renderer.seekTrack(b.firstTrackNumberEnqueued,function(a){a?c&&c(a):d._renderer.play(c)}):c&&c(Error("result format invalid"))})})})}else c&&c(Error("play obj invalid"))};d.prototype.browser=function(a,b,c,d,f){a?(b||(b=0),
c||(c=0),d||(d=0),a.browseChildren(b,c,d,null,function(c,b,d){if(!c&&b)for(var e=0;e<b.length;e++)b[e]&&(b[e].id&&"Q:"==b[e].id.substr(0,2)&&"Q:"==b[e].parentID&&b[e].res&&delete b[e].res,b[e].albumArtURI&&"http://"!=b[e].albumArtURI.substr(0,7)&&(b[e].albumArtURI="http://"+a.ip+":"+a.port+b[e].albumArtURI),b[e].info&&b[e].info.albumArtURI&&"http://"!=b[e].info.albumArtURI.substr(0,7)&&(b[e].info.albumArtURI="http://"+a.ip+":"+a.port+b[e].info.albumArtURI));f&&f(c,b,d)})):f&&f(Error("play obj invalid"))};
d.prototype.getZoneGroupState=function(a){var b=this;this._getZoneGroupState(function(c,d){if(c)a&&a(c);else if(d&&d.zoneGroupState){c=!1;for(var e=0;e<d.zoneGroupState.length;e++){var g=d.zoneGroupState[e];g&&g.coordinator==b.getUUID()&&(c=!0);for(var h=0;h<g.members.length;h++){var k=g.members[h];k&&k.uuid==b.getUUID()&&k.name&&(b.zoneName=k.name)}}a&&a(null,d.zoneGroupState,c)}else a&&a(Error("unknow zone group state"))})};d.prototype.getZoneGroupState=function(a){var b=this;this._getZoneGroupState(function(c,
d){if(c)a&&a(c);else if(d&&d.zoneGroupState){c=!1;for(var e=0;e<d.zoneGroupState.length;e++){var g=d.zoneGroupState[e];g&&g.coordinator==b.getUUID()&&(c=!0);for(var h=0;h<g.members.length;h++){var k=g.members[h];k&&k.uuid==b.getUUID()&&k.name&&(b.zoneName=k.name)}}a&&a(null,d.zoneGroupState,c)}else a&&a(Error("unknow zone group state"))})};d.prototype.playLineIn=function(a){var b=this.getUUID();b?this._renderer.playResource({url:"x-rincon-stream:"+b},"",a):a&&a(Error("no uuid"))};d.prototype.clearQueue=
function(a,b){var c=0;"Q:1"==a&&(c=1);this._removeAllTracks(c,0,b)};d.prototype.volumeUp=function(a){this._setRelativeVolume(0,"Master",5,a)};d.prototype.volumeDown=function(a){this._setRelativeVolume(0,"Master",-5,a)};d.prototype.getPlayInfo=function(a){if(this._renderer){var b=function(b){g--;b?a&&a(b):0==g&&(c&&"x-sonosapi-stream"==c.substr(0,17)&&d&&f&&f.trackMetaData&&(d["class"]&&(f.trackMetaData["class"]=d["class"]),d.title&&(f.trackMetaData.title=d.title),f.trackMetaData.streamContent&&(f.trackMetaData.artist=
f.trackMetaData.streamContent)),f&&f.trackMetaData&&f.trackMetaData.creator&&!f.trackMetaData.artist&&(f.trackMetaData.artist=f.trackMetaData.creator),a&&a(null,f))},c,d,f,g=2,h=this;this._renderer.getPlayInfo(function(a,c){!a&&c&&(c.trackMetaData&&c.trackMetaData.albumArtURI&&"http://"!=c.trackMetaData.albumArtURI.substr(0,7)&&(c.trackMetaData.albumArtURI="http://"+h._renderer.ip+":"+h._renderer.port+c.trackMetaData.albumArtURI),f=c);b&&b(a);a&&(b=null)});this._renderer.getMediaInfo(function(a,e){!a&&
...
a.command.forEach(function(a){b(d.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))});break;case "status":a.status&&a.status.forEach(function(a){b(d.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))})}return e},h=function(a,b){var c=[],f=xnm.aio.sonos.DM.getCommandStatusMap(a,d);f&&(a=e(f,a,b),c=c.concat(a));return c};if(!a.sys)return null;var k=JSON.parse(JSON.stringify(a)),l=null;switch(c.catagory){case "command":l=h(a,c);k.command=l;break;case "status":l=h(a,c);k.status=l;break;
default:return null}return l&&0!=l.length?k:null},getCommandStatusMap:function(a,c){return a.type?d[a.type]:null}}}();
