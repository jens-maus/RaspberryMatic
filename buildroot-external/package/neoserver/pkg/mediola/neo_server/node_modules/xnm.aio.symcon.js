...
xnm.aio.symcon.TYPE_INT=1;xnm.aio.symcon.TYPE_FLOAT=2;xnm.aio.symcon.TYPE_STRING=3;xnm.aio.symcon.Server=function(a,b,c,d,f,e){try{this._logger=require("x.logger.js").getLogger("xnm.aio.symcon.Server")}catch(g){this._logger={log:function(){}}}this.ip=a;this.port=b?b:82;this.user=c;this.password=d;this.webusr="webfront";this.webid=f;this.webpwd=e;this._id=1;this.scripts={};this.instances={};this.variables={};this._scanning=!1;this._scanCallbacks=[];this._scanListener=[];this.CommandHandler=null};
xnm.aio.symcon.Server.prototype._doRequest=function(a,b,c){b='{"jsonrpc": "2.0", "params": '+JSON.stringify(b)+', "method": "'+a+'", "id": '+this._id++ +"}";"WFC_"==a.substr(0,4)?(this._logger.log("trace","do WFC_ request with cred.:"+this.webusr+" pwd:"+this.webpwd),a=this.webpwd?"Basic "+(new Buffer(this.webusr+":"+this.webpwd)).toString("base64"):null):(this._logger.log("trace","do request with cred.:"+this.user+" pwd:"+this.password),a=this.user&&this.password?"Basic "+(new Buffer(this.user+":"+
this.password)).toString("base64"):null);var d={host:this.ip,port:this.port,path:"/api/",method:"POST",headers:{"Content-Type":"application/json; charset=UTF-8","Content-Length":b.length,"User-Agent":"mediola",Connection:"close"}};a&&(d.headers.Authorization=a);this._logger.log("trace","do request:"+JSON.stringify(d));this._logger.log("trace","with data:"+b);var f=this,e=require("http").request(d,function(a){a.setEncoding("utf-8");var b="";a.on("data",function(a){b+=a});a.on("end",function(){f._logger.log("trace",
...
d&&(g=parseFloat(b.ext));break;case 3:"setValue"==d&&(g=b.ext)}break;case 3:"run"==d&&(g=0);break;default:c&&c(Error("device type not supported"));return}if(null==g||"undefined"==typeof g)c&&c(Error("command invalid"));else if(this.webid){var h=this;this._doRequest("WFC_Execute",[parseInt(this.webid),parseInt(e),parseInt(f),g],function(a){if(!a)var d=Error("http requeset error");else if(a.error&&(h._logger.log("trace","execute command error:"+JSON.stringify(a)),d=Error(a.error),a.errorCode)){"run"==
b.value?(a="IPS_RunScript",d=[parseInt(f)]):(a="SetValue",d=[parseInt(f),g]);h._doRequest(a,d,function(a){var b;a?a.error&&(b=Error(a.error)):b=Error("http requeset error");c&&c(b)});return}c&&c(d)})}else c&&c(Error("no web front object id"))}else c&&c(Error("argument invalid"))};
...
xnm.aio.symcon.DM=function(){var a=function(a,c){this.code=a;this.message=c;this.stack=Error().stack};a.prototype=Error();a.prototype.name="IPSymconDMError";a.prototype.code=0;a.ERROR_CODE={DEFAULT:0,INDEX_EXISTS:1,ID_EXISTS:2,INVALID:3,UNKNOW:4,NOT_FOUND:5,IP_EXISTS:6};return{SYS:"ipsync",getGatewayType:function(){return{sys:this.SYS,name:"IP-Symcon",port:82}},createGateway:function(b,c,d){c=a.ERROR_CODE;b?b.ip?b.port?b.username?b.password?d(null,b):d(new a(c.INVALID,"password is invalid")):d(new a(c.INVALID,
"username is invalid")):d(new a(c.INVALID,"port is invalid")):d(new a(c.INVALID,"ip is invalid")):d(new a(c.INVALID,"info is invalid"))},updateGateway:function(b,c,d,f){b=a.ERROR_CODE;c?c.ip?c.port?c.username?c.password?f(null,c):f(new a(b.INVALID,"password is invalid")):f(new a(b.INVALID,"username is invalid")):f(new a(b.INVALID,"port is invalid")):f(new a(b.INVALID,"ip is invalid")):f(new a(b.INVALID,"update is invalid"))},deleteGateway:function(a,c,d){d()},createDevice:function(b,c,d){var f=a.ERROR_CODE;
if(b)if(b.gateway)if(c.data&&c.data.gateways&&0!==c.data.gateways.length){c=c.data.gateways;var e;for(e=0;e<c.length;e++)if(c[e].index===b.gateway){var g=c[e];break}g?b.type?b.objectid?d(null,b):d(new a(f.INVALID,"address is invalid")):d(new a(f.INVALID,"type is invalid")):d(new a(f.NOT_FOUND,"gateway with index "+b.gateway+" not exists"))}else d(new a(f.NOT_FOUND,"gateway with index "+b.gateway+" not exists"));else d(new a(f.INVALID,"gateway is invalid"));else d(new a(f.INVALID,"info is invalid"))},
updateDevice:function(b,c,d){var f=a.ERROR_CODE;if(b)if(b.gateway){c=c.data.gateways;var e;for(e=0;e<c.length;e++)if(c[e].index===b.gateway){var g=c[e];break}g?b.type?b.objectid?d(null,b):d(new a(f.INVALID,"address is invalid")):d(new a(f.INVALID,"type is invalid")):d(new a(f.NOT_FOUND,"gateway with index "+b.gateway+" not exists"))}else d(new a(f.INVALID,"gateway is invalid"));else d(new a(f.INVALID,"info is invalid"))},deleteDevice:function(a,c,d){d()},getCommandStatusMap:function(a){var b=function(a){if(null==
a.valuetype||"undefined"==typeof a.valuetype)return null;switch(parseInt(a.valuetype)){case 0:var b={command:[{type:"enum",name:"on",ref:{value:"on"}},{type:"enum",name:"off",ref:{value:"off"}}],status:[{type:"enum",name:"status",ref:{value:"state",options:["off","on"]}},{type:"string",name:"formatted status",ref:{value:"stateF"}}]};break;case 1:b={command:[{type:"int",name:"set value",ref:{value:"setValue",ext:"%d"}}],status:[{type:"int",name:"status",ref:{value:"state",ext:"%d"}},{type:"string",
...
name:"run",ref:{value:"run"}}]};default:return null}},applyUIFilter:function(a,c){var b=function(a,b){if("*"==a)return!0;for(var c=!1,d=0;d<a.length;d++)if(a[d]==b){c=!0;break}return c},f=function(a,c,d){var e=[];switch(d.catagory){case "command":a.command&&a.command.forEach(function(a){b(d.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))});break;case "status":a.status&&a.status.forEach(function(a){b(d.elems,a.type)&&(a=JSON.parse(JSON.stringify(a)),e.push(a))})}return e},e=function(a,b){var c=
[],d=xnm.aio.symcon.DM.getCommandStatusMap(a);d&&(a=f(d,a,b),c=c.concat(a));return c};if(!a.sys)return null;var g=JSON.parse(JSON.stringify(a)),h=null;switch(c.catagory){case "command":h=e(a,c);g.command=h;break;case "status":h=e(a,c);g.status=h;break;default:return null}return h&&0!=h.length?g:null}}}();xnm.aio.symcon.Handler=function(){};xnm.aio.symcon.Handler.prototype.Server=xnm.aio.symcon.Server;
xnm.aio.symcon.Handler.prototype.getStateValues=function(a,b){return(new xnm.aio.symcon.Server).getStateValues(a.type,b)};xnm.aio.symcon.Handler.prototype.getDeviceCommandList=function(a,b){return[]};xnm.aio.symcon.Handler.prototype.TYPE_BOOL=xnm.aio.symcon.TYPE_BOOL;xnm.aio.symcon.Handler.prototype.TYPE_INT=xnm.aio.symcon.TYPE_INT;xnm.aio.symcon.Handler.prototype.TYPE_FLOAT=xnm.aio.symcon.TYPE_FLOAT;xnm.aio.symcon.Handler.prototype.TYPE_STRING=xnm.aio.symcon.TYPE_STRING;
xnm.aio.symcon.Handler.prototype.devicemanager=xnm.aio.symcon.DM;xnm.aio.symcon.Handler.prototype.registModule=function(a){a.register(this.devicemanager.SYS,"symcon")};xnm.aio.symcon.Handler.prototype.resolveGateway=function(a){return a&&a.ip&&a.username&&a.password?new xnm.aio.symcon.Server(a.ip,a.port,a.username,a.password,a.webid,a.webpwd):null};xnm.aio.symcon.Handler.prototype.getWebfronts=function(a,b){(a=this.resolveGateway(a))?a._scanWebfront(function(a,d){b&&b(a,d)}):b&&b(Error("gateway json invalid"))};
