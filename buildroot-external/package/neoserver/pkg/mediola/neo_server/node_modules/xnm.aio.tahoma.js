...
g.length);(c=d.getCookie(this.username,this.password))&&(a.headers.Cookie=c);c=require("https");"undefined"!=typeof process&&null!=process&&process.env&&(process.env.NODE_TLS_REJECT_UNAUTHORIZED="0");-1!=a.path.indexOf("/externalAPI/json/login")?this._logger.log("trace","do request: /externalAPI/json/login with username & password"):this._logger.log("trace","do request:"+JSON.stringify(a));var h=this,f=m,l=c.request(a,function(a){a.setEncoding("utf-8");var b="";a.on("data",function(a){b+=a});a.on("end",
function(){h._logger.log("trace","receive code:"+a.statusCode+" headers:"+JSON.stringify(a.headers)+" response:"+b);f&&(f(null,b,a),f=null)})});if(l.on)l.on("error",function(a){h._logger.log("trace","do request error:"+a.message);f&&(f(a),f=null)});l.setTimeout&&l.setTimeout(2E3,function(){h._logger.log("trace","do request timeout");l.abort();f&&(f(Error("timeout")),f=null)});l.setAllowRedirect&&l.setAllowRedirect(!1);g&&(this._logger.log("trace","do request send body:"+g),l.write(g));l.end()};c.prototype.toJSON=
function(){return{sys:"tahoma",username:this.username,password:this.password}};c.prototype.equalsTo=function(a){return a&&a instanceof c?this.username==a.username&&this.password==a.password:!1};return c}();
