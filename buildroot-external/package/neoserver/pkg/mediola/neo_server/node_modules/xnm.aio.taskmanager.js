...
a.device.ipDevice?{sys:"ACTION",type:"IP",code:xnm.aio.taskmanager.intToStrHex(a.espId)}:{sys:"ACTION",type:a.sys,code:a.command.ref.value}}b.id=a.id;return b});return{group:{id:a.id,sys:"GROUP",actionids:c,conditionids:d,triggerids:e,active:a.active?"1":"0"},actions:k,triggers:h,conditions:g}};
...
xnm.aio.taskmanager.saveVxTask=function(a,b,c){function d(a,b,c){XC.debugf("# del elements: "+a);var d=0,e=q[a],l=f[a];if(0===e.length)return c();e.forEach(function(f){var h=m.filter(function(a){return a.id==f&&l.includes(a.sys)})[0];if(!h)return c(Error(a+" not found"));xnm.aio.taskmanager.deleteVxElement(g,h,function(a,f){if(a)return c(a);n[b].splice(n[b].indexOf(h.id),1);++d===e.length&&c()})})}function e(c,f,e){XC.debugf("# add elements: "+c);var d=k[c];if(!d||!d.length)return e();var h=0;d.forEach(function(l){l.id=
xnm.aio.taskmanager.getNextVxElementId(n[f]);p[c]+=l.id;n[f].push(l.id);xnm.aio.taskmanager.addVxElement(g,l,function(c,f){if(c)return XC.debugf(c),e(Error("add element failed"));"ACTION"===l.sys&&"AIO"===l.type&&b.push(l);"ACTION"===l.sys&&"CLOUD"===l.type&&(a._localCloudId=l.id);++h===d.length&&e()})}.bind(this))}function h(b){XC.debugf("# del all");if(!a.id)return b();var c={id:a.id,options:"all"};a.alarmPin&&(c.pin=a.alarmPin);a.aio._executeRequest("DelGroup",c,null,function(c,f){if(c)return a.isAlarmTask?
b({msg:"pin error"}):b(Error("del group failed"));b()})}var g=a.aio,k=xnm.aio.taskmanager.mapProTaskToVx(a),m=null,f={groups:["GROUP"],actions:["ACTION","DEVICEACTION"],triggers:["TASK","ASTRO","EVENT","SEVENT"],conditions:["TIME"]},p={actions:"",triggers:"",conditions:""},n={groups:[],actions:[],triggers:[]},q={actions:[],triggers:[],conditions:[]},r=xnm.aio.taskmanager.hasDeleteAll(a.aio)?[h.bind(this)]:[d.bind(this,"triggers","triggers"),d.bind(this,"conditions","triggers"),d.bind(this,"actions",
"actions")];r=[].concat([function(b){function c(a){return a&&a.match(/.{1,2}/g)||[]}XC.debugf("# init");xnm.aio.taskmanager.getVxTasks(g,function(e,d){if(e)return b(e);m=d;if(a.id){e=d.filter(function(b){return"GROUP"===b.sys&&b.id==a.id})[0];if(!e)return b(Error("group not found"));q.actions=c(e.actionids);q.conditions=c(e.conditionids);q.triggers=c(e.triggerids)}n.groups=xnm.aio.taskmanager.getVxIdList(d,f.groups);n.actions=xnm.aio.taskmanager.getVxIdList(d,f.actions).concat(xnm.aio.taskmanager.getVxIdList(d,
f.conditions));n.triggers=xnm.aio.taskmanager.getVxIdList(d,f.triggers);b()})}],$jscomp.arrayFromIterable(r),[e.bind(this,"triggers","triggers"),e.bind(this,"conditions","triggers"),e.bind(this,"actions","actions"),function(b){var c=k.group;c.actionids=p.actions;c.triggerids=p.triggers;c.conditionids=p.conditions;c.id||(c.id=xnm.aio.taskmanager.getNextVxElementId(n.groups));a.id=c.id;var e=xnm.aio.taskmanager.SystemData,d=e.getGateways(function(b){return b.info.mac==a.aio._mac})[0];d.tasks||(d.tasks=
[]);var f=d.tasks.find(function(b){return b.id==a.id});f?f.name=a.name:d.tasks.push({id:a.id,name:a.name});e.saveGateway(d);a.alarmPin&&(c.pin=a.alarmPin);g._executeRequest("AddGroup",c,null,function(c,d){if(c)return a.isAlarmTask?b({msg:"pin error"}):b(Error("save group failed"));b()})}]);try{(new xnm.aio.taskmanager.FunctionStack(r)).start(c)}catch(t){XC.debugf(t)}};
...
xnm.aio.taskmanager.deleteVxTask=function(a,b){function c(b,c,f){XC.debugf("# del elements: "+b);var g=0,k=h[b],m=e[b];if(0===k.length)return f();k.forEach(function(b){var c=d.filter(function(a){return a.id==b&&m.includes(a.sys)})[0];if(!c)return f();xnm.aio.taskmanager.deleteVxElement(a.aio,c,function(a,b){if(a)return f(a);++g===k.length&&f()})})}(function(a){var b=xnm.aio.taskmanager.SystemData,c=b.getGateways(function(b){return b.info.mac==a.aio._mac})[0];if(c.tasks){for(var d=c.tasks.length;d--;)c.tasks[d].id==
