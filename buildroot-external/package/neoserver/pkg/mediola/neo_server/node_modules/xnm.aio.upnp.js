...
$jscomp.polyfill("Array.prototype.find",function(e){return e?e:function(b,a){return $jscomp.findInternal(this,b,a).v}},"es6","es3");var xnm=xnm||{};xnm.aio=xnm.aio||{};xnm.aio.upnp=xnm.aio.upnp||{};
xnm.aio.upnp.Player=function(){var e=function(b){"undefined"!=typeof require&&require?(this._logger=require("x.logger.js").getLogger("xnm.aio.bose.SoundTouch"),require("x.logger.js").setLevel("trace","xnm.aio.bose.SoundTouch")):this._logger={log:{}};this._renderer=null;b&&b.ip&&b.port&&b.uuid&&b.avTransport&&b.renderingControl&&(this._renderer=this._getRenderer(b.ip,b.port,b.uuid,b.avTransport,b.renderingControl))};e.prototype.getIP=function(){return this._renderer?this._renderer.ip:null};e.prototype.setIP=
function(b){this._renderer&&(this._renderer.ip=b)};e.prototype.getPort=function(){return this._renderer?this._renderer.port:null};e.prototype.setPort=function(b){this._renderer&&(this._renderer.port=b)};e.prototype.getUUID=function(){return this._renderer?this._renderer.uuid:null};e.prototype.getPlayInfo=function(b){this._renderer?this._renderer.getPlayInfo(function(a,c){b&&b(a,c)}):b&&b(Error("no renderer"))};e.prototype.play=function(b,a,c){if(b&&a&&a["class"]&&a.id)if(this._renderer){var d=this;
b.browseMetaData(a.id,function(f,h){f?c&&c(f):0!=a["class"].indexOf("object.container")||a.res&&a.res.url?a.res&&a.res.url?d._renderer.playResource({url:a.res.url},h.result,c):c&&c(Error("resource url invalid")):b.uuid?d._renderer.playContainer({containerID:a.id,server_uuid:b.uuid},h.result,c):c&&c(Error("media server uuid invalid"))})}else c&&c(Error("renderer not inited"));else c&&c(Error("play obj invalid"))};e.prototype.browser=function(b,a,c,d,f){b?(a||(a=0),c||(c=0),d||(d=0),b.browseChildren(a,
c,d,null,function(a,c,b){f&&f(a,c,b)})):f&&f(Error("play obj invalid"))};e.prototype.executeCommandCreator=function(b,a,c){a&&a.value?this._executeCommand(a.value,a.ext,function(a){c&&c(a)}):c&&c(Error("command value is empty"))};e.prototype.getStatesCreator=function(b,a,c){this._getStates(function(b,f){if(b)c&&c(b);else{b=[];for(var d=0;d<a.length;d++){var e=a[d];if(e&&e.id&&e.status){var g="?";switch(e.status.value){case "power":case "muted":case "volume":case "repeatMode":case "shuffleMode":case "playState":case "durationS":case "positionS":case "bass":g=
f[e.status.value];break;case "artist":case "title":case "album":case "radio":(g=f[e.status.value])||(g="")}if("undefined"==typeof g||null==g)g="?";b.push({id:e.id,status:g})}}c&&c(null,b)}})};e.prototype._executeCommand=function(b,a,c){if(this._renderer){var d=this;switch(b){case "play":this._clickKey("PLAY",c);break;case "pause":this._clickKey("PAUSE",c);break;case "tooglePlay":this._clickKey("PLAY_PAUSE",c);break;case "stop":this._clickKey("STOP",c);break;case "previous":this._clickKey("PREV_TRACK",
...
c);break;case "bookmark":this._clickKey("BOOKMARK",c);break;case "preset":if(!a){c&&c(Error("preset value invalid"));break}this._clickKey("PRESET_"+a,c);break;case "shuffleOn":this._clickKey("SHUFFLE_ON",c);break;case "shuffleOff":this._clickKey("SHUFFLE_OFF",c);break;case "toggleShuffle":this._getNowPlaying(function(b,a){b?c&&c(b):a&&a.shuffleSetting?(b="SHUFFLE_ON","SHUFFLE_ON"==a.shuffleSetting&&(b="SHUFFLE_OFF"),d._clickKey(b,c)):c&&c(Error("no shuffle setting"))});break;case "setRepeatMode":if(!a){c&&
...
(c.shuffleMode=d.shuffleSetting),d.playStatus&&(c.playState=d.playStatus),d.duration&&(c.durationS=d.duration),d.position&&(c.positionS=d.position),d.position&&(c.positionS=d.position),d.track&&(c.title=d.track),d.stationName&&(c.radio=d.stationName),d.artist&&(c.artist=d.artist),d.album&&(c.album=d.album));a()})};e.prototype._getRenderer=function(b,a,c,d,e){var f=require("xnm.aio.upnp.js");if(f.discovery&&f.discovery._player&&(f=f.discovery._player[c])&&f._renderer)return f._renderer;f=new (require("x.upnp.js").MediaRenderer);
f.init(b,a,c,{avTransport:d,renderingControl:e});return f};e.prototype._clickKey=function(b,a){var c=this;this._pressKey(b,function(d){d?a&&a(d):c._releaseKey(b,function(b){a&&a(b)})})};e.prototype._getInfo=function(b){var a=this;this._sendRequest("GET","/info",null,function(c,d){c?b&&b(c):(c=d.find("info"))&&c[0]?(c=$(c[0]),a.mac=c.attr("deviceID"),(c=c.find("name"))&&c[0]&&(a.name=$(c[0]).text()),b&&b(null,{name:a.name,mac:a.mac})):b&&b(Error("get info xml invalid"))})};e.prototype._pressKey=function(b,
a){this._sendRequest("POST","/key",'<key state="press" sender="Gabbo">'+b+"</key>",a)};e.prototype._releaseKey=function(b,a){this._sendRequest("POST","/key",'<key state="release" sender="Gabbo">'+b+"</key>",a)};e.prototype._getSources=function(b){var a=this;this._sendRequest("GET","/sources",null,function(c,d){if(c)b&&b(c);else{var e=[];d.find("sourceItem").each(function(){var a=$(this);e.push({source:a.attr("source"),sourceAccount:a.attr("sourceAccount")?a.attr("sourceAccount"):null,isLocal:a.attr("isLocal"),
name:a.text()?a.text():null})});a.sources=e;b&&b(null,e)}})};e.prototype._setSource=function(b,a,c){b='<ContentItem source="'+b+'"';a&&(b+=' sourceAccount="'+a+'"');this._sendRequest("POST","/select",b+"></ContentItem>",c)};e.prototype._setBass=function(b,a){this._sendRequest("POST","/bass","<bass>"+b+"</bass>",a)};e.prototype._getBass=function(b){this._sendRequest("GET","/bass",null,function(a,c){if(a)b&&b(a);else{var d={},e=c.find("targetbass");e&&e[0]&&(d.targetbass=parseInt($(e[0]).text()));(e=
c.find("actualbass"))&&e[0]&&(d.actualbass=parseInt($(e[0]).text()));b&&b(a,d)}})};e.prototype._getBassCap=function(b){var a=this;this._sendRequest("GET","/bassCapabilities",null,function(c,d){if(c)b&&b(c);else if((c=d.find("bassAvailable"))&&c[0])if((c=$(c[0]).text())&&"false"!=c.toLowerCase()){c=0;var e=d.find("bassMin");e&&e[0]&&(c=$(e[0]).text());e=100;(d=d.find("bassMax"))&&d[0]&&(e=$(d[0]).text());a.bass=c+"-"+e;b&&b(null,{bass:a.bass})}else b&&b(null,null);else b&&b(Error("get bass cap xml invalid"))})};
e.prototype._getZone=function(b){this._sendRequest("GET","/getZone",null,function(a,c){if(a)b&&b(a);else{var d={};if((a=c.find("zone"))&&a[0]){c=$(a[0]).attr("master");if(!c){b&&b(null,d);return}d.master=c}$(a[0]).find("member").each(function(){var a=$(this),b=a.attr("ipaddress");a=a.text();b&&a&&(d.members||(d.members={}),d.members[a]=b)});b&&b(null,d)}})};e.prototype._setZone=function(b,a,c){b='<zone master="'+b+'" senderIPAddress="0.0.0.0">';for(var d in a)a.hasOwnProperty(d)&&a[d]&&(b+='<member ipaddress="'+
a[d]+'">'+d+"</member>");this._sendRequest("POST","/setZone",b+"</zone>",c)};e.prototype._getVolume=function(b){this._sendRequest("GET","/volume",null,function(a,c){if(a)b&&b(a);else{var d={},e=c.find("targetvolume");e&&e[0]&&(d.targetvolume=parseInt($(e[0]).text()));(e=c.find("actualvolume"))&&e[0]&&(d.actualvolume=parseInt($(e[0]).text()));(e=c.find("muteenabled"))&&e[0]&&(d.muteenabled=$(e[0]).text());b&&b(a,d)}})};e.prototype._setVolume=function(b,a){this._sendRequest("POST","/volume","<volume>"+
b+"</volume>",a)};e.prototype._getNowPlaying=function(b){this._sendRequest("GET","/now_playing",null,function(a,c){if(a)b&&b(a);else{a={};var d=c.find("nowPlaying");if(d&&d[0]){var e=$(d[0]).getAttributes();a=JSON.parse(JSON.stringify(e))}(d=c.find("ContentItem"))&&d[0]&&(e=$(d[0]).getAttributes(),a.content=JSON.parse(JSON.stringify(e)),(d=$(d[0]).find("itemName"))&&d[0]&&(a.content.itemName=$(d[0]).text()));(d=c.find("track"))&&d[0]&&(a.track=$(d[0]).text());(d=c.find("artist"))&&d[0]&&(a.artist=
$(d[0]).text());(d=c.find("album"))&&d[0]&&(a.album=$(d[0]).text());(d=c.find("stationName"))&&d[0]&&(a.stationName=$(d[0]).text());(d=c.find("art"))&&d[0]&&(a.artImageStatus=$(d[0]).attr("artImageStatus"),a.art=$(d[0]).text());(d=c.find("playStatus"))&&d[0]&&(a.playStatus=$(d[0]).text());(d=c.find("description"))&&d[0]&&(a.description=$(d[0]).text());(d=c.find("stationLocation"))&&d[0]&&(a.description=$(d[0]).text());(d=c.find("time"))&&d[0]&&(a.duration=$(d[0]).attr("total"),a.position=$(d[0]).text());
(d=c.find("shuffleSetting"))&&d[0]&&(a.shuffleSetting=$(d[0]).text());(d=c.find("repeatSetting"))&&d[0]&&(a.repeatSetting=$(d[0]).text());b&&b(null,a)}})};e.prototype._sendRequest=function(b,a,c,d){this._doRequest(this.getIP(),8090,b,a,c,function(a,b){if(a)d&&d(a);else try{var c=$($.parseXML(b)),e=c.find("error");if(e&&e[0]){var f=$(e[0]),h=f.text(),m=f.attr("name");d&&d(Error(h+":"+m))}else d&&d(null,c)}catch(n){d&&d(n)}})};e.prototype._doRequest=function(b,a,c,d,e,h){var f=this;b="http://"+b+":"+
a+d;this._logger.log("trace","send "+c+" to url:"+b);e&&this._logger.log("trace","with data "+e);var g=h;h={url:b,type:c,timeout:2E3,contentType:"text/xml",dataType:"text",cache:!0,success:function(a){f._logger.log("trace","http request success:"+a);g&&(g(null,a),g=null)},error:function(a,b){a="http request error:"+(a?a.status:"0")+"-"+b;f._logger.log("trace",a);g&&(g(Error(a)),g=null)}};"post"==c.toLowerCase()&&e&&(h.data=e);$.ajax(h)};e.prototype.toJSON=function(){return{sys:"upnp",ip:this.getIP(),
port:this.getPort(),uuid:this.getUUID(),name:this._renderer?this._renderer.name:null,model:this._renderer?this._renderer.modelName:null,modelDescription:this._renderer?this._renderer.modelDescription:null,manufacturer:this._renderer?this._renderer.manufacturer:null,avTransport:this._renderer?this._renderer.avTransport:null,renderingControl:this._renderer?this._renderer.renderingControl:null,connectionManager:this._renderer?this._renderer.connectionManager:null}};e.prototype.equalsTo=function(b){return b&&
b instanceof SoundTouch?this.getIP()==b.getIP()&&this.getPort()==b.getPort():!1};return e}();xnm.aio.upnp.InMemoryPlayList=function(){return function(){}}();
xnm.aio.upnp.Discovery=function(){var e=function(){"undefined"!=typeof require&&require?(this._logger=require("x.logger.js").getLogger("xnm.aio.bose.Discovery"),require("x.logger.js").setLevel("info","xnm.aio.bose.Discovery")):this._logger={log:{}};this._upnpCB=null;this._mediaServer={};this._player={};this._searchCB=[];this._autoSearchTimeout=null;this.search(null)};e.prototype.getBufferedPlayer=function(b){return this._player[b]};e.prototype.getBufferedPlayers=function(){var b=[],a;for(a in this._player)this._player.hasOwnProperty(a)&&
b.push(this._player[a]);return b};e.prototype.getBufferedMediaServers=function(){var b=[],a;for(a in this._mediaServer)this._mediaServer.hasOwnProperty(a)&&b.push(this._mediaServer[a]);return b};e.prototype.search=function(b){var a=this._searchCB.length;b&&-1==this._searchCB.indexOf(b)&&this._searchCB.push(b);if(0==a){this._autoSearchTimeout&&(clearTimeout(this._autoSearchTimeout),this._autoSearchTimeout=null);this._search();var c=this;setTimeout(function(){c._autoSearchTimeout=setTimeout(function(){c._logger.log("trace",
"auto search timeout, search again");c.search(null)},6E4);var a=c._searchCB;c._searchCB=[];a.forEach(function(a){a&&a()})},5E3)}};e.prototype._search=function(){if(!this._upnpCB){var b=this;this._upnpCB=function(a){b._findUpnpDevice(a)}}this._mediaServer={};this._player={};var a=require("x.upnp.js").getDiscoveryService();a.addCallback("upnp:rootdevice",this._upnpCB);a.discoverTarget("upnp:rootdevice")};e.prototype._findUpnpDevice=function(b){if(b){var a=b.uuid;switch(b.deviceType){case "urn:schemas-upnp-org:device:MediaServer:1":this._logger.log("trace",
