...
proc action_create_backup {} {
    ...
    rega system.Save()
    cd /
    catch {exec tar --exclude=usr/local/tmp --exclude=usr/local/lost+found -czf $tmpdir/usr_local.tar.gz usr/local}
    cd $tmpdir/
    ...
    catch {exec tar -cf /usr/local/tmp/last_backup.sbk usr_local.tar.gz signature firmware_version key_index}
    cd /
    exec rm -rf $tmpdir
