...
proc action_create_backup {} {
    ...
    rega system.Save()
    cd /
    catch { exec tar --owner=root --group=root --exclude=usr/local/tmp --exclude=usr/local/lost+found --one-file-system --ignore-failed-read -czf $tmpdir/usr_local.tar.gz usr/local }
    cd $tmpdir/
