...
#     interface: [string] Bezeichnung der Schnittstelle, über die das Gerät angeschlossen ist
#     type:      [string] Bezeichnung des Gerätetyps
...
!   Write(" DEVICE_TYPE {"# device.HssType() # "}"); 
    ...
    var rssiDevice = "";
    var rssiPeer = "";
    if (interface.Name() == "HmIP-RF")
    {
      rssiDevice = xmlrpc.GetValue(device.Interface(), device.Address() # ":0", "RSSI_DEVICE");
      rssiPeer = xmlrpc.GetValue(device.Interface(), device.Address() # ":0", "RSSI_PEER");
    }
    Write(" RSSI_DEVICE {"# rssiDevice # "}");
    Write(" RSSI_PEER {"# rssiPeer # "}");
...
append result ",\"rssiDevice\":[json_toString $device(RSSI_DEVICE)]"
append result ",\"rssiPeer\":[json_toString $device(RSSI_PEER)]"
