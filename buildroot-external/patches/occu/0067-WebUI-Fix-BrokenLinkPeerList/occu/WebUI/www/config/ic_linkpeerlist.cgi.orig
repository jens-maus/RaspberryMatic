...
proc put_page {} {
  ...
  puts "<div id=\"ic_linkpeerlist\" class=\"j_translate\">"
    ...
    puts "<div id=\"id_body\">"
      ...
      puts "  s += \"<tr>\";"
      puts "  s += \"<td align='center' valign='middle'><div class='FooterButton' onclick='WebUI.goBack()'>\" + translateKey('footerBtnPageBack') + \"</div></td>\";"
      puts "  s += \"<td align='center' valign='middle'><div class='FooterButton CLASS22100' onclick='loadNewLinkPage();' >\"+ translateKey('footerBtnNewLink') +\"</div></td>\";"
      puts "  s += \"</tr>\";"
      puts "  s += \"</table>\";"
      puts "  setFooter(s);"
...
#       append SENTRY(ACTION) "<div class=\"CLASS21000\" onclick=\"OpenSetProfiles('$iface', '$link(SENDER)', '$link(RECEIVER)');\">${btnEdit}</div>"
  ...
  if {[llength $tablestruct] == 0} then {
    puts "<TR>"
    #puts "<TD height=\"100\" align=\"center\" COLSPAN=\"7\">Es sind keine Verkn&uuml;pfungen vorhanden.</TD>"
    puts "<TD height=\"100\" align=\"center\" COLSPAN=\"7\">\${lblNoLinksAvailable}</TD>"
    puts "</TR>"
  } else {
    if {$sortby == "LINK"} then {
...
proc put_tableheader {} {
  ...
  puts "<TR>"
  puts "<TD align=\"center\" [expr {$sortby=="SENDER"?"class=\"header_active\"":""} ] >\${thName}</TD>"
  puts "<TD align=\"center\">\${thSerialNumber}</TD>"
  puts "<TD align=\"center\" [expr {$sortby=="LINK"?"class=\"header_active\"":""} ]>\${thName}</TD>"
  puts "<TD align=\"center\">\${thDescription}</TD>"
  puts "<TD align=\"center\">\${thAction}</TD>"
  puts "<TD align=\"center\" [expr {$sortby=="RECEIVER"?"class=\"header_active\"":""} ]>\${thName}</TD>"
  puts "<TD align=\"center\">\${thSerialNumber}</TD>"
  puts "</TR>"
  puts "</THEAD>"
}
