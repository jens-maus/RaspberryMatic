...
#	Party-/Urlaubs-Endzeit	
	append HTML_PARAMS(separate_1) "<tr><td>$PROFILE_PNAME(PARTY_END_TIME)</td>"
	append HTML_PARAMS(separate_1) "<td><input type=\"text\" value=\"[format {%1.0f} $ps(PARTY_END_DAY)]\" name=\"PARTY_END_DAY\" size=\"2\" id=\"separate_${special_input_id}_7\" style=\"text-align:center\" >\${partyDays}"
	
	append HTML_PARAMS(separate_1) "<select id=\"separate_${special_input_id}_12\" name=\"PARTY_END_HOUR\">"
	for {set loop 0} {$loop <= 23} {incr loop} {
		if {$loop < 10} {set zero "0"} else {set zero ""}		
		if {$loop == $ps(PARTY_END_HOUR)} then {
 			append HTML_PARAMS(separate_1) "<option selected value=\"$loop\">$zero$loop</option>"
		} else {
			append HTML_PARAMS(separate_1) "<option value=\"$loop\">$zero$loop</option>"
		}	
	
	}
	#append HTML_PARAMS(separate_1) "</select>Uhr "
	append HTML_PARAMS(separate_1) "</select>&nbsp;:&nbsp;"

	append HTML_PARAMS(separate_1) "<select id=\"separate_${special_input_id}_13\" name=\"PARTY_END_MINUTE\">"
	if {$ps(PARTY_END_MINUTE) == "0" } then {
			append HTML_PARAMS(separate_1) "<option selected value=\"0\">00</option>"
			append HTML_PARAMS(separate_1) "<option value=\"1\">30</option>"
	} else {
			append HTML_PARAMS(separate_1) "<option value=\"0\">00</option>"
			append HTML_PARAMS(separate_1) "<option selected value=\"1\">30</option>"
	}

# solange darf die Party höchstens dauern
	array_clear param_descr
	array set param_descr $ps_descr(PARTY_END_DAY)
	set maxDay [format {%1.0f} $param_descr(MAX)]

	#append HTML_PARAMS(separate_1) "</select>Minuten</td>"
	append HTML_PARAMS(separate_1) "</tr>"
	append HTML_PARAMS(separate_1) "<tr><td></td><td style=\"padding-top:0px; padding-bottom:10px;\">max. $maxDay \${lblDays}</td></tr>"
