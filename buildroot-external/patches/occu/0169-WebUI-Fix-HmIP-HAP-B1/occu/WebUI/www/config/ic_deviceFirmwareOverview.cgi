      ...
      if {[catch {set firmwareUpdateState $dev_descr(FIRMWARE_UPDATE_STATE)}] == 0} {
        switch $firmwareUpdateState {
          ...
          "LIVE_NEW_FIRMWARE_AVAILABLE" {
            ...
            if { ([string compare "HmIPW-DRAP" $dev_descr(TYPE)] == 0 || [string first "HmIP-HAP" $dev_descr(TYPE)] == 0 ) && ([regexp {[0-1]\.[0-9]*\.[0-9]*} $dev_descr(FIRMWARE)] || [regexp {2\.0\.[0-9]*} $dev_descr(FIRMWARE)]) } {
