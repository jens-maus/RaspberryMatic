...
# die Texte der Platzhalter einlesen
  puts "<script type=\"text/javascript\">getLangInfo('MULTI_MODE_INPUT_TRANSMITTER_5', '$dev_descr_receiver(TYPE)');</script>"
...
#1
  ...
  append HTML_PARAMS(separate_$prn) "<tr class=\"_hidden\">"
    append HTML_PARAMS(separate_$prn) "<td>\${condThresholdLo}</td>"
    append HTML_PARAMS(separate_$prn) "<td><input type='text' id='separate_receiver_$prn\_$pref' name='$scvl' value='$ps($scvl)' size='4' class='alignCenter' onblur='ProofAndSetValue(this.id, this.id, $min, $max, 1);'/>&nbsp&nbsp;($min - $max)</td>"
  append HTML_PARAMS(separate_$prn) "</tr>"

  set scvh SHORT_COND_VALUE_HI
  incr pref
  append HTML_PARAMS(separate_$prn) "<tr class=\"_hidden\">"
    append HTML_PARAMS(separate_$prn) "<td>\${condThresholdHi}</td>"
    append HTML_PARAMS(separate_$prn) "<td><input type='text' id='separate_receiver_$prn\_$pref' name='$scvh' value='$ps($scvh)' size='4' class='alignCenter' onblur='ProofAndSetValue(this.id, this.id, $min, $max, 1);'/>&nbsp&nbsp;($min - $max)</td>"
  append HTML_PARAMS(separate_$prn) "</tr>"
