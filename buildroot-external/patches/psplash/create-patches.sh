...
for file in ${logofiles}; do
  dir=$(dirname "${file}")
  gdk-pixbuf-csource --macros --name=POKY_IMG "${dir}/logo.png" | sed 's/guint8/uint8_t/g' >/tmp/psplash-poky-img.h
  diff -N -u --label=psplash/psplash-poky-img.h.orig --label=psplash/psplash-poky-img.h psplash-poky-img.h.orig /tmp/psplash-poky-img.h >"${dir}/0001-logo.patch"
